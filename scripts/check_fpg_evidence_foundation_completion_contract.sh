#!/usr/bin/env bash
# Validate bd-bp8fl.3.12.1 fpg evidence foundation completion evidence.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT_PATH="${1:-${ROOT}/tests/conformance/fpg_evidence_foundation_completion_contract.v1.json}"
OUT_DIR="${2:-${ROOT}/target/conformance}"
REPORT_PATH="${OUT_DIR}/fpg_evidence_foundation_completion_contract.report.json"
LOG_PATH="${OUT_DIR}/fpg_evidence_foundation_completion_contract.log.jsonl"
...
EXPECTED_SCHEMA = "fpg_evidence_foundation_completion_contract.v1"
EXPECTED_BEAD = "bd-bp8fl.3.12"
EXPECTED_COMPLETION_BEAD = "bd-bp8fl.3.12.1"
...
EXPECTED_FRESHNESS_POLICY = {
    ...
    "stale_result": "block_foundation_gate_evidence",
    "foundation_evidence_allowed_when_stale": False,
...
def append_event(event: str, status: str, details: dict[str, Any]) -> None:
    events.append(
        {
            "schema_version": "fpg_evidence_foundation_completion_contract.log.v1",
            "event": event,
            "status": status,
...
def validate_source_artifacts(manifest: dict[str, Any]) -> dict[str, pathlib.Path]:
    ...
    for key in sorted(EXPECTED_SOURCE_KEYS):
        ...
        if path is not None:
            paths[key] = path
    append_event(
        "fpg_evidence_foundation_completion.source_artifacts",
...
def validate_completion_evidence(manifest: dict[str, Any], paths: dict[str, pathlib.Path]) -> dict[str, list[str]]:
    ...
    unit_refs = validate_test_refs(
        ...
        "unit_primary",
        test_text,
        {"gate_artifact_covers_foundation_rows"},
    )
    integration_refs = validate_test_refs(
        ...
        test_text,
        {
            ...
            "gate_rejects_missing_foundation_anchor",
    ...
    append_event(
        "fpg_evidence_foundation_completion.evidence_refs",
        "fail" if errors else "pass",
        {
            "unit_refs": unit_refs,
...
def validate_source_gate_contract(manifest: dict[str, Any], paths: dict[str, pathlib.Path]) -> dict[str, Any]:
    ...
    append_event(
        "fpg_evidence_foundation_completion.source_gate_contract",
        "fail" if errors else "pass",
        {
            "gap_count": len(rows),
...
report = {
    "schema_version": "fpg_evidence_foundation_completion_contract.report.v1",
    "status": status,
    "bead": EXPECTED_COMPLETION_BEAD,
    "source_bead": EXPECTED_BEAD,
...
if errors:
    print(
        "FAIL fpg evidence foundation completion contract: "
        + "; ".join(errors[:8]),
        file=sys.stderr,
    )
    sys.exit(1)
...
print(
    "PASS fpg evidence foundation completion contract "
