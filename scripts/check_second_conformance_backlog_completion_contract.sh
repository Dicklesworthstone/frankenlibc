#!/usr/bin/env bash
# check_second_conformance_backlog_completion_contract.sh -- bd-r5k32.15 completion gate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT="${FRANKENLIBC_SECOND_CONFORMANCE_BACKLOG_COMPLETION_CONTRACT:-${ROOT}/tests/conformance/second_conformance_backlog_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_SECOND_CONFORMANCE_BACKLOG_COMPLETION_OUT_DIR:-${ROOT}/target/conformance/second_conformance_backlog_completion}"
REPORT="${FRANKENLIBC_SECOND_CONFORMANCE_BACKLOG_COMPLETION_REPORT:-${OUT_DIR}/second_conformance_backlog_completion_contract.report.json}"
LOG="${FRANKENLIBC_SECOND_CONFORMANCE_BACKLOG_COMPLETION_LOG:-${OUT_DIR}/second_conformance_backlog_completion_contract.events.jsonl}"
...
SCHEMA = "second_conformance_backlog_completion_contract.v1"
REPORT_SCHEMA = "second_conformance_backlog_completion_contract.report.v1"
BEAD_ID = "bd-r5k32.15"
PARENT_BEAD = "bd-r5k32"
TRACE_ID = "bd-r5k32.15::second-conformance-backlog::completion::v1"
...
REQUIRED_POSITIVE_TESTS = {
    "contract_binds_second_conformance_backlog_closeout",
    "checker_accepts_second_conformance_backlog_completion_contract",
    "checker_emits_completion_telemetry",
}
REQUIRED_NEGATIVE_TESTS = {
    "checker_rejects_missing_child_closeout",
    "checker_rejects_stale_rpc_first_wave_symbols",
...
def primary_signature() -> str:
    ...
    for signature in FAILURE_PRIORITY:
        if signature in present:
            return signature
    return "second_conformance_backlog.completion_contract_failed"


def event(name: str, status: str, failure_signature: str = "none", **fields: Any) -> dict[str, Any]:
    return {
        "schema_version": "second_conformance_backlog_completion_contract.event.v1",
        "timestamp": now(),
    ...
    if label == "br_dep_cycles":
        try:
            ...
            if payload.get("count") != 0:
                add_error("base_gate_failed", f"{label}: expected count=0, got {payload!r}")
        except Exception as exc:
...
def validate_child_closeouts(contract: dict[str, Any]) -> None:
    ...
    expected_count = snapshot.get("required_closed_child_count")
    if expected_count != 14:
        add_error("child_closeout_drift", f"required_closed_child_count expected 14, got {expected_count!r}")
    expected_ids = {f"bd-r5k32.{idx}" for idx in range(1, 15)}
    ...
    for row in closeouts:
        ...
        if child_id in seen:
            add_error("child_closeout_drift", f"duplicate child closeout {child_id}")
        if isinstance(child_id, str):
            seen.add(child_id)
        if child_id not in expected_ids:
            add_error("child_closeout_drift", f"unexpected child closeout id {child_id!r}")
        if status != "closed":
            add_error("child_closeout_drift", f"{child_id} status must be closed, got {status!r}")
        if len(fragments) < 2:
...
def validate_prioritizer_advancement(contract: dict[str, Any]) -> None:
    ...
    campaign_id = advancement.get("campaign_id")
    rpc = None
    for row in campaigns:
        ...
        if obj.get("campaign_id") == campaign_id:
            ...
            break
    if rpc is None:
        add_error("prioritizer_stale_wave", f"missing campaign {campaign_id}")
        rpc = {}
    completed = set(string_array(advancement.get("completed_wave_symbols"), "prioritizer_advancement.completed_wave_symbols", "prioritizer_stale_wave"))
    expected_next = string_array(advancement.get("expected_next_first_wave_symbols"), "prioritizer_advancement.expected_next_first_wave_symbols", "prioritizer_stale_wave")
    actual_next = string_array(rpc.get("first_wave_symbols"), "rpc.first_wave_symbols", "prioritizer_stale_wave")
    overlap = completed.intersection(actual_next)
    if overlap:
        add_error("prioritizer_stale_wave", f"completed RPC symbols still advertised as first-wave work: {sorted(overlap)}")
    if actual_next != expected_next:
        add_error("prioritizer_stale_wave", f"RPC next first_wave_symbols drift: expected {expected_next}, got {actual_next}")
    for key, expected_key in [
        ("current_coverage_pct", "expected_current_coverage_pct"),
        ("target_covered", "expected_target_covered"),
        ("target_uncovered", "expected_target_uncovered"),
    ]:
        if rpc.get(key) != advancement.get(expected_key):
            add_error("prioritizer_stale_wave", f"rpc.{key} expected {advancement.get(expected_key)!r}, got {rpc.get(key)!r}")
    events.append(
        event(
            ...
            next_first_wave_symbols=actual_next,
        )
    )
...
def validate_test_surface(contract: dict[str, Any]) -> None:
    ...
    test_path = ROOT / "crates/frankenlibc-harness/tests/second_conformance_backlog_completion_contract_test.rs"
    try:
...
def validate_telemetry(contract: dict[str, Any]) -> None:
    ...
    missing_events = sorted(required - emitted - {"second_conformance_backlog.completion_contract_validated"})
    ...
    sample = event("second_conformance_backlog.completion_contract_validated", "pass")
...
validate_child_closeouts(contract_obj)
validate_coverage(contract_obj)
validate_prioritizer_advancement(contract_obj)
validate_base_gates()
...
events.append(
    event(
        "second_conformance_backlog.completion_contract_validated" if status == "pass" else "second_conformance_backlog.completion_contract_failed",
        status,
        failure_signature,
...
report = {
    ...
    "errors": errors,
    "summary": {
        "closed_child_count": 14,
        "target_covered_symbols": 939,
        "symbols_with_fixtures": 937,
        "fixture_cases": 2320,
        "selected_target_uncovered_symbols": 2826,
    },
}
...
if status == "pass":
    print(f"PASS second conformance backlog completion contract ({REPORT})")
    sys.exit(0)

print(f"FAIL second conformance backlog completion contract: {failure_signature} ({REPORT})", file=sys.stderr)
for error in errors:
