#!/usr/bin/env bash
# Validate bd-3u0.1 workload matrix completion evidence.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
CONTRACT="${FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_CONTRACT:-${ROOT}/tests/conformance/workload_matrix_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_OUT_DIR:-${ROOT}/target/conformance/workload_matrix_completion_contract}"
REPORT="${FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_REPORT:-${OUT_DIR}/report.json}"
LOG="${FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_LOG:-${OUT_DIR}/events.jsonl}"
GATE_STDOUT="${FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_GATE_STDOUT:-${OUT_DIR}/gate_stdout.txt}"
GATE_STDERR="${FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_GATE_STDERR:-${OUT_DIR}/gate_stderr.txt}"
...
SCHEMA = "workload_matrix_completion_contract.v1"
REPORT_SCHEMA = "workload_matrix_completion_contract.report.v1"
BEAD_ID = "bd-3u0.1"
ORIGINAL_BEAD = "bd-3u0"
TRACE_ID = "bd-3u0.1::workload-matrix::v1"
...
UNIT_TESTS = {
    ...
    "workloads_have_required_fields",
    "subsystem_impact_consistent",
    "replacement_blockers_match_support_matrix",
    "milestones_reference_valid_workloads",
    "summary_consistent",
...
EXPECTED_SUMMARY = {
    ...
    "replace_blocked": 0,
    "top_blocker": None,
    "most_blocked_workload": None,
    "milestone_count": 6,
}
COMPLETION_EVENTS = {
    "workload_matrix_completion.source_artifacts",
    "workload_matrix_completion.unit_bindings",
    "workload_matrix_completion.e2e_gate_replayed",
    "workload_matrix_completion.telemetry_contract",
    "workload_matrix_completion.completion_contract_validated",
    "workload_matrix_completion.completion_contract_failed",
}
REPORT_FIELDS = {
    "schema_version",
    "bead_id",
...
def primary_signature() -> str:
    ...
    for signature in FAILURE_PRIORITY:
        if signature in present:
            return signature
    return "workload_matrix_completion_contract_failed"
    ...
    events.append(
        {
            ...
            "api_family": "hard_parts",
            "symbol": "workload_matrix",
            "decision_path": "completion_contract>workload_matrix_gate",
            "outcome": status,
...
def validate_sources(contract: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    if not any(error["failure_signature"] in {"malformed_contract", "missing_source_artifact"} for error in errors):
        append_event("workload_matrix_completion.source_artifacts", "pass", {"source_count": len(by_id)})
    return by_id
...
def validate_unit(contract: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    harness_text = read_text(ROOT / "crates/frankenlibc-harness/tests/workload_matrix_test.rs", "workload_matrix_test", "missing_unit_binding")
    for test_name in sorted(actual_tests):
        if f"fn {test_name}(" not in harness_text:
            add_error("missing_unit_binding", f"workload_matrix_test.rs missing fn {test_name}")
    require_commands_use_rch(unit.get("required_commands"), "unit_primary.required_commands", "missing_unit_binding")

    workload_matrix = load_json(ROOT / "tests/conformance/workload_matrix.json", "workload_matrix", "missing_unit_binding")
    summary = workload_matrix.get("summary", {}) if isinstance(workload_matrix.get("summary"), dict) else {}
    milestone_count = len(workload_matrix.get("milestone_mapping", {}).get("milestones", []))
    policy_summary = {**summary, "milestone_count": milestone_count}
    ...
    if not isinstance(required_summary, dict):
        ...
        for key, expected in EXPECTED_SUMMARY.items():
            ...
            if policy_summary.get(key) != expected:
                add_error("missing_unit_binding", f"workload_matrix.summary.{key} drifted: {policy_summary.get(key)} != {expected}")
    if not any(error["failure_signature"] == "missing_unit_binding" for error in errors):
        append_event(
            "workload_matrix_completion.unit_bindings",
            "pass",
            {"unit_test_count": len(actual_tests), "policy_summary": policy_summary},
        )
    return len(actual_tests), policy_summary
...
def validate_e2e(contract: dict[str, Any]) -> None:
    ...
    if e2e.get("gate_script") != "scripts/check_workload_matrix.sh":
        add_error("missing_e2e_binding", "e2e_primary.gate_script must be scripts/check_workload_matrix.sh")
    if e2e.get("workload_matrix") != "tests/conformance/workload_matrix.json":
        add_error("missing_e2e_binding", "e2e_primary.workload_matrix must be tests/conformance/workload_matrix.json")
    require_commands_use_rch(e2e.get("required_commands"), "e2e_primary.required_commands", "missing_e2e_binding")

    result = subprocess.run(
        ["bash", "scripts/check_workload_matrix.sh"],
        cwd=ROOT,
        text=True,
    ...
    if result.returncode != 0:
        add_error("base_gate_failed", f"check_workload_matrix.sh failed with status {result.returncode}: {result.stderr[-2000:]}")
        return
...
def finish(source_count: int, unit_count: int, policy_summary: dict[str, Any]) -> None:
    ...
    if status == "pass":
        append_event("workload_matrix_completion.completion_contract_validated", "pass")
    else:
        append_event("workload_matrix_completion.completion_contract_failed", "fail")
    report = {
    ...
    if errors:
        print(f"FAIL workload matrix completion contract errors={len(errors)}", file=sys.stderr)
        for error in errors[:16]:
            print(f"- {error['failure_signature']}: {error['message']}", file=sys.stderr)
        sys.exit(1)
    print(
        "PASS workload matrix completion contract "
