#!/usr/bin/env bash
# Validate bd-bp8fl.3.9.1 fpg online-control completion evidence.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT_PATH="${1:-${ROOT}/tests/conformance/fpg_online_control_completion_contract.v1.json}"
OUT_DIR="${2:-${ROOT}/target/conformance}"
REPORT_PATH="${OUT_DIR}/fpg_online_control_completion_contract.report.json"
LOG_PATH="${OUT_DIR}/fpg_online_control_completion_contract.log.jsonl"
...
EXPECTED_SCHEMA = "fpg_online_control_completion_contract.v1"
EXPECTED_BEAD = "bd-bp8fl.3.9"
EXPECTED_COMPLETION_BEAD = "bd-bp8fl.3.9.1"
...
EXPECTED_UNIT_TEST_REFS = {
    "gate_artifact_covers_online_control_rows",
...
EXPECTED_CONFORMANCE_TEST_REFS = {
    ...
    "gate_rejects_missing_online_control_anchor",
}
...
def append_event(event: str, status: str, details: dict[str, Any]) -> None:
    events.append(
        {
            "schema_version": "fpg_online_control_completion_contract.log.v1",
            "event": event,
            "status": status,
...
def validate_source_artifacts(manifest: dict[str, Any]) -> dict[str, pathlib.Path]:
    ...
    for key in sorted(EXPECTED_SOURCE_KEYS):
        ...
        if path is not None:
            paths[key] = path
    append_event(
        "fpg_online_control_completion.source_artifacts",
...
def validate_completion_evidence(
    ...
    append_event(
        "fpg_online_control_completion.evidence_refs",
        "fail" if errors else "pass",
        {
            "unit_refs": unit_refs,
...
def validate_source_gate_contract(manifest: dict[str, Any], paths: dict[str, pathlib.Path]) -> dict[str, Any]:
    ...
    if isinstance(policy, dict) and isinstance(expected_policy, dict):
        for key in (
            "default_decision",
            "allow_status_without_full_closure",
            "block_status_without_all_anchors",
            "block_replacement_levels_without_evidence",
            "missing_or_stale_result",
        ):
    ...
    append_event(
        "fpg_online_control_completion.source_gate_contract",
        "fail" if errors else "pass",
        {
            "gap_count": len(rows),
...
report = {
    "schema_version": "fpg_online_control_completion_contract.report.v1",
    "status": status,
    "bead": EXPECTED_COMPLETION_BEAD,
    "source_bead": EXPECTED_BEAD,
...
if errors:
    print(
        "FAIL fpg online-control completion contract: "
        + "; ".join(errors[:8]),
        file=sys.stderr,
    )
    sys.exit(1)
...
print(
    "PASS fpg online-control completion contract "
