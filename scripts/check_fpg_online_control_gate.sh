#!/usr/bin/env bash
# check_fpg_online_control_gate.sh -- bd-bp8fl.3.9
#
# Static fail-closed validator for the thirteen fpg-proof-online-control rows.
# Emits deterministic JSON and JSONL artifacts under target/conformance.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
GATE="${FRANKENLIBC_FPG_ONLINE_CONTROL_GATE:-${ROOT}/tests/conformance/fpg_online_control_gate.v1.json}"
...
REPORT="${FRANKENLIBC_FPG_ONLINE_CONTROL_REPORT:-${OUT_DIR}/fpg_online_control_gate.report.json}"
LOG="${FRANKENLIBC_FPG_ONLINE_CONTROL_LOG:-${OUT_DIR}/fpg_online_control_gate.log.jsonl}"
...
TRACE_ID = "bd-bp8fl.3.9:fpg-online-control"
EXPECTED_GAP_IDS = [
...
if isinstance(gate, dict):
    ...
    if gate.get("manifest_id") != "fpg-online-control-gate":
        fail("gate manifest_id must be fpg-online-control-gate")
    if gate.get("bead") != "bd-bp8fl.3.9":
        fail(f"gate bead must be bd-bp8fl.3.9, got {gate.get('bead')!r}")
    if gate.get("owner_family_group") != "fpg-proof-online-control":
    ...
    policy_ok = True
    if policy.get("default_decision") != "block_done_until_online_control_evidence_current":
        fail("claim_policy.default_decision must block DONE until online-control evidence is current")
        policy_ok = False
    ...
    if row_ids != EXPECTED_GAP_IDS:
        fail(f"gate row IDs must match online-control gap IDs in order: {row_ids!r}")
    for row in rows:
    ...
    if "fpg-proof-online-control" in owner_groups_text and "`bd-bp8fl.3.9`" in owner_groups_text:
        checks["owner_group_binding"] = "pass"
    else:
        fail("owner-family groups markdown must bind fpg-proof-online-control to bd-bp8fl.3.9")

    group_batches = groups.get("batches", []) if isinstance(groups, dict) else []
    batch = next((b for b in group_batches if b.get("batch_id") == "fpg-proof-online-control"), None)
    if batch is None:
        fail("feature_parity_gap_groups missing fpg-proof-online-control batch")
    elif batch.get("gap_ids") != EXPECTED_GAP_IDS or batch.get("gap_count") != len(EXPECTED_GAP_IDS):
        fail("feature_parity_gap_groups online-control batch must carry the thirteen expected gap IDs")
    ...
    for row in rows if isinstance(rows, list) else []:
        ...
        if row.get("replacement_level") in policy.get("block_replacement_levels_without_evidence", []):
            fail(f"{gid}: replacement_level {row.get('replacement_level')} is blocked for online-control gaps")
            claim_ok = False
...
report = {
    "schema_version": "v1",
    "bead": "bd-bp8fl.3.9",
    "manifest_id": "fpg-online-control-gate",
    "status": status,
