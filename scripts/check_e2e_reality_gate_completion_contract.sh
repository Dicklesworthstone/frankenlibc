...
CONTRACT="${FRANKENLIBC_E2E_REALITY_CONTRACT:-$ROOT/tests/conformance/e2e_reality_gate_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_E2E_REALITY_OUT_DIR:-$ROOT/target/conformance/e2e_reality_gate_completion_contract}"
REPORT="${FRANKENLIBC_E2E_REALITY_REPORT:-$OUT_DIR/e2e_reality_gate_completion_contract.report.json}"
LOG="${FRANKENLIBC_E2E_REALITY_LOG:-$OUT_DIR/e2e_reality_gate_completion_contract.log.jsonl}"
...
EXPECTED_SCHEMA = "e2e_reality_gate_completion_contract.v1"
EXPECTED_REPORT_SCHEMA = "e2e_reality_gate_completion_contract.report.v1"
EXPECTED_MANIFEST = "bd-mtj.1-e2e-reality-gate-completion-contract"
SOURCE_BEAD = "bd-mtj"
COMPLETION_BEAD = "bd-mtj.1"
REQUIRED_SOURCE_IDS = {
    "e2e_scenario_manifest",
    "e2e_manifest_contract",
    "e2e_manifest_checker",
    "e2e_manifest_test",
    "e2e_suite_script",
    "e2e_validator",
    "ld_preload_smoke_summary",
    "ld_preload_smoke_index",
    "ld_preload_smoke_script",
    "ld_preload_smoke_index_test",
    "real_program_smoke_suite",
    "real_program_smoke_contract",
    "real_program_smoke_checker",
    "real_program_smoke_test",
    "standalone_link_run_smoke",
    "standalone_link_run_smoke_contract",
    "standalone_link_run_smoke_checker",
    "standalone_link_run_smoke_test",
    "dual_mode_e2e_contract",
    "dual_mode_e2e_checker",
    "dual_mode_e2e_test",
    "strict_hardened_evidence",
    "strict_hardened_checker",
    "strict_hardened_test",
    "libc_map",
    "libc_map_export_golden",
    "libc_map_export_golden_test",
    "completion_checker",
    "completion_test",
}
REQUIRED_LANES = {
    ...
    events.append(
        {
            ...
            "failure_signature": "none" if status == "pass" else "e2e_reality_gate_completion_failed",
            "details": details or {},
        }
    )
...
def write_outputs(status: str, summary: dict[str, Any] | None = None) -> None:
    if status != "pass":
        append_event(
            "e2e_reality_completion_contract_fail",
            "fail",
            [rel(CONTRACT)],
...
print(
    "PASS: e2e reality gate completion contract "
