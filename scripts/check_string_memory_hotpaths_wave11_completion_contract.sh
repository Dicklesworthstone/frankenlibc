#!/usr/bin/env bash
# check_string_memory_hotpaths_wave11_completion_contract.sh -- bd-trfzu completion gate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT="${FRANKENLIBC_STRING_WAVE11_COMPLETION_CONTRACT:-${ROOT}/tests/conformance/string_memory_hotpaths_wave11_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_STRING_WAVE11_COMPLETION_OUT_DIR:-${ROOT}/target/conformance/string_memory_hotpaths_wave11_completion}"
REPORT="${FRANKENLIBC_STRING_WAVE11_COMPLETION_REPORT:-${OUT_DIR}/string_memory_hotpaths_wave11_completion_contract.report.json}"
LOG="${FRANKENLIBC_STRING_WAVE11_COMPLETION_LOG:-${OUT_DIR}/string_memory_hotpaths_wave11_completion_contract.events.jsonl}"
...
SCHEMA = "string_memory_hotpaths_wave11_completion_contract.v1"
REPORT_SCHEMA = "string_memory_hotpaths_wave11_completion_contract.report.v1"
BEAD_ID = "bd-trfzu"
TRACE_ID = "bd-trfzu::string-memory-hotpaths-wave11::completion::v1"
CAMPAIGN_ID = "fcq-string-memory-hotpaths"
WAVE_ID = "wave-11-string-memory-hotpaths"
FIXTURE_PATH = "tests/conformance/fixtures/string_memory_hotpaths_wave11.json"
FIXTURE_FILE = "string_memory_hotpaths_wave11.json"
...
FAILURE_PRIORITY = [
    ...
    "string_wave11_fixture_symbol_coverage",
    "string_wave11_fixture_mode_coverage",
    "string_wave11_fixture_schema",
...
def primary_signature() -> str:
    ...
    for signature in FAILURE_PRIORITY:
        if signature in present:
            return signature
    return "string_wave11_completion_contract_failed"
...
def validate_top_level(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    if completion.get("source_fixture_commit") != "4dcd65f9":
        add_error("malformed_contract", "source_fixture_commit mismatch")
    if completion.get("coverage_refresh_commit") != "eb256972":
        add_error("malformed_contract", "coverage_refresh_commit mismatch")
    if completion.get("coverage_close_commit") != "650e6ad8":
    ...
    if completion.get("harness_test") != "crates/frankenlibc-harness/tests/string_memory_hotpaths_wave11_conformance_test.rs":
...
def validate_fixture(completion: dict[str, Any]) -> None:
    start_errors = len(errors)
    fixture = as_object(load_json(resolve(FIXTURE_PATH), "fixture", "string_wave11_fixture_schema"), "fixture")
    ...
    summary["string_wave11_symbol_count"] = len(required_symbols)
    summary["required_mode_count"] = len(required_modes)
    cases = as_array(fixture.get("cases"), "fixture.cases", "string_wave11_fixture_schema")
    summary["fixture_case_count"] = len(cases)
    if fixture.get("family") != "string_memory_hotpaths_wave11":
        add_error("string_wave11_fixture_schema", "fixture family mismatch")
    campaign = as_object(fixture.get("campaign"), "fixture.campaign", "string_wave11_fixture_schema")
    ...
    if campaign.get("wave_id") != WAVE_ID:
        add_error("string_wave11_fixture_schema", "fixture wave_id mismatch")
    if campaign.get("ambient_state_policy") != AMBIENT_POLICY:
        add_error("string_wave11_fixture_schema", "fixture ambient policy mismatch")
    modes_by_symbol: dict[str, set[str]] = {symbol: set() for symbol in required_symbols}
    for case in cases:
        obj = as_object(case, "fixture.cases[]", "string_wave11_fixture_schema")
    ...
    if missing_modes:
        add_error("string_wave11_fixture_mode_coverage", f"missing fixture modes: {missing_modes}")
    expected_count = completion.get("expected_fixture_case_count")
    if expected_count != len(cases):
        add_error("string_wave11_fixture_schema", f"fixture case count {len(cases)} != expected {expected_count}")
    mark_check(
        start_errors,
        "string_wave11_fixture_validated",
        "string_wave11_fixture_symbol_coverage",
...
def validate_coverage(completion: dict[str, Any]) -> None:
    ...
    next_symbols = string_set(campaign.get("first_wave_symbols"), "string prioritizer first_wave_symbols", "coverage_accounting_drift")
    overlap = sorted(required_symbols & next_symbols)
    if overlap:
        add_error("completed_symbol_still_claimed", f"completed wave-11 symbols still listed as next work: {overlap}")
    mark_check(
        start_errors,
        "coverage_accounting_validated",
        "coverage_accounting_drift",
...
def validate_commands(completion: dict[str, Any]) -> None:
    ...
    required_needles = [
        ...
        "cargo check -p frankenlibc-harness --test string_memory_hotpaths_wave11_completion_contract_test",
        "cargo clippy -p frankenlibc-harness --test string_memory_hotpaths_wave11_completion_contract_test -- -D warnings",
        "cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave11_conformance_test",
    ]
...
def validate_tests(completion: dict[str, Any]) -> None:
    ...
    test_source = read_text(
        ROOT / "crates/frankenlibc-harness/tests/string_memory_hotpaths_wave11_completion_contract_test.rs",
...
events.append(event("string_wave11_completion_contract_validated", status, primary_signature() if errors else "none"))
