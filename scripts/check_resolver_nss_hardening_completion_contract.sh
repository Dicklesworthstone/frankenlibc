#!/usr/bin/env bash
# check_resolver_nss_hardening_completion_contract.sh - bd-1j4.3.1 completion evidence gate
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT="${FRANKENLIBC_RESOLVER_NSS_HARDENING_CONTRACT:-${ROOT}/tests/conformance/resolver_nss_hardening_completion_contract.v1.json}"
REPORT="${FRANKENLIBC_RESOLVER_NSS_HARDENING_REPORT:-${ROOT}/target/conformance/resolver_nss_hardening_completion_contract.report.json}"
LOG="${FRANKENLIBC_RESOLVER_NSS_HARDENING_LOG:-${ROOT}/target/conformance/resolver_nss_hardening_completion_contract.log.jsonl}"
...
ORIGINAL_BEAD = "bd-1j4.3"
COMPLETION_DEBT_BEAD = "bd-1j4.3.1"
...
def row(event: str, status: str = "pass", **fields: Any) -> dict[str, Any]:
    return {
        "timestamp": utc_now(),
        "trace_id": f"{COMPLETION_DEBT_BEAD}:resolver-nss-hardening",
        "event": event,
    ...
    for artifact_id, path_value in artifacts.items():
        ...
        if artifact_id in directory_artifacts:
            ...
            try:
                ...
                continue
        rows.append(row("resolver_nss_hardening.source_ref", artifact_id=artifact_id, path=path_value))
...
def validate_missing_bindings(
    ...
    for index, binding in enumerate(bindings):
        ...
        if isinstance(item_id, str) and isinstance(section, str):
            rows.append(row("resolver_nss_hardening.missing_item_bound", item_id=item_id, section=section))
...
def validate_required_artifacts(section: dict[str, Any], paths: dict[str, Path], label: str, errors: list[str]) -> None:
...
def validate_fuzz(
    ...
    if not any("fuzz_resolv" in command for command in commands):
        errors.append("fuzz_primary.commands must include fuzz_resolv")
    if not any("fuzz_resolver" in command for command in commands):
        errors.append("fuzz_primary.commands must include fuzz_resolver")
    ...
    if seen != {"fuzz_resolv_corpus", "fuzz_resolver_corpus"}:
        errors.append(f"fuzz corpus requirements must bind fuzz_resolv_corpus and fuzz_resolver_corpus, got {sorted(seen)}")
    return total
...
def validate_golden(
    ...
    fixture_id = golden.get("fixture")
    if not isinstance(fixture_id, str) or fixture_id not in paths:
        errors.append("golden_primary.fixture must reference a known artifact")
        return 0
    fixture = load_json(paths[fixture_id], "resolver fixture", errors)
    cases = fixture.get("cases")
    if not isinstance(cases, list):
        errors.append("resolver fixture cases must be an array")
        return 0
    case_by_name = {
        case.get("name"): case for case in cases if isinstance(case, dict) and isinstance(case.get("name"), str)
    }
    required_cases = require_string_list(golden.get("required_cases"), "golden_primary.required_cases", errors)
    for case_name in required_cases:
        if case_name not in case_by_name:
            errors.append(f"golden case missing from resolver fixture: {case_name}")
        else:
    ...
    retry = require_dict(golden.get("required_retry_policy"), "golden_primary.required_retry_policy", errors)
    actual_retry = require_dict(fixture.get("dns_protocol"), "resolver_fixture.dns_protocol", errors).get("retry_policy")
    actual_retry = require_dict(actual_retry, "resolver_fixture.dns_protocol.retry_policy", errors)
    for field, expected in retry.items():
        if actual_retry.get(field) != expected:
            errors.append(f"resolver retry policy {field} expected {expected}, got {actual_retry.get(field)}")
    modes = {case_by_name[name].get("mode") for name in required_cases if name in case_by_name}
    if "strict" not in modes or "hardened" not in modes:
        errors.append(f"golden cases must include strict and hardened modes, got {sorted(str(mode) for mode in modes)}")
...
if contract.get("schema_version") != "resolver_nss_hardening_completion_contract.v1":
    errors.append("schema_version must be resolver_nss_hardening_completion_contract.v1")
...
if unit_ref_count < 6:
    errors.append(f"unit_primary must bind at least 6 unit refs, got {unit_ref_count}")

e2e = require_dict(evidence.get("e2e_primary"), "e2e_primary", errors)
validate_required_artifacts(e2e, paths, "e2e_primary", errors)
e2e_commands = require_string_list(e2e.get("commands"), "e2e_primary.commands", errors)
if not any("run_hermetic_nss_resolver_lab.sh" in command for command in e2e_commands):
    errors.append("e2e_primary.commands must include hermetic NSS resolver lab runner")

fuzz_seed_count = validate_fuzz(evidence, paths, errors, rows)
golden_case_count = validate_golden(evidence, paths, errors, rows)

conformance = require_dict(evidence.get("conformance_primary"), "conformance_primary", errors)
validate_required_artifacts(conformance, paths, "conformance_primary", errors)
conformance_ref_count = validate_test_refs(conformance, texts, "conformance_primary", errors)
if conformance_ref_count < 3:
    errors.append(f"conformance_primary must bind at least 3 conformance refs, got {conformance_ref_count}")

validate_telemetry(evidence, errors)

status = "fail" if errors else "pass"
failure_signature = "resolver_nss_hardening_completion_contract_failed" if errors else "none"
report = {
    "schema_version": "resolver_nss_hardening_completion_contract.report.v1",
    "timestamp": utc_now(),
    "trace_id": f"{COMPLETION_DEBT_BEAD}:resolver-nss-hardening",
    "event": "resolver_nss_hardening.completion_contract_validated" if not errors else "resolver_nss_hardening.completion_contract_failed",
    "completion_debt_bead": COMPLETION_DEBT_BEAD,
    "original_bead": ORIGINAL_BEAD,
    "source_commit": source_commit,
    "status": status,
...
if errors:
    print(f"FAIL: resolver/NSS hardening completion contract errors={len(errors)}", file=sys.stderr)
    for error in errors:
        print(f"  - {error}", file=sys.stderr)
    sys.exit(1)

print(
    "PASS: resolver/NSS hardening completion contract "
    f"unit_refs={unit_ref_count} golden_cases={golden_case_count} "
