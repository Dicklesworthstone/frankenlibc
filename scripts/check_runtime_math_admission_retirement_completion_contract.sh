...
CONTRACT="${FRANKENLIBC_RUNTIME_MATH_ADMISSION_RETIREMENT_COMPLETION_CONTRACT:-$ROOT/tests/conformance/runtime_math_admission_retirement_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_RUNTIME_MATH_ADMISSION_RETIREMENT_COMPLETION_OUT_DIR:-$ROOT/target/conformance}"
REPORT="${FRANKENLIBC_RUNTIME_MATH_ADMISSION_RETIREMENT_COMPLETION_REPORT:-$OUT_DIR/runtime_math_admission_retirement_completion_contract.report.json}"
LOG="${FRANKENLIBC_RUNTIME_MATH_ADMISSION_RETIREMENT_COMPLETION_LOG:-$OUT_DIR/runtime_math_admission_retirement_completion_contract.log.jsonl}"
...
EXPECTED_SCHEMA = "runtime_math_admission_retirement_completion_contract.v1"
EXPECTED_REPORT_SCHEMA = "runtime_math_admission_retirement_completion_contract.report.v1"
ORIGINAL_BEAD = "bd-w2c3.5.3"
COMPLETION_BEAD = "bd-w2c3.5.3.1"
...
def validate_admission_report(manifest: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    ...
    require(report.get("bead") == expected.get("bead"), "admission report bead drifted")
    ...
    actual_policies = as_string_list(report.get("policies_enforced"), "admission_report.policies_enforced")
    expected_policies = as_string_list(expected.get("policies_enforced"), "required admission policies")
    require(
        sorted(actual_policies) == sorted(expected_policies),
        "admission policies must exactly match required set",
    )
    ...
    expected_integrity = as_string_list(expected.get("required_integrity_entries"), "required integrity entries")
    require(sorted(integrity.keys()) == sorted(expected_integrity), "artifact_integrity keys must exactly match required set")
    for entry_name in expected_integrity:
        ...
        if isinstance(entry, dict):
            ...
            require(
                ...
                f"artifact_integrity.{entry_name}.sha256 invalid",
            )
            require(
                isinstance(entry.get("size_bytes"), int) and entry.get("size_bytes", 0) > 0,
                f"artifact_integrity.{entry_name}.size_bytes invalid",
            )
    ...
    for entry in ledger:
        ...
        if entry.get("tier") == "production_monitor":
            require(entry.get("admission_status") == "ADMITTED", f"production monitor module not admitted: {entry.get('module')}")
    ...
    return {
        ...
        "policy_count": len(actual_policies),
...
def validate_supporting_artifacts(manifest: dict[str, Any], loaded: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    governance_counts = {
        "production_core": len(classifications.get("production_core", [])) if isinstance(classifications.get("production_core"), list) else 0,
        "production_monitor": len(classifications.get("production_monitor", [])) if isinstance(classifications.get("production_monitor"), list) else 0,
        "research": len(classifications.get("research", [])) if isinstance(classifications.get("research"), list) else 0,
    }
    ...
    if isinstance(production_modules, list) and isinstance(research_only, list):
        overlap = set(production_modules) & set(research_only)
        require(not overlap, f"production and research-only manifests overlap: {sorted(overlap)}")
    ...
    for key in ["production_core_assessments", "production_monitor_assessments"]:
        rows = value.get(key, [])
        require(isinstance(rows, list) and rows, f"math value proof {key} must be non-empty")
        if isinstance(rows, list):
            all_assessments.extend(rows)
    threshold = expected_value.get("retention_threshold")
    for row in all_assessments:
        if isinstance(row, dict):
            module = row.get("module", "?")
            score = row.get("score")
            verdict = row.get("verdict")
            require(verdict == "retain", f"math value proof module not retained: {module}")
            require(isinstance(score, (int, float)) and score >= threshold, f"math value proof module below retention threshold: {module}")
...
def validate_missing_item_bindings(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    require(ids == {"tests.unit.primary", "tests.e2e.primary"}, "missing item bindings must be exactly unit + e2e")
    ...
    for binding in bindings:
        ...
        if binding_id == "tests.unit.primary":
            require(any("runtime_math_admission_retirement_completion_contract_test" in command for command in commands), "unit binding must reference completion contract test")
            ...
            require(any("math_value_proof_test" in command for command in commands), "unit binding must reference math_value_proof_test")
            require(any("math_retirement_test" in command for command in commands), "unit binding must reference math_retirement_test")
        if binding_id == "tests.e2e.primary":
            for required_script in [
                "check_runtime_math_admission_retirement_completion_contract.sh",
                "check_runtime_math_admission.sh",
                "check_math_value_proof.sh",
                "check_math_retirement.sh",
                "check_math_production_set_policy.sh",
            ]:
                require(any(required_script in command for command in commands), f"e2e binding must reference {required_script}")
...
events = [
    event(
        "runtime_math_admission_retirement_completion_summary",
        "pass",
        "admission_value_retirement_checked",
        artifact_refs,
        admission=admission_summary,
        supporting=supporting_summary,
    ),
    event(
        "runtime_math_admission_retirement_bindings",
        "pass",
        "unit_e2e_bound",
        artifact_refs,
        ci=ci_summary,
    ...
    event(
        "runtime_math_admission_retirement_completion_contract_pass",
        "pass",
        "ready_for_closeout",
        artifact_refs,
...
if status == "fail":
    events = [
        event(
            "runtime_math_admission_retirement_completion_contract_fail",
            "fail",
            "contract_rejected",
            artifact_refs,
...
if errors:
    print(f"runtime_math_admission_retirement_completion_contract: FAIL errors={len(errors)}")
    for message in errors:
        print(f"ERROR: {message}")
    raise SystemExit(1)

print(
    "runtime_math_admission_retirement_completion_contract: PASS "
    f"modules={admission_summary['total_modules']} "
    f"admitted={admission_summary['admitted']} "
    f"retired={admission_summary['retired']} "
    f"value_assessed={supporting_summary['value_assessed']} "
    f"retirement={supporting_summary['retirement_policy_status']} "
