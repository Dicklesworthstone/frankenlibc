#!/usr/bin/env bash
# check_cxa_thread_atexit_init_array_completion_contract.sh - bd-yf86e.1 completion gate
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
CONTRACT="${FRANKENLIBC_CXA_THREAD_ATEXIT_INIT_ARRAY_COMPLETION_CONTRACT:-$ROOT/tests/conformance/cxa_thread_atexit_init_array_completion_contract.v1.json}"
REPORT="${FRANKENLIBC_CXA_THREAD_ATEXIT_INIT_ARRAY_COMPLETION_REPORT:-$ROOT/target/conformance/cxa_thread_atexit_init_array_completion_contract.report.json}"
LOG="${FRANKENLIBC_CXA_THREAD_ATEXIT_INIT_ARRAY_COMPLETION_LOG:-$ROOT/target/conformance/cxa_thread_atexit_init_array_completion_contract.log.jsonl}"
...
SCHEMA = "cxa_thread_atexit_init_array_completion_contract.v1"
MANIFEST_ID = "bd-yf86e.1-cxa-thread-atexit-init-array-completion-contract"
ORIGINAL_BEAD = "bd-yf86e"
COMPLETION_BEAD = "bd-yf86e.1"
MISSING_ITEM = "tests.conformance.primary"
VALIDATED_EVENT = "cxa_thread_atexit_init_array_completion_contract_validated"
INVARIANT_EVENT = "cxa_thread_atexit_init_array_source_invariant"
FAILED_EVENT = "cxa_thread_atexit_init_array_completion_contract_failed"
...
emit(
    ...
    artifact_refs=[
        ...
        str(source_artifacts.get("startup_abi_tests", "")),
        str(source_artifacts.get("unistd_abi_tests", "")),
    ],
    failure_signature="" if not errors else "cxa_thread_atexit_init_array_completion_contract_failed",
)
...
report = {
    "schema_version": "cxa_thread_atexit_init_array_completion_contract.report.v1",
...
if errors:
    print("check_cxa_thread_atexit_init_array_completion_contract: FAIL", flush=True)
    for message in errors:
        print(f"FAIL: {message}", flush=True)
    raise SystemExit(1)

print("check_cxa_thread_atexit_init_array_completion_contract: PASS")
PY
