#!/usr/bin/env bash
# check_string_memory_hotpaths_wave06_completion_contract.sh -- bd-56m5d.2 completion gate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT="${FRANKENLIBC_STRING_WAVE06_COMPLETION_CONTRACT:-${ROOT}/tests/conformance/string_memory_hotpaths_wave06_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_STRING_WAVE06_COMPLETION_OUT_DIR:-${ROOT}/target/conformance/string_memory_hotpaths_wave06_completion}"
REPORT="${FRANKENLIBC_STRING_WAVE06_COMPLETION_REPORT:-${OUT_DIR}/string_memory_hotpaths_wave06_completion_contract.report.json}"
LOG="${FRANKENLIBC_STRING_WAVE06_COMPLETION_LOG:-${OUT_DIR}/string_memory_hotpaths_wave06_completion_contract.events.jsonl}"
...
SCHEMA = "string_memory_hotpaths_wave06_completion_contract.v1"
REPORT_SCHEMA = "string_memory_hotpaths_wave06_completion_contract.report.v1"
BEAD_ID = "bd-56m5d.2"
EPIC_ID = "bd-56m5d"
TRACE_ID = "bd-56m5d.2::string-memory-hotpaths-wave06::completion::v1"
CAMPAIGN_ID = "fcq-string-memory-hotpaths"
WAVE_ID = "wave-06-string-memory-hotpaths"
FIXTURE_FILE = "string_memory_hotpaths_wave06.json"
FIXTURE_PATH = "tests/conformance/fixtures/string_memory_hotpaths_wave06.json"
...
REQUIRED_ARTIFACT_IDS = {
    ...
    "string_wave06_fixture",
    "fixture_executor",
    "string_wave06_harness_test",
    "completion_contract",
    "completion_gate",
...
REQUIRED_PROOFS = {
    "string-wave06-code-proof": {"commit": "61e7ae41"},
    "bd-56m5d.1": {"commit": "4280d5c9", "tracker_close_commit": "36dcbfb4"},
}
REQUIRED_TELEMETRY_EVENTS = {
    "source_artifacts_validated",
    "dependency_proofs_validated",
    "string_wave06_fixture_validated",
    "coverage_accounting_validated",
    "validation_commands_validated",
    "test_surfaces_validated",
    "telemetry_contract_validated",
    "string_wave06_completion_contract_validated",
}
REQUIRED_POSITIVE_TESTS = {
    "contract_binds_string_wave06_sources",
    "checker_accepts_string_wave06_completion_contract",
    "checker_emits_structured_string_wave06_telemetry",
}
REQUIRED_NEGATIVE_TESTS = {
    "checker_rejects_missing_string_wave06_fixture_symbol",
    "checker_rejects_stale_string_wave06_coverage_accounting",
...
FAILURE_PRIORITY = [
    ...
    "string_wave06_fixture_symbol_coverage",
    "string_wave06_fixture_mode_coverage",
    "string_wave06_fixture_schema",
...
def primary_signature() -> str:
    ...
    for signature in FAILURE_PRIORITY:
        if signature in present:
            return signature
    return "string_wave06_completion_contract_failed"
...
def validate_dependency_proofs(completion: dict[str, Any]) -> None:
    ...
    mark_check(start_errors, "dependency_proofs_validated", "dependency_proof_missing", proof_count=len(proofs))
...
def validate_fixture(completion: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> tuple[set[str], set[str]]:
    ...
    summary["string_wave06_symbol_count"] = len(required_symbols)
    summary["required_mode_count"] = len(required_modes)
    if completion.get("fixture_file") != FIXTURE_PATH:
        add_error("string_wave06_fixture_schema", "fixture_file mismatch")
    fixture = load_json(artifact_path(artifacts, "string_wave06_fixture", FIXTURE_PATH), "string wave06 fixture", "string_wave06_fixture_schema")
    campaign = as_object(fixture.get("campaign"), "fixture.campaign", "string_wave06_fixture_schema")
    if fixture.get("version") != "v1" or fixture.get("family") != "string_memory_hotpaths_wave06":
        add_error("string_wave06_fixture_schema", "fixture version/family mismatch")
    if campaign.get("campaign_id") != CAMPAIGN_ID or campaign.get("wave_id") != WAVE_ID:
        add_error("string_wave06_fixture_schema", "fixture campaign mismatch")
    if campaign.get("ambient_state_policy") != AMBIENT_POLICY:
        add_error("string_wave06_fixture_schema", "fixture ambient-state policy mismatch")
    declared = string_set(campaign.get("first_wave_symbols"), "fixture.campaign.first_wave_symbols", "string_wave06_fixture_symbol_coverage")
    if declared != required_symbols:
        add_error("string_wave06_fixture_symbol_coverage", f"fixture first-wave symbols drifted: missing={sorted(required_symbols - declared)} extra={sorted(declared - required_symbols)}")
    if as_array(campaign.get("residual_symbols"), "fixture.campaign.residual_symbols", "string_wave06_fixture_schema"):
        add_error("string_wave06_fixture_schema", "fixture residual_symbols must be empty")
    structured = string_set(fixture.get("structured_log_fields"), "fixture.structured_log_fields", "string_wave06_fixture_schema")
    ...
    if not required_fields <= structured:
        add_error("string_wave06_fixture_schema", f"fixture structured fields missing {sorted(required_fields - structured)}")
    ...
    cases = as_array(fixture.get("cases"), "fixture.cases", "string_wave06_fixture_schema")
    summary["fixture_case_count"] = len(cases)
    for case in cases:
        obj = as_object(case, "fixture.cases[]", "string_wave06_fixture_schema")
        ...
        if mode not in required_modes:
            add_error("string_wave06_fixture_mode_coverage", f"unsupported fixture mode {mode} for {symbol}")
        if obj.get("expected_errno") != 0:
            add_error("string_wave06_fixture_schema", f"case {obj.get('name')} exposes errno")
        inputs = as_object(obj.get("inputs"), "fixture.cases[].inputs", "string_wave06_fixture_schema")
        if inputs.get("symbol") != symbol:
            add_error("string_wave06_fixture_schema", f"case {obj.get('name')} inputs.symbol mismatch")
        if inputs.get("ambient_state_policy") != AMBIENT_POLICY:
            add_error("string_wave06_fixture_schema", f"case {obj.get('name')} ambient policy mismatch")
        if isinstance(expected_output, str):
            for field in required_fields:
                if f"{field}=" not in expected_output:
                    add_error("string_wave06_fixture_schema", f"case {obj.get('name')} expected_output missing {field}")
            for token in forbidden_tokens:
                if token in expected_output:
                    add_error("string_wave06_fixture_schema", f"case {obj.get('name')} leaks ambient token {token}")
        else:
            add_error("string_wave06_fixture_schema", f"case {obj.get('name')} expected_output must be a string")
    for symbol in required_symbols:
        modes = modes_by_symbol.get(symbol, set())
        if modes != required_modes:
            add_error("string_wave06_fixture_mode_coverage", f"{symbol} modes {sorted(modes)} != {sorted(required_modes)}")
    mark_check(
        start_errors,
        "string_wave06_fixture_validated",
        "string_wave06_fixture_symbol_coverage",
...
def find_by_key(rows: Any, key: str, expected: str, module: str | None = None) -> dict[str, Any] | None:
    for row in as_array(rows, f"rows[{key}]", "coverage_accounting_drift"):
        obj = as_object(row, "coverage row", "coverage_accounting_drift")
        if obj.get(key) == expected and (module is None or obj.get("module") == module):
            return obj
    return None
...
def validate_coverage(
    ...
    if campaign is None:
        ...
        if float(campaign.get("current_coverage_pct", 0.0)) < float(accounting.get("campaign_current_coverage_pct_min", 0.0)):
            add_error("coverage_accounting_drift", "string coverage percent below wave-06 floor")
        if int(campaign.get("target_covered", 0)) < int(accounting.get("campaign_target_covered_min", 0)):
            add_error("coverage_accounting_drift", "string target_covered below wave-06 floor")
        if int(campaign.get("target_uncovered", 999999)) > int(accounting.get("campaign_target_uncovered_max", 999999)):
            add_error("coverage_accounting_drift", "string target_uncovered above wave-06 ceiling")
    ...
    for symbol in required_symbols:
        row = find_by_key(coverage.get("symbols"), "symbol", symbol, "string_abi")
        if row is None:
        ...
        per_row = find_by_key(per_symbol.get("per_symbol_report"), "symbol", symbol, "string_abi")
        if per_row is None:
    ...
    mark_check(start_errors, "coverage_accounting_validated", "coverage_accounting_drift", campaign_id=CAMPAIGN_ID)
...
def validate_validation_commands(completion: dict[str, Any]) -> None:
    ...
    required_needles = [
        "jq empty tests/conformance/string_memory_hotpaths_wave06_completion_contract.v1.json",
        "bash -n scripts/check_string_memory_hotpaths_wave06_completion_contract.sh",
        "bash scripts/check_string_memory_hotpaths_wave06_completion_contract.sh",
        "bash scripts/check_symbol_fixture_coverage.sh",
        "bash scripts/check_per_symbol_fixture_tests.sh --validate-only",
        "bash scripts/check_fixture_coverage_prioritizer.sh",
        "AGENT_NAME=BrownTern br --no-db dep cycles --json",
        " cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave06_completion_contract_test ",
        " cargo check -p frankenlibc-harness --test string_memory_hotpaths_wave06_completion_contract_test",
        " cargo clippy -p frankenlibc-harness --test string_memory_hotpaths_wave06_completion_contract_test ",
        " cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave06_conformance_test ",
    ]
    joined = "\n".join(command_texts)
    for needle in required_needles:
        if needle not in joined:
            add_error("missing_validation_command", f"missing validation command containing: {needle}")
    mark_check(start_errors, "validation_commands_validated", "non_rch_validation_command", command_count=len(command_texts))


def validate_test_surfaces(
    ...
    harness_text = read_text(
        artifact_path(artifacts, "string_wave06_harness_test", "crates/frankenlibc-harness/tests/string_memory_hotpaths_wave06_conformance_test.rs"),
        "string wave06 harness test",
        "missing_test_binding",
    )
    completion_text = read_text(
        artifact_path(artifacts, "completion_harness_test", "crates/frankenlibc-harness/tests/string_memory_hotpaths_wave06_completion_contract_test.rs"),
...
def validate_telemetry_contract(completion: dict[str, Any]) -> None:
    ...
    mark_check(start_errors, "telemetry_contract_validated", "missing_telemetry_event", declared_event_count=len(declared))
...
validate_test_surfaces(contract, completion, artifacts)
...
for row in events:
    ...
    elif event_name == "string_wave06_fixture_validated":
        checks["string_wave06_fixture"] = "fail"
...
if status == "pass":
    print(
        "check_string_memory_hotpaths_wave06_completion_contract: PASS "
        f"symbols={summary['string_wave06_symbol_count']} cases={summary['fixture_case_count']} "
        f"events={len(events)} report={rel(REPORT)}"
    )
else:
    print(
        "check_string_memory_hotpaths_wave06_completion_contract: FAIL "
