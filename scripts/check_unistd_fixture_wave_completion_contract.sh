#!/usr/bin/env bash
# check_unistd_fixture_wave_completion_contract.sh -- bd-qce2t.3 completion gate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT="${FRANKENLIBC_UNISTD_FIXTURE_WAVE_CONTRACT:-${ROOT}/tests/conformance/unistd_fixture_wave_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_UNISTD_FIXTURE_WAVE_OUT_DIR:-${ROOT}/target/conformance/unistd_fixture_wave_completion}"
REPORT="${FRANKENLIBC_UNISTD_FIXTURE_WAVE_REPORT:-${OUT_DIR}/unistd_fixture_wave_completion_contract.report.json}"
LOG="${FRANKENLIBC_UNISTD_FIXTURE_WAVE_LOG:-${OUT_DIR}/unistd_fixture_wave_completion_contract.events.jsonl}"
...
SCHEMA = "unistd_fixture_wave_completion_contract.v1"
REPORT_SCHEMA = "unistd_fixture_wave_completion_contract.report.v1"
BEAD_ID = "bd-qce2t.3"
EPIC_ID = "bd-qce2t"
TRACE_ID = "bd-qce2t.3::unistd-fixture-wave::completion::v1"
CAMPAIGN_ID = "fcq-unistd-process-filesystem"
FIXTURE_FILE = "unistd_process_filesystem.json"
...
REQUIRED_TELEMETRY_EVENTS = {
    ...
    "unistd_fixture_validated",
    "coverage_accounting_validated",
    "validation_commands_validated",
    "test_surfaces_validated",
    "telemetry_contract_validated",
    "unistd_fixture_wave_completion_contract_validated",
}
REQUIRED_POSITIVE_TESTS = {
    "contract_binds_unistd_fixture_wave_sources",
    "checker_accepts_unistd_fixture_wave_completion_contract",
    "checker_emits_structured_unistd_completion_telemetry",
}
REQUIRED_NEGATIVE_TESTS = {
    "checker_rejects_missing_unistd_fixture_symbol",
    "checker_rejects_stale_unistd_coverage_accounting",
...
FAILURE_PRIORITY = [
    ...
    "unistd_fixture_symbol_coverage",
    "unistd_fixture_mode_coverage",
    "unistd_fixture_schema",
...
def primary_signature() -> str:
    ...
    for signature in FAILURE_PRIORITY:
        if signature in present:
            return signature
    return "unistd_fixture_wave_completion_contract_failed"
...
def commit_binding_valid(commit: Any) -> bool:
    if not isinstance(commit, str) or len(commit) < 7:
        return False
    return all(ch in "0123456789abcdefABCDEF" for ch in commit)
...
def validate_dependency_proofs(completion: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> None:
    ...
    for bead_id in ["bd-qce2t.1", "bd-qce2t.2"]:
        ...
        if not ledger_closed and not proof_committed:
            missing.append(bead_id)
        if not commit_binding_valid(commit):
...
def validate_unistd_fixture(completion: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> None:
    fixture = load_json(
        resolve(artifacts.get("unistd_fixture", {}).get("path", "tests/conformance/fixtures/unistd_process_filesystem.json")),
        "unistd fixture",
        "unistd_fixture_schema",
    )
    ...
    if fixture.get("family") != "unistd/process-filesystem":
        add_error("unistd_fixture_schema", "unistd fixture family mismatch")
    campaign = as_object(fixture.get("campaign"), "unistd_fixture.campaign", "unistd_fixture_schema")
    if campaign.get("bead") != "bd-qce2t.1" or campaign.get("campaign_id") != CAMPAIGN_ID:
        add_error("unistd_fixture_schema", "unistd fixture campaign binding mismatch")
    if campaign.get("ambient_state_policy") != "forbid_exact_process_or_filesystem_metadata":
        add_error("unistd_fixture_schema", "unistd fixture ambient_state_policy mismatch")
    ...
    if set(fixture.get("structured_log_fields", [])) != required_fields:
        add_error("unistd_fixture_schema", "structured_log_fields mismatch")

    modes_by_symbol: dict[str, set[str]] = {symbol: set() for symbol in required_symbols}
    case_count = 0
    for case in as_array(fixture.get("cases"), "unistd_fixture.cases", "unistd_fixture_schema"):
        obj = as_object(case, "unistd_fixture.cases[]", "unistd_fixture_schema")
        symbol = obj.get("function")
        mode = obj.get("mode")
        if isinstance(symbol, str) and isinstance(mode, str):
            case_count += 1
            modes_by_symbol.setdefault(symbol, set()).add(mode)
        inputs = as_object(obj.get("inputs"), f"unistd_fixture.case[{symbol}].inputs", "unistd_fixture_schema")
        for field in ["symbol", "expected", "ambient_state_policy", "oracle_source"]:
            if not isinstance(inputs.get(field), str) or not inputs.get(field):
                add_error("unistd_fixture_schema", f"case {obj.get('name')} missing inputs.{field}")
        if isinstance(symbol, str) and inputs.get("symbol") != symbol:
            add_error("unistd_fixture_schema", f"case {obj.get('name')} inputs.symbol mismatch")
        expected_output = obj.get("expected_output")
        if not isinstance(expected_output, str):
            add_error("unistd_fixture_schema", f"case {obj.get('name')} missing expected_output")
            expected_output = ""
        for field in sorted(required_fields):
            if f"{field}=" not in expected_output:
                add_error("unistd_fixture_schema", f"case {obj.get('name')} expected_output missing {field} token")
        for token in forbidden_tokens:
            if token in expected_output:
                add_error("unistd_fixture_schema", f"case {obj.get('name')} leaks ambient token {token}")
        if obj.get("expected_errno") != 0:
            add_error("unistd_fixture_schema", f"case {obj.get('name')} expected_errno must be 0")
    ...
    if mode_failures:
        add_error("unistd_fixture_mode_coverage", f"symbols missing required modes: {mode_failures}")

    events.append(
        event(
            "unistd_fixture_validated",
            "pass" if not mode_failures else "fail",
            "none" if not mode_failures else "unistd_fixture_mode_coverage",
...
def validate_coverage_accounting(completion: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> None:
    ...
    if "tests/conformance/fixtures/unistd_process_filesystem.json" not in top_wave.get("fixture_files", []):
        add_error("coverage_accounting_drift", "top blocker wave plan missing unistd fixture file")

    per_symbol_rows = {
        row.get("symbol"): row
        for row in as_array(per_symbol.get("per_symbol_report"), "per_symbol.per_symbol_report", "coverage_accounting_drift")
        if isinstance(row, dict) and row.get("module") == "unistd_abi"
    }
    coverage_rows = {
        row.get("symbol"): row
...
def validate_validation_commands(completion: dict[str, Any]) -> None:
    ...
    required_fragments = [
        "cargo test -p frankenlibc-harness --test unistd_fixture_wave_completion_contract_test",
        "cargo check -p frankenlibc-harness --test unistd_fixture_wave_completion_contract_test",
        "cargo clippy -p frankenlibc-harness --test unistd_fixture_wave_completion_contract_test -- -D warnings",
    ]
    ...
    if not any("br dep cycles --no-db --json" in command for command in commands):
        add_error("non_rch_validation_command", "missing br dep cycles --no-db validation command")
    events.append(
        event(
            "validation_commands_validated",
            "pass",
...
def validate_test_surfaces(artifacts: dict[str, dict[str, Any]]) -> None:
    text = read_text(
        resolve(artifacts.get("completion_harness_test", {}).get("path", "crates/frankenlibc-harness/tests/unistd_fixture_wave_completion_contract_test.rs")),
...
validate_unistd_fixture(completion, artifacts)
...
checks = {
    ...
    "unistd_fixture": "pass" if not any(e["failure_signature"].startswith("unistd_fixture") for e in errors) else "fail",
...
report = {
    ...
    "summary": {
        ...
        "unistd_first_wave_symbol_count": len(completion.get("required_first_wave_symbols", [])),
...
print(
    "PASS: unistd fixture wave completion contract "
    f"symbols={report['summary']['unistd_first_wave_symbol_count']} "
