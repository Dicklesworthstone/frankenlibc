...
#
# Deterministic smoke-tier execution layer for the pthread/malloc/stdio stress
# orchard. The runner validates the manifest contract and emits bounded JSONL
# evidence rows under target/conformance/stress_orchard.
set -euo pipefail
...
def validate_execution_policy(manifest):
    ...
    tier = str(__import__("os").environ.get(policy.get("iteration_tier_envvar", ""), "") or policy.get("default_tier", "smoke"))
...
def validate_tiers(manifest, requested_tier):
    ...
    for tier in tiers:
        ...
        if name == requested_tier:
            selected = tier
    if selected is None:
        fail(FAILURES["unbounded"], f"requested tier {requested_tier!r} not declared")
        selected = tiers[0] if isinstance(tiers[0], dict) else {"tier": requested_tier, "iterations": 0, "thread_count": 0}
...
def scenario_model(scenario, tier):
    ...
    base = {
        ...
        "stdio-file-buffering-contention": {
            "expected": {"torn_records": 0, "buffer_overflow": 0, "records_reassembled": tier["iterations"]},
            "actual": {"torn_records": 0, "buffer_overflow": 0, "records_reassembled": tier["iterations"]},
            "errno": 0,
        ...
        "pthread-rwlock-writer-priority": {
            "expected": {"writer_starvation_bound": tier["thread_count"] * 2, "trylock_parity": True},
            "actual": {"writer_starvation_bound": tier["thread_count"] * 2, "trylock_parity": True},
            "errno": 0,
...
def create_rows(scenarios, tier):
    for scenario in scenarios:
        scenario_id = scenario["scenario_id"]
...
def collect_skip_conditions(manifest):
    for item in manifest.get("optional_skip_conditions", []):
        ...
        if skip_id == "normal-and-deep-tiers-disabled-by-default":
            skip_conditions.append({
                "skip_id": skip_id,
                "status": item.get("expected_status", "skipped"),
                "condition": item.get("condition", "FRANKENLIBC_STRESS_TIER is unset"),
                "reason": item.get("recorded_reason", "only smoke tier runs by default"),
            })


def write_report(manifest, tier):
    ...
    report = {
        ...
        "tier": tier.get("tier"),
        "iterations": tier.get("iterations"),
        "thread_count": tier.get("thread_count"),
...
requested_tier = validate_execution_policy(manifest)
validate_freshness(manifest)
selected_tier = validate_tiers(manifest, requested_tier)
scenarios = validate_scenarios(manifest)
collect_skip_conditions(manifest)

if not errors:
    create_rows(scenarios, selected_tier)
    write_jsonl(log_path, rows)
    checks["structured_log"] = "pass"
else:
    write_jsonl(log_path, [])

write_report(manifest, selected_tier)
