#!/usr/bin/env bash
# check_standalone_link_run_smoke_dashboard_completion_contract.sh - bd-zyck1.113.1 completion gate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
CONTRACT="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_CONTRACT:-$ROOT/tests/conformance/standalone_link_run_smoke_dashboard_completion_contract.v1.json}"
DASHBOARD="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_DASHBOARD:-$ROOT/tests/conformance/l1_dry_run_readiness_dashboard.v1.json}"
OUT_DIR="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_OUT_DIR:-$ROOT/target/conformance/standalone_link_run_smoke_dashboard_completion_contract}"
REPORT="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_REPORT:-$OUT_DIR/report.json}"
LOG="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_LOG:-$OUT_DIR/events.jsonl}"
GATE_DIR="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_GATE_DIR:-$OUT_DIR/source_gates}"
SMOKE_REPORT="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_SMOKE_REPORT:-$GATE_DIR/standalone_link_run_smoke.report.json}"
SMOKE_LOG="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_SMOKE_LOG:-$GATE_DIR/standalone_link_run_smoke.log.jsonl}"
SMOKE_TARGET_DIR="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_SMOKE_TARGET_DIR:-$GATE_DIR/standalone_link_run_smoke}"
SMOKE_COMPLETION_REPORT="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_SMOKE_COMPLETION_REPORT:-$GATE_DIR/standalone_link_run_smoke_completion.report.json}"
SMOKE_COMPLETION_LOG="${FRANKENLIBC_STANDALONE_SMOKE_DASHBOARD_COMPLETION_SMOKE_COMPLETION_LOG:-$GATE_DIR/standalone_link_run_smoke_completion.log.jsonl}"

mkdir -p "$(dirname "$REPORT")" "$(dirname "$LOG")" "$GATE_DIR" "$SMOKE_TARGET_DIR"
...
SMOKE_COMPLETION_REPORT="$SMOKE_COMPLETION_REPORT" \
SMOKE_COMPLETION_LOG="$SMOKE_COMPLETION_LOG" \
python3 - <<'PY'
...
SMOKE_COMPLETION_REPORT = pathlib.Path(os.environ["SMOKE_COMPLETION_REPORT"])
SMOKE_COMPLETION_LOG = pathlib.Path(os.environ["SMOKE_COMPLETION_LOG"])

EXPECTED_SCHEMA = "standalone_link_run_smoke_dashboard_completion_contract.v1"
EXPECTED_REPORT_SCHEMA = "standalone_link_run_smoke_dashboard_completion_contract.report.v1"
EXPECTED_BEAD = "bd-zyck1.113.1"
EXPECTED_ORIGINAL_BEAD = "bd-zyck1.113"
EXPECTED_TRACE_ID = "bd-zyck1.113.1::standalone-link-run-smoke-dashboard::completion::v1"
EXPECTED_MISSING_ITEMS = {"tests.conformance.primary", "telemetry.primary"}
REQUIRED_SOURCE_ARTIFACTS = {
    "l1_dashboard",
    "l1_dashboard_harness",
    "standalone_smoke_manifest",
    "standalone_smoke_gate",
    "standalone_smoke_harness",
    "standalone_smoke_completion_contract",
    "standalone_smoke_completion_gate",
    "standalone_smoke_completion_harness",
    "completion_checker",
    "completion_harness",
}
PASS_EVENTS = {
    ...
    events.append(
        {
            ...
            "stream": "conformance",
            "gate": "standalone_link_run_smoke_dashboard_completion_contract",
            "scenario_id": event,
            "mode": "strict",
            "api_family": "standalone",
            "symbol": "standalone_link_run_smoke",
            "oracle_kind": "l1_dashboard_completion",
            "expected": "pass",
            "actual": outcome,
            "decision_path": "dashboard->standalone_smoke->telemetry",
            "outcome": outcome,
            "errno": 0,
            "latency_ns": 0,
...
def run_command(command: list[str], *, marker: str, label: str, env: dict[str, str] | None = None) -> None:
...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if keys != REQUIRED_SOURCE_ARTIFACTS:
        err(f"source_artifacts mismatch: missing={sorted(REQUIRED_SOURCE_ARTIFACTS - keys)} extra={sorted(keys - REQUIRED_SOURCE_ARTIFACTS)}")
    for key, path_text in artifacts.items():
        repo_path(path_text, f"source_artifacts.{key}")
    emit("standalone_smoke_dashboard_sources_validated", source_artifact_count=len(keys))
...
def validate_dashboard(dashboard: dict[str, Any], required: dict[str, Any]) -> dict[str, Any]:
    ...
    prefix = str(required.get("standalone_smoke_row_prefix", ""))
    smoke_rows = [row for row in rows if isinstance(row, dict) and str(row.get("row_id", "")).startswith(prefix)]
    required_count = int(required.get("standalone_smoke_row_count", 0))
    if len(smoke_rows) != required_count:
        err(f"dashboard_row_count_drift: expected {required_count} standalone smoke rows, got {len(smoke_rows)}")

    required_kind = required.get("required_row_kind")
    required_artifact = required.get("required_evidence_artifact")
    row_by_id = {row.get("row_id"): row for row in smoke_rows if isinstance(row.get("row_id"), str)}
    ...
    summary = {"dashboard_rows": len(rows), "standalone_smoke_rows": len(smoke_rows)}
    emit("standalone_smoke_dashboard_rows_validated", **summary)
    return summary
...
def validate_bindings(manifest: dict[str, Any]) -> int:
    ...
    test_surfaces = {
        "crates/frankenlibc-harness/tests/l1_dry_run_readiness_dashboard_test.rs": {"standalone_link_run_smoke_rows_are_explicit"},
        "crates/frankenlibc-harness/tests/standalone_link_run_smoke_test.rs": {"dry_run_blocks_l2_claim_without_candidate_artifact"},
        "crates/frankenlibc-harness/tests/standalone_link_run_smoke_dashboard_completion_contract_test.rs": REQUIRED_POSITIVE_TESTS | REQUIRED_NEGATIVE_TESTS,
    }
    ...
    emit("standalone_smoke_dashboard_bindings_validated", binding_count=len(ids))
    return len(ids)
...
def replay_source_gates(required: dict[str, Any]) -> None:
    ...
    run_command(
        ...
        marker=str(markers.get("standalone_smoke_dry_run", "")),
        label="standalone_smoke_dry_run",
        env={
    ...
    run_command(
        ["bash", "scripts/check_standalone_link_run_smoke_completion_contract.sh"],
        marker=str(markers.get("standalone_smoke_completion_gate", "")),
        label="standalone_smoke_completion_gate",
        env={
            "FRANKENLIBC_STANDALONE_SMOKE_COMPLETION_REPORT": str(SMOKE_COMPLETION_REPORT),
            "FRANKENLIBC_STANDALONE_SMOKE_COMPLETION_LOG": str(SMOKE_COMPLETION_LOG),
            "FRANKENLIBC_STANDALONE_SMOKE_COMPLETION_SMOKE_REPORT": str(SMOKE_REPORT),
            "FRANKENLIBC_STANDALONE_SMOKE_COMPLETION_SMOKE_LOG": str(SMOKE_LOG),
            "FRANKENLIBC_STANDALONE_SMOKE_COMPLETION_TARGET_DIR": str(SMOKE_TARGET_DIR),
        },
    )
    emit("standalone_smoke_dashboard_source_gates_replayed", gate_count=2)
...
def write_outputs(status: str, summary: dict[str, Any]) -> None:
    ...
    final = FAIL_EVENT if status != "pass" else "standalone_smoke_dashboard_completion_contract_pass"
...
def main() -> int:
    ...
    print(
        "PASS: standalone link-run smoke dashboard completion contract "
        f"dashboard_rows={summary['dashboard_rows']} "
        f"standalone_rows={summary['standalone_smoke_rows']} "
