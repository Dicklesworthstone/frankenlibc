#!/usr/bin/env bash
# check_unistd_process_filesystem_wave4_completion_contract.sh -- bd-mk28y.3 completion gate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTRACT="${FRANKENLIBC_UNISTD_WAVE4_COMPLETION_CONTRACT:-${ROOT}/tests/conformance/unistd_process_filesystem_wave4_completion_contract.v1.json}"
OUT_DIR="${FRANKENLIBC_UNISTD_WAVE4_COMPLETION_OUT_DIR:-${ROOT}/target/conformance/unistd_process_filesystem_wave4_completion}"
REPORT="${FRANKENLIBC_UNISTD_WAVE4_COMPLETION_REPORT:-${OUT_DIR}/unistd_process_filesystem_wave4_completion_contract.report.json}"
LOG="${FRANKENLIBC_UNISTD_WAVE4_COMPLETION_LOG:-${OUT_DIR}/unistd_process_filesystem_wave4_completion_contract.events.jsonl}"
...
SCHEMA = "unistd_process_filesystem_wave4_completion_contract.v1"
REPORT_SCHEMA = "unistd_process_filesystem_wave4_completion_contract.report.v1"
BEAD_ID = "bd-mk28y.3"
EPIC_ID = "bd-mk28y"
TRACE_ID = "bd-mk28y.3::unistd-process-filesystem-wave4::completion::v1"
CAMPAIGN_ID = "fcq-unistd-process-filesystem"
WAVE_ID = "wave-04-unistd-process-filesystem-aio-random-argp"
FIXTURE_FILE = "unistd_process_filesystem_wave04.json"
FIXTURE_PATH = "tests/conformance/fixtures/unistd_process_filesystem_wave04.json"
UNISTD_MODULE = "unistd_abi"
AMBIENT_POLICY = "forbid_ambient_random_values_alarm_state_argp_streams_or_aio_fd_metadata"
...
REQUIRED_DEPENDENCIES = {"bd-mk28y.1", "bd-mk28y.2"}
...
REQUIRED_POSITIVE_TESTS = {
    "contract_binds_unistd_wave4_sources",
    "checker_accepts_unistd_wave4_completion_contract",
    "checker_emits_structured_unistd_wave4_telemetry",
}
REQUIRED_NEGATIVE_TESTS = {
    "checker_rejects_missing_unistd_wave4_fixture_symbol",
    "checker_rejects_stale_unistd_wave4_coverage_accounting",
...
FAILURE_PRIORITY = [
    ...
    "unistd_wave4_fixture_symbol_coverage",
    "unistd_wave4_fixture_mode_coverage",
    "unistd_wave4_fixture_schema",
...
def primary_signature() -> str:
    ...
    for signature in FAILURE_PRIORITY:
        if signature in present:
            return signature
    return "unistd_wave4_completion_contract_failed"
...
def validate_unistd_wave4_fixture(completion: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> None:
    start_errors = len(errors)
    fixture = load_json(
        artifact_path(artifacts, "unistd_wave4_fixture", FIXTURE_PATH),
        "unistd wave-04 fixture",
        "unistd_wave4_fixture_schema",
    )
    ...
    if fixture.get("family") != "unistd/process-filesystem":
        add_error("unistd_wave4_fixture_schema", "unistd wave-04 fixture family mismatch")
    campaign = as_object(fixture.get("campaign"), "unistd_wave4_fixture.campaign", "unistd_wave4_fixture_schema")
    if campaign.get("bead") != "bd-mk28y.1":
        add_error("unistd_wave4_fixture_schema", "unistd wave-04 fixture bead binding mismatch")
    if campaign.get("campaign_id") != CAMPAIGN_ID or campaign.get("wave_id") != WAVE_ID:
        add_error("unistd_wave4_fixture_schema", "unistd wave-04 fixture campaign/wave binding mismatch")
    if campaign.get("ambient_state_policy") != AMBIENT_POLICY:
        add_error("unistd_wave4_fixture_schema", "unistd wave-04 ambient_state_policy mismatch")
    ...
    if campaign.get("residual_symbols") not in ([], None):
        add_error("unistd_wave4_fixture_symbol_coverage", "fixture must not leave residual wave-04 symbols")
    if set(fixture.get("structured_log_fields", [])) != required_fields:
        add_error("unistd_wave4_fixture_schema", "structured_log_fields mismatch")

    modes_by_symbol: dict[str, set[str]] = {symbol: set() for symbol in required_symbols}
    case_count = 0
    for case in as_array(fixture.get("cases"), "unistd_wave4_fixture.cases", "unistd_wave4_fixture_schema"):
        obj = as_object(case, "unistd_wave4_fixture.cases[]", "unistd_wave4_fixture_schema")
        symbol = obj.get("function")
        mode = obj.get("mode")
        if isinstance(symbol, str) and isinstance(mode, str):
            case_count += 1
            modes_by_symbol.setdefault(symbol, set()).add(mode)
        inputs = as_object(obj.get("inputs"), f"unistd_wave4_fixture.case[{symbol}].inputs", "unistd_wave4_fixture_schema")
        for field in ["symbol", "expected", "ambient_state_policy", "oracle_source"]:
            if not isinstance(inputs.get(field), str) or not inputs.get(field):
                add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} missing inputs.{field}")
        if isinstance(symbol, str) and inputs.get("symbol") != symbol:
            add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} inputs.symbol mismatch")
        if inputs.get("ambient_state_policy") != AMBIENT_POLICY:
            add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} ambient-state policy mismatch")
        expected_output = obj.get("expected_output")
        if not isinstance(expected_output, str):
            add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} missing expected_output")
            expected_output = ""
        for field in sorted(required_fields):
            if f"{field}=" not in expected_output:
                add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} expected_output missing {field} token")
        for token in forbidden_tokens:
            if token in expected_output:
                add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} leaks ambient token {token}")
        if obj.get("expected_errno") != 0:
            add_error("unistd_wave4_fixture_schema", f"case {obj.get('name')} expected_errno must be 0")
    ...
    if mode_failures:
        add_error("unistd_wave4_fixture_mode_coverage", f"symbols missing required modes: {mode_failures}")

    failed = len(errors) > start_errors
    signature = "none"
    if failed:
        signature = "unistd_wave4_fixture_mode_coverage" if mode_failures else "unistd_wave4_fixture_schema"
    events.append(
        event(
            "unistd_wave4_fixture_validated",
            "fail" if failed else "pass",
...
def validate_validation_commands(completion: dict[str, Any]) -> None:
    ...
    required_fragments = [
        ...
        "cargo check -p frankenlibc-harness --test unistd_process_filesystem_wave4_completion_contract_test",
        "cargo clippy -p frankenlibc-harness --test unistd_process_filesystem_wave4_completion_contract_test -- -D warnings",
    ]
...
def validate_test_surfaces(completion: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> None:
    start_errors = len(errors)
    text = read_text(
        artifact_path(artifacts, "completion_harness_test", "crates/frankenlibc-harness/tests/unistd_process_filesystem_wave4_completion_contract_test.rs"),
    ...
    fixture_test = read_text(
        artifact_path(artifacts, "unistd_wave4_harness_test", "crates/frankenlibc-harness/tests/unistd_process_filesystem_wave04_conformance_test.rs"),
        "unistd wave-04 fixture harness test",
        "missing_test_binding",
    )
    required_needles = string_set(
    ...
    for needle in sorted(required_needles):
        if needle not in fixture_test:
            add_error("missing_test_binding", f"unistd wave-04 fixture harness missing {needle}")
...
def build_report(completion: dict[str, Any]) -> dict[str, Any]:
    ...
    for error in errors:
        ...
        elif signature.startswith("unistd_wave4_fixture"):
            checks["unistd_wave4_fixture"] = "fail"
    ...
    return {
        ...
        "summary": {
            ...
            "unistd_wave4_symbol_count": len(completion.get("required_first_wave_symbols", [])),
...
def main() -> int:
    ...
    validate_unistd_wave4_fixture(completion, artifacts)
    ...
    if errors:
        print(f"FAIL unistd process/filesystem wave-04 completion contract errors={len(errors)} report={rel(REPORT)}", file=sys.stderr)
        return 1
    print(
        "PASS unistd process/filesystem wave-04 completion contract "
        f"symbols={report['summary']['unistd_wave4_symbol_count']} "
