...
EXPECTED_SOURCE_COMMIT_FRESHNESS_POLICY = {
    ...
    "stale_result": "block_ported_surface_gate_evidence",
    "ported_surface_evidence_allowed_when_stale": False,
...
if isinstance(gate, dict):
    ...
    if gate.get("source_commit_freshness_policy") != EXPECTED_SOURCE_COMMIT_FRESHNESS_POLICY:
        fail("source_commit_freshness_policy must match the stale ported-surface gate block contract")
