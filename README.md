...
Canonical checked smoke artifact: `tests/conformance/ld_preload_smoke_summary.v1.json` (run `rc0923-corpus-20260924T075200Z`, checked September 24, 2026) reports 152 passes / 0 fails / 4 skips overall, with strict 76/0/2 and hardened 76/0/2 across the curated preload smoke battery.

| Mode | Pass | Fail | Skip | Programs exercised |
|---|---:|---:|---:|---|
| `strict` | 76 | 0 | 2 | coreutils (`ls`, `cat`, `echo`, `env`, `sort`, `wc`, `date`), `python3` incl. C extensions, `busybox`, `sqlite3`, sed, grep, awk, find, tar gzip/xz, make, perl, git, a C++ runtime program, glibc `FILE`-layout / small-stack-thread / hardened-first-heal / TZ `localtime` / argp fixtures, `getent`, `iconv`, `node -e`, integration fixture, stress iterations |
| `hardened` | 76 | 0 | 2 | same battery with `FRANKENLIBC_MODE=hardened` |
| **Total** | **152** | **0** | **4** | The 4 skips are the optional `redis-cli --version` and `nginx -v` probes when those binaries are not installed |
...
- The preload smoke battery currently has 152 pass / 0 fail / 4 optional skips across strict and hardened modes; see `docs/planning/COMPATIBILITY.md` for workload status details.
...
| Curated `LD_PRELOAD` smoke battery | 152 pass / 0 fail / 4 optional skip; strict and hardened green |
