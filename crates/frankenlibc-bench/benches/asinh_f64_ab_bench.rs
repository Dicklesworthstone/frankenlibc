...
fn run<F: Fn(f64) -> f64>(g: &mut criterion::BenchmarkGroup<'_, criterion::measurement::WallTime>, name: &str, inp: &[f64], f: F) {
    let one = || { let mut a = 0.0f64; for &x in inp { a += f(black_box(x)); } a };
...
fn bench(c: &mut Criterion) {
    ...
    assert!(worst <= 2, "asinh candidate exceeds 2 ULP vs glibc (worst {worst} at x={wx})");

    // Timing input: the [1,16) midrange band, both signs.
    let inp: Vec<f64> = (0..64).map(|k| {
