...
fn host() -> Fn_ {
    ...
    let a = *H.get_or_init(|| unsafe {
        ...
        assert!(!h.is_null());
        libc::dlsym(h, b"strcat\0".as_ptr().cast()) as usize
    });
...
fn bench(c: &mut Criterion) {
    ...
    for &(dlen, slen) in &[
        ...
        buf[..dlen].copy_from_slice(&prefix);
        buf[dlen] = 0;
        let reset = |b: &mut [u8]| {
            b[dlen] = 0;
        }; // O(1): strcat only writes [dlen..]
        // byte-identity
        reset(&mut buf);
        let mut b2 = buf.clone();
        unsafe {
            frankenlibc_abi::string_abi::strcat(buf.as_mut_ptr().cast(), sp);
            g(b2.as_mut_ptr().cast(), sp);
        }
        ...
        for _ in 0..100 {
            ...
            for _ in 0..it {
                reset(&mut buf);
                black_box(unsafe {
                    frankenlibc_abi::string_abi::strcat(
                        black_box(buf.as_mut_ptr().cast()),
                        black_box(sp),
                    )
                });
            }
            ...
            for _ in 0..it {
                reset(&mut buf);
                black_box(unsafe { g(black_box(buf.as_mut_ptr().cast()), black_box(sp)) });
            }
            gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        let (fp, gp) = (pctl(&fs, 0.5), pctl(&gs, 0.5));
        println!(
            "STRCAT dlen={dlen} slen={slen} fl_p50={fp:.2}ns glibc_p50={gp:.2}ns ratio_fl_over_glibc={:.2}",
            fp / gp
        );
    }
