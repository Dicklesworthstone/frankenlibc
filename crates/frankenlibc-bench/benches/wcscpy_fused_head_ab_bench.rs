...
fn host() -> CpyFn {
    ...
    let a = *H.get_or_init(|| unsafe {
        ...
        assert!(!h.is_null());
        libc::dlsym(h, b"wcscpy\0".as_ptr().cast()) as usize
    });
...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("wcscpy_fused");
    grp.sample_size(10);
    let it = 4000u64;
    for &n in &[4usize, 16, 32, 64, 128, 256, 1024] {
        ...
        unsafe {
            old_wcscpy(a.as_mut_ptr(), sp);
            new_wcscpy(b.as_mut_ptr(), sp);
            g(cc.as_mut_ptr(), sp);
        }
        assert_eq!(a[..=n], b[..=n], "new n={n}");
        assert_eq!(a[..=n], cc[..=n], "glibc n={n}");
        let (mut os, mut ns, mut gs) = (Vec::new(), Vec::new(), Vec::new());
        for _ in 0..80 {
            let t = Instant::now();
            for _ in 0..it {
                unsafe {
                    old_wcscpy(black_box(a.as_mut_ptr()), black_box(sp));
                }
            }
            ...
            for _ in 0..it {
                unsafe {
                    new_wcscpy(black_box(b.as_mut_ptr()), black_box(sp));
                }
            }
            ns.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { g(black_box(cc.as_mut_ptr()), black_box(sp)) });
            }
            gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        let (op, np, gp) = (pctl(&os, 0.5), pctl(&ns, 0.5), pctl(&gs, 0.5));
        println!(
            "WCSCPY_FUSED n={n} old={op:.2} new={np:.2} glibc={gp:.2} new/old={:.3} new/glibc={:.3} old/glibc={:.3}",
            np / op,
            np / gp,
            op / gp
        );
