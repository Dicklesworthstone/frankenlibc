...
fn run(g: &mut criterion::BenchmarkGroup<'_, criterion::measurement::WallTime>, name: &str, inp: &[(f32, f32)], f: impl Fn(f32, f32) -> f32) {
    ...
    g.bench_function(name, |b| b.iter(|| black_box(f(black_box(inp[0].0), black_box(inp[0].1)))));
}
...
fn bench(c: &mut Criterion) {
    ...
    let edges: Vec<(f32,f32)> = vec![
        (5.3,2.0),(-5.3,2.0),(5.3,-2.0),(1.0,1.0),(0.0,1.0),(-0.0,1.0),
        (f32::from_bits(1),f32::from_bits(3)),(1e30,3.0),(0.1,0.3),(7.0,7.0),(2.5,1.0),
    ];
    let mut mism = 0u64; let mut first = String::new();
    let mut check = |x: f32, y: f32| {
        let (cb, gb) = (candidate(x,y).to_bits(), unsafe { h_fmodf(x,y) }.to_bits());
        if cb != gb && !(candidate(x,y).is_nan() && unsafe { h_fmodf(x,y) }.is_nan()) {
            mism += 1; if first.is_empty() { first = format!("fmodf({x:e},{y:e}) cand={:#010x} glibc={:#010x}", cb, gb); }
        }
    };
    for &(x,y) in &edges { check(x,y); }
    for _ in 0..3_000_000u32 {
        let x = f32::from_bits(rng() as u32); let y = f32::from_bits((rng()>>32) as u32);
        if x.is_finite() && y.is_finite() && y != 0.0 { check(x,y); }
    }
    ...
    g.sample_size(10);
    run(&mut g, "fl_deployed_typical", &typical, |x,y| math::fmodf(x,y));
    run(&mut g, "candidate_typical", &typical, candidate);
    run(&mut g, "glibc_typical", &typical, |x,y| unsafe { h_fmodf(x,y) });
    run(&mut g, "fl_deployed_stress", &stress, |x,y| math::fmodf(x,y));
    run(&mut g, "candidate_stress", &stress, candidate);
    run(&mut g, "glibc_stress", &stress, |x,y| unsafe { h_fmodf(x,y) });
    g.finish();
}
