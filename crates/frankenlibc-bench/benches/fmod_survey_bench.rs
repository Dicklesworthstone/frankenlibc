...
unsafe extern "C" {
    #[link_name = "fmod"] fn h_fmod(x: f64, y: f64) -> f64;
...
fn survey(g: &mut criterion::BenchmarkGroup<'_, criterion::measurement::WallTime>, name: &str, inp: &[(f64, f64)], fl: impl Fn(f64, f64) -> f64, gl: impl Fn(f64, f64) -> f64) {
    ...
    let ratio = a / b;
    let flag = if ratio > 1.30 { "  <-- LIBM-SLOW: exact-algo lever candidate" } else { "" };
...
fn bench(c: &mut Criterion) {
    ...
    g.sample_size(10);
    survey(&mut g, "fmod_typical", &typical, |x, y| math::fmod(x, y), |x, y| unsafe { h_fmod(x, y) });
    survey(&mut g, "fmod_stress", &stress, |x, y| math::fmod(x, y), |x, y| unsafe { h_fmod(x, y) });
    survey(&mut g, "remainder_typical", &typical, |x, y| math::remainder(x, y), |x, y| unsafe { h_remainder(x, y) });
    survey(&mut g, "remainder_stress", &stress, |x, y| math::remainder(x, y), |x, y| unsafe { h_remainder(x, y) });
    g.finish();
}
