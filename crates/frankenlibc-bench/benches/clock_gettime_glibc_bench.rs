...
fn measure_clock_getres(f: CgrFn, clock_id: c_int, res: *mut libc::timespec, iters: u64) -> f64 {
...
fn bench(c: &mut Criterion) {
    ...
    for &clk in &[libc::CLOCK_REALTIME, libc::CLOCK_MONOTONIC] {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..it {
                black_box(unsafe {
                    frankenlibc_abi::time_abi::clock_gettime(black_box(clk), &mut ts)
                });
            }
            fs.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { g_cg(black_box(clk), &mut ts) });
            }
            gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        println!(
            "CLOCK_GETTIME clk={clk} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fs, 0.5),
            pctl(&gs, 0.5),
    ...
    }
    {
        ...
        let mut fl_res = libc::timespec {
            tv_sec: 0,
            tv_nsec: 0,
        };
        let mut glibc_res = libc::timespec {
            tv_sec: 0,
            tv_nsec: 0,
        };
        ...
        let (fl, glibc_a, glibc_b) = (pctl(&fs, 0.5), pctl(&gs_a, 0.5), pctl(&gs_b, 0.5));
        println!(
    ...
    }
    {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..it {
                black_box(unsafe {
                    frankenlibc_abi::time_abi::gettimeofday(&mut tv, std::ptr::null_mut())
                });
            }
            fs.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { g_gtod(&mut tv, std::ptr::null_mut()) });
            }
            gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        println!(
