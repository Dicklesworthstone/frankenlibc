...
fn host() -> Fn_ {
    ...
    let a = *H.get_or_init(|| unsafe {
        ...
        assert!(!h.is_null());
        let mut p = libc::dlsym(h, b"wcswcs\0".as_ptr().cast()); if p.is_null() { p = libc::dlsym(h, b"wcsstr\0".as_ptr().cast()); }
...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("wcswcs"); grp.sample_size(20);
    for &n in &[4096usize, 16384] {
        // adversarial: haystack all 'a' (no NUL until end), needle 'a'*16 + 'b' (absent) → many partial matches.
        let mut hay: Vec<u32> = std::iter::repeat(b'a' as u32).take(n).collect(); hay.push(0);
        let mut ndl: Vec<u32> = std::iter::repeat(b'a' as u32).take(16).collect(); ndl.push(b'b' as u32); ndl.push(0);
        ...
        for _ in 0..60 {
            let t=Instant::now();
            for _ in 0..it { black_box(unsafe{frankenlibc_abi::glibc_internal_abi::wcswcs(black_box(hp.cast()), black_box(np.cast()))}); }
            fs.push(t.elapsed().as_nanos() as f64/it as f64);
            let t=Instant::now();
            for _ in 0..it { black_box(unsafe{g(black_box(hp), black_box(np))}); }
            gs.push(t.elapsed().as_nanos() as f64/it as f64);
        }
        let(fp,gp)=(pctl(&fs,0.5),pctl(&gs,0.5));
        println!("WCSWCS_ADV n={n} fl_p50={fp:.1}ns glibc_p50={gp:.1}ns ratio_fl_over_glibc={:.2}", fp/gp);
    }
