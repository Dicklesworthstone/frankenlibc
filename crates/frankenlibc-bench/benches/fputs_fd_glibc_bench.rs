...
fn h() -> *mut libc::c_void {
    ...
    *H.get_or_init(|| unsafe {
        let p = libc::dlmopen(libc::LM_ID_NEWLM, b"libc.so.6\0".as_ptr().cast(), libc::RTLD_LAZY | libc::RTLD_LOCAL);
        assert!(!p.is_null()); p as usize
    }) as *mut libc::c_void
}
fn dl<T: Copy>(n: &[u8]) -> T { let p = unsafe { libc::dlsym(h(), n.as_ptr().cast()) }; assert!(!p.is_null(), "dlsym"); unsafe { std::mem::transmute_copy::<usize, T>(&(p as usize)) } }
...
fn bench(c: &mut Criterion) {
    ...
    let mut grp = c.benchmark_group("fputs_fd"); grp.sample_size(10);
    let it = 20000u64;
    for &n in &[8usize, 38, 200] {
        ...
        unsafe { fl::fputs(sp, ff); g_fputs(sp, gf); }
        let (mut fs, mut gs) = (Vec::new(), Vec::new());
        for _ in 0..100 {
            let t = Instant::now(); for _ in 0..it { black_box(unsafe { fl::fputs(black_box(sp), ff) }); } unsafe { fl::fflush(ff); } fs.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now(); for _ in 0..it { black_box(unsafe { g_fputs(black_box(sp), gf) }); } unsafe { g_fflush(gf); } gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        println!("FPUTS_FD n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}", pctl(&fs,0.5), pctl(&gs,0.5), pctl(&fs,0.5)/pctl(&gs,0.5));
    }
