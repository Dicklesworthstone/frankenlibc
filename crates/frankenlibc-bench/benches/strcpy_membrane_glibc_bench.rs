...
fn bench(c: &mut Criterion) {
    ...
    let mut grp = c.benchmark_group("strcpy_membrane");
    grp.sample_size(10);
    let it = 4000u64;
    for &n in &[4usize, 16, 32, 64, 128, 256, 1024] {
        let mut src: Vec<u8> = std::iter::repeat(b'a').take(n).collect();
        src.push(0);
        let sp = src.as_ptr() as *const c_char;
        let (mut dfl, mut dgl) = (vec![0u8; n + 1], vec![0u8; n + 1]);
        unsafe {
            frankenlibc_abi::string_abi::strcpy(dfl.as_mut_ptr() as *mut c_char, sp);
            g_cpy(dgl.as_mut_ptr() as *mut c_char, sp);
        }
        ...
        for _ in 0..80 {
            ...
            for _ in 0..it {
                black_box(unsafe {
                    frankenlibc_abi::string_abi::strcpy(
                        black_box(dfl.as_mut_ptr() as *mut c_char),
                        black_box(sp),
                    )
                });
            }
            ...
            for _ in 0..it {
                black_box(unsafe {
                    g_cpy(black_box(dgl.as_mut_ptr() as *mut c_char), black_box(sp))
                });
            }
    ...
    for &n in &[4usize, 16, 32, 64, 128, 256, 1024] {
        ...
        unsafe {
            frankenlibc_abi::string_abi::strcat(dfl.as_mut_ptr() as *mut c_char, sp);
            g_cat(dgl.as_mut_ptr() as *mut c_char, sp);
        }
        ...
        for _ in 0..80 {
            ...
            for _ in 0..it {
                unsafe {
                    dfl[pre] = 0;
                    black_box(frankenlibc_abi::string_abi::strcat(
                        black_box(dfl.as_mut_ptr() as *mut c_char),
                        black_box(sp),
                    ));
                }
            ...
            for _ in 0..it {
                unsafe {
                    dgl[pre] = 0;
                    black_box(g_cat(
                        black_box(dgl.as_mut_ptr() as *mut c_char),
                        black_box(sp),
                    ));
                }
            }
        ...
        println!(
            "STRCAT n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fs, 0.5),
            pctl(&gs, 0.5),
            pctl(&fs, 0.5) / pctl(&gs, 0.5)
        );
