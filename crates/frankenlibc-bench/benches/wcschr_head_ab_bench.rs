...
fn host() -> Fn_ {
    ...
    let a = *H.get_or_init(|| unsafe {
        ...
        assert!(!h.is_null());
        libc::dlsym(h, b"wcschr\0".as_ptr().cast()) as usize
    });
...
unsafe fn new_scan(s: *const u32, c: u32) -> (usize, bool) {
    ...
    loop {
        if i >= 32 && ((s as usize) + i * 4) & 0xFFF <= 0x1000 - 128 {
            ...
            let any = (x0.simd_eq(cv) | x0.simd_eq(zv))
                | (x1.simd_eq(cv) | x1.simd_eq(zv))
                | (x2.simd_eq(cv) | x2.simd_eq(zv))
                | (x3.simd_eq(cv) | x3.simd_eq(zv));
            if !any.any() {
                i += 4 * LANES;
                continue;
            }
...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("wcschr_head");
    grp.sample_size(10);
    ...
    for &n in &[4usize, 8, 16, 32, 48, 64, 96, 128, 256, 1024] {
        ...
        for _ in 0..80 {
            ...
            for _ in 0..it {
                black_box(unsafe {
                    frankenlibc_abi::wchar_abi::bench_wide_find_or_nul_simd(black_box(p), miss)
                });
            }
            os.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { new_scan(black_box(p), miss) });
            }
            ns.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { new2_scan(black_box(p), miss) });
            }
            n2s.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { g(black_box(p), miss) });
            }
        ...
        println!(
            "WCSCHR_HEAD n={n} old={op:.2} new={np:.2} new2={n2p:.2} glibc={gp:.2} new/old={:.3} new2/old={:.3} new2/glibc={:.3}",
            np / op,
            n2p / op,
            n2p / gp
        );
