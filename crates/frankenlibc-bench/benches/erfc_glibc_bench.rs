...
fn bench(c: &mut Criterion) {
    ...
    ge.bench_function("frankenlibc_core", |b| {
        b.iter(|| { let mut s = 0.0; for &x in ARGS { s += black_box(frankenlibc_core::math::erfc(black_box(x))); } black_box(s) })
    });
    ge.bench_function("host_glibc_inprocess", |b| {
        b.iter(|| { let mut s = 0.0; for &x in ARGS { s += black_box(unsafe { erfc(black_box(x)) }); } black_box(s) })
    });
    ge.finish();
    let mut gf = c.benchmark_group("erf");
    gf.bench_function("frankenlibc_core", |b| {
        b.iter(|| { let mut s = 0.0; for &x in ARGS { s += black_box(frankenlibc_core::math::erf(black_box(x))); } black_box(s) })
    });
    gf.bench_function("host_glibc_inprocess", |b| {
        b.iter(|| { let mut s = 0.0; for &x in ARGS { s += black_box(unsafe { erf(black_box(x)) }); } black_box(s) })
    });
    gf.finish();
}
