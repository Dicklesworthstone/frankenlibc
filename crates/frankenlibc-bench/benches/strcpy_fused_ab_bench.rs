...
fn host() -> CpyFn {
    ...
    let a = *H.get_or_init(|| unsafe {
        ...
        assert!(!h.is_null());
        libc::dlsym(h, b"strcpy\0".as_ptr().cast()) as usize
    });
...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("strcpy_fused");
    grp.sample_size(10);
    let it = 4000u64;
    for &n in &[4usize, 8, 16, 32, 64, 128, 256, 1024] {
        ...
        unsafe {
            g(cc.as_mut_ptr() as *mut c_char, sp as *const c_char);
        }
        assert_eq!(la, n, "two_pass len n={n}");
        assert_eq!(lb, n, "fused len n={n}");
        assert_eq!(a[..=n], b[..=n], "fused bytes n={n}");
        assert_eq!(a[..=n], cc[..=n], "glibc bytes n={n}");
        let (mut os, mut ns, mut gs) = (Vec::new(), Vec::new(), Vec::new());
        for _ in 0..80 {
            let t = Instant::now();
            for _ in 0..it {
                unsafe {
                    black_box(frankenlibc_abi::string_abi::bench_strcpy_two_pass(
            ...
            for _ in 0..it {
                unsafe {
                    black_box(frankenlibc_abi::string_abi::bench_strcpy_fused(
                        black_box(b.as_mut_ptr()),
                        black_box(sp),
                    ));
                }
            }
            ...
            for _ in 0..it {
                unsafe {
                    black_box(g(
                        black_box(cc.as_mut_ptr() as *mut c_char),
                        black_box(sp as *const c_char),
                    ));
                }
            }
        ...
        println!(
            "STRCPY_FUSED n={n} two_pass={op:.2} fused={np:.2} glibc={gp:.2} fused/two_pass={:.3} fused/glibc={:.3} two_pass/glibc={:.3}",
            np / op,
            np / gp,
            op / gp
        );
