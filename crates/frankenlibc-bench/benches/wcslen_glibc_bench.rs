...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("wcslen");
    grp.sample_size(30);
    for &n in &[4usize, 16, 32, 48, 64, 96, 128, 256, 1024] {
        ...
        let p = buf.as_ptr();
        assert_eq!(
            unsafe { frankenlibc_abi::wchar_abi::wcslen(p) },
            unsafe { g(p) },
            "wcslen fl!=glibc n={n}"
        );
        let it = 2000u64;
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { frankenlibc_abi::wchar_abi::wcslen(black_box(p)) });
            }
        ...
        println!(
            "WCSLEN n={n} fl_p50={fp:.2}ns glibc_p50={gp:.2}ns ratio_fl_over_glibc={:.2}",
            fp / gp
        );
    }
