...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("strrchr");
    grp.sample_size(30);
    for &size in &[8usize, 16, 64, 1024, 65536] {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..it {
                black_box(unsafe {
                    frankenlibc_abi::string_abi::strrchr(black_box(p), black_box(b'/' as c_int))
                });
            }
            fs.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { g(black_box(p), black_box(b'/' as c_int)) });
            }
            gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        let (fp, gp) = (pctl(&fs, 0.50), pctl(&gs, 0.50));
        println!(
            "STRRCHR size={size} fl_p50={fp:.2}ns glibc_p50={gp:.2}ns ratio_fl_over_glibc={:.2}",
            fp / gp
        );
    }
