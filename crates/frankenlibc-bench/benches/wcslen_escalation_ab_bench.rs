...
fn host() -> Fn_ {
    ...
    let a = *H.get_or_init(|| unsafe {
        ...
        assert!(!h.is_null());
        libc::dlsym(h, b"wcslen\0".as_ptr().cast()) as usize
    });
...
fn bench(c: &mut Criterion) {
    let g = host();
    let mut grp = c.benchmark_group("wcslen_esc");
    grp.sample_size(10);
    for &n in &[4usize, 16, 32, 48, 64, 96, 128, 256, 1024] {
        ...
        assert_eq!(o, n, "old n={n}");
        assert_eq!(ne, n, "new n={n}");
        assert_eq!(gl, n, "glibc n={n}");
        let it = 4000u64;
        let (mut os, mut ns, mut gs) = (Vec::new(), Vec::new(), Vec::new());
        for _ in 0..80 {
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { old_scan(black_box(p)) });
            }
            os.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { new_scan(black_box(p)) });
            }
            ns.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box(unsafe { g(black_box(p)) });
            }
            gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        let (op, np, gp) = (pctl(&os, 0.5), pctl(&ns, 0.5), pctl(&gs, 0.5));
        println!(
            "WCSLEN_ESC n={n} old={op:.2} new={np:.2} glibc={gp:.2} new/old={:.3} new/glibc={:.3} old/glibc={:.3}",
            np / op,
            np / gp,
            op / gp
        );
