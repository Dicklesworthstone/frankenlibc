...
fn main(){
  ...
  // matches glibc EXACTLY, not just match/nomatch (the subtle part of the fast path).
  {
    ...
    for pfx in ["","a","ab","z"]{ for sfx in ["","z","az","a"]{
      ...
      for txt in ["","a","z","az","za","aXz","aXzYz","zzaz","aaz","azaza","XaYbZzQ","ababzz","abXYabZaz","nomatch","aa","zz","abc","azazaz"]{
        let ct=std::ffi::CString::new(txt).unwrap();
        let mut fpm=[libc::regmatch_t{rm_so:-9,rm_eo:-9}]; let mut gpm=[libc::regmatch_t{rm_so:-9,rm_eo:-9}];
        let fm=unsafe{frankenlibc_abi::string_abi::regexec(&fre as *const _ as *const std::ffi::c_void,ct.as_ptr(),1,fpm.as_mut_ptr() as *mut std::ffi::c_void,0)};
        let gm=unsafe{ge(&gre,ct.as_ptr(),1,gpm.as_mut_ptr(),0)};
        assert_eq!(fm==0,gm==0,"match {pfx}.*{sfx} on {txt:?}");
        if fm==0{ assert_eq!((fpm[0].rm_so,fpm[0].rm_eo),(gpm[0].rm_so,gpm[0].rm_eo),"SPAN {pfx}.*{sfx} on {txt:?}"); }
        checks+=1;
      }
    }}
