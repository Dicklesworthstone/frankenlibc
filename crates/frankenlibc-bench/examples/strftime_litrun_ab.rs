...
fn main() {
    ...
    for fmt in [
        &b"just text no directives\0"[..],
        &b"%H:%M\0"[..],
        &b"a longer literal string of text with no percent directives at all here\0"[..],
        &b"%Y-%m-%d %H:%M:%S\0"[..],
    ] {
        let fp = fmt.as_ptr() as *const i8;
        unsafe {
            ta::strftime(fbp, 256, fp, tmpp);
            g_sf(gbp, 256, fp, tmpp);
        }
        let same = fb
            .iter()
            .zip(gb.iter())
        ...
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { ta::strftime(black_box(fbp), 256, fp, tmpp) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_sf(black_box(gbp), 256, fp, tmpp) });
                }
            },
        );
        println!(
            "strftime [{:<40}] fl={:7.2}ns glibc={:7.2}ns fl/glibc={:.3}{} match={}",
