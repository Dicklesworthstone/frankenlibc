...
fn main() {
    ...
    let g_pbrk: PbrkFn =
        unsafe { std::mem::transmute(libc::dlsym(h, b"strpbrk\0".as_ptr().cast())) };
    ...
    for &n in &[64usize, 256, 1024] {
        ...
        assert_eq!(
            unsafe { fa::strcspn(sp, reject) },
            unsafe { g_spn(sp, reject) },
            "strcspn n={n}"
        );
        assert_eq!(
            unsafe { fa::strpbrk(sp, reject) }.is_null(),
            unsafe { g_pbrk(sp, reject) }.is_null(),
            "strpbrk n={n}"
        );
        ...
        for r in 0..50 {
            if r % 2 == 0 {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { fa::strcspn(black_box(sp), reject) });
                }
                fl.push(t.elapsed().as_nanos() as f64 / iters as f64);
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { g_spn(black_box(sp), reject) });
                }
        ...
        println!(
            "strcspn n={n:<5} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}{}",
            f / gg,
