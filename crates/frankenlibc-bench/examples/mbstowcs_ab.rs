...
fn main() {
    ...
    unsafe {
        let _ = setlocale(6 /*LC_ALL*/, b"C.UTF-8\0".as_ptr() as *const i8);
    }
    unsafe {
        libc::setlocale(libc::LC_ALL, b"C.UTF-8\0".as_ptr() as *const i8);
    }
    let g: MbFn =
        unsafe { std::mem::transmute(libc::dlsym(h, b"mbstowcs\0".as_ptr() as *const i8)) };
    use frankenlibc_abi::wchar_abi as wa;
    for (tag, src) in [(
        "ascii64",
        &b"the quick brown fox jumps over the lazy dog 0123456789 abcde\0"[..],
    )] {
        let sp = src.as_ptr() as *const i8;
        let n = src.len() - 1;
        let mut fd = vec![0u32; n + 8];
        let mut gd = vec![0u32; n + 8];
        let fr = unsafe { wa::mbstowcs(fd.as_mut_ptr(), sp as *const u8, n) };
        let gr = unsafe { g(gd.as_mut_ptr() as *mut i32, sp, n) };
        assert_eq!(fr, gr, "{tag} count");
        assert_eq!(&fd[..n], &gd[..n], "{tag} data");
        ...
        for r in 0..50 {
            if r % 2 == 0 {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { wa::mbstowcs(fd.as_mut_ptr(), sp as *const u8, n) });
                }
                fl.push(t.elapsed().as_nanos() as f64 / iters as f64);
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { g(gd.as_mut_ptr() as *mut i32, sp, n) });
                }
        ...
        eprintln!(
            "mbstowcs {tag:<8} n={n} fl={f:6.1} glibc={gg:6.1} fl/glibc={:.3}{}",
            f / gg,
            if f / gg > 1.25 {
                "  <-- LOSS"
            } else if f / gg < 0.9 {
