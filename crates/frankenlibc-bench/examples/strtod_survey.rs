...
fn main(){
  ...
  for txt in ["0","42","-1.5e10","3.14159","2.5","123456.789","1e-300","1000000"]{
    let s:Vec<i8>=txt.bytes().map(|b|b as i8).chain(std::iter::once(0)).collect(); let p=s.as_ptr();
    let(f,g)=bench2(||{for _ in 0..iters{black_box(unsafe{sa::strtod(black_box(p),std::ptr::null_mut())});}},
                    ||{for _ in 0..iters{black_box(unsafe{g_strtod(black_box(p),std::ptr::null_mut())});}});
    println!("strtod {:<14} fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",txt,f/iters as f64,g/iters as f64,f/g,tag(f/g));
  }
  for txt in ["0","42","3.14159"]{
    let s:Vec<i8>=txt.bytes().map(|b|b as i8).chain(std::iter::once(0)).collect(); let p=s.as_ptr();
    let(f,g)=bench2(||{for _ in 0..iters{black_box(unsafe{sa::atof(black_box(p))});}},
                    ||{for _ in 0..iters{black_box(unsafe{g_atof(black_box(p))});}});
    println!("atof   {:<14} fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",txt,f/iters as f64,g/iters as f64,f/g,tag(f/g));
  }
}
