...
fn main(){
  ...
  let sbase=src.as_ptr(); let dbase=dst.as_mut_ptr();
  // verify correctness on a few configs first
  for (o_d,o_s,n) in [(0usize,0usize,4096usize),(16,16,4096),(16,0,4096),(0,16,16384)]{
    let dp=unsafe{((dbase as usize+31)&!31) as *mut u8}; let dp=unsafe{dp.add(o_d)};
    let sp=unsafe{((sbase as usize+31)&!31) as *const u8}; let sp=unsafe{sp.add(o_s)};
    unsafe{fa::memcpy(dp as *mut _, sp as *const _, n);}
    assert!(unsafe{std::slice::from_raw_parts(dp,n)}.iter().all(|&b|b==0x5a),"memcpy correctness {o_d} {o_s} {n}");
  }
  println!("memcpy correctness OK");
  for &n in &[4096usize,16384]{
    for (tag,od,os) in [("dstalign",0usize,0usize),("dst+16",16,0),("both+16",16,16),("src+16",0,16)]{
      let dp=unsafe{(((dbase as usize+31)&!31) as *mut u8).add(od)} as *mut core::ffi::c_void;
      let sp=unsafe{(((sbase as usize+31)&!31) as *const u8).add(os)} as *const core::ffi::c_void;
      ...
      println!("memcpy n={n:<6} {tag:<9} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}",f/gg);
    }
  }
}
