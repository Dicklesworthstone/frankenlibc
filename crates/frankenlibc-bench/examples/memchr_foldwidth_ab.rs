...
fn scan64(hs:&[u8],needle:u8)->Option<usize>{
  let n=hs.len(); let nd=Simd::<u8,64>::splat(needle); let mut base=0;
  while n-base>=256{
    ...
    if (p0|p1|p2|p3).any(){ for j in base..base+256{ if hs[j]==needle{return Some(j);} } }
    base+=256;
  }
...
fn scan32(hs:&[u8],needle:u8)->Option<usize>{
  let n=hs.len(); let nd=Simd::<u8,32>::splat(needle); let mut base=0;
  while n-base>=256{
    let mut acc=Simd::<u8,32>::splat(0).simd_ne(Simd::splat(0)); // all-false mask
    let mut o=base;
    while o<base+256{ acc|=Simd::<u8,32>::from_slice(&hs[o..o+32]).simd_eq(nd); o+=32; }
    if acc.any(){ for j in base..base+256{ if hs[j]==needle{return Some(j);} } }
    base+=256;
  }
  hs[base..].iter().position(|&b|b==needle).map(|j|base+j)
}
fn main(){
  for &n in &[1024usize,4096,16384]{
    ...
    for r in 0..60{
      let o=||{let t=Instant::now();for _ in 0..iters{black_box(scan64(black_box(&hs),needle));}t.elapsed().as_nanos()as f64/iters as f64};
      let nw=||{let t=Instant::now();for _ in 0..iters{black_box(scan32(black_box(&hs),needle));}t.elapsed().as_nanos()as f64/iters as f64};
      if r%2==0{ov.push(o());nv.push(nw());}else{nv.push(nw());ov.push(o());}
    }
    let(o,nn)=(pctl(&ov,0.1),pctl(&nv,0.1));
    println!("memchr-fold n={n:<6} OLD(simd64)={o:7.1}ns NEW(simd32)={nn:7.1}ns  new/old={:.3} ({:.2}x)",nn/o,o/nn);
  }
}
