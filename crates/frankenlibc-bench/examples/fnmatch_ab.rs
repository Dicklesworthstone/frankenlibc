...
fn main() {
    ...
    for (pat, txt) in [
        ("*.txt", "document_final_v2.txt"),
        ("foo*", "foobarbazqux"),
        ("a?c", "abc"),
        ("[abc]*", "cxxxxx"),
        ("*abc*", "xxxxxxxxxxxxxxxxxxxxabcyyy"),
        ("*a*b*c*", "xxxaxxxbxxxcxxx"),
        ("literal", "literal"),
    ] {
        ...
        for r in 0..80 {
            if r % 2 == 0 {
                ...
                for _ in 0..lit {
                    black_box(unsafe {
                        frankenlibc_abi::string_abi::fnmatch(cp.as_ptr(), ct.as_ptr(), 0)
                    });
                }
                fl.push(t.elapsed().as_nanos() as f64 / lit as f64);
                let t = Instant::now();
                for _ in 0..lit {
                    black_box(unsafe { g(cp.as_ptr(), ct.as_ptr(), 0) });
                }
        ...
        println!(
            "FNMATCH {pat:<9} fl={f10:.1} glibc={g10:.1} fl/glibc={:.3}",
            f10 / g10
        );
    }
}
