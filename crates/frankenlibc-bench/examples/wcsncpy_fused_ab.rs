...
unsafe fn copy_n(dst:*mut u32,src:*const u32,count:usize){
  let mut i=0;
  while i+32<=count{
    for k in 0..4{ let v=Simd::<u32,8>::from_slice(unsafe{std::slice::from_raw_parts(src.add(i+k*8),8)}); v.copy_to_slice(unsafe{std::slice::from_raw_parts_mut(dst.add(i+k*8),8)}); }
    i+=32;
  }
...
unsafe fn two_pass(dst:*mut u32,src:*const u32,n:usize){
  ...
  if copy<n{ for j in copy..n{ unsafe{*dst.add(j)=0}; } }
}
...
unsafe fn fused(dst:*mut u32,src:*const u32,n:usize){
  ...
  while i+32<=n && (unsafe{src.add(i)} as usize & 0xFFF)<=0x1000-128 {
    ...
    if c0.simd_min(c1).simd_min(c2.simd_min(c3)).simd_eq(z).any(){
      for (k,c) in [c0,c1,c2,c3].iter().enumerate(){
        ...
        if m!=0{ let nul=i+k*8+m.trailing_zeros()as usize;
          for j in i+k*8..=nul{ unsafe{*dst.add(j)=*src.add(j)}; }
          for j in nul+1..n{ unsafe{*dst.add(j)=0}; } return; }
  ...
  while i<n{
    ...
    if c==0{ for j in i..n{ unsafe{*dst.add(j)=0}; } return; }
    unsafe{*dst.add(i)=c}; i+=1;
  }
}
...
fn main(){
  for &(n,slen) in &[(16usize,16usize),(64,64),(256,256),(1024,1024),(4096,4096),(64,40),(256,200),(1024,900)]{
    ...
    let sp=src.as_ptr();
    let mut d1=vec![7u32;n+8]; let mut d2=vec![7u32;n+8]; let mut d3=vec![7u32;n+8];
    unsafe{two_pass_8lane(d1.as_mut_ptr(),sp,n); fused(d2.as_mut_ptr(),sp,n); two_pass(d3.as_mut_ptr(),sp,n);}
    assert_eq!(&d1[..n],&d2[..n],"data n={n} slen={slen}"); assert_eq!(&d1[..n],&d3[..n],"data2 n={n}");
    ...
    for r in 0..60{
      ...
      let o=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{two_pass_8lane(black_box(p1),black_box(sp),n)});}t.elapsed().as_nanos()as f64/iters as f64};
      let nw=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fused(black_box(p2),black_box(sp),n)});}t.elapsed().as_nanos()as f64/iters as f64};
      if r%2==0{ov.push(o());nv.push(nw());}else{nv.push(nw());ov.push(o());}
    }
    let(o,nn)=(pctl(&ov,0.1),pctl(&nv,0.1));
    eprintln!("wcsncpy n={n:<5} slen={slen:<5} OLD(8lane2pass)={o:7.2}ns fused={nn:7.2}ns  fused/old={:.3} ({:.2}x)",nn/o,o/nn);
  }
}
