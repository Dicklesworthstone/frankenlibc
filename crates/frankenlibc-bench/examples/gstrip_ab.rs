...
fn new(s0:&str)->String{
  ...
  if let Some(e_pos)=s.bytes().position(|b|b==b'e'||b==b'E'){
    ...
    if b[..e_pos].contains(&b'.'){
      let mut end=e_pos;
      while end>0 && b[end-1]==b'0'{end-=1;}
      if end>0 && b[end-1]==b'.'{end-=1;}
      if end<e_pos{ s.drain(end..e_pos); }
    }
  }
  s
}
fn main(){
  ...
  let ins=["1.500000e+02","1.234568e+03","1.000000e-04","9.990000e+10","3.141590E+00","-4.200000e+01","1.000000e+100","7.000000e-308","1.230000e+05","5.000000e+00"];
  ...
  println!("gstrip OLD(to_string+format!)={o:.1}ns NEW(drain)={nn:.1}ns  new/old={:.3} ({:.2}x faster)",nn/o,o/nn);
}
