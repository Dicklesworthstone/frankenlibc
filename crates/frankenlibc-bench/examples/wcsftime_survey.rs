...
fn main(){
  ...
  let g_wsf:WsfFn=unsafe{std::mem::transmute(libc::dlsym(h,b"wcsftime\0".as_ptr().cast()))};
  use frankenlibc_abi::time_abi as ta; use frankenlibc_abi::wchar_abi as wa;
  ...
  let iters=50_000u64;
  let mut fb=[0i32;256]; let mut gb=[0i32;256];
  let fbp=fb.as_mut_ptr(); let gbp=gb.as_mut_ptr();
  let mk=|s:&str|->Vec<i32>{ s.chars().map(|c|c as i32).chain(std::iter::once(0)).collect() };
  for fmt in ["%Y-%m-%d %H:%M:%S","%H:%M","%A","just text no directives"]{
    let w=mk(fmt); let fp=w.as_ptr();
    // byte check
    let fn2=unsafe{wa::wcsftime(fbp as *mut i32,256,fp as *const i32,tmpp as *const std::ffi::c_void)};
    let gn=unsafe{g_wsf(gbp,256,fp,tmpp)};
    let same=fn2==gn && fb[..fn2].iter().zip(gb[..gn].iter()).all(|(a,b)|a==b);
    let(f,g)=bench2(||{for _ in 0..iters{black_box(unsafe{wa::wcsftime(black_box(fbp as *mut i32),256,fp as *const i32,tmpp as *const std::ffi::c_void)});}},
                    ||{for _ in 0..iters{black_box(unsafe{g_wsf(black_box(gbp),256,fp,tmpp)});}});
    println!("wcsftime {:<24} fl={:7.2}ns glibc={:7.2}ns fl/glibc={:.3}{} match={}",fmt,f/iters as f64,g/iters as f64,f/g,tag(f/g),same);
  }
}
