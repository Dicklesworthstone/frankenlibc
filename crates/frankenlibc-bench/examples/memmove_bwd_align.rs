...
fn main(){
  ...
  for (osrc,n) in [(0usize,4096usize),(16,16384),(3,8192)]{
    ...
    let mut refbuf=vec![0u8;n]; for i in 0..n{ refbuf[i]=unsafe{*d0.sub(0).add(i).sub(0)}; let _=&mut refbuf; }
    // capture original src region first
    for i in 0..n{ refbuf[i]=unsafe{*s0.add(i)}; }
    unsafe{fa::memmove(d0 as *mut _, s0 as *const _, n);}
    for i in 0..n{ assert_eq!(unsafe{*d0.add(i)}, refbuf[i], "memmove bwd correctness osrc={osrc} n={n} i={i}"); }
    for b in buf.iter_mut(){ *b=0x5au8; }
  }
  ...
  for &n in &[4096usize,16384]{
    for (tag,osrc) in [("end_al",0usize),("end+16",16),("end+8",8)]{
      ...
      let iters=200_000u64; let(mut fl,mut gl)=(Vec::new(),Vec::new());
      for r in 0..50{
        if r%2==0{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memmove(d0,sp,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(d0,sp,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
        }else{let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(d0,sp,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memmove(d0,sp,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);}
      }
      let(f,gg)=(pctl(&fl,0.1),pctl(&gl,0.1));
      println!("memmove-bwd n={n:<6} {tag:<7} (dst+n)&31={:2} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}",(d0 as usize+n)&31,f/gg);
    }
  }
}
