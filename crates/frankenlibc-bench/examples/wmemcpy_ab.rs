...
fn main(){
  ...
  for &n in &[8usize,32,128,512,1024,2048,4096,8192,16384,32768]{
    let src=vec![0x41424344u32;n+16]; let mut dst=vec![0u32;n+16];
    let (dp,sp)=(dst.as_mut_ptr(),src.as_ptr());
    let iters=if n<=512{2_000_000u64}else{300_000};
    let(mut fl,mut gl)=(Vec::new(),Vec::new());
    for r in 0..60{
      if r%2==0{let t=Instant::now();for _ in 0..iters{black_box(unsafe{wa::wmemcpy(dp,sp,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
        ...
        let t=Instant::now();for _ in 0..iters{black_box(unsafe{wa::wmemcpy(dp,sp,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);}
    }
    let(f,gg)=(pctl(&fl,0.1),pctl(&gl,0.1));
    println!("wmemcpy nw={n:<6}({:>6}B) fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}{}",n*4,f/gg,if f/gg>1.25{"  <-- LOSS"}else if f/gg<0.95{"  win"}else{"  ~par"});
  }
}
