...
unsafe fn scan8(s:*const u32,c:u32)->(Option<usize>,usize){
  ...
  loop{
    ...
    if (eqc|eqz).any(){
      ...
      if zm!=0{ let p=zm.trailing_zeros() as usize; let cmb=eqc.to_bitmask()&((1u64<<p)-1);
        if cmb!=0{ last=Some(i+(63-cmb.leading_zeros() as usize)); } return(last,i+p+1); }
...
unsafe fn scan32(s:*const u32,c:u32)->(Option<usize>,usize){
  ...
  loop{
    while (unsafe{s.add(i)} as usize & 0xFFF)<=0x1000-128 {
      let c0=Simd::<u32,L>::from_array(unsafe{core::ptr::read(s.add(i).cast::<[u32;L]>())});
      let c1=Simd::<u32,L>::from_array(unsafe{core::ptr::read(s.add(i+8).cast::<[u32;L]>())});
      let c2=Simd::<u32,L>::from_array(unsafe{core::ptr::read(s.add(i+16).cast::<[u32;L]>())});
      let c3=Simd::<u32,L>::from_array(unsafe{core::ptr::read(s.add(i+24).cast::<[u32;L]>())});
      let e0=c0.simd_eq(cv)|c0.simd_eq(zv); let e1=c1.simd_eq(cv)|c1.simd_eq(zv);
      let e2=c2.simd_eq(cv)|c2.simd_eq(zv); let e3=c3.simd_eq(cv)|c3.simd_eq(zv);
      if (e0|e1|e2|e3).any(){
        let cm=c0.simd_eq(cv).to_bitmask()|(c1.simd_eq(cv).to_bitmask()<<8)|(c2.simd_eq(cv).to_bitmask()<<16)|(c3.simd_eq(cv).to_bitmask()<<24);
        let zm=c0.simd_eq(zv).to_bitmask()|(c1.simd_eq(zv).to_bitmask()<<8)|(c2.simd_eq(zv).to_bitmask()<<16)|(c3.simd_eq(zv).to_bitmask()<<24);
        if zm!=0{ let p=zm.trailing_zeros() as usize; let cmb=cm&((1u64<<p)-1);
          if cmb!=0{ last=Some(i+(63-cmb.leading_zeros() as usize)); } return(last,i+p+1); }
...
fn main(){
  for &n in &[16usize,64,256,1024,4096]{
    ...
    for (tag,c) in [("absent",b'Z' as u32),("mid",(b'a' as u32)+((n/2%20)as u32))]{
      assert_eq!(unsafe{scan8(sp,c)},unsafe{scan32(sp,c)},"mismatch n={n} {tag}");
      ...
      for r in 0..50{
        ...
        let nw=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{scan32(black_box(sp),c)});}t.elapsed().as_nanos()as f64/iters as f64};
