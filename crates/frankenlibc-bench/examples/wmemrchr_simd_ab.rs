...
unsafe fn scalar_rev(s:*const u32,c:u32,n:usize)->isize{
  ...
  match (0..n).rev().find(|&i| sl[i]==c){ Some(i)=>i as isize, None=>-1 }
}
...
unsafe fn simd_rev(s:*const u32,c:u32,n:usize)->isize{
  const L:usize=8; let cv=Simd::<u32,L>::splat(c); let mut i=n;
  // top remainder (highest indices first) scalar
  let rem=n%L;
  for _ in 0..rem{ i-=1; if unsafe{*s.add(i)}==c{ return i as isize; } }
  ...
  while i>=L{
    ...
    if m!=0{ return (i + (63 - m.leading_zeros() as usize)) as isize; }
  }
  -1
}
...
fn main(){
  for &n in &[16usize,64,256,1024,4096]{
    ...
    for (tag,setpos,c) in [("absent",None,b'Z' as u32),("front",Some(0usize),b'Z' as u32),("end",Some(n-1),b'Z' as u32),("mid",Some(n/2),b'Z' as u32)]{
      let mut b2=base.clone();
      if let Some(p)=setpos{ b2[p]=c; }
      let p2=b2.as_ptr();
      assert_eq!(unsafe{scalar_rev(p2,c,n)},unsafe{simd_rev(p2,c,n)},"mismatch n={n} {tag}");
      let iters=300_000u64; let(mut ov,mut nv)=(Vec::new(),Vec::new());
      for r in 0..50{
        let o=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{scalar_rev(black_box(p2),c,n)});}t.elapsed().as_nanos()as f64/iters as f64};
        let nw=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{simd_rev(black_box(p2),c,n)});}t.elapsed().as_nanos()as f64/iters as f64};
        if r%2==0{ov.push(o());nv.push(nw());}else{nv.push(nw());ov.push(o());}
      }
      let(o,nn)=(pctl(&ov,0.1),pctl(&nv,0.1));
      eprintln!("wmemrchr n={n:<5} {tag:<7} scalar={o:8.2}ns simd={nn:8.2}ns  simd/scalar={:.3} ({:.2}x)",nn/o,o/nn);
    }
    let _=sp;
  }
}
