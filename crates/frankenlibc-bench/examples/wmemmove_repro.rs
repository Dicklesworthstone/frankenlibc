...
fn main(){
  ...
  { let tmp:Vec<i32>=(0..n).map(|k|base[so+k]).collect(); for k in 0..n{ refb[dom+k]=tmp[k]; } }
  // fl
  let mut fb=base.clone();
  unsafe{ fl::wmemmove(fb.as_mut_ptr().add(dom) as *mut u32, fb.as_ptr().add(so) as *const u32, n); }
  ...
  unsafe{ g(gb.as_mut_ptr().add(dom),gb.as_ptr().add(so),n); }
  eprintln!("ref  ={:?}",&refb);
  eprintln!("fl   ={:?}",&fb);
  eprintln!("glibc={:?}",&gb);
  ...
  unsafe{ libc::memmove(fm.as_mut_ptr().add(dstb) as *mut _, fm.as_ptr().add(srcb) as *const _, lenb); }
  // glibc memmove
  let mut gm=bytes.clone();
  unsafe{ g_mm(gm.as_mut_ptr().add(dstb),gm.as_ptr().add(srcb),lenb); }
  eprintln!("memmove fl==glibc: {}", fm==gm);
  if fm!=gm { let fw:Vec<i32>=fm.chunks(4).map(|c|i32::from_ne_bytes(c.try_into().unwrap())).collect();
