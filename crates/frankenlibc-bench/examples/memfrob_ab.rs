...
unsafe fn scalar(p: *mut u8, n: usize) {
    for i in 0..n {
        unsafe { *p.add(i) ^= 42 };
    }
}
...
fn main() {
    ...
    for &n in &[16usize, 64, 256, 1024, 4096] {
        ...
        let mut b = a.clone();
        unsafe {
            scalar(a.as_mut_ptr(), n);
            simd(b.as_mut_ptr(), n);
        }
        assert_eq!(a, b, "mismatch n={n}");
        let mut buf: Vec<u8> = (0..n).map(|x| (x * 3 + 1) as u8).collect();
        ...
        for r in 0..40 {
            if r % 2 == 0 {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { ua::memfrob(black_box(bp as *mut std::ffi::c_void), n) });
                }
                fl.push(t.elapsed().as_nanos() as f64 / iters as f64);
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { g_mf(black_box(bp as *mut std::ffi::c_void), n) });
                }
        ...
        for r in 0..40 {
            let so = || {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { scalar(black_box(bp), n) });
                }
            ...
            let mo = || {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { simd(black_box(bp), n) });
                }
                t.elapsed().as_nanos() as f64 / iters as f64
            };
            if r % 2 == 0 {
        ...
        eprintln!(
            "memfrob n={n:<5} fl={f:6.2} glibc={g:6.2} fl/glibc={:.3} | AB scalar={sc:6.2} simd={sm:6.2} simd/scalar={:.3}",
            f / g,
            sm / sc
        );
    }
