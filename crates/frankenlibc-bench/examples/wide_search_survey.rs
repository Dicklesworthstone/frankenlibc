...
fn main() {
    ...
    for &n in &[16usize, 64, 256, 1024] {
        let s: Vec<u32> = (0..n as u32)
            .map(|x| b'a' as u32 + (x % 20))
            .chain(std::iter::once(0))
            .collect();
        let sp = s.as_ptr();
        let spi = sp as *const i32;
        // wcsrchr: search for a char present once near the middle
        let c = (b'a' as u32) + ((n / 2 % 20) as u32);
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { wa::wcsrchr(black_box(sp), c) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_rchr(black_box(spi), c as i32) });
                }
            },
        );
        println!(
            "wcsrchr  n={n:<5} fl={:6.2} glibc={:6.2} fl/glibc={:.3}{}",
            f / iters as f64,
            g / iters as f64,
            f / g,
            tag(f / g)
        ...
        let set: Vec<u32> = vec![b'X' as u32, b'Y' as u32, b'Z' as u32, 0];
        let setp = set.as_ptr();
        let seti = setp as *const i32;
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { wa::wcspbrk(black_box(sp), setp) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_pbrk(black_box(spi), seti) });
                }
            },
        );
        println!(
            "wcspbrk  n={n:<5} fl={:6.2} glibc={:6.2} fl/glibc={:.3}{}",
            f / iters as f64,
            g / iters as f64,
            f / g,
            tag(f / g)
        ...
        let acc: Vec<u32> = (0..20u32)
            .map(|x| b'a' as u32 + x)
            .chain(std::iter::once(0))
            .collect();
        let accp = acc.as_ptr();
        let acci = accp as *const i32;
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { wa::wcsspn(black_box(sp), accp) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_spn(black_box(spi), acci) });
                }
            },
        );
        println!(
            "wcsspn   n={n:<5} fl={:6.2} glibc={:6.2} fl/glibc={:.3}{}",
            f / iters as f64,
            g / iters as f64,
            f / g,
            tag(f / g)
        ...
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { wa::wcscspn(black_box(sp), setp) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_cspn(black_box(spi), seti) });
                }
            },
        );
        println!(
            "wcscspn  n={n:<5} fl={:6.2} glibc={:6.2} fl/glibc={:.3}{}",
            f / iters as f64,
            g / iters as f64,
            f / g,
            tag(f / g)
        ...
        let ndl: Vec<u32> = vec![b'q' as u32, b'q' as u32, b'q' as u32, 0];
        let np = ndl.as_ptr();
        let ni = np as *const i32;
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { wa::wcsstr(black_box(sp), np) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_str(black_box(spi), ni) });
                }
            },
        );
        println!(
            "wcsstr   n={n:<5} fl={:6.2} glibc={:6.2} fl/glibc={:.3}{}",
            f / iters as f64,
            g / iters as f64,
            f / g,
            tag(f / g)
        ...
        let s2 = s.clone();
        let s2p = s2.as_ptr() as *const i32;
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { wa::wcscasecmp(black_box(sp), black_box(s2.as_ptr())) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_case(black_box(spi), black_box(s2p)) });
                }
            },
        );
        println!(
            "wcscasecmp n={n:<3} fl={:6.2} glibc={:6.2} fl/glibc={:.3}{}",
            f / iters as f64,
            g / iters as f64,
            f / g,
            tag(f / g)
