...
fn main(){
  ...
  for _ in 0..10_000{ m::bench_malloc_guards_reresolve(); m::bench_malloc_guards_forslot(); }
  let iters=20_000_000u64;
  let(mut ov,mut nv)=(Vec::new(),Vec::new());
  for r in 0..80{
    let o=||{let t=Instant::now();for _ in 0..iters{ black_box(m::bench_malloc_guards_reresolve()); } t.elapsed().as_nanos()as f64/iters as f64};
    let nw=||{let t=Instant::now();for _ in 0..iters{ black_box(m::bench_malloc_guards_forslot()); } t.elapsed().as_nanos()as f64/iters as f64};
    if r%2==0{ov.push(o());nv.push(nw());}else{nv.push(nw());ov.push(o());}
  }
  let(o,nn)=(pctl(&ov,0.1),pctl(&nv,0.1));
  println!("malloc-guards OLD(double-resolve)={o:.2}ns NEW(reuse-slot)={nn:.2}ns  delta={:.2}ns  new/old={:.3} ({:.2}x)",o-nn,nn/o,o/nn);
}
