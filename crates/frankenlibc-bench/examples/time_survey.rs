...
fn main() {
    ...
    for &e in &epochs {
        ...
        let (f, g) = bench2(
            || {
                for _ in 0..iters {
                    black_box(unsafe { ta::gmtime_r(black_box(ep), ftp) });
                }
            },
            || {
                for _ in 0..iters {
                    black_box(unsafe { g_gmr(black_box(ep), gtp) });
                }
            },
        );
        ff.push(f);
    ...
    println!(
        "gmtime_r  fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",
        f / iters as f64,
        g / iters as f64,
        f / g,
        tag(f / g)
    ...
    let (f, g) = bench2(
        || {
            for _ in 0..iters {
                let mut t = mk_tm();
                black_box(unsafe { ta::timegm(black_box(&mut t)) });
            }
        },
        || {
            for _ in 0..iters {
                let mut t = mk_tm();
                black_box(unsafe { g_tg(black_box(&mut t)) });
            }
        },
    );
    println!(
        "timegm    fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",
        f / iters as f64,
        g / iters as f64,
        f / g,
        tag(f / g)
    ...
    let (f, g) = bench2(
        || {
            for _ in 0..iters {
                let mut t = mk_tm();
                black_box(unsafe { ta::mktime(black_box(&mut t)) });
            }
        },
        || {
            for _ in 0..iters {
                let mut t = mk_tm();
                black_box(unsafe { g_mk(black_box(&mut t)) });
            }
        },
    );
    println!(
        "mktime    fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",
        f / iters as f64,
        g / iters as f64,
        f / g,
        tag(f / g)
