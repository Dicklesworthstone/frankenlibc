...
fn main() {
    ...
    for (name, fl_ptr, gsym) in [
        ...
        for &z in &zs {
            ...
            let b = unsafe { g(z) };
            assert!(
                (a.re - b.re).abs() <= 1e-12 * (1.0 + b.re.abs())
                    && (a.im - b.im).abs() <= 1e-12 * (1.0 + b.im.abs()),
                "{name} {z:?}: fl=({},{}) g=({},{})",
                a.re,
                a.im,
                b.re,
        ...
        for r in 0..40 {
            if r % 2 == 0 {
                ...
                for _ in 0..iters {
                    for &z in &zs {
                        black_box(unsafe { fl(black_box(z)) });
                    }
                }
                flv.push(t.elapsed().as_nanos() as f64 / (iters * n) as f64);
                let t = Instant::now();
                for _ in 0..iters {
                    for &z in &zs {
                        black_box(unsafe { g(black_box(z)) });
                    }
                }
                glv.push(t.elapsed().as_nanos() as f64 / (iters * n) as f64);
            } else {
                let t = Instant::now();
                for _ in 0..iters {
                    for &z in &zs {
        ...
        let (f, gg) = (pctl(&flv, 0.1), pctl(&glv, 0.1));
        println!(
            "{:<7} fl={f:6.2} glibc={gg:6.2} fl/glibc={:.3}{}",
            name,
            f / gg,
            if f / gg > 1.25 {
                "  <-- LOSS"
