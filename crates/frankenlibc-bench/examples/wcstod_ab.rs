...
fn wide(s: &str, pad: usize) -> Vec<i32> {
...
fn main() {
    ...
    for (tag, s, pad) in [
        ("short", "3.14159", 0usize),
        ("sci", "1.5e-10", 0),
        ("long-buf", "2.71828", 4096),
    ] {
        let w = wide(s, pad);
        let p = w.as_ptr();
        // correctness
        let fv = unsafe { wa::wcstod(p, std::ptr::null_mut()) };
        let gv = unsafe { g(p, std::ptr::null_mut()) };
        assert!(
            (fv - gv).abs() < 1e-12,
            "val mismatch {tag}: fl={fv} glibc={gv}"
        );
        ...
        println!(
            "wcstod {tag:<9} fl={f:7.1} glibc={gg:7.1} fl/glibc={:.3}{}",
            f / gg,
            if f / gg > 1.25 {
                "  <-- LOSS"
            } else if f / gg < 0.95 {
