...
fn main(){
  ...
  unsafe{ let sl:unsafe extern "C" fn(i32,*const i8)->*mut i8=std::mem::transmute(libc::dlsym(h,b"setlocale\0".as_ptr().cast())); sl(6,b"C.UTF-8\0".as_ptr().cast()); }
  macro_rules! g{($n:literal)=>{unsafe{std::mem::transmute::<_,Fn1>(libc::dlsym(h,concat!($n,"\0").as_ptr().cast()))}}}
  use frankenlibc_abi::wchar_abi as wa;
  let iters=30_000u64;
  // (name, fl fn, glibc fn)
  macro_rules! row{($name:literal,$fl:path,$g:expr)=>{{
    let gf:Fn1=$g;
    let(f,gg)=bench2(
      ||{for _ in 0..iters{ for w in 0..128i32{ black_box(unsafe{$fl(black_box(w as u32))}); } }},
      ||{for _ in 0..iters{ for w in 0..128i32{ black_box(unsafe{gf(black_box(w))}); } }});
    let per_f=f/(iters as f64*128.0); let per_g=gg/(iters as f64*128.0);
    println!("{:<10} fl={per_f:5.2}ns glibc={per_g:5.2}ns fl/glibc={:.3}{}",$name,per_f/per_g,if per_f/per_g>1.25{"  <-- LOSS"}else if per_f/per_g<0.9{"  win"}else{"  ~par"});
  }}}
