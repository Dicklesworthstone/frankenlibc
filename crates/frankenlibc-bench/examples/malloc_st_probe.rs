...
fn main() {
    ...
    for &sz in &[16usize, 64, 256, 1024] {
        // warm
        for _ in 0..1000 { unsafe { let p = fl::malloc(sz); fl::free(p); let q = g_malloc(sz); g_free(q); } }
        let (mut fs, mut gs) = (Vec::new(), Vec::new());
        for _ in 0..80 {
            let t = Instant::now(); for _ in 0..it { unsafe { let p = fl::malloc(sz); std::hint::black_box(p); fl::free(p); } } fs.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now(); for _ in 0..it { unsafe { let p = g_malloc(sz); std::hint::black_box(p); g_free(p); } } gs.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        let (fp, gp) = (pctl(&fs, 0.5), pctl(&gs, 0.5));
        println!("MALLOC_FREE sz={sz} fl={fp:.2} glibc={gp:.2} fl/glibc={:.3}", fp/gp);
    }
}
