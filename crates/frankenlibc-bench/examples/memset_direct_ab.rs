...
fn main(){
  ...
  let mut checks=0u64;
  for n in [1usize,7,8,15,16,17,31,32,33,47,48,63,64,65,79,95,96,127,128,129,255,256,257,512,1023,1024,2047,2048,2049,4096].iter().copied(){
    for align in 0..16usize{
      for &val in &[0u8,0x5a,0xff,1]{
        let mut fb=vec![0xa5u8;n+align+32]; let mut gb=vec![0xa5u8;n+align+32];
        unsafe{fa::memset(fb.as_mut_ptr().add(align).cast(),val as i32,n);}
        unsafe{g(gb.as_mut_ptr().add(align).cast(),val as i32,n);}
  ...
  for &n in &[32usize,64,96,127,256,1024,4096,8192,16384,65536]{
    ...
    for r in 0..60{
      if r%2==0{
        let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memset(p,0x5a,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
        let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(p,0x5a,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
      }else{
        let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(p,0x5a,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
        let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memset(p,0x5a,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
      }
    ...
    println!("memset n={n:<6} fl={f:7.1} glibc={gg:7.1} fl/glibc={:.3}{}",f/gg,if f/gg>1.25{"  <-- LOSS"}else if f/gg<0.95{"  win"}else{"  ~par"});
  }
}
