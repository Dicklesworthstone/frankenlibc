...
fn main(){
  ...
  let g_ltr:LrFn=unsafe{std::mem::transmute(libc::dlsym(h,b"localtime_r\0".as_ptr().cast()))};
  use frankenlibc_abi::time_abi as ta;
  let iters=200_000u64;
  let e:i64=1_700_000_000; let ep=&e as *const i64;
  let(f,g)=bench2(||{for _ in 0..iters{black_box(unsafe{ta::gmtime(ep)});}},
                  ||{for _ in 0..iters{black_box(unsafe{g_gm(ep)});}});
  println!("gmtime      fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",f/iters as f64,g/iters as f64,f/g,tag(f/g));
  let(f,g)=bench2(||{for _ in 0..iters{black_box(unsafe{ta::localtime(ep)});}},
                  ||{for _ in 0..iters{black_box(unsafe{g_lt(ep)});}});
  println!("localtime   fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",f/iters as f64,g/iters as f64,f/g,tag(f/g));
  let mut ft:libc::tm=unsafe{std::mem::zeroed()}; let mut gt:libc::tm=unsafe{std::mem::zeroed()};
  let ftp=&mut ft as *mut libc::tm; let gtp=&mut gt as *mut libc::tm;
  let(f,g)=bench2(||{for _ in 0..iters{black_box(unsafe{ta::localtime_r(ep,ftp)});}},
                  ||{for _ in 0..iters{black_box(unsafe{g_ltr(ep,gtp)});}});
  println!("localtime_r fl={:6.2}ns glibc={:6.2}ns fl/glibc={:.3}{}",f/iters as f64,g/iters as f64,f/g,tag(f/g));
}
