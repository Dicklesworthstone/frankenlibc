...
fn old(x:f64,p:usize)->String{ let mut s=String::new(); let _=write!(s,"{x:.p$}"); s }
...
fn new(x:f64,p:usize)->Option<String>{
  ...
  if scaled.fract()!=0.0 || scaled>=1.8446744073709552e19 { return None; }
  ...
  if ds.len() < p+1 { let pad = p+1-ds.len(); ds = "0".repeat(pad)+&ds; }
  let point = ds.len()-p;
  let mut out=String::with_capacity(ds.len()+2);
  if x.is_sign_negative(){ out.push('-'); }
  out.push_str(&ds[..point]);
  if p>0 { out.push('.'); out.push_str(&ds[point..]); }
  Some(out)
}
fn main(){
  // dyadic non-integers + integers + <1 values
  let cases:&[(f64,usize)]=&[
    (2.5,6),(0.5,6),(0.25,6),(10.75,2),(3.125,6),(100.5,2),(0.125,3),(-8.5,2),
    (42.0,6),(0.0,6),(1234.5,4),(0.75,2),(255.25,2),(0.5,1),(63.5,6),(-0.25,4),
  ];
  for &(x,p) in cases{ if let Some(n)=new(x,p){ assert_eq!(old(x,p),n,"MISMATCH {x} .{p}: old={} new={}",old(x,p),n); } }
  ...
  println!("dyadf OLD(std .Pf)={o:.1}ns NEW(u128-build)={nn:.1}ns  new/old={:.3} ({:.2}x faster)",nn/o,o/nn);
}
