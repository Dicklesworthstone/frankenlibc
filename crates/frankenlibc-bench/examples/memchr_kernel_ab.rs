...
fn main() {
    ...
    for &n in &[48usize, 64, 96, 128, 192, 240] {
        ...
        assert_eq!(
            mem::memchr_medium_32bloop_for_bench(&buf, b'Z'),
        ...
        for r in 0..120 {
            ...
            let mut run_old = || {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(mem::memchr_medium_32bloop_for_bench(&buf, b'Z'));
                }
            ...
            let mut run_new = || {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(mem::memchr_medium_64lane_for_bench(&buf, b'Z'));
                }
        ...
        println!(
            "MEMCHR-KERNEL n={n:<4} p10: old={o10:.2} new={n10:.2} ratio={:.3} | p50 ratio={:.3}  {}",
            n10 / o10,
            n50 / o50,
            if n10 < o10 { "NEW WINS" } else { "old" }
        );
    }
}
