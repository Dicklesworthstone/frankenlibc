...
fn scan64(s: &[u32], c: u32) -> Option<usize> {
    ...
    let z = Simd::<u32, 64>::splat(0);
    let mut b = 0;
    while b + 256 <= s.len() {
        let hit = |k: usize| {
            let p = Simd::<u32, 64>::from_slice(&s[b + k * 64..b + (k + 1) * 64]);
            p.simd_min(p ^ t)
        };
        if hit(0)
            .simd_min(hit(1))
            .simd_min(hit(2).simd_min(hit(3)))
            .simd_eq(z)
            .any()
        {
            for j in b..b + 256 {
...
fn scan8(s: &[u32], c: u32, panels: usize) -> Option<usize> {
    ...
    while b + blk <= s.len() {
        let mut folded = {
            let p = Simd::<u32, 8>::from_slice(&s[b..b + 8]);
            p.simd_min(p ^ t)
        };
        for k in 1..panels {
            let p = Simd::<u32, 8>::from_slice(&s[b + k * 8..b + (k + 1) * 8]);
            folded = folded.simd_min(p.simd_min(p ^ t));
        }
...
fn main() {
    for &n in &[256usize, 1024, 4096, 16384] {
        ...
        let c = b'z' as u32; // absent needle, no nul -> full scan
        assert_eq!(scan64(&s, c), scan8(&s, c, 8), "m8 n={n}");
        assert_eq!(scan64(&s, c), scan8(&s, c, 16), "m16 n={n}");
        let iters = 300_000u64;
        let (mut v64, mut v8, mut v16) = (Vec::new(), Vec::new(), Vec::new());
        for _ in 0..50 {
            let t = Instant::now();
            for _ in 0..iters {
                black_box(scan64(black_box(&s), c));
            }
            v64.push(t.elapsed().as_nanos() as f64 / iters as f64);
            let t = Instant::now();
            for _ in 0..iters {
                black_box(scan8(black_box(&s), c, 8));
            }
            v8.push(t.elapsed().as_nanos() as f64 / iters as f64);
            let t = Instant::now();
            for _ in 0..iters {
                black_box(scan8(black_box(&s), c, 16));
            }
            v16.push(t.elapsed().as_nanos() as f64 / iters as f64);
        }
        let (a, b, d) = (pctl(&v64, 0.1), pctl(&v8, 0.1), pctl(&v16, 0.1));
        println!(
            "wcschr-fold n={n:<6} 64lane(1024B)={a:6.1} 8x8(256B)={b:6.1} 16x8(512B)={d:6.1}  256/orig={:.3} 512/orig={:.3}",
            b / a,
            d / a
        );
    }
