...
#[inline]
unsafe fn copy_8lane(dst: *mut u32, src: *const u32, count: usize) {
    let mut i = 0;
    while i + 8 <= count {
...
unsafe fn old(dst: *mut u32, src: *const u32, n: usize) {
    let c = unsafe { scan_n(src, n) };
    if c > 0 {
        unsafe { copy_8lane(dst, src, c) };
    }
    unsafe { *dst.add(c) = 0 };
}
...
unsafe fn new(dst: *mut u32, src: *const u32, n: usize) {
    let c = unsafe { fused_ncopy(dst, src, n) };
    unsafe { *dst.add(c) = 0 };
}
...
fn main() {
    for &(n, slen) in &[
        ...
        unsafe {
            old(d1.as_mut_ptr(), sp, n);
            new(d2.as_mut_ptr(), sp, n);
        }
        ...
        for r in 0..60 {
            let o = || {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { old(black_box(p1), black_box(sp), n) });
                }
            ...
            let nw = || {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { new(black_box(p2), black_box(sp), n) });
                }
                t.elapsed().as_nanos() as f64 / iters as f64
            };
            if r % 2 == 0 {
        ...
        eprintln!(
            "wcsncat n={n:<5} slen={slen:<5} OLD={o:7.2}ns fused={nn:7.2}ns  fused/old={:.3} ({:.2}x)",
            nn / o,
            o / nn
        );
    }
