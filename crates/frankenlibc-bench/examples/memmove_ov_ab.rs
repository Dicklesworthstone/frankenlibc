...
fn main() {
    ...
    let mut checks = 0u64;
    for n in [
        1usize, 7, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129, 255, 256, 300, 1000, 4096,
    ]
    .iter()
    .copied()
    {
        for delta in [
            -129i64, -128, -65, -64, -33, -32, -17, -16, -8, -3, -1, 1, 3, 8, 16, 17, 32, 33, 64,
            65, 128, 129,
        ]
        .iter()
        .copied()
        {
            ...
            let mut fb = init.clone();
            let mut gb = init.clone();
            // src at pad; dst at pad+delta
            let sidx = pad;
            let didx = (pad as i64 + delta) as usize;
            unsafe {
                fa::memmove(
                    fb.as_mut_ptr().add(didx).cast(),
                    fb.as_ptr().add(sidx).cast(),
                    n,
                );
            }
            unsafe {
                g(
                    gb.as_mut_ptr().add(didx).cast(),
                    gb.as_ptr().add(sidx).cast(),
                    n,
                );
            }
            assert_eq!(fb, gb, "MISMATCH n={n} delta={delta}");
            checks += 1;
        }
    }
    ...
    for &n in &[256usize, 1024, 4096, 16384, 65536] {
        let mut buf = vec![0x5au8; n + 256];
        let base = buf.as_mut_ptr();
        let (dpb, spb) = (
            unsafe { base.add(64) } as *mut core::ffi::c_void,
            base as *const core::ffi::c_void,
        );
        let (dpf, spf) = (base as *mut core::ffi::c_void, unsafe { base.add(64) }
        ...
        for (tag, dp, sp) in [("bwd", dpb, spb), ("fwd", dpf, spf)] {
            ...
            for r in 0..50 {
                if r % 2 == 0 {
                    let t = Instant::now();
                    for _ in 0..iters {
                        black_box(unsafe { fa::memmove(dp, sp, n) });
                    }
            ...
            println!(
                "memmove-{tag} n={n:<6} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}{}",
                f / gg,
                if f / gg > 1.25 {
                    "  <-- LOSS"
                } else if f / gg < 0.95 {
