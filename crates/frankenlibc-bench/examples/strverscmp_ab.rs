...
fn main() {
    ...
    for (a, b) in [
        ...
        ("foo-1.2.3", "foo-1.2.10"),
        ("aaaa", "aaab"),
        ("version-2024-06-15", "version-2024-06-16"),
    ] {
        ...
        for r in 0..80 {
            if r % 2 == 0 {
                ...
                for _ in 0..lit {
                    black_box(unsafe {
                        frankenlibc_abi::string_abi::strverscmp(ca.as_ptr(), cb.as_ptr())
                    });
                }
                fl.push(t.elapsed().as_nanos() as f64 / lit as f64);
                let t = Instant::now();
                for _ in 0..lit {
                    black_box(unsafe { g(ca.as_ptr(), cb.as_ptr()) });
                }
        ...
        println!(
            "STRVERSCMP {a:<20} fl={f10:.1} glibc={g10:.1} fl/glibc={:.3}",
            f10 / g10
        );
    }
}
