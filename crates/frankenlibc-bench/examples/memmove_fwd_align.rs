...
fn main(){
  ...
  for (od,n) in [(0usize,4096usize),(16,16384),(3,8192)]{
    ...
    let mut refbuf=vec![0u8;n]; for i in 0..n{ refbuf[i]=unsafe{*s0.add(i)}; }
    unsafe{fa::memmove(d0 as *mut _, s0 as *const _, n);}
    for i in 0..n{ assert_eq!(unsafe{*d0.add(i)}, refbuf[i], "memmove fwd correctness od={od} n={n} i={i}"); }
    // restore
    for (i,b) in buf.iter_mut().enumerate(){ *b = 0x5au8; let _=(i,); }
  }
  ...
  for &n in &[4096usize,16384]{
    for (tag,od) in [("dstalign",0usize),("dst+16",16),("dst+8",8)]{
      ...
      for r in 0..50{
        if r%2==0{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memmove(d0,s0,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(d0,s0,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
        }else{let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(d0,s0,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memmove(d0,s0,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);}
      }
      let(f,gg)=(pctl(&fl,0.1),pctl(&gl,0.1));
      println!("memmove-fwd n={n:<6} {tag:<9} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}",f/gg);
    }
  }
}
