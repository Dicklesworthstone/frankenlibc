...
fn main(){
  ...
  for cp in 0..128u32{
    let mut m=0u16;
    unsafe{
      if wa::iswalnum(cp)!=0 { m|=1<<0; }
      if wa::iswalpha(cp)!=0 { m|=1<<1; }
      if wa::iswblank(cp)!=0 { m|=1<<2; }
      if wa::iswcntrl(cp)!=0 { m|=1<<3; }
      if wa::iswdigit(cp)!=0 { m|=1<<4; }
      if wa::iswgraph(cp)!=0 { m|=1<<5; }
      if wa::iswlower(cp)!=0 { m|=1<<6; }
      if wa::iswprint(cp)!=0 { m|=1<<7; }
      if wa::iswpunct(cp)!=0 { m|=1<<8; }
      if wa::iswspace(cp)!=0 { m|=1<<9; }
      if wa::iswupper(cp)!=0 { m|=1<<10; }
      if wa::iswxdigit(cp)!=0 { m|=1<<11; }
    }
    if cp%8==0 { out.push_str("    "); }
