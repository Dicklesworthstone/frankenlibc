...
fn main(){
  ...
  for (od,n) in [(0usize,4096usize),(16,4096),(3,16384),(31,8192)]{
    ...
    unsafe{fa::memset(dp as *mut _, 0xab, n);}
    assert!(unsafe{std::slice::from_raw_parts(dp,n)}.iter().all(|&b|b==0xab),"memset correctness od={od} n={n}");
    unsafe{fa::memset(dp as *mut _, 0x00, n);}
  }
  println!("memset correctness OK");
  for &n in &[4096usize,16384]{
    for (tag,od) in [("dstalign",0usize),("dst+16",16),("dst+3",3),("dst+1",1)]{
      let dp=unsafe{(((base as usize+31)&!31) as *mut u8).add(od)} as *mut core::ffi::c_void;
      let iters=300_000u64; let(mut fl,mut gl)=(Vec::new(),Vec::new());
      for r in 0..50{
        if r%2==0{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memset(dp,0xab,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(dp,0xab,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
        }else{let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(dp,0xab,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memset(dp,0xab,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);}
      }
      let(f,gg)=(pctl(&fl,0.1),pctl(&gl,0.1));
      println!("memset n={n:<6} {tag:<9} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}",f/gg);
    }
  }
}
