...
fn main() {
    ...
    let cap = 1usize << 20;
    let src = vec![0x5au8; cap + 64];
    let mut dst = vec![0u8; cap + 64];
    let sp =
        unsafe { ((src.as_ptr() as usize + 31) & !31) as *const u8 } as *const core::ffi::c_void;
    let dp =
        unsafe { ((dst.as_mut_ptr() as usize + 31) & !31) as *mut u8 } as *mut core::ffi::c_void;
    for &n in &[16384usize, 32768, 65536, 131072, 262144, 524288] {
        ...
        println!(
            "memcpy n={n:<7} fl={f:9.1} glibc={gg:9.1} fl/glibc={:.3}{}",
            f / gg,
            if f / gg > 1.15 { "  <-- LOSS" } else { "" }
        );
    }
}
