...
fn main() {
    ...
    let mut run = |src_off: usize, dst_off: usize, n: usize, tag: &'static str| {
        ...
        unsafe {
            black_box(libc::memmove(
                base_f.add(dst_off) as *mut _,
                base_f.add(src_off) as *const _,
                n,
            ));
        ...
        if !eq && first.is_none() {
            first = Some((
                n,
                dst_off as isize - src_off as isize,
                (src_off.max(dst_off) + n) & 31,
                tag,
            ));
        }
        if !eq {
    ...
    for base_off in 0..64usize {
        for &n in &[
            1usize, 8, 16, 31, 32, 33, 63, 64, 65, 120, 127, 128, 129, 132, 160, 200, 255, 256,
            260, 384, 512, 600,
        ] {
