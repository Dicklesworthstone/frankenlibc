...
fn main(){
  ...
  for &n in &[4096usize,16384]{
    for (tag,o1,o2) in [("aligned32",0usize,0usize),("both+16",16,16),("skew",0,16)]{
      ...
      for r in 0..50{
        if r%2==0{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memcmp(ap,bp,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(ap,bp,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
        }else{let t=Instant::now();for _ in 0..iters{black_box(unsafe{g(ap,bp,n)});}gl.push(t.elapsed().as_nanos()as f64/iters as f64);
          let t=Instant::now();for _ in 0..iters{black_box(unsafe{fa::memcmp(ap,bp,n)});}fl.push(t.elapsed().as_nanos()as f64/iters as f64);}
      }
      let(f,gg)=(pctl(&fl,0.1),pctl(&gl,0.1));
      println!("memcmp n={n:<6} {tag:<10} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}",f/gg);
    }
  }
}
