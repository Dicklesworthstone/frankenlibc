...
fn main() {
    ...
    for &(re, im) in &zs {
        ...
        let (s, c) = math::sincos(im);
        let new = (r * c, r * s);
        assert_eq!(old.0.to_bits(), new.0.to_bits(), "re mismatch");
        assert_eq!(old.1.to_bits(), new.1.to_bits(), "im mismatch");
    }
    ...
    for r in 0..60 {
        let old = || {
            ...
            for _ in 0..iters {
                for &(re, im) in &zs {
                    let r = math::exp(black_box(re));
                    black_box((r * math::cos(black_box(im)), r * math::sin(black_box(im))));
                }
            }
        ...
        let new = || {
            ...
            for _ in 0..iters {
                for &(re, im) in &zs {
                    let r = math::exp(black_box(re));
                    let (s, c) = math::sincos(black_box(im));
                    black_box((r * c, r * s));
                }
            }
            t.elapsed().as_nanos() as f64 / (iters * n) as f64
    ...
    println!(
        "cexp-core  OLD(sin+cos)={o:.2}ns  NEW(sincos)={nn:.2}ns  new/old={:.3}  ({:.2}x faster)",
        nn / o,
        o / nn
    );
}
