...
unsafe fn fused8(dst:*mut u32,src:*const u32)->usize{
  let z=Simd::<u32,8>::splat(0); let pb=src as usize; let align=(pb&31)>>2;
  ...
  if m0!=0{let nul=m0.trailing_zeros()as usize-align; for j in 0..=nul{unsafe{*dst.add(j)=*src.add(j)};} return nul;}
  let first=8-align; for j in 0..first{unsafe{*dst.add(j)=*src.add(j)};}
  let mut i=first;
  loop{
    let v=Simd::<u32,8>::from_slice(unsafe{std::slice::from_raw_parts(src.add(i),8)});
    let m=v.simd_eq(z).to_bitmask();
    if m!=0{let nul=m.trailing_zeros()as usize; for j in 0..=nul{unsafe{*dst.add(i+j)=*src.add(i+j)};} return i+nul;}
    ...
    while (unsafe{src.add(i)} as usize & 0xFFF) <= 0x1000-128 {
      ...
      if c0.simd_min(c1).simd_min(c2.simd_min(c3)).simd_eq(z).any(){
        for (k,c) in [c0,c1,c2,c3].iter().enumerate(){
          ...
          if m!=0{let nul=m.trailing_zeros()as usize; let off=i+k*8;
            for j in 0..=nul{unsafe{*dst.add(off+j)=*src.add(off+j)};} return off+nul;}
...
fn main(){
  for &n in &[16usize,64,256,1024,4096]{
    ...
    let sp=src.as_ptr(); let mut d1=vec![0u32;n+8]; let mut d2=vec![0u32;n+8];
    let r1=unsafe{fused8(d1.as_mut_ptr(),sp)}; let r2=unsafe{fused32(d2.as_mut_ptr(),sp)};
    assert_eq!(r1,n,"len8 n={n}"); assert_eq!(r2,n,"len32 n={n}"); assert_eq!(&d1[..=n],&d2[..=n],"data n={n}");
    let iters=300_000u64; let(mut ov,mut nv)=(Vec::new(),Vec::new());
    let p1=d1.as_mut_ptr(); let p2=d2.as_mut_ptr();
    for r in 0..60{
      let o=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fused8(black_box(p1),black_box(sp))});}t.elapsed().as_nanos()as f64/iters as f64};
      let nw=||{let t=Instant::now();for _ in 0..iters{black_box(unsafe{fused32(black_box(p2),black_box(sp))});}t.elapsed().as_nanos()as f64/iters as f64};
      if r%2==0{ov.push(o());nv.push(nw());}else{nv.push(nw());ov.push(o());}
    }
    let(o,nn)=(pctl(&ov,0.1),pctl(&nv,0.1));
    eprintln!("wfused n={n:<5} 8lane={o:7.2}ns 128B={nn:7.2}ns  new/old={:.3} ({:.2}x)",nn/o,o/nn);
  }
}
