...
fn main() {
    ...
    for &n in &[64usize, 256, 1024, 4096] {
        ...
        let sp = s.as_ptr();
        let needle = b'z' as u32; // absent
        assert_eq!(
            unsafe { wa::wmemchr(sp, needle, n) }.is_null(),
            unsafe { g(sp as *const i32, needle as i32, n) }.is_null(),
            "wmemchr n={n}"
        );
        ...
        for r in 0..50 {
            if r % 2 == 0 {
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { wa::wmemchr(black_box(sp), needle, n) });
                }
                fl.push(t.elapsed().as_nanos() as f64 / iters as f64);
                let t = Instant::now();
                for _ in 0..iters {
                    black_box(unsafe { g(black_box(sp as *const i32), needle as i32, n) });
                }
        ...
        println!(
            "wmemchr n={n:<5} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}{}",
            f / gg,
