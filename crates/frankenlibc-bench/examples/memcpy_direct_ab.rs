...
fn main() {
    ...
    let mut checks = 0u64;
    for n in [
        1usize, 15, 16, 63, 64, 65, 127, 128, 129, 255, 256, 1023, 2048, 4095, 4096, 8192, 131071,
        131072, 131073, 262144,
    ]
    .iter()
    .copied()
    {
        for sa in 0..8usize {
            for da in 0..8usize {
                ...
                let mut fb = vec![0xa5u8; n + da + 16];
                let mut gb = vec![0xa5u8; n + da + 16];
                unsafe {
                    fa::memcpy(
                        fb.as_mut_ptr().add(da).cast(),
                        src.as_ptr().add(sa).cast(),
                        n,
                    );
                }
                unsafe {
                    g(
                        gb.as_mut_ptr().add(da).cast(),
                        src.as_ptr().add(sa).cast(),
                        n,
                    );
                }
                assert_eq!(fb, gb, "MISMATCH n={n} sa={sa} da={da}");
                checks += 1;
            }
        }
    }
    ...
    for &n in &[
        256usize, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144,
    ] {
        let src = vec![0x5au8; n + 256];
        let mut dst = vec![0u8; n + 256];
        let (dp, sp) = (
            unsafe { dst.as_mut_ptr().add(96) } as *mut core::ffi::c_void,
        ...
        println!(
            "memcpy n={n:<6} fl={f:8.1} glibc={gg:8.1} fl/glibc={:.3}{}",
            f / gg,
            if f / gg > 1.25 {
                "  <-- LOSS"
            } else if f / gg < 0.95 {
