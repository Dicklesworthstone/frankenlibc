...
fn main() {
    ...
    for &n in &[8usize, 38, 200] {
        ...
        println!(
            "FPUTS_ST n={n} fl={fp:.2} glibc={gp:.2} fl/glibc={:.3}",
            fp / gp
        );
    }
    ...
    for _ in 0..80 {
        let t = Instant::now();
        for _ in 0..it {
            black_box_usize(unsafe {
                frankenlibc_abi::stdio_abi::bench_canonical_stream_id_cost(black_box_ptr(ff))
            });
        }
        cs.push(t.elapsed().as_nanos() as f64 / it as f64);
    }
    println!(
        "CANONICAL_STREAM_ID_COST ns={:.2} (per call; the lock the ptr-keyed fast path skips)",
        pctl(&cs, 0.5)
    );
    ...
    let sp = s.as_ptr() as *const c_char;
    unsafe {
        assert!(fl::bench_fputs_oldpath(sp, ff));
        assert!(fl::bench_fputs_newpath(sp, ff));
        fl::fflush(ff);
    }
    ...
    for _ in 0..120 {
        ...
        for _ in 0..it {
            unsafe {
                black_box_bool(fl::bench_fputs_oldpath(sp, ff));
            }
        }
        unsafe {
            fl::fflush(ff);
        }
        os.push(t.elapsed().as_nanos() as f64 / it as f64);
        let t = Instant::now();
        for _ in 0..it {
            unsafe {
                black_box_bool(fl::bench_fputs_newpath(sp, ff));
            }
        }
        unsafe {
            fl::fflush(ff);
        }
        ns.push(t.elapsed().as_nanos() as f64 / it as f64);
    }
    let (op, np) = (pctl(&os, 0.5), pctl(&ns, 0.5));
    println!(
        "FPUTS_LOOKUP_AB old={op:.2} new={np:.2} new/old={:.3} saved={:.2}ns",
        np / op,
        op - np
    );
    ...
    unsafe {
        assert_eq!(fl::bench_feof_oldpath(ff), fl::bench_feof_newpath(ff));
    }
    let (mut fo, mut fn_) = (Vec::new(), Vec::new());
    for _ in 0..120 {
        let t = Instant::now();
        for _ in 0..it {
            black_box_i32(unsafe { fl::bench_feof_oldpath(black_box_ptr(ff)) });
        }
        fo.push(t.elapsed().as_nanos() as f64 / it as f64);
        let t = Instant::now();
        for _ in 0..it {
            black_box_i32(unsafe { fl::bench_feof_newpath(black_box_ptr(ff)) });
        }
        fn_.push(t.elapsed().as_nanos() as f64 / it as f64);
    }
    let (fop, fnp) = (pctl(&fo, 0.5), pctl(&fn_, 0.5));
    println!(
        "FEOF_AB old={fop:.2} new={fnp:.2} new/old={:.3} saved={:.2}ns",
        fnp / fop,
        fop - fnp
    );
    ...
    let ff2 = unsafe { fl::fopen(path, mode) };
    assert!(!ff2.is_null());
    unsafe {
        fl::setvbuf(ff2, std::ptr::null_mut(), libc::_IOFBF, cap);
    }
    let s: Vec<u8> = std::iter::repeat(b'x')
        .take(16)
        .chain(std::iter::once(0))
    ...
    for _ in 0..120 {
        ...
        for _ in 0..it {
            unsafe {
                black_box_i32(fl::fputs(sp, ff));
            }
        }
        unsafe {
            fl::fflush(ff);
        }
        single.push(t.elapsed().as_nanos() as f64 / it as f64);
        let t = Instant::now();
        for _ in 0..(it / 2) {
            unsafe {
                black_box_i32(fl::fputs(sp, ff));
                black_box_i32(fl::fputs(sp, ff2));
            }
        }
        unsafe {
            fl::fflush(ff);
        ...
        inter.push(t.elapsed().as_nanos() as f64 / it as f64);
    }
    let (sp_, ip_) = (pctl(&single, 0.5), pctl(&inter, 0.5));
    println!(
        "INTERLEAVE single={sp_:.2} two_stream={ip_:.2} inter/single={:.3}",
        ip_ / sp_
    );
    ...
    for &wn in &[8usize, 16, 64, 200] {
        let ws: Vec<i32> = std::iter::repeat(b'y' as i32)
            .take(wn)
            .chain(std::iter::once(0))
            .collect();
        let wp = ws.as_ptr();
        unsafe {
            frankenlibc_abi::wchar_abi::fputws(wp, ff);
            frankenlibc_abi::wchar_abi::bench_fputws_percall(wp, ff);
            g_fputws(wp, gf as *mut libc::c_void);
            fl::fflush(ff);
            g_fflush(gf);
        }
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..it {
                black_box_i32(unsafe { frankenlibc_abi::wchar_abi::fputws(wp, ff) });
            }
            unsafe {
                fl::fflush(ff);
            }
            nb.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box_i32(unsafe { frankenlibc_abi::wchar_abi::bench_fputws_percall(wp, ff) });
            }
            unsafe {
                fl::fflush(ff);
            }
            ob.push(t.elapsed().as_nanos() as f64 / it as f64);
            let t = Instant::now();
            for _ in 0..it {
                black_box_i32(unsafe { g_fputws(wp, gf as *mut libc::c_void) });
            }
            unsafe {
                g_fflush(gf);
            }
            gb.push(t.elapsed().as_nanos() as f64 / it as f64);
        }
        let (np, op, gp) = (pctl(&nb, 0.5), pctl(&ob, 0.5), pctl(&gb, 0.5));
        println!(
            "FPUTWS wn={wn} new={np:.2} old={op:.2} glibc={gp:.2} new/old={:.3} new/glibc={:.3}",
            np / op,
            np / gp
        );
    }
    ...
    let rf = unsafe { fl::fopen(rpath, rmode) };
    assert!(!rf.is_null());
    unsafe {
        fl::setvbuf(rf, std::ptr::null_mut(), libc::_IOFBF, cap);
    }
    let mut gbuf = [0u8; 256];
    unsafe {
        fl::fgetc(rf);
    } // populate the write cache for rf
    unsafe {
        assert!(fl::bench_fgets_oldpath(gbuf.as_mut_ptr() as *mut c_char, 256, rf).is_null());
        assert!(fl::bench_fgets_newpath(gbuf.as_mut_ptr() as *mut c_char, 256, rf).is_null());
    }
    ...
    for _ in 0..120 {
        ...
        for _ in 0..it {
            std::hint::black_box(unsafe {
                fl::bench_fgets_oldpath(gbuf.as_mut_ptr() as *mut c_char, 256, rf)
            });
        }
        go.push(t.elapsed().as_nanos() as f64 / it as f64);
        let t = Instant::now();
        for _ in 0..it {
            std::hint::black_box(unsafe {
                fl::bench_fgets_newpath(gbuf.as_mut_ptr() as *mut c_char, 256, rf)
            });
        }
        gn.push(t.elapsed().as_nanos() as f64 / it as f64);
    }
    let (gop, gnp) = (pctl(&go, 0.5), pctl(&gn, 0.5));
    println!(
        "FGETS_AB old={gop:.2} new={gnp:.2} new/old={:.3} saved={:.2}ns",
        gnp / gop,
        gop - gnp
    );
    ...
    let flf = unsafe { fl::fopen(fpath, rmode) };
    assert!(!flf.is_null());
    unsafe {
        fl::setvbuf(flf, std::ptr::null_mut(), libc::_IOFBF, cap);
    }
    let glf = unsafe { g_fopen(fpath, rmode) };
    assert!(!glf.is_null());
    unsafe {
        g_setvbuf(glf, std::ptr::null_mut(), libc::_IOFBF, cap);
    }
    let (mut flp, mut glp): (*mut c_char, *mut c_char) =
        (std::ptr::null_mut(), std::ptr::null_mut());
    let (mut fnc, mut gnc): (usize, usize) = (0, 0);
    // warm
    unsafe {
        while frankenlibc_abi::stdio_abi::getline(&mut flp, &mut fnc, flf) != -1 {}
        fl::rewind(flf);
        while g_getline(&mut glp, &mut gnc, glf) != -1 {}
        g_rewind(glf as *mut libc::c_void);
    }
    let iters = 2000u64;
    ...
    for _ in 0..80 {
        ...
        fl_v.push(t.elapsed().as_nanos() as f64 / (iters * nlines as u64) as f64);
        let t = Instant::now();
        for _ in 0..iters {
            unsafe {
                while g_getline(&mut glp, &mut gnc, glf) != -1 {}
        ...
        gl_v.push(t.elapsed().as_nanos() as f64 / (iters * nlines as u64) as f64);
    }
    let (flpl, glpl) = (pctl(&fl_v, 0.5), pctl(&gl_v, 0.5));
    println!(
        "GETLINE_PERLINE fl={flpl:.2} glibc={glpl:.2} fl/glibc={:.3} (ns/line incl amortized rewind)",
        flpl / glpl
    );
    ...
    type FreadFn =
        unsafe extern "C" fn(*mut libc::c_void, usize, usize, *mut libc::c_void) -> usize;
    ...
    let flr = unsafe { fl::fopen(bpath, rmode) };
    assert!(!flr.is_null());
    unsafe {
        fl::setvbuf(flr, std::ptr::null_mut(), libc::_IOFBF, cap);
    }
    let gr = unsafe { g_fopen(bpath, rmode) };
    assert!(!gr.is_null());
    unsafe {
        g_setvbuf(gr, std::ptr::null_mut(), libc::_IOFBF, cap);
    }
    ...
    for _ in 0..80 {
        ...
        for _ in 0..riter {
            unsafe {
                while frankenlibc_abi::stdio_abi::fread(chunk.as_mut_ptr().cast(), 1, 4096, flr)
                    == 4096
                {}
                fl::rewind(flr);
            }
        }
        frv.push(t.elapsed().as_nanos() as f64 / (riter * chunks_per) as f64);
        let t = Instant::now();
        for _ in 0..riter {
            unsafe {
                while g_fread(chunk.as_mut_ptr().cast(), 1, 4096, gr) == 4096 {}
                g_rewind(gr as *mut libc::c_void);
            }
        }
        grv.push(t.elapsed().as_nanos() as f64 / (riter * chunks_per) as f64);
    }
    let (frp, grp) = (pctl(&frv, 0.5), pctl(&grv, 0.5));
    println!(
        "FREAD_4K fl={frp:.2} glibc={grp:.2} fl/glibc={:.3} (ns per 4KiB fread)",
        frp / grp
    );
    ...
    for _ in 0..80 {
        ...
        for _ in 0..witer {
            for _ in 0..wchunks {
                unsafe {
                    std::hint::black_box(frankenlibc_abi::stdio_abi::fwrite(
                        src4k.as_ptr().cast(),
                        1,
                        4096,
                        ff,
        ...
        fwv.push(t.elapsed().as_nanos() as f64 / (witer * wchunks) as f64);
        let t = Instant::now();
        for _ in 0..witer {
            for _ in 0..wchunks {
                unsafe {
                    std::hint::black_box(g_fwrite(src4k.as_ptr().cast(), 1, 4096, gf));
                }
            }
            unsafe {
                g_fflush(gf);
        ...
        gwv.push(t.elapsed().as_nanos() as f64 / (witer * wchunks) as f64);
    }
    let (fwp, gwp) = (pctl(&fwv, 0.5), pctl(&gwv, 0.5));
    println!(
        "FWRITE_4K fl={fwp:.2} glibc={gwp:.2} fl/glibc={:.3} (ns per 4KiB fwrite)",
        fwp / gwp
    );
    ...
    let wnum: Vec<i32> = [b'1', b'2', b'3', b'4', b'5', b'6', b'7', b'8', 0]
        .iter()
        .map(|&b| b as i32)
        .collect();
    let wp2 = wnum.as_ptr();
    ...
    for _ in 0..100 {
        let t = Instant::now();
        for _ in 0..wit {
            black_box_i64(unsafe {
                frankenlibc_abi::wchar_abi::wcstol(wp2, std::ptr::null_mut(), 10)
            });
        }
        flw.push(t.elapsed().as_nanos() as f64 / wit as f64);
        let t = Instant::now();
        for _ in 0..wit {
            black_box_i64(unsafe { g_wcstol(wp2, std::ptr::null_mut(), 10) });
        }
        glw.push(t.elapsed().as_nanos() as f64 / wit as f64);
    }
    let (flwp, glwp) = (pctl(&flw, 0.5), pctl(&glw, 0.5));
    println!(
        "WCSTOL fl={flwp:.2} glibc={glwp:.2} fl/glibc={:.3}",
        flwp / glwp
    );
    ...
    let master: Vec<i32> = (0..nq)
        .map(|i| ((i * 2654435761usize) & 0x7fffffff) as i32)
        .collect();
    ...
    for _ in 0..80 {
        ...
        for _ in 0..qit {
            ...
            unsafe {
                frankenlibc_abi::stdlib_abi::qsort(arr.as_mut_ptr().cast(), nq, 4, Some(cmp_i32));
            }
            std::hint::black_box(arr[0]);
        }
        fqv.push(t.elapsed().as_nanos() as f64 / qit as f64);
        let t = Instant::now();
        for _ in 0..qit {
            arr.copy_from_slice(&master);
            unsafe {
                g_qsort(arr.as_mut_ptr().cast(), nq, 4, cmp_i32);
            }
            std::hint::black_box(arr[0]);
        }
        gqv.push(t.elapsed().as_nanos() as f64 / qit as f64);
    }
    let (fqp, gqp) = (pctl(&fqv, 0.5), pctl(&gqv, 0.5));
    println!(
        "QSORT_1024 fl={fqp:.2} glibc={gqp:.2} fl/glibc={:.3} (ns/sort incl reset)",
        fqp / gqp
    );
    ...
    for _ in 0..80 {
        ...
        fsv.push(t.elapsed().as_nanos() as f64 / qit as f64);
        let t = Instant::now();
        for _ in 0..qit {
            arr.copy_from_slice(&sorted);
            unsafe {
        ...
        gsv.push(t.elapsed().as_nanos() as f64 / qit as f64);
    }
    let (fsp, gsp) = (pctl(&fsv, 0.5), pctl(&gsv, 0.5));
    println!(
        "QSORT_SORTED fl={fsp:.2} glibc={gsp:.2} fl/glibc={:.3} (ns/sort incl reset)",
        fsp / gsp
    );
    ...
    type BsearchFn = unsafe extern "C" fn(
        *const libc::c_void,
        *const libc::c_void,
        usize,
        usize,
    ...
    for _ in 0..100 {
        ...
        fb.push(t.elapsed().as_nanos() as f64 / bit as f64);
        let t = Instant::now();
        for i in 0..bit {
            let k = keys[(i & 3) as usize];
            black_box_ptr3(unsafe {
        ...
        gb.push(t.elapsed().as_nanos() as f64 / bit as f64);
    }
    println!(
        "BSEARCH fl={:.2} glibc={:.2} fl/glibc={:.3}",
        pctl(&fb, 0.5),
        pctl(&gb, 0.5),
        pctl(&fb, 0.5) / pctl(&gb, 0.5)
    );
    // lfind: linear search (POSIX). Pays 2x tracked_region_fits (registry lookups); strict skips them.
    type LfindFn = unsafe extern "C" fn(
        *const libc::c_void,
        *const libc::c_void,
        *const usize,
        usize,
    ...
    let g_lfind: LfindFn = dl(h, b"lfind\0");
    {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..lit {
                black_box_ptr3(unsafe {
                    frankenlibc_abi::search_abi::lfind(
            ...
            fl2.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_ptr3(unsafe {
                    g_lfind(
            ...
            gl2.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        let _ = &mut nel;
        println!(
            "LFIND fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fl2, 0.5),
            pctl(&gl2, 0.5),
            pctl(&fl2, 0.5) / pctl(&gl2, 0.5)
        );
    }
    ...
    for &sn in &[8usize, 16, 32] {
        ...
        for _ in 0..80 {
            ...
            for _ in 0..sqit {
                ...
                std::hint::black_box(sa[0]);
            }
            fqs.push(t.elapsed().as_nanos() as f64 / sqit as f64);
            let t = Instant::now();
            for _ in 0..sqit {
                sa.copy_from_slice(&sm);
                unsafe {
                    g_qsort(sa.as_mut_ptr().cast(), sn, 4, cmp_i32);
                }
                std::hint::black_box(sa[0]);
            }
            gqs.push(t.elapsed().as_nanos() as f64 / sqit as f64);
        }
        println!(
            "QSORT_SMALL n={sn} fl={:.2} glibc={:.2} fl/glibc={:.3} (incl reset)",
            pctl(&fqs, 0.5),
            pctl(&gqs, 0.5),
            pctl(&fqs, 0.5) / pctl(&gqs, 0.5)
        );
    }
    // qsort_r: reentrant sort (thunk comparator). fl vs glibc (GNU qsort_r has arg LAST like fl).
    unsafe extern "C" fn cmp_i32_r(
        a: *const libc::c_void,
        b: *const libc::c_void,
    ...
    type CmpRFn =
        unsafe extern "C" fn(*const libc::c_void, *const libc::c_void, *mut libc::c_void) -> i32;
    type QsortRFn =
        unsafe extern "C" fn(*mut libc::c_void, usize, usize, CmpRFn, *mut libc::c_void);
    let g_qsort_r: QsortRFn = dl(h, b"qsort_r\0");
    {
        ...
        for _ in 0..80 {
            ...
            fq.push(t.elapsed().as_nanos() as f64 / sqit as f64);
            let t = Instant::now();
            for _ in 0..sqit {
                sa.copy_from_slice(&sm);
                unsafe {
            ...
            gq.push(t.elapsed().as_nanos() as f64 / sqit as f64);
        }
        println!(
            "QSORT_R n=8 fl={:.2} glibc={:.2} fl/glibc={:.3} (incl reset)",
            pctl(&fq, 0.5),
            pctl(&gq, 0.5),
            pctl(&fq, 0.5) / pctl(&gq, 0.5)
        );
    }
    ...
    for pat in [
        b"3.14159\0".as_ref(),
        b"1.7976931348623157e308\0".as_ref(),
        b"42\0".as_ref(),
    ] {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..dit {
                std::hint::black_box(unsafe {
                    frankenlibc_abi::stdlib_abi::strtod(dp, std::ptr::null_mut())
                });
            }
            fdv.push(t.elapsed().as_nanos() as f64 / dit as f64);
            let t = Instant::now();
            for _ in 0..dit {
                std::hint::black_box(unsafe { g_strtod(dp, std::ptr::null_mut()) });
            }
            gdv.push(t.elapsed().as_nanos() as f64 / dit as f64);
        }
        let (fdp, gdp) = (pctl(&fdv, 0.5), pctl(&gdv, 0.5));
        let s = std::str::from_utf8(&pat[..pat.len() - 1]).unwrap();
        println!(
            "STRTOD '{s}' fl={fdp:.2} glibc={gdp:.2} fl/glibc={:.3}",
            fdp / gdp
        );
    }
    ...
    for &n in &[64usize, 256, 1024] {
        let mut sv: Vec<u8> = std::iter::repeat(b'a').take(n).collect();
        sv[5] = b'b';
        sv.push(0);
        ...
        for _ in 0..100 {
            ...
            for _ in 0..sit {
                std::hint::black_box(unsafe {
                    frankenlibc_abi::string_abi::strrchr(scp, b'b' as i32)
                });
            }
            fsv.push(t.elapsed().as_nanos() as f64 / sit as f64);
            let t = Instant::now();
            for _ in 0..sit {
                std::hint::black_box(unsafe { g_strrchr(scp, b'b' as i32) });
            }
            gsv.push(t.elapsed().as_nanos() as f64 / sit as f64);
        }
        let (fsp, gsp) = (pctl(&fsv, 0.5), pctl(&gsv, 0.5));
        println!(
            "STRRCHR n={n} fl={fsp:.2} glibc={gsp:.2} fl/glibc={:.3}",
            fsp / gsp
        );
    }
    ...
    for &n in &[64usize, 256, 1024] {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..cit {
                std::hint::black_box(unsafe {
                    frankenlibc_abi::string_abi::strchr(scp, b'b' as i32)
                });
            }
            fcv.push(t.elapsed().as_nanos() as f64 / cit as f64);
            let t = Instant::now();
            for _ in 0..cit {
                std::hint::black_box(unsafe { g_strchr(scp, b'b' as i32) });
            }
            gcv.push(t.elapsed().as_nanos() as f64 / cit as f64);
            let t = Instant::now();
            for _ in 0..cit {
                std::hint::black_box(unsafe {
                    frankenlibc_abi::string_abi::memchr(scp.cast(), b'b' as i32, n)
                });
            }
            fmv.push(t.elapsed().as_nanos() as f64 / cit as f64);
            let t = Instant::now();
            for _ in 0..cit {
                std::hint::black_box(unsafe { g_memchr(scp.cast(), b'b' as i32, n) });
            }
            gmv.push(t.elapsed().as_nanos() as f64 / cit as f64);
        }
        println!(
            "STRCHR n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fcv, 0.5),
            pctl(&gcv, 0.5),
            pctl(&fcv, 0.5) / pctl(&gcv, 0.5)
        );
        println!(
            "MEMCHR n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fmv, 0.5),
            pctl(&gmv, 0.5),
            pctl(&fmv, 0.5) / pctl(&gmv, 0.5)
        );
    }
    ...
    for &n in &[16usize, 64, 256, 1024] {
        let sv: Vec<u8> = std::iter::repeat(b'a')
            .take(n)
            .chain(std::iter::once(0))
            .collect();
        let scp = sv.as_ptr() as *const c_char;
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                std::hint::black_box(unsafe { frankenlibc_abi::string_abi::strlen(scp) });
            }
            flv2.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                std::hint::black_box(unsafe { g_strlen(scp) });
            }
            glv2.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "STRLEN n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&flv2, 0.5),
            pctl(&glv2, 0.5),
            pctl(&flv2, 0.5) / pctl(&glv2, 0.5)
        );
    }
    ...
    for &n in &[8usize, 16, 24, 32, 48, 64, 128] {
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                std::hint::black_box(unsafe { frankenlibc_abi::string_abi::memcpy(dp, sp, n) });
            }
            flv.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                std::hint::black_box(unsafe { g_memcpy(dp, sp, n) });
            }
            glv.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "MEMCPY n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&flv, 0.5),
            pctl(&glv, 0.5),
            pctl(&flv, 0.5) / pctl(&glv, 0.5)
        );
    }
    ...
    let g_unlock: MtxFn = dl(h, b"pthread_mutex_unlock\0");
    {
        ...
        for _ in 0..1000 {
            unsafe {
                frankenlibc_abi::pthread_abi::pthread_mutex_lock(&mut fm);
                frankenlibc_abi::pthread_abi::pthread_mutex_unlock(&mut fm);
                g_lock(&mut gm);
                g_unlock(&mut gm);
            }
        }
        ...
        for _ in 0..100 {
            ...
            for _ in 0..lit {
                unsafe {
                    black_box_i32(frankenlibc_abi::pthread_abi::pthread_mutex_lock(&mut fm));
                    black_box_i32(frankenlibc_abi::pthread_abi::pthread_mutex_unlock(&mut fm));
                }
            }
            flv.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                unsafe {
                    black_box_i32(g_lock(&mut gm));
                    black_box_i32(g_unlock(&mut gm));
                }
            }
            glv.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "MUTEX_LOCKUNLOCK fl={:.2} glibc={:.2} fl/glibc={:.3} (ns/lock+unlock pair)",
            pctl(&flv, 0.5),
            pctl(&glv, 0.5),
            pctl(&flv, 0.5) / pctl(&glv, 0.5)
        );
    ...
    for &n in &[8usize, 16, 24, 32, 64, 256] {
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { frankenlibc_abi::string_abi::memcmp(ap, bp, n) });
            }
            flv.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_memcmp(ap, bp, n) });
            }
            glv.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "MEMCMP n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&flv, 0.5),
            pctl(&glv, 0.5),
            pctl(&flv, 0.5) / pctl(&glv, 0.5)
        );
    ...
    for &n in &[8usize, 16, 32, 64, 128] {
        ...
        let mut dbuf = vec![0u8; n + 1];
        let dp = dbuf.as_mut_ptr() as *mut c_char;
        // strcmp: compare against an equal copy differing at last byte
        let mut s2: Vec<u8> = s.clone();
        s2[n - 1] = b'b';
        let s2c = s2.as_ptr() as *const c_char;
        let (mut fcp, mut gcp, mut fcm, mut gcm) = (Vec::new(), Vec::new(), Vec::new(), Vec::new());
        let lit = 200_000u64;
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_ptr2(unsafe { frankenlibc_abi::string_abi::strcpy(dp, sc) });
            }
            fcp.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_ptr2(unsafe { g_strcpy(dp, sc) });
            }
            gcp.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { frankenlibc_abi::string_abi::strcmp(sc, s2c) });
            }
            fcm.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_strcmp(sc, s2c) });
            }
            gcm.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "STRCPY n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fcp, 0.5),
            pctl(&gcp, 0.5),
            pctl(&fcp, 0.5) / pctl(&gcp, 0.5)
        );
        println!(
            "STRCMP n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fcm, 0.5),
            pctl(&gcm, 0.5),
            pctl(&fcm, 0.5) / pctl(&gcm, 0.5)
        );
        ...
        for _ in 0..100 {
            ...
            for _ in 0..lit {
                black_box_usize(unsafe {
                    frankenlibc_abi::string_abi::bench_strcpy_fused(dp.cast(), sc.cast())
                });
            }
            fus.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "STRCPY_FUSED_AB n={n} twopass={:.2} fused={:.2} fused/twopass={:.3}",
            pctl(&fcp, 0.5),
            pctl(&fus, 0.5),
            pctl(&fus, 0.5) / pctl(&fcp, 0.5)
        );
    }
    ...
    for &n in &[8usize, 16, 32, 64] {
        let w: Vec<i32> = std::iter::repeat(b'a' as i32)
            .take(n)
            .chain(std::iter::once(0))
            .collect();
        let wp = w.as_ptr();
        let mut w2: Vec<i32> = w.clone();
        w2[n - 1] = b'b' as i32;
        let wp2 = w2.as_ptr();
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_usize(unsafe { frankenlibc_abi::wchar_abi::wcslen(wp.cast::<u32>()) });
            }
            fl_.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_usize(unsafe { g_wcslen(wp) });
            }
            gl_.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe {
                    frankenlibc_abi::wchar_abi::wcscmp(wp.cast::<u32>(), wp2.cast::<u32>())
                });
            }
            fc.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_wcscmp(wp, wp2) });
            }
            gc.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "WCSLEN n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fl_, 0.5),
            pctl(&gl_, 0.5),
            pctl(&fl_, 0.5) / pctl(&gl_, 0.5)
        );
        println!(
            "WCSCMP n={n} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fc, 0.5),
            pctl(&gc, 0.5),
            pctl(&fc, 0.5) / pctl(&gc, 0.5)
        );
    }
    ...
    let g_errno_loc: ErrnoLocFn = dl(h, b"__errno_location\0");
    {
        ...
        for _ in 0..100 {
            ...
            for _ in 0..lit {
                black_box_ptr(
                    unsafe { frankenlibc_abi::errno_abi::__errno_location() } as *mut libc::c_void
                );
            }
            fe.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_ptr(unsafe { g_errno_loc() } as *mut libc::c_void);
            }
            ge.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "ERRNO_LOC fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fe, 0.5),
            pctl(&ge, 0.5),
            pctl(&fe, 0.5) / pctl(&ge, 0.5)
        );
    }
    ...
    let g_lseek: LseekFn = dl(h, b"lseek\0");
    {
        ...
        if devnull >= 0 {
            ...
            for _ in 0..100 {
                ...
                for _ in 0..lit {
                    black_box_i64(unsafe {
                        frankenlibc_abi::unistd_abi::lseek(devnull, 0, libc::SEEK_SET)
                    });
                }
                fls.push(t.elapsed().as_nanos() as f64 / lit as f64);
                let t = Instant::now();
                for _ in 0..lit {
                    black_box_i64(unsafe { g_lseek(devnull, 0, libc::SEEK_SET) });
                }
                gls.push(t.elapsed().as_nanos() as f64 / lit as f64);
            }
            println!(
                "LSEEK fl={:.2} glibc={:.2} fl/glibc={:.3}",
                pctl(&fls, 0.5),
                pctl(&gls, 0.5),
                pctl(&fls, 0.5) / pctl(&gls, 0.5)
            );
            unsafe {
                libc::close(devnull);
            }
    ...
    let g_random: RandomFn = dl(h, b"random\0");
    {
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { frankenlibc_abi::stdlib_abi::rand() });
            }
            fr.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_rand() });
            }
            gr.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i64(unsafe { frankenlibc_abi::stdlib_abi::random() as i64 });
            }
            frnd.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i64(unsafe { g_random() as i64 });
            }
            grnd.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "RAND fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fr, 0.5),
            pctl(&gr, 0.5),
            pctl(&fr, 0.5) / pctl(&gr, 0.5)
        );
        println!(
            "RANDOM fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&frnd, 0.5),
            pctl(&grnd, 0.5),
            pctl(&frnd, 0.5) / pctl(&grnd, 0.5)
        );
    }
    ...
    for (label, af, s) in [
        ("v4", libc::AF_INET, b"192.168.1.100\0".as_slice()),
        (
            "v6",
            libc::AF_INET6,
            b"2001:db8::8a2e:370:7334\0".as_slice(),
        ),
    ] {
        let sp = s.as_ptr() as *const c_char;
        let mut buf = [0u8; 16];
        ...
        for _ in 0..100 {
            ...
            for _ in 0..lit {
                black_box_i32(unsafe {
                    frankenlibc_abi::inet_abi::inet_pton(af, sp, buf.as_mut_ptr().cast())
                });
                std::hint::black_box(buf[0]);
            }
            fp.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_pton(af, sp, buf.as_mut_ptr().cast()) });
                std::hint::black_box(buf[0]);
            }
            gp.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "INET_PTON {label} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fp, 0.5),
            pctl(&gp, 0.5),
            pctl(&fp, 0.5) / pctl(&gp, 0.5)
        );
    }
    ...
    {
        type NtopFn =
            unsafe extern "C" fn(i32, *const libc::c_void, *mut c_char, u32) -> *const c_char;
        ...
        for (label, af, src) in [
            ...
            for _ in 0..100 {
                ...
                for _ in 0..lit {
                    std::hint::black_box(unsafe {
                        frankenlibc_abi::inet_abi::inet_ntop(af, sp, outb.as_mut_ptr().cast(), 64)
                    });
                ...
                let t = Instant::now();
                for _ in 0..lit {
                    std::hint::black_box(unsafe { g_ntop(af, sp, outb.as_mut_ptr().cast(), 64) });
                    std::hint::black_box(outb[0]);
                }
                gp.push(t.elapsed().as_nanos() as f64 / lit as f64);
            }
            println!(
                "INET_NTOP {label} fl={:.2} glibc={:.2} fl/glibc={:.3}",
                pctl(&fp, 0.5),
                pctl(&gp, 0.5),
                pctl(&fp, 0.5) / pctl(&gp, 0.5)
            );
    ...
    // strict fast-path — pay full decide()+observe()+tracked_region_fits+bounded-cstr. dlmopen glibc.
    {
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { frankenlibc_abi::inet_abi::inet_aton(sp, &mut out) });
                std::hint::black_box(out);
            }
            fa.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_aton(sp, &mut out) });
                std::hint::black_box(out);
            }
            ga.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                std::hint::black_box(unsafe { frankenlibc_abi::inet_abi::inet_addr(sp) });
            }
            fd2.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                std::hint::black_box(unsafe { g_addr(sp) });
            }
            gd2.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "INET_ATON fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fa, 0.5),
            pctl(&ga, 0.5),
            pctl(&fa, 0.5) / pctl(&ga, 0.5)
        );
        println!(
            "INET_ADDR fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fd2, 0.5),
            pctl(&gd2, 0.5),
            pctl(&fd2, 0.5) / pctl(&gd2, 0.5)
        );
    }
    ...
    for &(dn, sn) in &[(8usize, 8usize), (32, 16), (64, 32)] {
        let src: Vec<u8> = std::iter::repeat(b'b')
            .take(sn)
            .chain(std::iter::once(0))
            .collect();
        ...
        for _ in 0..100 {
            ...
            for _ in 0..lit {
                unsafe {
                    dbuf[dn] = 0;
                    black_box_ptr2(frankenlibc_abi::string_abi::strcat(dp, scp));
                }
            }
            fc.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                unsafe {
                    dbuf[dn] = 0;
                    black_box_ptr2(g_strcat(dp, scp));
                }
            }
            gc.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "STRCAT dn={dn} sn={sn} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fc, 0.5),
            pctl(&gc, 0.5),
            pctl(&fc, 0.5) / pctl(&gc, 0.5)
        );
    ...
    let g_strspn: StrspnFn = dl(h, b"strspn\0");
    {
        ...
        let mut ndl: Vec<u8> = std::iter::repeat(b'a').take(32).collect();
        ndl[31] = b'c';
        let (hp, hl, np, nl) = (
            hay.as_ptr() as *const libc::c_void,
            hay.len(),
            ndl.as_ptr() as *const libc::c_void,
            ndl.len(),
        );
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_ptr3(unsafe { frankenlibc_abi::string_abi::memmem(hp, hl, np, nl) });
            }
            fm.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_ptr3(unsafe { g_memmem(hp, hl, np, nl) });
            }
            gm.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "MEMMEM_PATHO fl={:.1} glibc={:.1} fl/glibc={:.3}",
            pctl(&fm, 0.5),
            pctl(&gm, 0.5),
            pctl(&fm, 0.5) / pctl(&gm, 0.5)
        );
    }
    {
        ...
        let hay: Vec<u8> = (0..256u32)
            .map(|i| b'a' + (i.wrapping_mul(37) % 23) as u8)
            .collect();
        for nl in [3usize, 8, 16] {
            ...
            for _ in 0..100 {
                ...
                for _ in 0..lit {
                    black_box_ptr3(unsafe {
                        frankenlibc_abi::string_abi::memmem(hp, hln, np, nln)
                    });
                }
                fm.push(t.elapsed().as_nanos() as f64 / lit as f64);
                let t = Instant::now();
                for _ in 0..lit {
                    black_box_ptr3(unsafe { g_memmem(hp, hln, np, nln) });
                }
                gm.push(t.elapsed().as_nanos() as f64 / lit as f64);
            }
            println!(
                "MEMMEM_COMMON nl={nl} fl={:.1} glibc={:.1} fl/glibc={:.3}",
                pctl(&fm, 0.5),
                pctl(&gm, 0.5),
                pctl(&fm, 0.5) / pctl(&gm, 0.5)
            );
    ...
    }
    {
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_usize(unsafe { frankenlibc_abi::string_abi::strspn(sp, ap) });
            }
            fs2.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_usize(unsafe { g_strspn(sp, ap) });
            }
            gs2.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "STRSPN fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fs2, 0.5),
            pctl(&gs2, 0.5),
            pctl(&fs2, 0.5) / pctl(&gs2, 0.5)
        );
    }
    ...
    for &sz in &[16usize, 64, 256] {
        ...
        for _ in 0..1000 {
            unsafe {
                let p = frankenlibc_abi::malloc_abi::malloc(sz);
                frankenlibc_abi::malloc_abi::free(p);
                let q = g_malloc(sz);
                g_free(q);
            }
        }
        ...
        println!(
            "MALLOC_FREE sz={sz} fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&flv, 0.5),
            pctl(&glv, 0.5),
            pctl(&flv, 0.5) / pctl(&glv, 0.5)
        );
    ...
    let g_mktime: MktimeFn = dl(h, b"mktime\0");
    {
        ...
        let mut gt = mk();
        unsafe {
            frankenlibc_abi::time_abi::mktime(&mut ft);
            g_mktime(&mut gt);
        }
        let (mut flv, mut glv) = (Vec::new(), Vec::new());
        let lit = 200_000u64;
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i64(unsafe { frankenlibc_abi::time_abi::mktime(&mut ft) });
            }
            flv.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i64(unsafe { g_mktime(&mut gt) });
            }
            glv.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "MKTIME fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&flv, 0.5),
            pctl(&glv, 0.5),
            pctl(&flv, 0.5) / pctl(&glv, 0.5)
        );
    }
    {
        ...
        for _ in 0..100 {
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { frankenlibc_abi::stdlib_abi::atoi(sp) });
            }
            fa.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i32(unsafe { g_atoi(sp) });
            }
            ga.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i64(unsafe {
                    frankenlibc_abi::stdlib_abi::strtol(sp, std::ptr::null_mut(), 10)
                });
            }
            fs2.push(t.elapsed().as_nanos() as f64 / lit as f64);
            let t = Instant::now();
            for _ in 0..lit {
                black_box_i64(unsafe { g_strtol(sp, std::ptr::null_mut(), 10) });
            }
            gs2.push(t.elapsed().as_nanos() as f64 / lit as f64);
        }
        println!(
            "ATOI fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fa, 0.5),
            pctl(&ga, 0.5),
            pctl(&fa, 0.5) / pctl(&ga, 0.5)
        );
        println!(
            "STRTOL fl={:.2} glibc={:.2} fl/glibc={:.3}",
            pctl(&fs2, 0.5),
            pctl(&gs2, 0.5),
            pctl(&fs2, 0.5) / pctl(&gs2, 0.5)
        );
...
#[inline(never)]
fn black_box_ptr2(v: *mut c_char) -> *mut c_char {
    std::hint::black_box(v)
}

#[inline(never)]
fn black_box_i64(v: i64) -> i64 {
    std::hint::black_box(v)
}

#[inline(never)]
fn black_box_i32(v: i32) -> i32 {
    std::hint::black_box(v)
}

#[inline(never)]
fn black_box_bool(v: bool) -> bool {
    std::hint::black_box(v)
}

#[inline(never)]
fn black_box_usize(v: usize) -> usize {
    std::hint::black_box(v)
}
#[inline(never)]
fn black_box_ptr(v: *mut libc::c_void) -> *mut libc::c_void {
    std::hint::black_box(v)
}

#[inline(never)]
fn black_box_ptr3(v: *mut libc::c_void) -> *mut libc::c_void {
    std::hint::black_box(v)
}
