...
//!
//! NOTE ON PLACEMENT: this package's lib.rs is `#[path]`-included from
//! frankenlibc_conformance, but the BUILD SCRIPT must live here — this
//! package is what the harness graph links, so its build script is the one
//! cargo runs and whose link directives reach the final link.
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../frankenlibc_conformance/src/fortify_va_shim.c");
    ...
    let status = Command::new("cc")
        .args([
            ...
            "-Wextra",
            "-Werror",
            "../frankenlibc_conformance/src/fortify_va_shim.c",
            "-o",
    ...
    if !status.success() {
        panic!("cc failed compiling ../frankenlibc_conformance/src/fortify_va_shim.c");
    }
