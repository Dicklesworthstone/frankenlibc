...
mod tests {
    ...
    fn parse_rejects_numeric_overflow() {
        assert!(parse_maps_line("10000000000000000-10000000000001000 r--p 0 00:00 0").is_none());
        assert!(parse_maps_line("400000-401000 r--p 10000000000000000 00:00 0").is_none());
        assert!(
            parse_maps_line("400000-401000 r--p 0 00:00 18446744073709551616 /bin/cat").is_none()
        );
    }
    ...
    fn range_rejects_numeric_overflow() {
        assert!(parse_maps_range("10000000000000000-10000000000001000 r--p 0 00:00 0").is_none());
    }
