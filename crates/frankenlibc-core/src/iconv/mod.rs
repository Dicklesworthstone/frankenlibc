...
const PHASE1_CODEC_TABLE: [CodecSpec; 157] = [
    CodecSpec {
...
/// Inverse of [`decode_sbcs_full`]: full linear reverse lookup over all 256
/// entries (including the low half, since it is not ASCII), skipping `0xFFFF`
/// undefined slots. First match wins, matching glibc's canonical byte for the
/// bijective EBCDIC/ISO-646 tables.
fn encode_sbcs_full(ch: char, out: &mut [u8], table: &[u16; 256]) -> Result<usize, EncodeError> {
    if out.is_empty() {
        return Err(EncodeError::NoSpace);
    }
    let cp = ch as u32;
...
fn encode_char(enc: Encoding, ch: char, out: &mut [u8]) -> Result<usize, EncodeError> {
    match enc {
        ...
        Encoding::Cp037 => encode_sbcs_full(ch, out, &CP037_TO_UNICODE),
