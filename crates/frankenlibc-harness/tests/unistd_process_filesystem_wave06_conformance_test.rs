//! Deterministic wave-06 unistd/process/filesystem fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave06_conformance_test
...
const AMBIENT_POLICY: &str =
    "forbid_crypt_hash_salt_path_user_fd_daemon_gettext_dns_time_or_pointer_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave06.json");
...
#[test]
fn unistd_process_filesystem_wave06_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave06.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave06.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "bd-dx0rc");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-06-unistd-process-filesystem-crypt-terminal-gettext-dns"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("raw hashes")
            && fixture.description.contains("generated salt bytes")
            && fixture.description.contains("terminal paths")
            && fixture.description.contains("login names")
            && fixture.description.contains("file descriptors")
            && fixture.description.contains("gettext catalog files")
            && fixture.description.contains("DNS network data")
            && fixture.description.contains("process-global counters"),
        "fixture description must reject ambient hash/salt/path/user/fd/gettext/DNS capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave06_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn unistd_process_filesystem_wave06_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn unistd_process_filesystem_wave06_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for token in [
        "libcrypt",
        "POSIX ctermid",
...
#[test]
fn unistd_process_filesystem_wave06_coverage_artifacts_bind_first_wave() -> Result<(), String> {
    ...
    for symbol in FIRST_WAVE_SYMBOLS {
        ...
        assert!(
            json_array_has_str(
                &row["fixture_files"],
                "unistd_process_filesystem_wave06.json"
            ),
            "{symbol} coverage must cite unistd_process_filesystem_wave06.json"
        );
        assert!(
    ...
    assert!(
        campaign["current_coverage_pct"].as_f64().unwrap_or(0.0) >= 16.04,
        "unistd/process coverage percent must include wave-06 fixture rows"
    );
    assert!(
        campaign["target_covered"].as_u64().unwrap_or(0) >= 119,
        "unistd/process target_covered must include the 12 wave-06 symbols"
    );
    assert!(
        campaign["target_uncovered"].as_u64().unwrap_or(u64::MAX) <= 623,
        "unistd/process target_uncovered must shrink after wave-06 coverage"
    );
...
#[test]
fn unistd_process_filesystem_wave06_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-06 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
