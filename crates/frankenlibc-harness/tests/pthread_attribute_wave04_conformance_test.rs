//! Deterministic wave-04 pthread attribute fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test pthread_attribute_wave04_conformance_test
...
const AMBIENT_POLICY: &str =
    "forbid_pthread_object_address_native_thread_id_scheduler_mask_stack_or_global_counter_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/pthread_attribute_wave04.json");
...
#[test]
fn pthread_attribute_wave04_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/pthread_attribute_wave04.json");
    assert!(
        path.exists(),
        "pthread_attribute_wave04.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "pthread/sync");
    assert_eq!(fixture.campaign.bead, "bd-vm6j0.3");
    assert_eq!(fixture.campaign.campaign_id, "fcq-pthread-sync");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-04-pthread-attribute-setter-roundtrip"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("pthread attribute")
            && fixture
                .description
            ...
            && fixture.description.contains("CPU mask bytes")
            && fixture.description.contains("signal-mask bytes")
        ...
        "fixture description must reject ambient pthread attribute metadata capture"
    );
    Ok(())
}

#[test]
fn pthread_attribute_wave04_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "wave-04 fixture should not leave residual symbols"
    );
...
#[test]
fn pthread_attribute_wave04_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        for forbidden in [
            ...
            assert!(
                ...
                "case {} expected_output leaks ambient pthread attribute detail {forbidden}",
                case.name
            );
        }
    }
...
#[test]
fn pthread_attribute_wave04_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture
            .spec_reference
            .contains("POSIX/GNU pthread attribute")
            && fixture.spec_reference.contains("pthread_attr_init")
            && fixture
                .spec_reference
                .contains("pthread_attr_setaffinity_np")
            && fixture
                .spec_reference
                .contains("pthread_attr_setsigmask_np")
            && fixture.spec_reference.contains("pthread_attr_setstackaddr"),
        "fixture must bind pthread attribute init, setter, and stack surfaces"
    );
...
#[test]
fn pthread_attribute_wave04_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "pthread attribute wave-04 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
        ...
        assert_eq!(
            ...
            "wave-04 fixture should not use ambient host outputs for {}",
            case.name
        );
