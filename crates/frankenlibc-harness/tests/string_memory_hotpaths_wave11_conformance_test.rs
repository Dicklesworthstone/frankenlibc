//! Deterministic wave-11 string/memory hot-path fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave11_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave11.json");
...
#[test]
fn string_memory_hotpaths_wave11_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave11.json");
    assert!(
        path.exists(),
        "string_memory_hotpaths_wave11.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "string_memory_hotpaths_wave11");
    assert_eq!(fixture.campaign.bead, "pending-string-memory-wave11");
    assert_eq!(fixture.campaign.campaign_id, "fcq-string-memory-hotpaths");
    assert_eq!(fixture.campaign.wave_id, "wave-11-string-memory-hotpaths");
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave11_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave11_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave11_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave11_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
