//! Deterministic wave-14 string/memory hot-path fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave14_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave14.json");
...
#[test]
fn string_memory_hotpaths_wave14_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave14.json");
    assert!(
        path.exists(),
        "string_memory_hotpaths_wave14.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "string_memory_hotpaths_wave14");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.2");
    assert_eq!(fixture.campaign.campaign_id, "fcq-string-memory-hotpaths");
    assert_eq!(fixture.campaign.wave_id, "wave-14-string-memory-hotpaths");
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave14_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "wave-14 fixture should not leave residual symbols"
    );
...
#[test]
fn string_memory_hotpaths_wave14_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave14_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave14_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            "wave-14 fixture should not use ambient host outputs for {}",
            case.name
        );
