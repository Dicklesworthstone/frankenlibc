...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/symbol_drift_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_symbol_drift_completion_contract.sh")
}
...
fn unique_out_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let out = root.join("target/conformance").join(format!(
        "symbol_drift_completion_{label}_{}_{}",
        std::process::id(),
        stamp
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_SYMBOL_DRIFT_COMPLETION_CONTRACT", contract)
        .env(
            "FRANKENLIBC_SYMBOL_DRIFT_COMPLETION_REPORT",
            out_dir.join("symbol_drift_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_SYMBOL_DRIFT_COMPLETION_LOG",
            out_dir.join("symbol_drift_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
fn manifest_binds_all_completion_items() -> TestResult {
    ...
    assert_eq!(
        ...
        Some("symbol_drift_completion_contract.v1")
    );
    assert_eq!(manifest["original_bead"].as_str(), Some("bd-28s"));
    assert_eq!(manifest["completion_debt_bead"].as_str(), Some("bd-28s.1"));
    ...
    let details = &manifest["symbol_drift_contract"];
    assert_eq!(details["required_total_symbols"].as_u64(), Some(4119));
    assert_eq!(details["next_audit_threshold"].as_u64(), Some(900));
    assert_eq!(
        details["required_status_counts"]["Implemented"].as_u64(),
        Some(3705)
    );
    assert_eq!(
        details["required_status_counts"]["RawSyscall"].as_u64(),
        Some(414)
    );
    assert_eq!(details["required_duplicate_symbols"].as_u64(), Some(0));
...
fn checker_emits_report_and_jsonl() -> TestResult {
    ...
    let report = read_json(&out_dir.join("symbol_drift_completion_contract.report.json"))?;
    assert_eq!(
        report["schema_version"].as_str(),
        Some("symbol_drift_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["failure_signature"].as_str(), Some("none"));
    assert_eq!(report["summary"]["missing_item_count"].as_u64(), Some(3));
    assert_eq!(report["summary"]["symbol_count"].as_u64(), Some(4119));
    assert_eq!(
    ...
    assert_eq!(
        report["summary"]["duplicate_symbol_count"].as_u64(),
        Some(0)
    );
    assert_eq!(report["summary"]["stub_count"].as_u64(), Some(0));

    let rows = read_jsonl(&out_dir.join("symbol_drift_completion_contract.log.jsonl"))?;
...
fn checker_rejects_status_count_drift() -> TestResult {
    ...
    manifest["symbol_drift_contract"]["required_status_counts"]["RawSyscall"] =
        serde_json::json!(413);
