//! Deterministic wave-02 wchar/locale encoding fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test wchar_locale_encoding_wave02_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/wchar_locale_encoding_wave02.json");
...
#[test]
fn wchar_locale_encoding_wave02_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/wchar_locale_encoding_wave02.json");
    assert!(
        path.exists(),
        "wchar_locale_encoding_wave02.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "wchar/locale-encoding");
    assert_eq!(fixture.campaign.bead, "bd-vm6j0.5");
    assert_eq!(fixture.campaign.campaign_id, "fcq-wchar-locale-encoding");
    assert_eq!(fixture.campaign.wave_id, "wave-02-wchar-locale-encoding");
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("stateful multibyte/wchar")
            && fixture.description.contains("invalid-sequence")
            && fixture.description.contains("short-buffer")
            && fixture.description.contains("locale-parameter")
...
#[test]
fn wchar_locale_encoding_wave02_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn wchar_locale_encoding_wave02_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn wchar_locale_encoding_wave02_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
...
#[test]
fn wchar_locale_encoding_wave02_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "wchar/locale wave-02 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
