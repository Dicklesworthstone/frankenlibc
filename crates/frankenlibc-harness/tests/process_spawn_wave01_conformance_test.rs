//! Deterministic wave-01 process/spawn privilege and file-action fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test process_spawn_wave01_conformance_test
...
const AMBIENT_POLICY: &str = "forbid_ambient_pid_uid_gid_path_fd_or_process_state_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/process_spawn_wave01.json");
...
#[test]
fn process_spawn_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/process_spawn_wave01.json");
    assert!(
        path.exists(),
        "process_spawn_wave01.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "process/spawn");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.5");
    assert_eq!(fixture.campaign.campaign_id, "fcq-process-spawn");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-01-process-spawn-privilege-file-actions"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("process ids")
            && fixture.description.contains("raw uid/gid values")
            && fixture.description.contains("filesystem paths")
            && fixture.description.contains("open file descriptors")
            && fixture.description.contains("child stack addresses")
            && fixture.description.contains("argv/envp payloads")
            && fixture.description.contains("PATH contents")
        ...
        "fixture description must reject ambient process/spawn metadata capture"
    );
    Ok(())
}

#[test]
fn process_spawn_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn process_spawn_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        for forbidden in [
            ...
            assert!(
                ...
                "case {} expected_output leaks ambient process/spawn detail {forbidden}",
                case.name
            );
        }
    }
...
#[test]
fn process_spawn_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
...
#[test]
fn process_spawn_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "process/spawn wave-01 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
