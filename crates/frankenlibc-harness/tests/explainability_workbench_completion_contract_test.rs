//! Contract tests for bd-26xb.4.1 explainability workbench completion evidence.
...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/explainability_workbench_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_explainability_workbench_completion_contract.sh")
}
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let path = root.join("target/conformance").join(format!(
        "explainability-workbench-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<std::process::Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_EXPLAINABILITY_WORKBENCH_CONTRACT", contract)
        .env(
            "FRANKENLIBC_EXPLAINABILITY_WORKBENCH_REPORT",
            out_dir.join("explainability_workbench_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_EXPLAINABILITY_WORKBENCH_LOG",
            out_dir.join("explainability_workbench_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
#[test]
fn contract_binds_workbench_module_cli_and_existing_tests() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&contract_path(&root))?;
    assert_eq!(manifest["bead"].as_str(), Some("bd-26xb.4"));
    assert_eq!(
    ...
    let policy = &evidence["workbench_policy"];
    assert_eq!(policy["required_schema_version"].as_str(), Some("v1"));
    assert_eq!(policy["required_bead"].as_str(), Some("bd-26xb.4"));
    assert_eq!(
        string_set(&policy["required_modes"])?,
        BTreeSet::from(["hardened".to_string(), "strict".to_string()])
    );
    assert_eq!(
        string_set(&policy["required_render_formats"])?,
        BTreeSet::from(["ftui".to_string(), "json".to_string(), "plain".to_string()])
    );
    assert!(policy["minimum_traces"].as_u64().unwrap_or(0) >= 2);
    assert!(policy["minimum_artifact_links"].as_u64().unwrap_or(0) >= 1);
    assert!(policy["minimum_mode_divergences"].as_u64().unwrap_or(0) >= 1);

    let sources = source_texts(&root, &manifest)?;
    let unit_refs = assert_test_refs_exist(&evidence["unit_primary"], &sources)?;
    for expected in [
        "workbench_source::build_report_detects_mode_divergence_and_stage_chain",
        "workbench_source::artifact_index_entries_join_via_decision_id",
        "workbench_source::plain_render_mentions_divergence_and_artifacts",
        "workbench_source::ftui_render_mentions_title_and_scenario",
        "completion_harness_test::contract_binds_workbench_module_cli_and_existing_tests",
    ] {
        assert!(unit_refs.contains(expected), "missing unit ref {expected}");
    }
    let integration_refs = assert_test_refs_exist(&evidence["integration_primary"], &sources)?;
    assert!(integration_refs.contains(
        "existing_integration_test::explainability_workbench_command_writes_joined_report"
    ));
    assert!(
        integration_refs.contains(
            "completion_harness_test::checker_emits_report_log_and_validates_golden_shape"
        )
    );
    let e2e_refs = assert_test_refs_exist(&evidence["e2e_primary"], &sources)?;
    assert!(e2e_refs.contains("completion_harness_test::checker_rejects_missing_cli_flag"));
    assert!(
        e2e_refs.contains("completion_harness_test::checker_rejects_missing_required_source_ref")
    );
    Ok(())
}

#[test]
fn checker_emits_report_log_and_validates_golden_shape() -> TestResult {
    let root = workspace_root()?;
    let out_dir = run_passing_checker(&root, "pass")?;
    let report =
        read_json(&out_dir.join("explainability_workbench_completion_contract.report.json"))?;
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-26xb.4.1"));
    assert_eq!(report["summary"]["scenario_count"].as_u64(), Some(1));
    assert_eq!(report["summary"]["trace_count"].as_u64(), Some(2));
    assert_eq!(report["summary"]["artifact_link_count"].as_u64(), Some(1));
    assert_eq!(report["summary"]["divergence_count"].as_u64(), Some(1));
    assert_eq!(report["summary"]["render_format_count"].as_u64(), Some(3));

    let rows = read_jsonl(&out_dir.join("explainability_workbench_completion_contract.log.jsonl"))?;
    assert_eq!(
        rows.len(),
        5,
    ...
    for row in rows {
        ...
        assert!(
            row["trace_id"]
                .as_str()
                .is_some_and(|trace_id| trace_id.starts_with("bd-26xb.4.1:"))
        );
    }
    Ok(())
}
...
#[test]
fn checker_rejects_missing_cli_flag() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = read_json(&contract_path(&root))?;
    manifest["completion_debt_evidence"]["workbench_policy"]["required_cli_flags"]
        .as_array_mut()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "required_cli_flags array"))?
        .push(serde_json::Value::String(
            "--definitely-missing-workbench-flag".to_string(),
        ));

    let out_dir = unique_output_dir(&root, "missing-cli-flag")?;
    ...
    assert!(
        !output.status.success(),
        "checker should reject missing CLI flag:\n{}",
        checker_output_message(&output)
    );
    assert!(
        checker_output_message(&output).contains("existing integration test missing required text"),
        "failure should identify CLI evidence drift"
    );
    Ok(())
}
...
fn checker_rejects_missing_required_source_ref() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["implementation_refs"]
        ...
        .push(serde_json::Value::String(
            "crates/frankenlibc-harness/src/explainability_workbench.rs:999999".to_string(),
        ));
...
fn checker_rejects_local_cargo_validation_command() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["unit_primary"]["required_commands"][0] =
        serde_json::Value::String(
            "cargo test -p frankenlibc-harness explainability_workbench --lib".to_string(),
        );
