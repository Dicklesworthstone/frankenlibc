//! Deterministic wave-08 unistd/process/filesystem fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave08_conformance_test
...
const AMBIENT_POLICY: &str =
    "forbid_fd_thread_time_user_path_network_database_pointer_or_iterator_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave08.json");
...
#[test]
fn unistd_process_filesystem_wave08_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave08.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave08.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.1");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-08-unistd-process-filesystem-tty-utmp-epoll-ether"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("raw file descriptors")
            ...
            && fixture.description.contains("network database contents")
            && fixture.description.contains("pointer addresses")
            && fixture.description.contains("iterator state"),
        "fixture description must reject ambient fd/thread/time/user/path/database/pointer capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave08_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn unistd_process_filesystem_wave08_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert!(
            case.expected_errno == 0 || case.expected_errno == 9,
            "case {} exposes an unexpected errno class {}",
            case.name,
            case.expected_errno
        );
        assert!(
...
#[test]
fn unistd_process_filesystem_wave08_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for token in [
        "endservent",
        "endttyent",
...
#[test]
fn unistd_process_filesystem_wave08_coverage_artifacts_bind_first_wave() -> Result<(), String> {
    ...
    for symbol in FIRST_WAVE_SYMBOLS {
        ...
        assert!(
            json_array_has_str(
                &row["fixture_files"],
                "unistd_process_filesystem_wave08.json"
            ),
            "{symbol} coverage must cite unistd_process_filesystem_wave08.json"
        );
        assert!(
    ...
    assert!(
        campaign["current_coverage_pct"].as_f64().unwrap_or(0.0) >= 19.27,
        "unistd/process coverage percent must include wave-08 fixture rows"
    );
    assert!(
        campaign["target_covered"].as_u64().unwrap_or(0) >= 143,
        "unistd/process target_covered must include the 12 wave-08 symbols"
    );
    assert!(
        campaign["target_uncovered"].as_u64().unwrap_or(u64::MAX) <= 599,
        "unistd/process target_uncovered must shrink after wave-08 coverage"
    );
...
#[test]
fn unistd_process_filesystem_wave08_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-08 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
