//! Deterministic wave-02 fortify checked-wrapper fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test fortify_checked_wrapper_wave02_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave02.json");
...
#[test]
fn fortify_checked_wrapper_wave02_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave02.json");
    assert!(
        path.exists(),
        "fortify_checked_wrapper_wave02.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "fortify/checked-wrapper");
    assert_eq!(fixture.campaign.bead, "bd-vm6j0.6");
    assert_eq!(fixture.campaign.campaign_id, "fcq-fortify-bounds");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-02-fortify-checked-wrapper-bounds"
    );
    assert_eq!(
    ...
    assert!(
        fixture
            ...
            && fixture.description.contains("hostnames")
            && fixture.description.contains("domain names")
            && fixture.description.contains("group ids")
...
#[test]
fn fortify_checked_wrapper_wave02_covers_second_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn fortify_checked_wrapper_wave02_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn fortify_checked_wrapper_wave02_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in SECOND_WAVE_SYMBOLS {
        assert!(
...
#[test]
fn fortify_checked_wrapper_wave02_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
