...
#[test]
fn math_finite_special_wave03_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/math_finite_special_wave03.json");
    assert!(
        path.exists(),
...
#[test]
fn math_finite_special_wave03_covers_third_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn math_finite_special_wave03_logs_expected_classes_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        let expected = case.inputs["expected"]
            .as_str()
            .ok_or_else(|| format!("case {} missing expected class", case.name))?;
        assert!(
            matches!(
                expected,
...
#[test]
fn math_finite_special_wave03_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn math_finite_special_wave03_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
