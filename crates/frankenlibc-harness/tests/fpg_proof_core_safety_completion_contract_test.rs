...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/fpg_proof_core_safety_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_fpg_proof_core_safety_completion_contract.sh")
}
...
fn unique_out_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    let dir = root.join(format!(
        "target/conformance/fpg_proof_core_safety_completion_contract_test_{}_{}",
        std::process::id(),
        label
    ));
...
#[test]
fn manifest_binds_fpg_proof_core_safety_completion_items() -> TestResult {
    let root = repo_root()?;
    let manifest = read_json(&contract_path(&root))?;

    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("fpg_proof_core_safety_completion_contract.v1")
    );
    assert_eq!(manifest["bead"].as_str(), Some("bd-bp8fl.3.8"));
    assert_eq!(
    ...
    assert_eq!(source_artifacts.len(), 11);
    ...
    assert_eq!(contract["expected_input_artifact_count"].as_u64(), Some(6));
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some("block_proof_core_safety_gate_evidence")
    );
...
#[test]
fn checker_validates_fpg_proof_core_safety_contract_and_emits_report_log() -> TestResult {
    let root = repo_root()?;
    ...
    assert_eq!(report["source_bead"].as_str(), Some("bd-bp8fl.3.8"));
    assert_eq!(report["bead"].as_str(), Some("bd-bp8fl.3.8.1"));
    ...
    let rows = read_jsonl(&out_dir.join("fpg_proof_core_safety_completion_contract.log.jsonl"))?;
...
fn checker_rejects_missing_unit_test_ref() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_proof_core_safety_missing_unit_ref.json");
...
fn checker_rejects_missing_conformance_artifact_ref() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_proof_core_safety_missing_conformance_artifact.json");
    write_json(&mutated, &manifest)?;
    ...
    assert!(
        report["errors"]
            .to_string()
            .contains("conformance artifact refs mismatch"),
        "report should cite missing conformance artifacts: {report}"
    );
    Ok(())
}
...
fn checker_rejects_gap_count_drift() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_proof_core_safety_gap_count_drift.json");
    write_json(&mutated, &manifest)?;
...
fn checker_rejects_non_rch_cargo_command() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["unit_primary"]["required_commands"] =
        json!(["cargo test -p frankenlibc-harness --test fpg_proof_core_safety_gate_test"]);
    let mutated = out_dir.join("fpg_proof_core_safety_non_rch.json");
    write_json(&mutated, &manifest)?;
