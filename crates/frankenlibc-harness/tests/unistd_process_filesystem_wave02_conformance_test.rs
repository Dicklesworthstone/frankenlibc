//! Deterministic wave-02 unistd/process/filesystem conformance fixture tests.
//!
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave02.json");
...
#[test]
fn unistd_process_filesystem_wave02_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave02.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave02.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "bd-pz1g1.1");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-02-unistd-process-filesystem"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "forbid_ambient_process_filesystem_or_key_metadata"
    );
    assert!(
        fixture
            .description
            .contains("without recording ambient inode"),
        "fixture description must reject ambient filesystem metadata capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave02_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn unistd_process_filesystem_wave02_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            Some("forbid_ambient_process_filesystem_or_key_metadata"),
...
#[test]
fn unistd_process_filesystem_wave02_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture.spec_reference.contains("POSIX")
            && fixture.spec_reference.contains("GNU")
            && fixture.spec_reference.contains("Linux keyring")
...
#[test]
fn unistd_process_filesystem_wave02_coverage_artifacts_bind_first_wave() -> Result<(), String> {
...
#[test]
fn unistd_process_filesystem_wave02_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-02 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
