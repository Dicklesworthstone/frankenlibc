//! Conformance gate for the harness binary `runtime-math-linkage-proofs`
//! subcommand (bd-edrl3).
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("conformance")
        .join("runtime_math_linkage_proofs_cli_contract.v1.json")
}
...
fn tmp_dir() -> TestResult<PathBuf> {
    let ts = std::time::SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bd_edrl3_{}_{ts}", std::process::id()));
    std::fs::create_dir_all(&dir).map_err(|e| format!("mkdir {dir:?}: {e}"))?;
    Ok(dir)
}

#[test]
fn manifest_anchors_to_edrl3_with_subcommand_name() -> TestResult {
    let root = workspace_root()?;
    let m = load_json(&manifest_path(&root))?;
    require(
        json_string(&m, "manifest_id")? == "runtime-math-linkage-proofs-cli-contract",
        "manifest_id mismatch",
    )?;
    require(json_string(&m, "bead")? == "bd-edrl3", "bead mismatch")?;
    require(
        json_string(&m, "subcommand_name")? == "runtime-math-linkage-proofs",
...
fn manifest_policy_pins_required_invariants() -> TestResult {
    ...
    for (field, message) in [
        ...
        (
            "report_contains_summary_with_total_modules_count",
            "report_contains_summary_with_total_modules_count must be true",
        ),
        (
...
fn manifest_underlying_lib_function_is_pinned() -> TestResult {
    ...
    require(
        names.contains(&"frankenlibc_harness::runtime_math_linkage_proofs::run_and_write"),
...
#[test]
fn harness_source_registers_runtime_math_linkage_proofs_subcommand() -> TestResult {
    let root = workspace_root()?;
    let source = root
        .join("crates")
    ...
    require(
        body.contains("Command::RuntimeMathLinkageProofs"),
        "harness.rs must register Command::RuntimeMathLinkageProofs",
    )?;
    require(
        body.contains("runtime_math_linkage_proofs::run_and_write"),
        "harness.rs must call runtime_math_linkage_proofs::run_and_write",
    )?;
    Ok(())
}
...
fn cli_writes_log_and_report_files() -> TestResult {
    ...
    let dir = tmp_dir()?;
    let log = dir.join("linkage.log.jsonl");
    let report = dir.join("linkage.report.json");
    let root = workspace_root()?;
    let result = Command::new(&bin)
        .arg("runtime-math-linkage-proofs")
        .arg("--workspace-root")
        .arg(&root)
        .arg("--log")
        .arg(&log)
    ...
    require(
        ...
        "runtime-math-linkage-proofs CLI invocation must succeed",
    )?;
    ...
    require(
        ...
        "report.summary block must be present",
    )?;
    Ok(())
}

#[test]
fn cli_summary_records_total_modules_field() -> TestResult {
    let Some(bin) = find_harness_binary() else {
        return Ok(());
    };
    let dir = tmp_dir()?;
    let log = dir.join("mods.log.jsonl");
    let report = dir.join("mods.report.json");
    let root = workspace_root()?;
    ...
    require(
        summary
            ...
            .is_some(),
        "summary.total_modules must be u64",
    )?;
    require(
