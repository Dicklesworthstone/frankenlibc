//! Deterministic wave-04 residual fortify checked-wrapper fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test fortify_checked_wrapper_wave04_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave04.json");
...
#[test]
fn fortify_checked_wrapper_wave04_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave04.json");
    assert!(
        path.exists(),
        "fortify_checked_wrapper_wave04.json fixture must exist"
    );
    ...
    assert_eq!(
        ...
        "wave-04-fortify-checked-wrapper-residual"
    );
...
#[test]
fn fortify_checked_wrapper_wave04_covers_residual_symbols_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    let expected: BTreeSet<_> = WAVE04_SYMBOLS.iter().copied().collect();
    let declared: BTreeSet<_> = fixture
        ...
        .map(String::as_str)
        .collect();
    assert_eq!(declared, expected, "wave04 symbol drift");
...
#[test]
fn fortify_checked_wrapper_wave04_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            "expected_errno must stay classified; wave04 markers embed errno instead"
        );
        assert!(
...
#[test]
fn fortify_checked_wrapper_wave04_respects_ambient_policy_scenarios() -> Result<(), String> {
    let fixture = load_fixture()?;
    // Every wave04 scenario must be a capture-free replay: bad fds, missing
...
#[test]
fn fortify_checked_wrapper_wave04_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
