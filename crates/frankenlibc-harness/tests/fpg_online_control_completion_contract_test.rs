...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/fpg_online_control_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_fpg_online_control_completion_contract.sh")
}
...
fn unique_out_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    let dir = root.join(format!(
        "target/conformance/fpg_online_control_completion_contract_test_{}_{}",
        std::process::id(),
        label
    ));
...
fn manifest_binds_fpg_online_control_completion_items() -> TestResult {
    ...
    assert_eq!(
        ...
        Some("fpg_online_control_completion_contract.v1")
    );
    assert_eq!(manifest["bead"].as_str(), Some("bd-bp8fl.3.9"));
    assert_eq!(
    ...
    assert_eq!(contract["expected_gap_count"].as_u64(), Some(13));
    assert_eq!(
        contract["expected_required_log_field_count"].as_u64(),
        Some(16)
    );
    assert_eq!(contract["expected_input_artifact_count"].as_u64(), Some(10));
    assert_eq!(
        contract["claim_policy"]["default_decision"].as_str(),
        Some("block_done_until_online_control_evidence_current")
    );
    assert_eq!(
        contract["source_commit_freshness_policy"]["stale_result"].as_str(),
        Some("block_online_control_gate_evidence")
    );
...
fn checker_validates_fpg_online_control_contract_and_emits_report_log() -> TestResult {
    ...
    let report = read_json(&out_dir.join("fpg_online_control_completion_contract.report.json"))?;
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["source_bead"].as_str(), Some("bd-bp8fl.3.9"));
    assert_eq!(report["bead"].as_str(), Some("bd-bp8fl.3.9.1"));
    assert_eq!(
    ...
    assert!(
        rows.iter()
            .all(|row| row["bead"].as_str() == Some("bd-bp8fl.3.9.1"))
    );
...
fn checker_rejects_missing_unit_test_ref() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_online_control_missing_unit_ref.json");
...
fn checker_rejects_missing_conformance_artifact_ref() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_online_control_missing_conformance_artifact.json");
    write_json(&mutated, &manifest)?;
...
fn checker_rejects_gap_count_drift() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_online_control_gap_count_drift.json");
    write_json(&mutated, &manifest)?;
...
fn checker_rejects_non_rch_cargo_command() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["unit_primary"]["required_commands"] =
        json!(["cargo test -p frankenlibc-harness --test fpg_online_control_gate_test"]);
    let mutated = out_dir.join("fpg_online_control_non_rch.json");
    write_json(&mutated, &manifest)?;
...
fn checker_rejects_source_commit_policy_drift() -> TestResult {
    ...
    manifest["required_source_gate_contract"]["source_commit_freshness_policy"]["online_control_evidence_allowed_when_stale"] =
        json!(true);
    let mutated = out_dir.join("fpg_online_control_freshness_drift.json");
    write_json(&mutated, &manifest)?;
