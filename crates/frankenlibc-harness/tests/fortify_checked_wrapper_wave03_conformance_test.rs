//! Deterministic wave-03 fortify checked-wrapper fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test fortify_checked_wrapper_wave03_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave03.json");
...
#[test]
fn fortify_checked_wrapper_wave03_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave03.json");
    assert!(
        path.exists(),
        "fortify_checked_wrapper_wave03.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "fortify/checked-wrapper");
    assert_eq!(fixture.campaign.bead, "bd-gmbqy.6");
    assert_eq!(fixture.campaign.campaign_id, "fcq-fortify-bounds");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-03-fortify-checked-wrapper-bounds"
    );
    assert_eq!(
    ...
    assert!(
        fixture
            ...
            && fixture.description.contains("object-size")
            && fixture.description.contains("safe-default")
            && fixture.description.contains("errno/status")
            && fixture.description.contains("abort")
...
#[test]
fn fortify_checked_wrapper_wave03_covers_third_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn fortify_checked_wrapper_wave03_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    let mut saw_safe_default = false;
    ...
    for case in &fixture.cases {
        ...
        saw_abort |= case.expected_output.contains("ABORT_SIGNAL_SIGABRT");
        saw_errno |= case.expected_output.contains("ERRNO_");
        saw_safe_default |= case.expected_output.contains("EMPTY_TIMEOUT_0");
        for forbidden in [
            "0x",
            "/tmp/",
            "/dev/",
    ...
    assert!(saw_abort, "wave-03 must include abort classification");
    assert!(
        saw_errno,
        "wave-03 must include errno/status classification"
    );
    assert!(
        saw_safe_default,
        "wave-03 must include safe-default classification"
    );
    Ok(())
}

#[test]
fn fortify_checked_wrapper_wave03_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in THIRD_WAVE_SYMBOLS {
        assert!(
...
#[test]
fn fortify_checked_wrapper_wave03_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
