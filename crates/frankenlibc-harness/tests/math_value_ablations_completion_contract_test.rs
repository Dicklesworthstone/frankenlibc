//! Completion-contract tests for bd-1rxj.1 math value-proof ablation evidence.
...
const REQUIRED_EVENTS: &[&str] = &[
    ...
    "math_value_ablations_completion_contract_validated",
];
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/math_value_ablations_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_math_value_ablations_completion_contract.sh")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_MATH_VALUE_ABLATIONS_CONTRACT", manifest)
        .env("FRANKENLIBC_MATH_VALUE_ABLATIONS_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_MATH_VALUE_ABLATIONS_COMPLETION_REPORT",
            out_dir.join("math_value_ablations_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_MATH_VALUE_ABLATIONS_COMPLETION_LOG",
            out_dir.join("math_value_ablations_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
#[test]
fn manifest_binds_math_value_ablation_e2e_sources() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
    assert_eq!(
        string_field(&manifest, "schema_version", "manifest")?,
        "math_value_ablations_completion_contract.v1"
    );
    assert_eq!(string_field(&manifest, "bead_id", "manifest")?, "bd-1rxj.1");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "bd-1rxj.1::math-value-ablations::v1"
    );
    ...
    let e2e = field(&manifest, "e2e_contract", "manifest")?;
    assert_eq!(
        string_field(e2e, "missing_item_id", "e2e_contract")?,
        "tests.e2e.primary"
    );
    assert_eq!(
        field(e2e, "required_experiment_count", "e2e_contract")?.as_u64(),
        Some(25)
    );
    assert_eq!(
        field(e2e, "required_log_row_count", "e2e_contract")?.as_u64(),
        Some(50)
    );

    let bindings = array_field(&manifest, "missing_item_bindings", "manifest")?;
    let e2e_binding = bindings
        .iter()
        .find(|binding| {
            binding.get("spec_item").and_then(Value::as_str) == Some("tests.e2e.primary")
        })
        .ok_or_else(|| test_error("tests.e2e.primary binding should exist"))?;
    let tests: BTreeSet<_> = array_field(e2e_binding, "required_positive_tests", "e2e_binding")?
        .iter()
        .filter_map(Value::as_str)
        .collect();
    assert!(tests.contains("gate_script_emits_logs_and_report"));
    assert!(tests.contains("checker_accepts_math_value_ablation_completion_contract"));
    Ok(())
}

#[test]
fn checker_accepts_math_value_ablation_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "math-value-ablations-ok")?;
    ...
    assert_eq!(
        field(summary, "experiment_count", "summary")?.as_u64(),
        Some(25)
    );
    assert_eq!(
        field(summary, "value_proof_module_count", "summary")?.as_u64(),
        Some(25)
    );
    assert_eq!(
        field(summary, "base_gate_log_rows", "summary")?.as_u64(),
        Some(50)
    );

    let artifact_refs: BTreeSet<_> = array_field(&report, "artifact_refs", "report")?
        .iter()
        .filter_map(Value::as_str)
        .collect();
    for required in [
        ...
        assert!(
            artifact_refs.contains(required),
            "missing artifact ref {required}"
        );
    }
...
fn checker_rejects_missing_e2e_binding() -> TestResult {
    ...
    let (manifest_path, out_dir) =
        write_mutated_manifest(&root, "math-value-ablations-no-binding", &manifest)?;
...
#[test]
fn checker_rejects_incomplete_mode_contract() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = load_json(&manifest_path(&root))?;
    manifest["e2e_contract"]["required_modes"] = json!(["strict"]);
    let (manifest_path, out_dir) =
        write_mutated_manifest(&root, "math-value-ablations-one-mode", &manifest)?;

    let output = run_checker(&root, &manifest_path, &out_dir)?;
    expect_checker_failure(&output)?;
    let report = load_json(&checker_report(&out_dir))?;
    assert_eq!(string_field(&report, "status", "report")?, "fail");
    assert!(failure_signatures(&report).contains("missing_e2e_contract"));
    Ok(())
}
