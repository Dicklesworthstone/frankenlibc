//! String/memory hot-path conformance fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test string_memory_hotpaths_conformance_test
...
fn string_memory_hotpaths_fixture_exists_and_names_campaign() {
    ...
    assert_eq!(fixture.campaign.wave_id, "wave-04-string-memory-hotpaths");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "forbid_pointer_locale_or_process_buffer_capture"
    );
    assert!(
        fixture
            .description
...
fn string_memory_hotpaths_bind_logs_without_ambient_leaks() {
    ...
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            Some("forbid_pointer_locale_or_process_buffer_capture"),
        ...
        for forbidden in ["0x", "ptr=", "address", "locale_t", "errno_buf", "/tmp"] {
            assert!(
...
fn string_memory_hotpaths_spec_reference_names_gnu_posix_and_formatting() {
    ...
    assert!(
        fixture.spec_reference.contains("GNU libc")
            ...
            && fixture.spec_reference.contains("strerror")
...
fn string_memory_hotpaths_coverage_artifacts_bind_first_wave() {
    ...
    for symbol in FIRST_WAVE_SYMBOLS {
        ...
        assert!(
            json_array_has_str(&row["fixture_files"], "string_memory_hotpaths.json"),
            "{symbol} coverage must cite string_memory_hotpaths.json"
        );
        assert!(
