//! Completion-contract tests for bd-w2c3.5.1.1 risk/pareto evidence.
...
const CONTRACT_REL: &str = "tests/conformance/runtime_math_risk_pareto_completion_contract.v1.json";
const CHECKER_REL: &str = "scripts/check_runtime_math_risk_pareto_completion_contract.sh";
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let dir = root.join("target/conformance").join(format!(
        "runtime-math-risk-pareto-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_RUNTIME_MATH_RISK_PARETO_CONTRACT", contract)
        .env("FRANKENLIBC_RUNTIME_MATH_RISK_PARETO_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_RUNTIME_MATH_RISK_PARETO_REPORT",
            out_dir.join("report.json"),
        )
        .env(
            "FRANKENLIBC_RUNTIME_MATH_RISK_PARETO_LOG",
...
#[test]
fn manifest_binds_risk_pareto_unit_and_e2e_items() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&root.join(CONTRACT_REL))?;
    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("runtime_math_risk_pareto_completion_contract.v1")
    );
    assert_eq!(manifest["original_bead"].as_str(), Some("bd-w2c3.5.1"));
    assert_eq!(
    ...
    let truth = &manifest["required_source_truth"];
    assert_eq!(
        truth["calibration_artifact"]["bead"].as_str(),
        Some("bd-w2c3.5.1")
    );
    assert_eq!(
        truth["calibration_artifact"]["expected_decisions_per_mode"].as_u64(),
        Some(256)
    );
    for mode in ["strict", "hardened"] {
        assert!(
            truth["calibration_artifact"]["required_modes"]
                .as_array()
                .into_iter()
                .flatten()
                .any(|value| value.as_str() == Some(mode)),
            "manifest should require {mode} calibration"
        );
    }
    for check_id in [
        ...
        assert!(
            truth["source_checker"]["required_check_ids"]
                .as_array()
                .into_iter()
                .flatten()
                .any(|value| value.as_str() == Some(check_id)),
            "manifest should bind source check {check_id}"
        );
    }
...
#[test]
fn checker_emits_report_log_and_validates_calibration() -> TestResult {
    ...
    assert_eq!(
        ...
        Some("runtime_math_risk_pareto_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["original_bead"].as_str(), Some("bd-w2c3.5.1"));
    assert_eq!(
    ...
    assert!(
        report["summary"]["test_ref_count"].as_u64().unwrap_or(0) >= 10,
        "completion contract should bind source and completion tests"
    );
    assert_eq!(
        report["source_summary"]["modes"]["strict"]["decisions"].as_u64(),
        Some(256)
    );
    assert_eq!(
        report["source_summary"]["modes"]["hardened"]["decisions"].as_u64(),
        Some(256)
    );
    assert_eq!(
        report["source_summary"]["source_check_count"].as_u64(),
        Some(6)
    );
...
#[test]
fn checker_rejects_missing_source_check() -> TestResult {
    let _lock = checker_lock();
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "missing-check")?;
    let mut manifest = read_json(&root.join(CONTRACT_REL))?;
    let checks = manifest["required_source_truth"]["source_checker"]["required_check_ids"]
        .as_array_mut()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "required_check_ids array"))?;
    checks.retain(|value| value.as_str() != Some("real_workload_divergence_guard"));
    ...
    assert!(
        ...
        "checker should fail without source check binding"
    );
    assert!(
        output_text(&output).contains("required_source_truth.source_checker.required_check_ids"),
        "{}",
        output_text(&output)
    );
...
fn checker_rejects_non_rch_cargo_validation_command() -> TestResult {
    ...
    commands.push(Value::String(
        "cargo test -p frankenlibc-harness --test runtime_math_risk_pareto_calibration_test"
            .to_string(),
    ));
