//! Deterministic resolver/NSS core fixture wave-01 tests.
//!
//! Run: cargo test -p frankenlibc-harness --test resolver_nss_core_wave01_conformance_test
...
const AMBIENT_POLICY: &str = "forbid_dns_network_host_path_pointer_fd_time_or_process_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/resolver_nss_core_wave01.json");
...
#[test]
fn resolver_nss_core_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/resolver_nss_core_wave01.json");
    assert!(
        path.exists(),
        "resolver_nss_core_wave01.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "resolver_nss_core_wave01");
    assert_eq!(fixture.campaign.bead, "bd-kgy3r.1");
    assert_eq!(fixture.campaign.campaign_id, "fcq-resolver-nss-core");
    assert_eq!(fixture.campaign.wave_id, "wave-01-resolver-nss-core");
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("without live DNS")
            && fixture.description.contains("raw pointer addresses")
        ...
        "fixture description must reject ambient DNS/path/pointer/fd/time/process capture"
    );
    Ok(())
}

#[test]
fn resolver_nss_core_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "resolver/NSS wave should not leave residual symbols"
    );
...
#[test]
fn resolver_nss_core_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn resolver_nss_core_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn resolver_nss_core_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            "resolver/NSS wave should not use ambient host outputs for {}",
            case.name
        );
