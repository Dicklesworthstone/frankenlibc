//! Contract tests for bd-3ot.2.1 controller-ablation completion evidence.
...
fn read_manifest(root: &Path) -> TestResult<serde_json::Value> {
    let path =
        root.join("tests/conformance/runtime_math_controller_ablation_completion_contract.v1.json");
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let path = root.join("target/conformance").join(format!(
        "runtime-math-controller-ablation-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<std::process::Output> {
    Ok(Command::new("bash")
        .arg(root.join("scripts/check_runtime_math_controller_ablation_completion_contract.sh"))
        .current_dir(root)
        .env("FRANKENLIBC_RUNTIME_MATH_ABLATION_CONTRACT", contract)
        .env(
            "FRANKENLIBC_RUNTIME_MATH_ABLATION_REPORT",
            out_dir.join("runtime_math_controller_ablation_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_RUNTIME_MATH_ABLATION_LOG",
            out_dir.join("runtime_math_controller_ablation_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
#[test]
fn manifest_binds_unit_e2e_and_migration_evidence() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_manifest(&root)?;
    assert_eq!(manifest["bead"].as_str(), Some("bd-3ot.2"));
    assert_eq!(
    ...
    assert_eq!(evidence["bead"].as_str(), Some("bd-3ot.2.1"));
    assert_eq!(evidence["original_bead"].as_str(), Some("bd-3ot.2"));
    assert!(
    ...
    let report_path = artifacts["controller_ablation_report"]
        .as_str()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "ablation report path"))?;
    let report: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(root.join(report_path))?)?;
    assert_eq!(report["schema_version"].as_str(), Some("v1"));
    assert_eq!(report["status"].as_str(), Some("pass"));

    let expected = &evidence["expected_partition"];
    for field in [
        "total_modules",
        "production_retain",
        "research_retire",
        ...
        assert_eq!(
            ...
            expected[field].as_u64(),
            "ablation report summary should match expected {field}"
        );
    }
    ...
    assert_eq!(
        migration_plan["feature_gate"].as_str(),
        evidence["migrations_primary"]["required_feature_gate"].as_str()
    );
    assert_eq!(migration_plan["total_to_retire"].as_u64(), Some(retired));
    assert!(
        migration_plan["modules"]
            .as_array()
    ...
    let production_manifest_path = artifacts["production_manifest"]
        .as_str()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "production manifest path"))?;
    let production_manifest: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(
        root.join(production_manifest_path),
    )?)?;
    assert!(
        string_set(&production_manifest["default_feature_set"])?
            .contains("runtime-math-production"),
        "default feature set should retain production runtime math"
    );
    assert!(
        string_set(&production_manifest["optional_feature_set"])?.contains("runtime-math-research"),
        "optional feature set should expose research annex"
    );
...
fn checker_emits_report_and_jsonl() -> TestResult {
    ...
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-3ot.2.1"));
    assert_eq!(report["original_bead"].as_str(), Some("bd-3ot.2"));
    assert_eq!(report["total_modules"].as_u64(), Some(69));
    assert_eq!(report["production_retain"].as_u64(), Some(25));
    assert_eq!(report["research_retire"].as_u64(), Some(44));
    ...
    assert!(json_str_field_is(
        row,
        "event",
        "runtime_math_controller_ablation_completion_contract_validated"
    ));
    assert!(json_str_field_is(row, "completion_debt_bead", "bd-3ot.2.1"));
    assert!(json_str_field_is(row, "status", "pass"));
    assert!(json_str_field_is(row, "failure_signature", "none"));
    assert_eq!(row["research_retire"].as_u64(), Some(44));

    Ok(())
}

#[test]
fn checker_rejects_missing_research_migration_binding() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "stale")?;
    let mut manifest = read_manifest(&root)?;
    manifest["completion_debt_evidence"]["migrations_primary"]["required_feature_gate"] =
        serde_json::json!("runtime-math-stale-research");
    let stale_contract = out_dir.join("stale_runtime_math_controller_ablation_contract.v1.json");
    write_json(&stale_contract, &manifest)?;

    let output = run_checker(&root, &stale_contract, &out_dir)?;
    assert!(
        !output.status.success(),
        "checker should reject stale research feature-gate binding"
    );
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&output.stdout),
    ...
    assert!(
        combined.contains("migration_plan.feature_gate must be"),
        "checker output should name migration feature gate drift: {combined}"
    );
    ...
    assert!(
        report["errors"]
            ...
            .any(|item| item
                .as_str()
                .unwrap_or("")
                .contains("migration_plan.feature_gate must be")),
        "report should retain migration feature-gate error"
    );
    ...
    assert!(json_str_field_is(
        &rows[0],
        "event",
        "runtime_math_controller_ablation_completion_contract_failed"
    ));
