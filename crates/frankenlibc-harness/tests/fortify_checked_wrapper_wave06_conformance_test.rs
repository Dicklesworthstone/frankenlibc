//! Deterministic wave-06 residual fortify checked-wrapper fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test fortify_checked_wrapper_wave06_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave06.json");
...
#[test]
fn fortify_checked_wrapper_wave06_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave06.json");
    assert!(
        path.exists(),
        "fortify_checked_wrapper_wave06.json fixture must exist"
    );
    ...
    assert_eq!(
        ...
        "wave06 belongs to the replacement-integration bead"
    );
    assert_eq!(fixture.campaign.campaign_id, "fcq-fortify-bounds");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-06-fortify-checked-wrapper-variadic-shim"
    );
    ...
    let residual: BTreeSet<_> = fixture
        ...
        .map(String::as_str)
        .collect();
    let expected_residual: BTreeSet<_> = ["__vfwprintf_chk", "__syslog_chk", "__vsyslog_chk"]
        .into_iter()
        .collect();
    assert_eq!(residual, expected_residual, "wave06 residual drift");
    assert_eq!(fixture.campaign.ambient_state_policy, AMBIENT_POLICY);
    Ok(())
}

#[test]
fn fortify_checked_wrapper_wave06_covers_residual_symbols_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    let expected: BTreeSet<_> = WAVE06_SYMBOLS.iter().copied().collect();
    let declared: BTreeSet<_> = fixture
        ...
        .map(String::as_str)
        .collect();
    assert_eq!(declared, expected, "wave06 symbol drift");
...
#[test]
fn fortify_checked_wrapper_wave06_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            "expected_errno must stay classified; wave06 markers embed errno instead"
        );
        assert!(
...
#[test]
fn fortify_checked_wrapper_wave06_respects_ambient_policy_scenarios() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn fortify_checked_wrapper_wave06_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
