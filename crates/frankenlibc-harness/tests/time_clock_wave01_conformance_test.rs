//! Deterministic wave-01 time/clock fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test time_clock_wave01_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/time_clock_wave01.json");
...
#[test]
fn time_clock_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/time_clock_wave01.json");
    assert!(path.exists(), "time_clock_wave01.json fixture must exist");

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "time/clock");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.10");
    assert_eq!(fixture.campaign.campaign_id, "fcq-time-clock");
    assert_eq!(fixture.campaign.wave_id, "wave-01-time-clock-calendar");
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("fixed UTC calendar")
            && fixture.description.contains("invalid-clock")
            && fixture.description.contains("zero-duration")
            && fixture.description.contains("gettimeofday")
            && fixture.description.contains("environment global shape")
            && fixture.description.contains("host wall-clock values")
            && fixture.description.contains("raw environment text")
        ...
        "fixture description must reject ambient time/environment capture"
    );
    Ok(())
}

#[test]
fn time_clock_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "time/clock wave should not leave residual symbols"
    );
...
#[test]
fn time_clock_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn time_clock_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
    ...
    assert!(
        fixture.spec_reference.contains("POSIX")
            && fixture.spec_reference.contains("Linux clock_adjtime")
            && fixture.spec_reference.contains("glibc environ"),
        "spec reference must name POSIX, Linux clock_adjtime, and glibc environ surfaces"
    );
    Ok(())
}

#[test]
fn time_clock_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
