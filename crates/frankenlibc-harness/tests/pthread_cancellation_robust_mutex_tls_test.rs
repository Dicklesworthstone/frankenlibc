...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/pthread_cancellation_robust_mutex_tls.v1.json")
}

fn script_path(root: &Path) -> PathBuf {
    root.join("scripts/check_pthread_cancellation_robust_mutex_tls.sh")
}
...
fn run_gate(root: &Path, manifest: Option<&Path>, out_dir: &Path) -> TestResult<Output> {
    ...
    command
        ...
        .current_dir(root)
        .env("FLC_PTHREAD_HARDPARTS_OUT_DIR", out_dir)
        .env(
            "FLC_PTHREAD_HARDPARTS_REPORT",
            out_dir.join("pthread-hardparts.report.json"),
        )
        .env(
            "FLC_PTHREAD_HARDPARTS_LOG",
            out_dir.join("pthread-hardparts.log.jsonl"),
        )
        .env("FLC_PTHREAD_HARDPARTS_TARGET_DIR", out_dir);
    if let Some(manifest) = manifest {
        command.env("FLC_PTHREAD_HARDPARTS_MANIFEST", manifest);
    }
    command
        .output()
...
fn run_negative_case(root: &Path, case_name: &str, manifest: &Value) -> TestResult<Value> {
    ...
    let report_path = out_dir.join("pthread-hardparts.report.json");
...
#[test]
fn manifest_defines_pthread_hardparts_schema_and_coverage() -> TestResult {
    ...
    assert_eq!(
        ...
        "pthread-cancellation-robust-mutex-tls-v1"
    );
    ...
    let required_symbols: HashSet<_> = array_field(&manifest, "required_symbols", "manifest")?
        .iter()
        .map(|field| {
            field
                .as_str()
        ...
        .into_iter()
        .collect();
    assert_eq!(required_symbols, REQUIRED_SYMBOLS.iter().copied().collect());
...
fn gate_emits_pass_report_and_structured_logs() -> TestResult {
    let root = workspace_root();
    let out_dir = unique_temp_dir("pthread-hardparts-pass")?;
    ...
    let report = load_json(&out_dir.join("pthread-hardparts.report.json"))?;
    ...
    let log = std::fs::read_to_string(out_dir.join("pthread-hardparts.log.jsonl"))?;
    let rows = log
        .lines()
    ...
    assert!(rows.iter().any(|row| {
        row.get("fixture_id").and_then(Value::as_str) == Some("pthread.tls.dtor.iteration.hardened")
            && row
                .get("covered_symbols")
                ...
                .is_some_and(|symbols| {
                    symbols
                        .iter()
                        .any(|symbol| symbol.as_str() == Some("__call_tls_dtors"))
                })
    }));
    Ok(())
}
...
fn gate_rejects_stale_artifact_and_missing_artifact_ref() -> TestResult {
    ...
    let stale_report = run_negative_case(&root, "pthread-hardparts-stale", &stale)?;
    ...
    let missing_report = run_negative_case(&root, "pthread-hardparts-missing-ref", &missing_ref)?;
    expect_failure_signature(&missing_report, "missing_source_artifact")
}

#[test]
fn gate_rejects_missing_kind_and_bad_pthread_claims() -> TestResult {
    let root = workspace_root();

    let mut missing_kind = load_json(&manifest_path(&root))?;
    mutable_rows(&mut missing_kind)?.retain(|row| {
        row.get("fixture_kind").and_then(Value::as_str) != Some("tls_destructor_iteration")
    });
    let missing_kind_report =
        run_negative_case(&root, "pthread-hardparts-missing-kind", &missing_kind)?;
    expect_failure_signature(&missing_kind_report, "missing_fixture_kind")?;

    let mut cancellation_overclaim = load_json(&manifest_path(&root))?;
    mutable_rows(&mut cancellation_overclaim)?[0]["expected"]["status"] =
        Value::String("pass".to_string());
    mutable_rows(&mut cancellation_overclaim)?[0]["expected"]["failure_signature"] =
        Value::String("none".to_string());
    let cancellation_report = run_negative_case(
        &root,
        "pthread-hardparts-cancel-overclaim",
        &cancellation_overclaim,
    )?;
    expect_failure_signature(&cancellation_report, "cancellation_contract_gap")?;

    let mut robust_bad_errno = load_json(&manifest_path(&root))?;
    mutable_rows(&mut robust_bad_errno)?[2]["expected"]["errno"] =
        Value::String("none".to_string());
    let robust_report = run_negative_case(
        &root,
        "pthread-hardparts-robust-bad-errno",
        &robust_bad_errno,
    )?;
    expect_failure_signature(&robust_report, "robust_mutex_classification")?;

    let mut timeout_bad_status = load_json(&manifest_path(&root))?;
    mutable_rows(&mut timeout_bad_status)?[4]["expected"]["status"] =
        Value::String("pass".to_string());
    mutable_rows(&mut timeout_bad_status)?[4]["expected"]["failure_signature"] =
        Value::String("none".to_string());
    let timeout_report = run_negative_case(
        &root,
        "pthread-hardparts-timeout-bad-status",
        &timeout_bad_status,
    )?;
    expect_failure_signature(&timeout_report, "timeout_classification")
}
