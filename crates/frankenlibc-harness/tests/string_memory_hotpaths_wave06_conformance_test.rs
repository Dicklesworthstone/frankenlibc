//! Deterministic wave-06 string/memory hot-path fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave06_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave06.json");
...
#[test]
fn string_memory_hotpaths_wave06_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave06.json");
    assert!(
        path.exists(),
        "string_memory_hotpaths_wave06.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "string_memory_hotpaths_wave06");
    assert_eq!(fixture.campaign.bead, "pending-string-memory-wave06");
    assert_eq!(fixture.campaign.campaign_id, "fcq-string-memory-hotpaths");
    assert_eq!(fixture.campaign.wave_id, "wave-06-string-memory-hotpaths");
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave06_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave06_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave06_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture.spec_reference.contains("GNU libc")
            && fixture.spec_reference.contains("POSIX")
            && fixture.spec_reference.contains("conversion")
            && fixture.spec_reference.contains("strverscmp")
            && fixture.spec_reference.contains("argz"),
        "fixture must bind GNU, POSIX, conversion, strverscmp, and argz semantics"
    );
    Ok(())
}

#[test]
fn string_memory_hotpaths_wave06_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "string/memory wave-06 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
