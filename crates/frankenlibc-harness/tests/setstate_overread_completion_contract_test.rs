//! Contract tests for bd-nrh5v.1 setstate overread conformance evidence.
...
fn read_manifest(root: &Path) -> TestResult<serde_json::Value> {
    let path = root.join("tests/conformance/setstate_overread_completion_contract.v1.json");
...
#[test]
fn manifest_binds_setstate_conformance_evidence() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_manifest(&root)?;
    let evidence = &manifest["completion_debt_evidence"];
    let conformance = &evidence["conformance_primary"];

    assert_eq!(manifest["bead"].as_str(), Some("bd-nrh5v.1"));
    assert_eq!(evidence["bead"].as_str(), Some("bd-nrh5v.1"));
    assert_eq!(evidence["original_bead"].as_str(), Some("bd-nrh5v"));
    assert_eq!(
    ...
    let missing_items = json_string_set(&evidence["missing_items"])?;
    assert_eq!(
        missing_items,
    ...
    assert_eq!(refs.len(), 4, "contract should bind four conformance refs");
    ...
    for symbol in ["initstate", "setstate", "random", "srandom"] {
        assert!(
            required_symbols.contains(symbol),
            "contract should bind symbol {symbol}"
        );
        assert!(
    ...
    assert!(
        implementation_refs.len() >= 10,
        "implementation refs should cover helper, ABI path, core path, and tests"
    );
...
fn checker_script_passes_and_emits_report() -> TestResult {
    let root = workspace_root()?;
    let report =
        root.join("target/conformance/setstate_overread_completion_contract.test.report.json");
    let log = root.join("target/conformance/setstate_overread_completion_contract.test.log.jsonl");

    let output = Command::new("bash")
        .arg(root.join("scripts/check_setstate_overread_completion_contract.sh"))
        .env("FRANKENLIBC_SETSTATE_REPORT", &report)
        .env("FRANKENLIBC_SETSTATE_LOG", &log)
        .current_dir(&root)
        .output()?;
    assert!(
    ...
    assert_eq!(report_json["bead"].as_str(), Some("bd-nrh5v.1"));
    assert_eq!(report_json["original_bead"].as_str(), Some("bd-nrh5v"));
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some("setstate_untracked_minimum_overread_or_missing_conformance_evidence")
    );
    ...
    assert_eq!(
        event["event"].as_str(),
        Some("setstate_overread_completion_contract_validated")
    );
