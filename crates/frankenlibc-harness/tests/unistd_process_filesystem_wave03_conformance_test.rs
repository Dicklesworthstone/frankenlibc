//! Deterministic wave-03 unistd/process/filesystem AIO/time fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave03_conformance_test
...
const AMBIENT_POLICY: &str = "forbid_ambient_aio_time_fd_or_scheduler_metadata";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave03.json");
...
#[test]
fn unistd_process_filesystem_wave03_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave03.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave03.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "bd-ph6um.1");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-03-unistd-process-filesystem-aio-time"
    );
    assert_eq!(
    ...
    assert!(
        fixture
            .description
            .contains("without recording file descriptors")
            && fixture.description.contains("kernel time offsets")
            && fixture.description.contains("scheduling races"),
        "fixture description must reject ambient AIO/time metadata capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave03_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn unistd_process_filesystem_wave03_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn unistd_process_filesystem_wave03_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture.spec_reference.contains("POSIX.1 AIO")
            && fixture.spec_reference.contains("XSI addseverity")
            && fixture.spec_reference.contains("Linux adjtimex")
            && fixture.spec_reference.contains("glibc LFS AIO"),
        "fixture must bind POSIX/XSI/Linux/glibc wave-03 semantics"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave03_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-03 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
