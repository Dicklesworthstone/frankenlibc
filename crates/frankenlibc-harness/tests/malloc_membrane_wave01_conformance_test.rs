//! Deterministic wave-01 malloc/membrane lifecycle fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test malloc_membrane_wave01_conformance_test
...
const AMBIENT_POLICY: &str = "forbid_allocator_pointer_address_heap_layout_rss_stderr_bytes_or_process_global_counter_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/malloc_membrane_wave01.json");
...
#[test]
fn malloc_membrane_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/malloc_membrane_wave01.json");
    assert!(
        path.exists(),
        "malloc_membrane_wave01.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "malloc/membrane");
    assert_eq!(
        fixture.campaign.bead,
        "tracker-blocked-empty-queue-discovery"
    );
    assert_eq!(fixture.campaign.campaign_id, "fcq-malloc-membrane");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-01-malloc-membrane-allocator-stats-lifecycle"
    );
    assert_eq!(
    ...
    assert!(
        fixture
            .description
            .contains("without recording heap pointer addresses")
            && fixture.description.contains("heap layout")
            && fixture.description.contains("rss")
            && fixture.description.contains("stderr bytes")
            && fixture
                .description
                .contains("process-global allocation counters"),
        "fixture description must reject ambient allocator metadata capture"
    );
    Ok(())
}

#[test]
fn malloc_membrane_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn malloc_membrane_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        for forbidden in [
            ...
            assert!(
                ...
                "case {} expected_output leaks ambient allocator detail {forbidden}",
                case.name
            );
        }
    }
...
#[test]
fn malloc_membrane_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture.spec_reference.contains("C11 aligned_alloc")
            && fixture.spec_reference.contains("GNU malloc extensions")
            && fixture.spec_reference.contains("mallinfo2")
            && fixture.spec_reference.contains("malloc_usable_size")
            && fixture.spec_reference.contains("cfree")
            && fixture.spec_reference.contains("glibc internal __libc_*"),
        "fixture must bind C11, GNU malloc extension, and glibc lifecycle surfaces"
    );
    Ok(())
}

#[test]
fn malloc_membrane_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "malloc/membrane wave-01 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
