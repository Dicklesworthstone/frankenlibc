//! Deterministic wave-02 process/spawn fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test process_spawn_wave02_conformance_test
...
const AMBIENT_POLICY: &str =
    "forbid_ambient_pid_uid_gid_fd_path_argv_envp_resource_or_timing_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/process_spawn_wave02.json");
...
#[test]
fn process_spawn_wave02_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/process_spawn_wave02.json");
    assert!(
        path.exists(),
        "process_spawn_wave02.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "process/spawn");
    assert_eq!(fixture.campaign.bead, "bd-vm6j0.4");
    assert_eq!(fixture.campaign.campaign_id, "fcq-process-spawn");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-02-process-spawn-attrs-wait-credentials"
    );
    assert_eq!(
...
#[test]
fn process_spawn_wave02_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn process_spawn_wave02_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn process_spawn_wave02_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture
            .spec_reference
            .contains("POSIX/GNU process and spawn")
            && fixture.spec_reference.contains("posix_spawn_file_actions")
            && fixture.spec_reference.contains("posix_spawnattr")
            && fixture.spec_reference.contains("setresuid")
            && fixture.spec_reference.contains("wait3"),
        "fixture must bind process spawn actions, attrs, credentials, and wait surfaces"
    );
...
#[test]
fn process_spawn_wave02_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "process/spawn wave-02 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
