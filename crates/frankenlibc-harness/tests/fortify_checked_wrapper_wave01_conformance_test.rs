//! Deterministic wave-01 fortify checked-wrapper fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test fortify_checked_wrapper_wave01_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave01.json");
...
#[test]
fn fortify_checked_wrapper_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave01.json");
    assert!(
        path.exists(),
        "fortify_checked_wrapper_wave01.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "fortify/checked-wrapper");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.8");
    assert_eq!(fixture.campaign.campaign_id, "fcq-fortify-bounds");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-01-fortify-checked-wrapper-bounds"
    );
    assert_eq!(
    ...
    assert!(
        fixture
            .description
            .contains("positive and negative behavior")
            && fixture.description.contains("raw buffer addresses")
            ...
            && fixture.description.contains("allocated pointer values")
            && fixture.description.contains("process ids")
            && fixture.description.contains("wall-clock timing"),
        "fixture description must reject ambient fortify metadata capture"
    );
    Ok(())
}

#[test]
fn fortify_checked_wrapper_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "fortify wave should not leave residual symbols"
    );
...
#[test]
fn fortify_checked_wrapper_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn fortify_checked_wrapper_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
    ...
    assert!(
        fixture.spec_reference.contains("_FORTIFY_SOURCE")
            && fixture.spec_reference.contains("object-size"),
        "spec reference must name fortify object-size semantics"
    );
    Ok(())
}

#[test]
fn fortify_checked_wrapper_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
