//! Conformance + telemetry-index gate for the CVE format-string
//! validation gate (bd-1m5.2 / completion-debt bd-1m5.2.1).
//!
//! Pins, at conformance level:
//! 1. The primary unit + e2e test file is present and contains the
//!    named test functions (4 unit tests + 1 e2e test).
//! 2. The python generator script exists and is executable.
//! 3. The telemetry report artifact path is stable AND, if present
//!    on disk, carries the required schema fields.
...
fn index_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("conformance")
        .join("cve_format_string_validation_index.v1.json")
}
...
#[test]
fn index_anchors_to_1m5_2_with_completion_debt_bead() -> TestResult {
    let root = workspace_root()?;
    let m = load_json(&index_path(&root))?;
    require(
        json_string(&m, "manifest_id")? == "cve-format-string-validation-index",
        "manifest_id",
    )?;
    require(json_string(&m, "bead")? == "bd-1m5.2", "bead")?;
    require(
        json_string(&m, "completion_debt_bead")? == "bd-1m5.2.1",
        "completion_debt_bead",
    )?;
    require(
        json_string(&m, "primary_unit_test_file")?
            == "crates/frankenlibc-harness/tests/cve_format_string_validation_test.rs",
        "primary_unit_test_file",
    )?;
    require(
        json_string(&m, "primary_e2e_test_function")?
            == "format_string_report_generates_successfully",
        "primary_e2e_test_function",
    )?;
    require(
        json_string(&m, "e2e_evidence_generator_script")?
            == "scripts/generate_cve_format_string_validation.py",
...
fn telemetry_report_path_pinned_to_canonical_location() -> TestResult {
    ...
    require(
        json_string(telemetry, "report_artifact_path")?
            == "tests/cve_arena/results/format_string_validation.v1.json",
