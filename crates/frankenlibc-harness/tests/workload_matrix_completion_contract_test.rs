//! Completion-contract tests for bd-3u0.1 hard-parts workload matrix evidence.
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/workload_matrix_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_workload_matrix_completion_contract.sh")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_CONTRACT", manifest)
        .env("FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_REPORT",
            out_dir.join("report.json"),
        )
        .env(
            "FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_LOG",
            out_dir.join("events.jsonl"),
        )
        .env(
            "FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_GATE_STDOUT",
            out_dir.join("gate_stdout.txt"),
        )
        .env(
            "FRANKENLIBC_WORKLOAD_MATRIX_COMPLETION_GATE_STDERR",
...
fn manifest_binds_unit_e2e_and_telemetry_items() -> TestResult {
    ...
    assert_eq!(
        ...
        Some("workload_matrix_completion_contract.v1")
    );
    assert_eq!(manifest["bead_id"].as_str(), Some("bd-3u0.1"));
    assert_eq!(manifest["original_bead"].as_str(), Some("bd-3u0"));
    assert_eq!(
    ...
    assert_eq!(
        manifest["unit_primary"]["required_summary"]["total_workloads"].as_u64(),
        Some(10)
    );
    Ok(())
}
...
#[test]
fn source_artifacts_bind_existing_workload_matrix_surfaces() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
...
fn checker_accepts_contract_and_emits_report_log() -> TestResult {
    ...
    assert!(
        String::from_utf8_lossy(&output.stdout)
            .contains("PASS workload matrix completion contract"),
        "{}",
        output_text(&output)
    );
    ...
    assert_eq!(
        ...
        Some("workload_matrix_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["bead_id"].as_str(), Some("bd-3u0.1"));
    assert_eq!(report["source_count"].as_u64(), Some(8));
    assert_eq!(report["unit_test_count"].as_u64(), Some(7));
    assert_eq!(
        report["policy_summary"]["total_workloads"].as_u64(),
        Some(10)
    );
    assert_eq!(
        report["policy_summary"]["milestone_count"].as_u64(),
        Some(6)
    );
    ...
    assert!(
        fs::read_to_string(out_dir.join("gate_stdout.txt"))?
            .contains("check_workload_matrix: PASS")
    );
...
fn checker_rejects_missing_unit_binding() -> TestResult {
    ...
    manifest["unit_primary"]["required_harness_tests"]
        ...
        .retain(|name| name.as_str() != Some("summary_consistent"));
...
fn checker_rejects_missing_e2e_stdout_binding() -> TestResult {
    ...
    manifest["e2e_primary"]["required_gate_stdout"][0] =
        json!("PASS: impossible workload matrix stdout marker");
