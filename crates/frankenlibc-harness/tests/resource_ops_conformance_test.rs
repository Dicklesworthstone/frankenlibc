//! Resource limit operations conformance test suite.
//!
...
#[test]
fn resource_ops_fixture_exists() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/resource_ops.json");
    assert!(path.exists(), "resource_ops.json fixture must exist");
    Ok(())
}
...
fn resource_ops_covers_resources() -> Result<(), String> {
    ...
    let patterns = ["nofile", "stack"];

    for pattern in patterns {
        assert!(
            case_names.iter().any(|name| name.contains(pattern)),
            "Missing test coverage for resource type: {}",
            pattern
        );
    }
    Ok(())
...
fn resource_ops_case_count_stable() -> Result<(), String> {
    ...
    assert!(
        ...
        "resource_ops fixture has {} cases, expected at least {}",
        fixture.cases.len(),
        EXPECTED_MIN_CASES
    );
...
// ---------------------------------------------------------------------------
// Isolated harness subprocess coverage (bd-3thu)
// ---------------------------------------------------------------------------
//
...
fn resource_ops_fixture_executes_with_host_parity_via_harness_matrix() -> Result<(), String> {
    ...
    for case in &fixture.cases {
        ...
        for mode in modes {
            ...
            assert_eq!(
                result.impl_output, expected_output,
                "resource_ops case {} ({mode}) mismatched fixture output via harness",
                case.name
            );
        }
    }
