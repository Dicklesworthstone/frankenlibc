//! Deterministic wave-04 stdio/libio write, scan, and position fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test stdio_libio_wave04_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/stdio_libio_wave04.json");
...
#[test]
fn stdio_libio_wave04_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/stdio_libio_wave04.json");
    assert!(path.exists(), "stdio_libio_wave04.json fixture must exist");

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "stdio/libio");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.3");
    assert_eq!(fixture.campaign.campaign_id, "fcq-stdio-libio");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-04-stdio-libio-stream-position-scan-write"
    );
    assert_eq!(
...
#[test]
fn stdio_libio_wave04_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn stdio_libio_wave04_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn stdio_libio_wave04_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
...
#[test]
fn stdio_libio_wave04_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "stdio/libio wave-04 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
