//! Deterministic wave-07 residual fortify checked-wrapper fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test fortify_checked_wrapper_wave07_conformance_test
...
const WAVE07_SYMBOLS: &[&str] = &["__syslog_chk", "__vsyslog_chk", "__vfwprintf_chk"];
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave07.json");
...
#[test]
fn fortify_checked_wrapper_wave07_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/fortify_checked_wrapper_wave07.json");
    assert!(
        path.exists(),
        "fortify_checked_wrapper_wave07.json fixture must exist"
    );
    ...
    assert_eq!(
        ...
        "wave07 belongs to the replacement-integration bead"
    );
    assert_eq!(fixture.campaign.campaign_id, "fcq-fortify-bounds");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-07-fortify-checked-wrapper-final"
    );
    ...
    // scenario) are recorded explicitly so wave07 cannot silently drop one.
    let residual: BTreeSet<_> = fixture
        ...
        .map(String::as_str)
        .collect();
    let expected_residual: BTreeSet<&str> = BTreeSet::new();
    assert_eq!(residual, expected_residual, "wave07 residual drift");
    assert_eq!(fixture.campaign.ambient_state_policy, AMBIENT_POLICY);
    Ok(())
}

#[test]
fn fortify_checked_wrapper_wave07_covers_residual_symbols_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    let expected: BTreeSet<_> = WAVE07_SYMBOLS.iter().copied().collect();
    let declared: BTreeSet<_> = fixture
        ...
        .map(String::as_str)
        .collect();
    assert_eq!(declared, expected, "wave07 symbol drift");
...
#[test]
fn fortify_checked_wrapper_wave07_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            "expected_errno must stay classified; wave07 markers embed errno instead"
        );
        assert!(
...
#[test]
fn fortify_checked_wrapper_wave07_respects_ambient_policy_scenarios() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    for case in &fixture.cases {
        ...
        assert!(
            scenario.ends_with("_without_capture")
                || scenario.starts_with("contained_")
                || scenario.ends_with("_no_crash"),
...
#[test]
fn fortify_checked_wrapper_wave07_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
