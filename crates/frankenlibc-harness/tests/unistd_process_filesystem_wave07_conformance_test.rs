//! Deterministic wave-07 unistd/process/filesystem fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave07_conformance_test
...
const AMBIENT_POLICY: &str =
    "forbid_locale_catalog_path_user_fd_database_encrypted_block_time_or_mutable_nss_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave07.json");
...
#[test]
fn unistd_process_filesystem_wave07_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave07.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave07.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "bd-5tx3r");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-07-unistd-process-filesystem-gettext-access-des-enddb"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("raw locale catalog files")
            ...
            && fixture.description.contains("resolver database contents")
            && fixture.description.contains("encrypted raw blocks")
            && fixture.description.contains("mutable NSS state"),
        "fixture description must reject ambient catalog/path/user/fd/database/block capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave07_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "wave-07 fixture should not leave residual symbols"
    );
...
#[test]
fn unistd_process_filesystem_wave07_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn unistd_process_filesystem_wave07_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for token in [
        "GNU dngettext",
        "GNU eaccess",
...
#[test]
fn unistd_process_filesystem_wave07_coverage_artifacts_bind_first_wave() -> Result<(), String> {
    ...
    for symbol in FIRST_WAVE_SYMBOLS {
        ...
        assert!(
            json_array_has_str(
                &row["fixture_files"],
                "unistd_process_filesystem_wave07.json"
            ),
            "{symbol} coverage must cite unistd_process_filesystem_wave07.json"
        );
        assert!(
    ...
    assert!(
        campaign["current_coverage_pct"].as_f64().unwrap_or(0.0) >= 17.65,
        "unistd/process coverage percent must include wave-07 fixture rows"
    );
    assert!(
        campaign["target_covered"].as_u64().unwrap_or(0) >= 131,
        "unistd/process target_covered must include the 12 wave-07 symbols"
    );
    assert!(
        campaign["target_uncovered"].as_u64().unwrap_or(u64::MAX) <= 611,
        "unistd/process target_uncovered must shrink after wave-07 coverage"
    );
...
#[test]
fn unistd_process_filesystem_wave07_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-07 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
        ...
        assert_eq!(
            ...
            "wave-07 fixture should not use ambient host outputs for {}",
            case.name
        );
