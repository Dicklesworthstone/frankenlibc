//! Completion-contract tests for bd-56m5d.2 string/memory wave-06 evidence.
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/string_memory_hotpaths_wave06_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_string_memory_hotpaths_wave06_completion_contract.sh")
}

fn checker_report(out_dir: &Path) -> PathBuf {
    out_dir.join("string_memory_hotpaths_wave06_completion_contract.report.json")
}

fn checker_log(out_dir: &Path) -> PathBuf {
    out_dir.join("string_memory_hotpaths_wave06_completion_contract.events.jsonl")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_STRING_WAVE06_COMPLETION_CONTRACT", manifest)
        .env("FRANKENLIBC_STRING_WAVE06_COMPLETION_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_STRING_WAVE06_COMPLETION_REPORT",
            checker_report(out_dir),
        )
        .env(
            "FRANKENLIBC_STRING_WAVE06_COMPLETION_LOG",
            checker_log(out_dir),
        )
        .output()?)
}
...
#[test]
fn contract_binds_string_wave06_sources() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
    assert_eq!(
        string_field(&manifest, "schema_version", "manifest")?,
        "string_memory_hotpaths_wave06_completion_contract.v1"
    );
    assert_eq!(
        string_field(&manifest, "bead_id", "manifest")?,
        "bd-56m5d.2"
    );
    assert_eq!(string_field(&manifest, "epic_id", "manifest")?, "bd-56m5d");
    ...
    assert_eq!(
        ...
        "wave-06-string-memory-hotpaths"
    );
    ...
    assert!(symbols.contains("__strtol_l"));
    assert!(symbols.contains("argz_add"));
    ...
    let proof_ids: BTreeSet<_> = proofs
        ...
        .collect();
    assert!(proof_ids.contains("string-wave06-code-proof"));
    assert!(proof_ids.contains("bd-56m5d.1"));
    Ok(())
}

#[test]
fn checker_accepts_string_wave06_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave06-completion-check")?;
    ...
    assert_eq!(
        ...
        Some("string_memory_hotpaths_wave06_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(
        report["summary"]["string_wave06_symbol_count"].as_u64(),
        Some(12)
    );
...
#[test]
fn checker_emits_structured_string_wave06_telemetry() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave06-completion-log")?;
    let output = run_checker(&root, &manifest_path(&root), &out_dir)?;
    expect_checker_success(&output)?;
    ...
    for row in rows {
        assert_eq!(row["bead_id"].as_str(), Some("bd-56m5d.2"));
        assert!(
            row["trace_id"]
                .as_str()
                .unwrap_or_default()
...
#[test]
fn checker_rejects_missing_string_wave06_fixture_symbol() -> TestResult {
    let root = workspace_root()?;
    ...
    symbols.retain(|value| value.as_str() != Some("__strtol_l"));
    symbols.push(Value::String(
        "missing_string_wave06_symbol_for_negative_test".to_string(),
    ));

    let out_dir = unique_output_dir(&root, "string-wave06-completion-missing-symbol")?;
    ...
    let report = load_json(&checker_report(&out_dir))?;
    assert!(failure_signatures(&report).contains("string_wave06_fixture_symbol_coverage"));
    Ok(())
}

#[test]
fn checker_rejects_stale_string_wave06_coverage_accounting() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = load_json(&manifest_path(&root))?;
    manifest["completion_contract"]["coverage_accounting"]["campaign_target_covered_min"] =
        Value::from(999);

    let out_dir = unique_output_dir(&root, "string-wave06-completion-stale-coverage")?;
...
fn checker_rejects_non_remote_rch_cargo_validation_command() -> TestResult {
    ...
    commands.push(Value::String(
        "cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave06_completion_contract_test".to_string(),
    ));

    let out_dir = unique_output_dir(&root, "string-wave06-completion-local-cargo")?;
...
fn checker_rejects_missing_required_telemetry_event() -> TestResult {
    ...
    events.retain(|value| value.as_str() != Some("string_wave06_completion_contract_validated"));

    let out_dir = unique_output_dir(&root, "string-wave06-completion-missing-event")?;
