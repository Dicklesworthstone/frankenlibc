//! Completion-contract tests for bd-w2c3.5.4 runtime-math closure evidence.
...
const CONTRACT_REL: &str =
    "tests/conformance/runtime_math_in_progress_closure_completion_contract.v1.json";
const CHECKER_REL: &str = "scripts/check_runtime_math_in_progress_closure_completion_contract.sh";
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let dir = root.join("target/conformance").join(format!(
        "runtime-math-in-progress-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_RUNTIME_MATH_IN_PROGRESS_CONTRACT", contract)
        .env("FRANKENLIBC_RUNTIME_MATH_IN_PROGRESS_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_RUNTIME_MATH_IN_PROGRESS_REPORT",
            out_dir.join("report.json"),
        )
        .env(
            "FRANKENLIBC_RUNTIME_MATH_IN_PROGRESS_LOG",
...
#[test]
fn manifest_binds_runtime_math_unit_and_e2e_closure() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&root.join(CONTRACT_REL))?;

    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("runtime_math_in_progress_closure_completion_contract.v1")
    );
    assert_eq!(manifest["original_bead"].as_str(), Some("bd-w2c3.5"));
    assert_eq!(manifest["runtime_math_epic_bead"].as_str(), Some("bd-5vr"));
    assert_eq!(
    ...
    let child_beads = manifest["required_source_truth"]["track_children"]
        .as_array()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "track_children array"))?
        .iter()
        .filter_map(|row| row["bead"].as_str())
        .collect::<BTreeSet<_>>();
    assert_eq!(
        child_beads,
        BTreeSet::from(["bd-w2c3.5.1", "bd-w2c3.5.2", "bd-w2c3.5.3"])
    );

    let epic = &manifest["required_source_truth"]["epic_closure"];
    assert_eq!(epic["source_bead"].as_str(), Some("bd-5vr"));
    assert!(epic["minimum_checks"].as_u64().unwrap_or(0) >= 9);
    for required in [
        "manifest",
        "linkage",
        "linkage_proofs",
        ...
        assert!(
            epic["required_checks"]
                .as_array()
                .into_iter()
                .flatten()
                .any(|value| value.as_str() == Some(required)),
            "epic closure should require {required}"
        );
    }
...
#[test]
fn checker_emits_report_log_and_validates_source_gates() -> TestResult {
    ...
    assert_eq!(
        ...
        Some("runtime_math_in_progress_closure_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["original_bead"].as_str(), Some("bd-w2c3.5"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-w2c3.5.4"));
    assert_eq!(report["summary"]["error_count"].as_u64(), Some(0));
    assert!(
        report["summary"]["source_artifact_count"]
            .as_u64()
            .unwrap_or(0)
            >= 12
    );
    assert!(
        ...
        "completion contract should bind all source harness refs"
    );
    assert_eq!(
        report["source_summary"]["admission_blocked"].as_u64(),
        Some(0)
    );
...
#[test]
fn checker_rejects_missing_epic_gate_binding() -> TestResult {
    let _lock = checker_lock();
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "missing-epic")?;
    let mut manifest = read_json(&root.join(CONTRACT_REL))?;
    let checks = manifest["required_source_truth"]["epic_closure"]["required_checks"]
        .as_array_mut()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "required_checks array"))?;
    checks.retain(|value| value.as_str() != Some("reverse_round_contracts"));
    ...
    assert!(
        ...
        "checker should fail without reverse_round_contracts binding"
    );
    assert!(
        output_text(&output).contains("required_source_truth.epic_closure.required_checks missing"),
        "{}",
        output_text(&output)
    );
...
fn checker_rejects_non_rch_cargo_validation_command() -> TestResult {
    ...
    commands.push(Value::String(
        "cargo test -p frankenlibc-harness --test runtime_math_epic_closure_test".to_string(),
    ));
