//! Deterministic wave-01 pthread sync and cancellation fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test pthread_sync_wave01_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/pthread_sync_wave01.json");
...
#[test]
fn pthread_sync_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/pthread_sync_wave01.json");
    assert!(path.exists(), "pthread_sync_wave01.json fixture must exist");

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "pthread/sync");
    assert_eq!(fixture.campaign.bead, "bd-kgy3r.5");
    assert_eq!(fixture.campaign.campaign_id, "fcq-pthread-sync");
    assert_eq!(fixture.campaign.wave_id, "wave-01-pthread-sync-alias-state");
    assert_eq!(
...
#[test]
fn pthread_sync_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn pthread_sync_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn pthread_sync_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture.spec_reference.contains("POSIX pthread")
            && fixture.spec_reference.contains("__pthread_mutex_init")
            && fixture
                .spec_reference
                .contains("__pthread_mutexattr_settype")
            && fixture.spec_reference.contains("__pthread_once")
            && fixture.spec_reference.contains("__pthread_key_create")
            && fixture.spec_reference.contains("cancellation"),
        "fixture must bind pthread sync aliases, TLS keys, once, and cancellation stubs"
    );
    Ok(())
}

#[test]
fn pthread_sync_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "pthread sync wave-01 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
