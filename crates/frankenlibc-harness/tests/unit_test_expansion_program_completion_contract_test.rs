//! Contract tests for bd-25n.1 unit-test expansion completion evidence.
...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/unit_test_expansion_program_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_unit_test_expansion_program_completion_contract.sh")
}
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let path = root.join("target/conformance").join(format!(
        "unit-test-expansion-program-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<std::process::Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_UNIT_TEST_EXPANSION_PROGRAM_CONTRACT", contract)
        .env(
            "FRANKENLIBC_UNIT_TEST_EXPANSION_PROGRAM_REPORT",
            out_dir.join("unit_test_expansion_program_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_UNIT_TEST_EXPANSION_PROGRAM_LOG",
            out_dir.join("unit_test_expansion_program_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
#[test]
fn contract_binds_closed_blockers_unit_refs_and_rch_commands() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&contract_path(&root))?;
    assert_eq!(manifest["bead"].as_str(), Some("bd-25n"));
    assert_eq!(manifest["completion_debt_bead"].as_str(), Some("bd-25n.1"));
    assert!(
    ...
    let parent_contract = &evidence["parent_closure_contract"];
    assert_eq!(
        string_set(&parent_contract["required_closed_blockers"])?,
        BTreeSet::from([
            "bd-1ff3".to_string(),
            "bd-1fk1".to_string(),
            "bd-2625".to_string(),
    ...
    assert_eq!(
        parent_contract["required_parent_status"].as_str(),
        Some("closed")
    );
    ...
    assert!(policy["minimum_hardened_cases"].as_u64().unwrap_or(0) >= 127);

    let sources = source_texts(&root, &manifest)?;
    let unit_refs = assert_test_refs_exist(&evidence["unit_primary"], &sources)?;
    for expected in [
        "existing_unit_pack_harness_test::all_required_families_have_fixtures",
        "pthread_thread_lifecycle_test::pthread_self_join_is_rejected_with_edeadlk",
        "pthread_cond_core_test::condvar_timedwait_rejects_null_abstime_and_invalid_nsec",
        "pthread_mutex_core_test::futex_mutex_destroy_while_locked_is_ebusy",
        "pthread_rwlock_trylock_test::tryrdlock_trywrlock_interleaved_cycle",
        "pthread_once_test::once_concurrent_threads_run_exactly_once",
        "pthread_tsd_test::tsd_destructor_runs_on_thread_exit",
        "completion_harness_test::contract_binds_closed_blockers_unit_refs_and_rch_commands",
    ] {
        assert!(unit_refs.contains(expected), "missing unit ref {expected}");
    }
    assert!(
        unit_refs.len() >= 27,
        "contract should bind both closure-pack and pthread unit evidence"
    );
    ...
    assert!(
        e2e_refs.contains(
            "completion_harness_test::checker_emits_report_log_and_replays_unit_pack_gate"
        )
    );
    assert!(
        e2e_refs.contains("completion_harness_test::checker_rejects_unclosed_blocker_contract")
    );
    Ok(())
}

#[test]
fn checker_emits_report_log_and_replays_unit_pack_gate() -> TestResult {
    let root = workspace_root()?;
    let out_dir = run_passing_checker(&root, "pass")?;
    let report =
        read_json(&out_dir.join("unit_test_expansion_program_completion_contract.report.json"))?;
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-25n.1"));
    assert_eq!(report["original_bead"].as_str(), Some("bd-25n"));
    assert_eq!(
        report["summary"]["closed_blocker_count"].as_u64(),
        report["summary"]["required_blocker_count"].as_u64()
    );
    assert!(
    ...
    assert!(report["summary"]["fixture_files"].as_u64().unwrap_or(0) >= 58);
    assert!(report["summary"]["total_cases"].as_u64().unwrap_or(0) >= 1284);
    assert!(report["summary"]["strict_cases"].as_u64().unwrap_or(0) >= 632);
    assert!(report["summary"]["hardened_cases"].as_u64().unwrap_or(0) >= 127);

    let rows =
        read_jsonl(&out_dir.join("unit_test_expansion_program_completion_contract.log.jsonl"))?;
    assert_eq!(
        rows.len(),
        3,
    ...
    for row in rows {
        ...
        assert!(
            row["trace_id"]
                .as_str()
                .is_some_and(|trace_id| trace_id.starts_with("bd-25n.1:"))
        );
    }
    Ok(())
}
...
#[test]
fn checker_rejects_unclosed_blocker_contract() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = read_json(&contract_path(&root))?;
    manifest["completion_debt_evidence"]["parent_closure_contract"]["required_closed_blockers"]
        .as_array_mut()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "blocker array"))?
        .push(serde_json::Value::String(
            "bd-definitely-missing".to_string(),
        ));

    let out_dir = unique_output_dir(&root, "missing-blocker")?;
    ...
    assert!(
        !output.status.success(),
        "checker should reject missing blocker:\n{}",
        checker_output_message(&output)
    );
    let combined = checker_output_message(&output);
    assert!(
        combined.contains("bd-definitely-missing")
            || combined.contains("required_closed_blockers must contain 7 blockers"),
        "failure should identify blocker drift:\n{combined}"
    );
    Ok(())
}
...
fn checker_rejects_local_cargo_validation_command() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["unit_primary"]["required_commands"][0] =
        serde_json::Value::String(
            "cargo test -p frankenlibc-harness --test unit_test_closure_packs_test".to_string(),
        );
