//! Completion-contract tests for bd-ph6um.3 unistd/process filesystem wave-03 evidence.
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/unistd_process_filesystem_wave3_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_unistd_process_filesystem_wave3_completion_contract.sh")
}

fn checker_report(out_dir: &Path) -> PathBuf {
    out_dir.join("unistd_process_filesystem_wave3_completion_contract.report.json")
}

fn checker_log(out_dir: &Path) -> PathBuf {
    out_dir.join("unistd_process_filesystem_wave3_completion_contract.events.jsonl")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_UNISTD_WAVE3_COMPLETION_CONTRACT", manifest)
        .env("FRANKENLIBC_UNISTD_WAVE3_COMPLETION_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_UNISTD_WAVE3_COMPLETION_REPORT",
            checker_report(out_dir),
        )
        .env(
            "FRANKENLIBC_UNISTD_WAVE3_COMPLETION_LOG",
            checker_log(out_dir),
        )
        .output()?)
}
...
#[test]
fn contract_binds_unistd_wave3_sources() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
    assert_eq!(
        string_field(&manifest, "schema_version", "manifest")?,
        "unistd_process_filesystem_wave3_completion_contract.v1"
    );
    assert_eq!(
        string_field(&manifest, "bead_id", "manifest")?,
        "bd-ph6um.3"
    );
    assert_eq!(string_field(&manifest, "epic_id", "manifest")?, "bd-ph6um");
    ...
    assert!(symbols.contains("addseverity"));
    assert!(symbols.contains("aio_return"));
    ...
    let proof_ids: BTreeSet<_> = proofs
        ...
        .collect();
    assert!(proof_ids.contains("bd-ph6um.1"));
    assert!(proof_ids.contains("bd-ph6um.2"));
    Ok(())
}

#[test]
fn checker_accepts_unistd_wave3_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-check")?;
    ...
    assert_eq!(
        ...
        Some("unistd_process_filesystem_wave3_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(
        report["summary"]["unistd_wave3_symbol_count"].as_u64(),
        Some(12)
    );
...
#[test]
fn checker_emits_structured_unistd_wave3_telemetry() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-log")?;
    let output = run_checker(&root, &manifest_path(&root), &out_dir)?;
    expect_checker_success(&output)?;
    ...
    for row in rows {
        assert_eq!(row["bead_id"].as_str(), Some("bd-ph6um.3"));
        assert!(
            row["trace_id"]
                .as_str()
                .unwrap_or_default()
...
#[test]
fn checker_rejects_missing_unistd_wave3_fixture_symbol() -> TestResult {
    let root = workspace_root()?;
    ...
    symbols.retain(|value| value.as_str() != Some("aio_return"));
    symbols.push(Value::String(
        "missing_unistd_wave3_symbol_for_negative_test".to_string(),
    ));

    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-missing-symbol")?;
    ...
    let report = load_json(&checker_report(&out_dir))?;
    assert!(failure_signatures(&report).contains("unistd_wave3_fixture_symbol_coverage"));
    Ok(())
}

#[test]
fn checker_rejects_stale_unistd_wave3_coverage_accounting() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = load_json(&manifest_path(&root))?;
    manifest["completion_contract"]["coverage_accounting"]["campaign_target_covered_min"] =
        Value::from(999);

    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-stale-coverage")?;
...
fn checker_rejects_non_remote_rch_cargo_validation_command() -> TestResult {
    ...
    for command in commands {
        if command.as_str().unwrap_or_default().contains(
            ...
            *command = Value::String(
                "cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave3_completion_contract_test -- --nocapture"
                    .to_string(),
            );
            break;
        }
    ...
    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-local-cargo")?;
...
fn checker_rejects_missing_required_telemetry_event() -> TestResult {
    ...
    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-missing-event")?;
...
fn checker_rejects_stale_dependency_proof_commit() -> TestResult {
    ...
    let proof = proofs
        .iter_mut()
        .find(|row| row["bead_id"].as_str() == Some("bd-ph6um.2"))
        .ok_or_else(|| test_error("bd-ph6um.2 dependency proof should exist"))?;
    proof["commit"] = Value::String("not-a-commit".to_string());

    let out_dir = unique_output_dir(&root, "unistd-wave3-completion-stale-commit")?;
