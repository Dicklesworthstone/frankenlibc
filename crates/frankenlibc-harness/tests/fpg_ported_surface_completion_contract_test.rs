...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/fpg_ported_surface_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_fpg_ported_surface_completion_contract.sh")
}
...
fn unique_out_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    let dir = root.join(format!(
        "target/conformance/fpg_ported_surface_completion_contract_test_{}_{}",
        std::process::id(),
        label
    ));
...
fn event_name(row: &Value) -> TestResult<String> {
    row["event"]
        .as_str()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "log event should be a string"))
        .map(str::to_owned)
        .map_err(Into::into)
}
...
#[test]
fn manifest_binds_fpg_ported_surface_completion_items() -> TestResult {
    let root = repo_root()?;
    let manifest = read_json(&contract_path(&root))?;

    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("fpg_ported_surface_completion_contract.v1")
    );
    assert_eq!(manifest["bead"].as_str(), Some("bd-bp8fl.3.13"));
    assert_eq!(
        manifest["completion_debt_bead"].as_str(),
        Some("bd-bp8fl.3.13.1")
    );
    assert_eq!(
    ...
    assert_eq!(contract["expected_gap_count"].as_u64(), Some(10));
    assert_eq!(
        contract["expected_required_log_field_count"].as_u64(),
        Some(16)
    );
    assert_eq!(contract["expected_input_artifact_count"].as_u64(), Some(9));
    assert_eq!(
        contract["claim_policy"]["default_decision"].as_str(),
        Some("block_done_until_ported_surface_evidence_current")
    );
    assert_eq!(
...
#[test]
fn checker_validates_fpg_ported_surface_contract_and_emits_report_log() -> TestResult {
    let root = repo_root()?;
    ...
    let report = read_json(&out_dir.join("fpg_ported_surface_completion_contract.report.json"))?;
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["source_bead"].as_str(), Some("bd-bp8fl.3.13"));
    assert_eq!(report["bead"].as_str(), Some("bd-bp8fl.3.13.1"));
    assert_eq!(
        report["source_gate_summary"]["gap_count"].as_u64(),
        Some(10)
    );
    ...
    let rows = read_jsonl(&out_dir.join("fpg_ported_surface_completion_contract.log.jsonl"))?;
    let events: BTreeSet<_> = rows
        .iter()
        .map(event_name)
        .collect::<TestResult<BTreeSet<_>>>()?;
    assert!(events.contains("fpg_ported_surface_completion.source_artifacts"));
    assert!(events.contains("fpg_ported_surface_completion.evidence_refs"));
    assert!(events.contains("fpg_ported_surface_completion.source_gate_contract"));
    assert!(
        rows.iter()
            .all(|row| row["bead"].as_str() == Some("bd-bp8fl.3.13.1"))
    );
...
fn checker_rejects_missing_integration_test_ref() -> TestResult {
    ...
    let mutated = out_dir.join("fpg_ported_surface_missing_integration_ref.json");
...
fn checker_rejects_gap_count_drift() -> TestResult {
    ...
    let mut manifest = read_json(&contract_path(&root))?;
    manifest["required_source_gate_contract"]["expected_gap_count"] = json!(9);
    let mutated = out_dir.join("fpg_ported_surface_gap_count_drift.json");
    write_json(&mutated, &manifest)?;
...
fn checker_rejects_non_rch_cargo_command() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["unit_primary"]["required_commands"] =
        json!(["cargo test -p frankenlibc-harness --test fpg_ported_surface_gate_test"]);
    let mutated = out_dir.join("fpg_ported_surface_non_rch.json");
    write_json(&mutated, &manifest)?;
...
fn checker_rejects_source_commit_policy_drift() -> TestResult {
    ...
    let mut manifest = read_json(&contract_path(&root))?;
    manifest["required_source_gate_contract"]["source_commit_freshness_policy"]["ported_surface_evidence_allowed_when_stale"] =
        json!(true);
    let mutated = out_dir.join("fpg_ported_surface_freshness_drift.json");
    write_json(&mutated, &manifest)?;
