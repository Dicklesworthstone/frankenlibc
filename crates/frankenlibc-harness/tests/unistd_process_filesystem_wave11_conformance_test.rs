//! Deterministic wave-11 unistd/process/filesystem fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave11_conformance_test
...
const AMBIENT_POLICY: &str =
    "forbid_stream_fd_path_pid_account_xattr_ifaddrs_terminal_stderr_or_file_state_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave11.json");
...
#[test]
fn unistd_process_filesystem_wave11_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave11.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave11.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "pending-unistd-wave11");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-11-unistd-process-filesystem-stream-xattr-lock-pty"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("stream contents")
            ...
            && fixture.description.contains("account names")
            && fixture.description.contains("xattr payloads")
            && fixture.description.contains("interface rows")
            && fixture.description.contains("terminal names")
            && fixture.description.contains("stderr bytes")
            && fixture.description.contains("ambient file state"),
        "fixture description must reject ambient stream/fd/path/pid/account/xattr/interface/terminal/stderr/file capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave11_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "wave-11 fixture should not leave residual symbols"
    );
...
#[test]
fn unistd_process_filesystem_wave11_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert!(
            matches!(case.expected_errno, 0 | 9 | 22),
...
#[test]
fn unistd_process_filesystem_wave11_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for token in [
        "fgetpwent",
        "fgetspent_r",
...
#[test]
fn unistd_process_filesystem_wave11_coverage_artifacts_bind_first_wave() -> Result<(), String> {
    ...
    for symbol in FIRST_WAVE_SYMBOLS {
        ...
        assert!(
            json_array_has_str(
                &row["fixture_files"],
                "unistd_process_filesystem_wave11.json"
            ),
            "{symbol} coverage must cite unistd_process_filesystem_wave11.json"
        );
        assert!(
    ...
    assert!(
        campaign["current_coverage_pct"].as_f64().unwrap_or(0.0) >= 24.0,
        "unistd/process coverage percent must include wave-11 fixture rows"
    );
    assert!(
        campaign["target_covered"].as_u64().unwrap_or(0) >= 179,
        "unistd/process target_covered must include the 12 wave-11 symbols"
    );
    assert!(
        campaign["target_uncovered"].as_u64().unwrap_or(u64::MAX) <= 563,
        "unistd/process target_uncovered must shrink after wave-11 coverage"
    );
...
#[test]
fn unistd_process_filesystem_wave11_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-11 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
        ...
        assert_eq!(
            ...
            "wave-11 fixture should not use ambient host outputs for {}",
            case.name
        );
