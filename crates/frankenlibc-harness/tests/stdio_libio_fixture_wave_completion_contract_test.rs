//! Completion-contract tests for bd-6cly1.3 stdio/libio fixture wave evidence.
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/stdio_libio_fixture_wave_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_stdio_libio_fixture_wave_completion_contract.sh")
}

fn checker_report(out_dir: &Path) -> PathBuf {
    out_dir.join("stdio_libio_fixture_wave_completion_contract.report.json")
}

fn checker_log(out_dir: &Path) -> PathBuf {
    out_dir.join("stdio_libio_fixture_wave_completion_contract.events.jsonl")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_STDIO_LIBIO_FIXTURE_WAVE_CONTRACT", manifest)
        .env("FRANKENLIBC_STDIO_LIBIO_FIXTURE_WAVE_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_STDIO_LIBIO_FIXTURE_WAVE_REPORT",
            checker_report(out_dir),
        )
        .env(
            "FRANKENLIBC_STDIO_LIBIO_FIXTURE_WAVE_LOG",
            checker_log(out_dir),
        )
        .output()?)
}
...
#[test]
fn contract_binds_stdio_libio_fixture_wave_sources() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
    assert_eq!(
        string_field(&manifest, "schema_version", "manifest")?,
        "stdio_libio_fixture_wave_completion_contract.v1"
    );
    assert_eq!(
        string_field(&manifest, "bead_id", "manifest")?,
        "bd-6cly1.3"
    );
    assert_eq!(string_field(&manifest, "epic_id", "manifest")?, "bd-6cly1");
    ...
    assert!(symbols.contains("_IO_2_1_stderr_"));
    assert!(symbols.contains("_IO_putc"));
    ...
    let proof_ids: BTreeSet<_> = proofs
        ...
        .collect();
    assert!(proof_ids.contains("bd-6cly1.1"));
    assert!(proof_ids.contains("bd-6cly1.2"));
    Ok(())
}

#[test]
fn checker_accepts_stdio_libio_fixture_wave_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "stdio-libio-fixture-wave-check")?;
    ...
    assert_eq!(
        ...
        Some("stdio_libio_fixture_wave_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(
        report["summary"]["stdio_libio_first_wave_symbol_count"].as_u64(),
        Some(12)
    );
...
#[test]
fn checker_emits_structured_stdio_libio_completion_telemetry() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "stdio-libio-fixture-wave-log")?;
    let output = run_checker(&root, &manifest_path(&root), &out_dir)?;
    expect_checker_success(&output)?;
    ...
    for row in rows {
        assert_eq!(row["bead_id"].as_str(), Some("bd-6cly1.3"));
        assert!(
            row["trace_id"]
                .as_str()
                .unwrap_or_default()
...
#[test]
fn checker_rejects_missing_stdio_libio_fixture_symbol() -> TestResult {
    let root = workspace_root()?;
    ...
    symbols.retain(|value| value.as_str() != Some("_IO_putc"));
    symbols.push(Value::String(
        "missing_stdio_libio_symbol_for_negative_test".to_string(),
    ));

    let out_dir = unique_output_dir(&root, "stdio-libio-fixture-wave-missing-symbol")?;
    ...
    let report = load_json(&checker_report(&out_dir))?;
    assert!(failure_signatures(&report).contains("stdio_libio_fixture_symbol_coverage"));
    Ok(())
}

#[test]
fn checker_rejects_stale_stdio_libio_coverage_accounting() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = load_json(&manifest_path(&root))?;
    manifest["completion_contract"]["coverage_accounting"]["campaign_target_covered_min"] =
        Value::from(999);

    let out_dir = unique_output_dir(&root, "stdio-libio-fixture-wave-stale-coverage")?;
...
fn checker_rejects_non_rch_cargo_validation_command() -> TestResult {
    ...
    for command in commands {
        if command.as_str().unwrap_or_default().contains(
            ...
            *command = Value::String(
                "cargo test -p frankenlibc-harness --test stdio_libio_fixture_wave_completion_contract_test -- --nocapture"
                    .to_string(),
            );
            break;
        }
    ...
    let out_dir = unique_output_dir(&root, "stdio-libio-fixture-wave-local-cargo")?;
...
fn checker_rejects_missing_required_telemetry_event() -> TestResult {
    ...
    let out_dir = unique_output_dir(&root, "stdio-libio-fixture-wave-missing-event")?;
