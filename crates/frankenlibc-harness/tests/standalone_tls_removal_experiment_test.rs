...
fn owned_tls_cache_feature_gate_is_wired_but_not_promoted() -> TestResult {
    ...
    require(
        substituted == 15,
        "owned-tls slices substitute crypt/gensalt, four NIS helper macros, resolver nsaddr, getmntent, cuserid, utmp state, pututxline return buffer, NSS systemd block flag, fstab state, ttyent state, and getdate tm",
    )?;
    require(
    ...
    require(
        ...
        "unistd ABI must route the crypt/gensalt, NIS helper, resolver nsaddr, getmntent, cuserid, utmp-state, pututxline return-buffer, NSS systemd block-flag, fstab-state, ttyent-state, and getdate-tm slices through owned TLS cache",
    )
}
