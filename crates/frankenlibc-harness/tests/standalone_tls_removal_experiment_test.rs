...
fn owned_tls_cache_feature_gate_is_wired_but_not_promoted() -> TestResult {
    ...
    require(
        substituted == 11,
        "owned-tls slices substitute crypt/gensalt, four NIS helper macros, resolver nsaddr, getmntent, cuserid, utmp state, and pututxline return buffer",
    )?;
    require(
    ...
    require(
        ...
        "unistd ABI must route the crypt/gensalt, NIS helper, resolver nsaddr, getmntent, cuserid, utmp-state, and pututxline return-buffer slices through owned TLS cache",
    )
}
