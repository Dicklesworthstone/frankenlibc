...
const EXPECTED_NON_TARGETED_TLS_EMITTER_COUNT: usize = 7;
...
fn owned_tls_cache_feature_gate_is_wired_but_not_promoted() -> TestResult {
    ...
    require(
        ...
        "resolver ABI must route backend caches, h_errno state, servent/protoent storage, printable-DNS helper buffers, and fixed fallback return buffers through owned TLS cache",
    )?;
    ...
    require(
        ...
        "passwd ABI must route non-reentrant passwd storage through owned TLS cache",
    )?;
