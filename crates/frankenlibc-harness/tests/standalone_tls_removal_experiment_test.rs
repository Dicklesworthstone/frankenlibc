...
const EXPECTED_NON_TARGETED_TLS_EMITTER_COUNT: usize = 8;
...
fn owned_tls_cache_feature_gate_is_wired_but_not_promoted() -> TestResult {
    ...
    require(
        ...
        "resolver ABI must route backend caches, h_errno state, servent storage, printable-DNS helper buffers, and fixed fallback return buffers through owned TLS cache",
    )?;
