//! Deterministic socket/network fixture wave-01 tests.
//!
//! Run: cargo test -p frankenlibc-harness --test socket_network_wave01_conformance_test
...
const AMBIENT_POLICY: &str = "forbid_fd_peer_address_thread_time_stderr_or_process_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/socket_network_wave01.json");
...
#[test]
fn socket_network_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/socket_network_wave01.json");
    assert!(
        path.exists(),
        "socket_network_wave01.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "socket_network_wave01");
    assert_eq!(fixture.campaign.bead, "bd-kgy3r.3");
    assert_eq!(fixture.campaign.campaign_id, "fcq-socket-network");
    assert_eq!(fixture.campaign.wave_id, "wave-01-socket-network");
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("fd numbers")
            && fixture.description.contains("peer addresses")
        ...
        "fixture description must reject fd/peer/thread/time/stderr/process capture"
    );
    Ok(())
}

#[test]
fn socket_network_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "socket/network wave should not leave residual symbols"
    );
...
#[test]
fn socket_network_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn socket_network_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn socket_network_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        ...
        assert_eq!(
            ...
            "socket/network wave should not use ambient host outputs for {}",
            case.name
        );
