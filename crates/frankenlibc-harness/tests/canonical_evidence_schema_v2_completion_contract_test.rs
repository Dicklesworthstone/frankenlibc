//! Contract tests for bd-33p.1.1 canonical evidence schema v2 completion evidence.
...
fn read_manifest(root: &Path) -> TestResult<serde_json::Value> {
    let path =
        root.join("tests/conformance/canonical_evidence_schema_v2_completion_contract.v1.json");
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let path = root.join("target/conformance").join(format!(
        "canonical-evidence-schema-v2-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<std::process::Output> {
    Ok(Command::new("bash")
        .arg(root.join("scripts/check_canonical_evidence_schema_v2_completion_contract.sh"))
        .current_dir(root)
        .env("FRANKENLIBC_CANONICAL_EVIDENCE_SCHEMA_CONTRACT", contract)
        .env(
            "FRANKENLIBC_CANONICAL_EVIDENCE_SCHEMA_REPORT",
            out_dir.join("canonical_evidence_schema_v2_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_CANONICAL_EVIDENCE_SCHEMA_LOG",
            out_dir.join("canonical_evidence_schema_v2_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
#[test]
fn manifest_binds_schema_v2_unit_e2e_conformance_and_telemetry_evidence() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_manifest(&root)?;
    assert_eq!(manifest["bead"].as_str(), Some("bd-33p.1"));
    assert_eq!(
    ...
    assert_eq!(evidence["bead"].as_str(), Some("bd-33p.1.1"));
    assert_eq!(evidence["original_bead"].as_str(), Some("bd-33p.1"));
    assert!(
    ...
    let schema_path = evidence["schema_artifact"]
        .as_str()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "schema artifact string"))?;
    let schema: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(root.join(schema_path))?)?;
    assert!(
        schema["schema_version"].as_u64().unwrap_or(0) >= 2,
        "schema artifact should be canonical schema v2"
    );

    let canonical = &evidence["canonical_fields"];
    let required_fields = string_set(&canonical["required_fields"])?;
    for field in ["timestamp", "trace_id", "level", "event"] {
        assert!(required_fields.contains(field), "missing required {field}");
        assert!(
            schema["required_fields"].get(field).is_some(),
            "log schema missing required field {field}"
        );
    }
    ...
    for group in [
        ...
        optional_fields.extend(string_set(&canonical[group])?);
    }
    for field in [
        "stream",
        "gate",
        ...
        assert!(optional_fields.contains(field), "missing optional {field}");
        assert!(
            schema["optional_fields"].get(field).is_some(),
            "log schema missing optional field {field}"
        );
    }

    let source_texts = source_texts(&root, &manifest)?;
    let structured_log = source_texts
        .get("structured_log")
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "structured log source"))?;
    for field in required_fields.iter().chain(optional_fields.iter()) {
        assert!(
            structured_log.contains(&format!("pub {field}:")),
            "LogEntry missing field {field}"
        );
    }
    for method in string_set(&canonical["required_builder_methods"])? {
        assert!(
            structured_log.contains(&format!("pub fn {method}")),
            "LogEntry missing builder {method}"
        );
    }
...
fn checker_emits_report_and_jsonl() -> TestResult {
    ...
    let report =
        read_json(&out_dir.join("canonical_evidence_schema_v2_completion_contract.report.json"))?;
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-33p.1.1"));
    assert_eq!(report["original_bead"].as_str(), Some("bd-33p.1"));
    assert!(
        string_set(&report["optional_fields"])?.contains("profile"),
        "report should include profile field binding"
    );
    assert!(
        report["test_refs"]
            .as_array()
            ...
            .any(|item| item.as_str()
                == Some("structured_log::validate_decision_requires_explainability_fields")),
        "report should include decision explainability unit ref"
    );

    let rows =
        read_jsonl(&out_dir.join("canonical_evidence_schema_v2_completion_contract.log.jsonl"))?;
    assert_eq!(rows.len(), 1, "checker should emit one telemetry row");
    let row = &rows[0];
    assert!(json_str_field_is(
        row,
        "event",
        "canonical_evidence_schema_v2_completion_contract_validated"
    ));
    assert!(json_str_field_is(row, "completion_debt_bead", "bd-33p.1.1"));
    assert!(json_str_field_is(row, "status", "pass"));
    assert!(json_str_field_is(row, "failure_signature", "none"));

    let fields = string_set(&row["optional_fields"])?;
    for field in ["stream", "gate", "span_id", "profile", "artifact_refs"] {
        assert!(fields.contains(field), "telemetry row missing {field}");
    }
...
#[test]
fn checker_rejects_missing_profile_field_binding() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "stale")?;
    let mut manifest = read_manifest(&root)?;
    manifest["completion_debt_evidence"]["canonical_fields"]["span_profile_fields"] =
        serde_json::json!(["span_id", "parent_span_id"]);
    let stale_contract = out_dir.join("stale_canonical_evidence_schema_v2_contract.v1.json");
    write_json(&stale_contract, &manifest)?;

    let output = run_checker(&root, &stale_contract, &out_dir)?;
    assert!(
        !output.status.success(),
        "checker should reject missing profile field binding"
    );
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&output.stdout),
    ...
    assert!(
        combined.contains("canonical_fields.span_profile_fields missing profile"),
        "checker output should name missing profile field: {combined}"
    );
    ...
    assert!(
        report["errors"]
            ...
            .any(|item| item
                .as_str()
                .unwrap_or("")
                .contains("canonical_fields.span_profile_fields missing profile")),
        "report should retain missing profile error"
    );
    ...
    assert!(json_str_field_is(
        &rows[0],
        "event",
        "canonical_evidence_schema_v2_completion_contract_failed"
    ));
