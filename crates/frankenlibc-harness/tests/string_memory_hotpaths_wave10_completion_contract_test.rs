//! Completion-contract tests for bd-c2qqr string/memory wave-10 evidence.
...
fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_string_memory_hotpaths_wave10_completion_contract.sh")
}

fn checker_report(out_dir: &Path) -> PathBuf {
    out_dir.join("string_memory_hotpaths_wave10_completion_contract.report.json")
}

fn checker_log(out_dir: &Path) -> PathBuf {
    out_dir.join("string_memory_hotpaths_wave10_completion_contract.events.jsonl")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_STRING_WAVE10_COMPLETION_CONTRACT", manifest)
        .env("FRANKENLIBC_STRING_WAVE10_COMPLETION_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_STRING_WAVE10_COMPLETION_REPORT",
            checker_report(out_dir),
        )
        .env(
            "FRANKENLIBC_STRING_WAVE10_COMPLETION_LOG",
            checker_log(out_dir),
        )
        .output()?)
}
...
#[test]
fn contract_binds_string_wave10_sources() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("string_memory_hotpaths_wave10_completion_contract.v1")
    );
    assert_eq!(manifest["bead_id"].as_str(), Some("bd-c2qqr"));
    ...
    assert_eq!(
        ...
        Some("wave-10-string-memory-hotpaths")
    );
    assert_eq!(
    ...
    assert!(symbols.contains("argz_create"));
    assert!(symbols.contains("memmem"));
    assert_eq!(
...
#[test]
fn checker_accepts_string_wave10_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave10-completion-check")?;
    ...
    assert_eq!(
        ...
        Some("string_memory_hotpaths_wave10_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(
        report["summary"]["string_wave10_symbol_count"].as_u64(),
        Some(12)
    );
...
#[test]
fn checker_emits_structured_string_wave10_telemetry() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave10-completion-telemetry")?;
    let output = run_checker(&root, &manifest_path(&root), &out_dir)?;
    expect_checker_success(&output)?;
    ...
    for row in rows {
        ...
        assert_eq!(row["bead_id"].as_str(), Some("bd-c2qqr"));
    }
    Ok(())
}

#[test]
fn checker_rejects_missing_string_wave10_fixture_symbol() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave10-missing-symbol")?;
    let mut manifest = load_json(&manifest_path(&root))?;
    manifest["completion_contract"]["required_first_wave_symbols"]
        .as_array_mut()
        .ok_or_else(|| test_error("required symbols should be array"))?
        .retain(|symbol| symbol.as_str() != Some("memmem"));
    ...
    let report = load_json(&checker_report(&out_dir))?;
    assert!(failure_signatures(&report).contains("string_wave10_fixture_symbol_coverage"));
    Ok(())
}

#[test]
fn checker_rejects_stale_string_wave10_coverage_accounting() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave10-stale-coverage")?;
    let mut manifest = load_json(&manifest_path(&root))?;
...
fn checker_rejects_non_remote_rch_cargo_validation_command() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave10-non-rch")?;
    let mut manifest = load_json(&manifest_path(&root))?;
...
fn checker_rejects_missing_required_telemetry_event() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "string-wave10-missing-telemetry")?;
    let mut manifest = load_json(&manifest_path(&root))?;
