//! Completion contract tests for bd-qerp.1.
...
const CONTRACT_REL: &str = "tests/conformance/regex_glob_free_completion_contract.v1.json";
const CHECKER_REL: &str = "scripts/check_regex_glob_free_completion_contract.sh";
const EXPECTED_SCHEMA: &str = "regex_glob_free_completion_contract.v1";
const EXPECTED_REPORT_SCHEMA: &str = "regex_glob_free_completion_contract.report.v1";
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let dir = root.join("target/conformance").join(format!(
        "regex-glob-free-completion-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_REGEX_GLOB_FREE_COMPLETION_CONTRACT", contract)
        .env("FRANKENLIBC_REGEX_GLOB_FREE_COMPLETION_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_REGEX_GLOB_FREE_COMPLETION_REPORT",
            checker_report(out_dir),
        )
        .env(
            "FRANKENLIBC_REGEX_GLOB_FREE_COMPLETION_LOG",
            checker_log(out_dir),
        )
        .output()?)
}
...
#[test]
fn contract_binds_regex_glob_free_conformance_surface() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&root.join(CONTRACT_REL))?;
    assert_eq!(string_field(&manifest, "schema_version")?, EXPECTED_SCHEMA);
    assert_eq!(string_field(&manifest, "bead_id")?, "bd-qerp.1");
    assert_eq!(string_field(&manifest, "original_bead")?, "bd-qerp");
    ...
    assert_eq!(
        test_contract
            .get("minimum_fixture_case_count")
            .and_then(Value::as_u64),
        Some(11)
    );
    assert_eq!(
        string_set(array_field(
            &Value::Object(test_contract.clone()),
            "required_functions"
        )?)?,
        BTreeSet::from([
            "fnmatch".to_string(),
...
#[test]
fn checker_accepts_regex_glob_free_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "accepts")?;
    let output = expect_checker_success(&root, &out_dir)?;
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("PASS: regex/glob free completion contract"),
    ...
    assert_eq!(
        summary.get("fixture_case_count").and_then(Value::as_u64),
        Some(11)
    );
    assert_eq!(
        summary
            .get("fixture_function_count")
            .and_then(Value::as_u64),
        Some(5)
    );
...
#[test]
fn checker_rejects_missing_frankenlibc_regfree_marker() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "missing-regfree")?;
    let mut contract = read_json(&root.join(CONTRACT_REL))?;
    contract["completion_contract"]["implementation_markers"]["conformance_executor"][3] =
        json!("not a real frankenlibc regfree marker");
    let contract_path = out_dir.join("contract-missing-regfree.json");
...
fn checker_rejects_fixture_case_count_drift() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "case-count-drift")?;
    let mut contract = read_json(&root.join(CONTRACT_REL))?;
    contract["completion_contract"]["test_contract"]["minimum_fixture_case_count"] = json!(9999);
    let contract_path = out_dir.join("contract-case-count-drift.json");
    write_json(&contract_path, &contract)?;

    let output = run_checker(&root, &contract_path, &out_dir)?;
    expect_checker_failure(output, "fixture case count below contract");
    Ok(())
}
