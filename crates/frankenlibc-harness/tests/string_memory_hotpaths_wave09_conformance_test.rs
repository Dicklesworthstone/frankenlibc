//! Deterministic wave-09 string/memory hot-path fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave09_conformance_test
...
const FIRST_WAVE_SYMBOLS: &[&str] = &["__argz_count", "__argz_next", "__argz_stringify", "__bzero"];
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave09.json");
...
#[test]
fn string_memory_hotpaths_wave09_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave09.json");
    assert!(
        path.exists(),
        "string_memory_hotpaths_wave09.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "string_memory_hotpaths_wave09");
    assert_eq!(fixture.campaign.bead, "pending-string-memory-wave09");
    assert_eq!(fixture.campaign.campaign_id, "fcq-string-memory-hotpaths");
    assert_eq!(fixture.campaign.wave_id, "wave-09-string-memory-hotpaths");
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave09_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave09_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave09_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for token in ["__argz_count", "__argz_next", "__argz_stringify", "__bzero"] {
        assert!(
...
#[test]
fn string_memory_hotpaths_wave09_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
