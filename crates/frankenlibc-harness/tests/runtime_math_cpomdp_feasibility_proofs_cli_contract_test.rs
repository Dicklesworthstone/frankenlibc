//! Conformance gate for the harness binary `runtime-math-cpomdp-feasibility-proofs`
//! subcommand (bd-v9enr).
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("conformance")
        .join("runtime_math_cpomdp_feasibility_proofs_cli_contract.v1.json")
}
...
fn tmp_dir() -> TestResult<PathBuf> {
    let ts = std::time::SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bd_v9enr_{}_{ts}", std::process::id()));
    std::fs::create_dir_all(&dir).map_err(|e| format!("mkdir {dir:?}: {e}"))?;
    Ok(dir)
}

#[test]
fn manifest_anchors_to_v9enr_with_subcommand_name() -> TestResult {
    let root = workspace_root()?;
    let m = load_json(&manifest_path(&root))?;
    require(
        json_string(&m, "manifest_id")? == "runtime-math-cpomdp-feasibility-proofs-cli-contract",
        "manifest_id mismatch",
    )?;
    require(json_string(&m, "bead")? == "bd-v9enr", "bead mismatch")?;
    require(
        json_string(&m, "subcommand_name")? == "runtime-math-cpomdp-feasibility-proofs",
...
fn manifest_underlying_lib_function_is_pinned() -> TestResult {
    ...
    require(
        names.contains(
            &"frankenlibc_harness::runtime_math_cpomdp_feasibility_proofs::run_and_write",
        ),
...
#[test]
fn harness_source_registers_runtime_math_cpomdp_feasibility_proofs_subcommand() -> TestResult {
    let root = workspace_root()?;
    let source = root
        .join("crates")
    ...
    require(
        body.contains("Command::RuntimeMathCpomdpFeasibilityProofs"),
        "harness.rs must register Command::RuntimeMathCpomdpFeasibilityProofs",
    )?;
    require(
        body.contains("runtime_math_cpomdp_feasibility_proofs::run_and_write"),
        "harness.rs must call runtime_math_cpomdp_feasibility_proofs::run_and_write",
    )?;
    Ok(())
}

#[test]
fn cli_writes_four_output_files_at_user_paths() -> TestResult {
    let Some(bin) = find_harness_binary() else {
        eprintln!("harness binary not built; gracefully skipping");
        return Ok(());
    };
    let dir = tmp_dir()?;
    let log = dir.join("cpomdp.log.jsonl");
    let report = dir.join("cpomdp.report.json");
    let feasibility = dir.join("feas.json");
    let sensitivity = dir.join("sens.json");
    let root = workspace_root()?;
    let result = Command::new(&bin)
        .arg("runtime-math-cpomdp-feasibility-proofs")
        .arg("--workspace-root")
        .arg(&root)
        .arg("--log")
        .arg(&log)
        .arg("--report")
        .arg(&report)
        .arg("--feasibility-artifact")
        .arg(&feasibility)
        .arg("--sensitivity-artifact")
        .arg(&sensitivity)
        .output()
    ...
    require(feasibility.exists(), "feasibility artifact must be written")?;
    require(sensitivity.exists(), "sensitivity artifact must be written")?;
