//! Completion-contract tests for bd-bp8fl.6.5.1 replacement guard recheck evidence.
...
const REQUIRED_EVENTS: &[&str] = &[
    ...
    "replacement_guard_interpose_replayed",
    "replacement_guard_replacement_replayed",
    ...
    "replacement_guard_recheck_completion_contract_validated",
];
...
fn manifest_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/replacement_guard_recheck_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_replacement_guard_recheck_completion_contract.sh")
}
...
fn run_checker(root: &Path, manifest: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env(
            "FRANKENLIBC_REPLACEMENT_GUARD_RECHECK_COMPLETION_CONTRACT",
            manifest,
        )
        .env(
            "FRANKENLIBC_REPLACEMENT_GUARD_RECHECK_COMPLETION_OUT_DIR",
            out_dir,
        )
        .env(
            "FRANKENLIBC_REPLACEMENT_GUARD_RECHECK_COMPLETION_REPORT",
            out_dir.join("replacement_guard_recheck_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_REPLACEMENT_GUARD_RECHECK_COMPLETION_LOG",
            out_dir.join("replacement_guard_recheck_completion_contract.events.jsonl"),
        )
        .output()?)
}
...
fn set_required_profile_value(manifest: &mut Value, key: &str, value: u64) -> TestResult {
    let obj = manifest
        .get_mut("completion_contract")
        .and_then(Value::as_object_mut)
        .and_then(|contract| contract.get_mut("required_replacement_profile"))
        .and_then(Value::as_object_mut)
        .ok_or_else(|| {
            test_error("completion_contract.required_replacement_profile should be an object")
        })?;
...
#[test]
fn contract_binds_replacement_guard_recheck_sources() -> TestResult {
    let root = workspace_root()?;
    let manifest = load_json(&manifest_path(&root))?;
    assert_eq!(
        string_field(&manifest, "schema_version", "manifest")?,
        "replacement_guard_recheck_completion_contract.v1"
    );
    assert_eq!(
...
#[test]
fn checker_accepts_replacement_guard_recheck_completion_contract() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "replacement-guard-recheck-check")?;
    let output = run_checker(&root, &manifest_path(&root), &out_dir)?;
    expect_checker_success(&output)?;
    assert!(
        String::from_utf8_lossy(&output.stdout)
            .contains("PASS replacement guard recheck completion contract")
    );

    let report = load_json(&checker_report(&out_dir))?;
    assert_eq!(
        string_field(&report, "schema_version", "report")?,
        "replacement_guard_recheck_completion_contract.report.v1"
    );
...
#[test]
fn checker_emits_structured_replacement_guard_telemetry() -> TestResult {
    let root = workspace_root()?;
    let out_dir = unique_output_dir(&root, "replacement-guard-recheck-telemetry")?;
    let output = run_checker(&root, &manifest_path(&root), &out_dir)?;
    expect_checker_success(&output)?;
    ...
    for row in &events {
        assert_eq!(string_field(row, "bead_id", "event")?, "bd-bp8fl.6.5.1");
        assert!(
            string_field(row, "trace_id", "event")?
                .starts_with("bd-bp8fl.6.5.1::replacement-guard-recheck::completion::v1::")
        );
    }
    Ok(())
}
...
fn checker_rejects_missing_e2e_binding() -> TestResult {
    ...
    let out_dir = unique_output_dir(&root, "replacement-guard-recheck-missing-e2e")?;
...
#[test]
fn checker_rejects_replacement_profile_drift() -> TestResult {
    let root = workspace_root()?;
    let mut manifest = load_json(&manifest_path(&root))?;
    set_required_profile_value(&mut manifest, "total_call_throughs", 1)?;

    let out_dir = unique_output_dir(&root, "replacement-guard-recheck-profile-drift")?;
    let bad_manifest = out_dir.join("bad_profile_drift.json");
    write_json(&bad_manifest, &manifest)?;

    let output = run_checker(&root, &bad_manifest, &out_dir)?;
    expect_checker_failure(&output)?;
    let report = load_json(&checker_report(&out_dir))?;
    assert!(failure_signatures(&report).contains("replacement_profile_drift"));
    Ok(())
}
...
fn checker_rejects_missing_telemetry_binding() -> TestResult {
    ...
    let out_dir = unique_output_dir(&root, "replacement-guard-recheck-missing-telemetry")?;
