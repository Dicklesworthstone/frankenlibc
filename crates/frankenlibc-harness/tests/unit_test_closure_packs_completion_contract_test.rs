//! Contract tests for bd-w2c3.9.1.1 unit test closure-pack completion evidence.
...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/unit_test_closure_packs_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_unit_test_closure_packs_completion_contract.sh")
}
...
fn unique_output_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let path = root.join("target/conformance").join(format!(
        "unit-test-closure-packs-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<std::process::Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_UNIT_TEST_CLOSURE_PACKS_CONTRACT", contract)
        .env(
            "FRANKENLIBC_UNIT_TEST_CLOSURE_PACKS_REPORT",
            out_dir.join("unit_test_closure_packs_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_UNIT_TEST_CLOSURE_PACKS_LOG",
            out_dir.join("unit_test_closure_packs_completion_contract.log.jsonl"),
        )
        .output()?)
}
...
#[test]
fn contract_binds_unit_pack_families_and_existing_tests() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&contract_path(&root))?;
    assert_eq!(manifest["bead"].as_str(), Some("bd-w2c3.9.1"));
    assert_eq!(
    ...
    assert!(
        policy["minimum_total_cases"].as_u64().unwrap_or(0) >= 470,
        "case threshold should preserve original closure count"
    );

    let sources = source_texts(&root, &manifest)?;
    let unit_refs = assert_test_refs_exist(&evidence["unit_primary"], &sources)?;
    for expected in [
        "existing_harness_test::all_required_families_have_fixtures",
        "existing_harness_test::new_closure_packs_have_cases",
        "existing_harness_test::closure_packs_have_strict_and_hardened_cases",
        "existing_harness_test::startup_fixture_expansion_covers_invariant_matrix",
        "existing_harness_test::total_fixture_count_meets_minimum",
        "completion_harness_test::contract_binds_unit_pack_families_and_existing_tests",
    ] {
    ...
    assert!(e2e_refs.contains("completion_harness_test::checker_rejects_missing_required_family"));
    Ok(())
}
...
fn checker_emits_report_log_and_replays_existing_gate() -> TestResult {
    ...
    let report =
        read_json(&out_dir.join("unit_test_closure_packs_completion_contract.report.json"))?;
    ...
    assert!(
        report["summary"]["fixture_files"].as_u64().unwrap_or(0) >= 48,
        "report should preserve fixture-pack threshold"
    );
    assert!(
        report["summary"]["total_cases"].as_u64().unwrap_or(0) >= 470,
        "report should preserve case threshold"
    );
    assert_eq!(
        report["summary"]["missing_families"]
            .as_array()
            .map(Vec::len),
        Some(0)
    );

    let rows = read_jsonl(&out_dir.join("unit_test_closure_packs_completion_contract.log.jsonl"))?;
    assert_eq!(
        rows.len(),
        3,
    ...
    for row in rows {
        ...
        assert!(
            row["trace_id"]
                .as_str()
                .is_some_and(|trace_id| trace_id.starts_with("bd-w2c3.9.1.1:"))
        );
    }
    Ok(())
}
...
fn checker_rejects_missing_required_family() -> TestResult {
    ...
    manifest["completion_debt_evidence"]["closure_pack_policy"]["required_families"]
        .as_array_mut()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "required_families array"))?
        .push(serde_json::Value::String(
            "definitely_missing_unit_pack_family".to_string(),
        ));
    ...
    assert!(
        combined.contains("closure_pack_policy.required_families must contain 12 families")
            || combined.contains("definitely_missing_unit_pack_family"),
        "failure should identify missing family:\n{combined}"
    );
    Ok(())
}
