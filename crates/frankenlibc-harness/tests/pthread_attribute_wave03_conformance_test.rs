//! Deterministic wave-03 pthread attribute fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test pthread_attribute_wave03_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/pthread_attribute_wave03.json");
...
#[test]
fn pthread_attribute_wave03_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/pthread_attribute_wave03.json");
    assert!(
        path.exists(),
        "pthread_attribute_wave03.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "pthread/sync");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.4");
    assert_eq!(fixture.campaign.campaign_id, "fcq-pthread-sync");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-03-pthread-attribute-introspection"
    );
    assert_eq!(
...
#[test]
fn pthread_attribute_wave03_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "wave-03 fixture should not leave residual symbols"
    );
...
#[test]
fn pthread_attribute_wave03_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn pthread_attribute_wave03_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture
            .spec_reference
            .contains("POSIX/GNU pthread attribute")
            && fixture.spec_reference.contains("pthread_atfork")
            && fixture
                .spec_reference
                .contains("pthread_attr_getaffinity_np")
            && fixture
                .spec_reference
                .contains("pthread_attr_getsigmask_np")
            && fixture.spec_reference.contains("pthread_attr_getstackaddr"),
        "fixture must bind pthread atfork and attribute getter surfaces"
    );
...
#[test]
fn pthread_attribute_wave03_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "pthread attribute wave-03 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
