//! Deterministic wave-05 stdio/libio stream-state fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test stdio_libio_wave05_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/stdio_libio_wave05.json");
...
#[test]
fn stdio_libio_wave05_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/stdio_libio_wave05.json");
    assert!(path.exists(), "stdio_libio_wave05.json fixture must exist");

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "stdio/libio");
    assert_eq!(fixture.campaign.bead, "bd-vm6j0.2");
    assert_eq!(fixture.campaign.campaign_id, "fcq-stdio-libio");
    assert_eq!(
        fixture.campaign.wave_id,
        "wave-05-stdio-libio-stream-state-unlocked-readers"
    );
    assert_eq!(
    ...
    assert!(
        fixture
            ...
            && fixture.description.contains("generated suffixes")
...
#[test]
fn stdio_libio_wave05_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn stdio_libio_wave05_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn stdio_libio_wave05_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
...
#[test]
fn stdio_libio_wave05_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "stdio/libio wave-05 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
