//! Integration test: fpg runtime-monitor evidence gate (bd-bp8fl.3.14)
//!
//! The gate binds 21 FEATURE_PARITY gap-summary rows to per-monitor replay,
//! calibration, failure-signature, strict/hardened decision-path, and source
//! anchors. Summary-only or stale monitor claims must fail closed.
...
fn gate_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/fpg_runtime_monitor_evidence_gate.v1.json")
}

fn report_path(root: &Path) -> PathBuf {
    root.join("target/conformance/fpg_runtime_monitor_evidence_gate.report.json")
}
...
fn run_gate(root: &Path) -> TestResult<std::process::Output> {
    Command::new(root.join("scripts/check_fpg_runtime_monitor_evidence_gate.sh"))
        .current_dir(root)
        .output()
        .map_err(|err| test_error(format!("failed to run fpg runtime-monitor gate: {err}")))
}

fn run_gate_with_fixture(root: &Path, case_name: &str, gate: &Value) -> TestResult<PathBuf> {
    let out_dir = root.join("target/conformance/fpg_runtime_monitor_negative");
    ...
    let output = Command::new(root.join("scripts/check_fpg_runtime_monitor_evidence_gate.sh"))
        .current_dir(root)
        .env("FRANKENLIBC_FPG_RUNTIME_MONITOR_GATE", &gate_fixture)
        .env("FRANKENLIBC_FPG_RUNTIME_MONITOR_REPORT", &report)
        .env("FRANKENLIBC_FPG_RUNTIME_MONITOR_LOG", &log)
        .output()
...
#[test]
fn gate_artifact_covers_runtime_monitor_rows() -> TestResult {
    ...
    for row in rows {
        ...
        for path in as_array(field(row, "monitor_paths", "row")?, "monitor_paths")? {
            let rel = path
                .as_str()
                .ok_or_else(|| test_error("monitor_paths entries must be strings"))?;
            ensure(
                safe_workspace_path(&root, rel)?.exists(),
                "monitor path must exist",
            )?;
        }
        let replay = as_object(field(row, "replay_artifact", "row")?, "replay_artifact")?;
        ensure(
            replay
                .get("strict_decision_path")
                .and_then(Value::as_str)
                .is_some()
                && replay
                    .get("hardened_decision_path")
                    .and_then(Value::as_str)
                    .is_some(),
            "replay artifact must define strict and hardened decision paths",
        )?;
        ensure(
...
#[test]
fn gate_rejects_missing_monitor_inventory() -> TestResult {
    let root = workspace_root();
    let mut gate = load_json(&gate_path(&root))?;
    let row = mutable_rows(&mut gate)?
        .get_mut(0)
        .ok_or_else(|| test_error("missing first gate row"))?;
    set_field(row, "monitor_paths", Value::Array(Vec::new()), "row")?;
    let report = run_gate_with_fixture(&root, "missing_monitor_inventory", &gate)?;
    expect_failed_check(&report, "monitor_inventory")
}

#[test]
fn gate_rejects_missing_calibration_state() -> TestResult {
    let root = workspace_root();
    let mut gate = load_json(&gate_path(&root))?;
    let row = mutable_rows(&mut gate)?
        .get_mut(1)
    ...
    let calibration = row
        .get_mut("calibration_artifact")
        .and_then(Value::as_object_mut)
        .ok_or_else(|| test_error("calibration_artifact must be an object"))?;
    calibration.insert("state".to_owned(), Value::String("missing".to_owned()));
    let report = run_gate_with_fixture(&root, "missing_calibration_state", &gate)?;
    expect_failed_check(&report, "replay_calibration")
}

#[test]
fn gate_rejects_stale_replay_source_commit() -> TestResult {
    let root = workspace_root();
    let mut gate = load_json(&gate_path(&root))?;
    let row = mutable_rows(&mut gate)?
        .get_mut(2)
...
#[test]
fn gate_rejects_summary_only_evidence() -> TestResult {
    let root = workspace_root();
    let mut gate = load_json(&gate_path(&root))?;
    let row = mutable_rows(&mut gate)?
        .get_mut(4)
        .ok_or_else(|| test_error("missing fifth gate row"))?;
    set_field(row, "replay_artifact", Value::Null, "row")?;
    set_field(row, "calibration_artifact", json!({}), "row")?;
    let report = run_gate_with_fixture(&root, "summary_only_evidence", &gate)?;
    expect_failed_check(&report, "replay_calibration")
}
