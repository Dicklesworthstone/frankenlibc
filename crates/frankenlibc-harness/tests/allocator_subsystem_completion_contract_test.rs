...
fn contract_path(root: &Path) -> PathBuf {
    root.join("tests/conformance/allocator_subsystem_completion_contract.v1.json")
}

fn checker_path(root: &Path) -> PathBuf {
    root.join("scripts/check_allocator_subsystem_completion_contract.sh")
}
...
fn unique_out_dir(root: &Path, label: &str) -> TestResult<PathBuf> {
    ...
    let out = root.join("target/conformance").join(format!(
        "allocator_subsystem_completion_contract_{label}_{}_{}",
        std::process::id(),
        nanos
    ));
...
fn run_checker(root: &Path, contract: &Path, out_dir: &Path) -> TestResult<Output> {
    Ok(Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_ALLOCATOR_SUBSYSTEM_CONTRACT", contract)
        .env("FRANKENLIBC_ALLOCATOR_SUBSYSTEM_OUT_DIR", out_dir)
        .env(
            "FRANKENLIBC_ALLOCATOR_SUBSYSTEM_REPORT",
            out_dir.join("allocator_subsystem_completion_contract.report.json"),
        )
        .env(
            "FRANKENLIBC_ALLOCATOR_SUBSYSTEM_LOG",
            out_dir.join("allocator_subsystem_completion_contract.log.jsonl"),
        )
        .output()?)
}
    ...
    for (idx, item) in array.iter().enumerate() {
        let item = item.as_str().ok_or_else(|| {
            io::Error::new(
                ...
                format!("array item {idx} should be string"),
            )
        })?;
...
#[test]
fn manifest_binds_allocator_subsystem_sources() -> TestResult {
    let root = repo_root()?;
    let manifest = read_json(&contract_path(&root))?;

    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("allocator_subsystem_completion_contract.v1")
    );
    assert_eq!(manifest["original_bead"].as_str(), Some("bd-2x5"));
    assert_eq!(manifest["completion_debt_bead"].as_str(), Some("bd-2x5.7"));
    let next_audit_score_threshold = manifest["next_audit_score_threshold"]
        .as_u64()
        .ok_or("next_audit_score_threshold must be integer")?;
    assert!(next_audit_score_threshold >= 800);
...
#[test]
fn checker_accepts_contract_and_replays_allocator_gates() -> TestResult {
    let root = repo_root()?;
    let out_dir = unique_out_dir(&root, "valid")?;
    let output = run_checker(&root, &contract_path(&root), &out_dir)?;
    assert!(output.status.success(), "{}", output_text(&output));

    let report = read_json(&out_dir.join("allocator_subsystem_completion_contract.report.json"))?;
    assert_eq!(
        report["schema_version"].as_str(),
        Some("allocator_subsystem_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["source_bead"].as_str(), Some("bd-2x5"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-2x5.7"));
    assert_eq!(report["summary"]["source_count"].as_u64(), Some(28));
    assert_eq!(report["summary"]["lane_count"].as_u64(), Some(5));
    assert_eq!(report["summary"]["missing_item_count"].as_u64(), Some(3));
    assert_eq!(report["summary"]["child_gate_count"].as_u64(), Some(6));

    let records = read_jsonl(&out_dir.join("allocator_subsystem_completion_contract.log.jsonl"))?;
...
fn checker_rejects_missing_lane_artifact() -> TestResult {
    ...
    manifest["proof_lanes"][0]["artifact_ids"] = json!(["arena_rs", "missing_artifact"]);
...
fn checker_rejects_bare_cargo_command() -> TestResult {
    ...
    manifest["missing_item_bindings"][0]["required_commands"] = json!([
        "cargo test -p frankenlibc-harness --test allocator_subsystem_completion_contract_test"
    ]);
...
fn checker_rejects_missing_required_telemetry_event() -> TestResult {
    ...
    manifest["telemetry_contract"]["required_events"] = json!(
        events
            .into_iter()
            .filter(|event| event != "allocator_subsystem_child_gates_replayed")
            .collect::<Vec<_>>()
    );
