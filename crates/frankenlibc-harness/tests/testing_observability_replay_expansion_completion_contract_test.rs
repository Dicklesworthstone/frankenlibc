...
const CONTRACT_REL: &str =
    "tests/conformance/testing_observability_replay_expansion_completion_contract.v1.json";
const CHECKER_REL: &str =
    "scripts/check_testing_observability_replay_expansion_completion_contract.sh";
const EXPECTED_MISSING_ITEMS: [&str; 4] = [
    "tests.unit.primary",
    "tests.e2e.primary",
    "tests.fuzz.primary",
...
fn unique_output_dir(root: &Path, label: &str) -> io::Result<PathBuf> {
    ...
    let dir = root.join("target").join("conformance").join(format!(
        "testing_observability_replay_expansion_contract_test-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn run_checker(root: &Path, contract_path: &Path, out_dir: &Path) -> io::Result<Output> {
    Command::new("bash")
        ...
        .current_dir(root)
        .env("FRANKENLIBC_TRACK8_COMPLETION_CONTRACT", contract_path)
        .env("FRANKENLIBC_TRACK8_COMPLETION_OUT_DIR", out_dir)
        .output()
}
...
#[test]
fn manifest_binds_all_track8_completion_evidence() -> TestResult {
    let root = workspace_root()?;
    let manifest = read_json(&root.join(CONTRACT_REL))?;
    assert_eq!(
        manifest["schema_version"].as_str(),
        Some("testing_observability_replay_expansion_completion_contract.v1")
    );
    assert_eq!(manifest["original_bead"].as_str(), Some("bd-w2c3.9"));
    assert_eq!(
    ...
    assert!(binding_covers["fuzz_phase1_inventory"].contains("tests.fuzz.primary"));
    assert!(binding_covers["fuzz_phase2_inventory"].contains("tests.fuzz.primary"));
    assert!(binding_covers["per_symbol_fixture_conformance"].contains("tests.conformance.primary"));
...
fn checker_emits_track8_completion_report_and_jsonl() -> TestResult {
    ...
    let report = read_json(
        &out_dir.join("testing_observability_replay_expansion_completion_contract.report.json"),
    )?;
    assert_eq!(
        report["schema_version"].as_str(),
        Some("testing_observability_replay_expansion_completion_contract.report.v1")
    );
    assert_eq!(report["status"].as_str(), Some("pass"));
    assert_eq!(report["original_bead"].as_str(), Some("bd-w2c3.9"));
    assert_eq!(report["completion_debt_bead"].as_str(), Some("bd-w2c3.9.4"));
    assert_eq!(report["summary"]["coverage_count"].as_u64(), Some(4));
    assert!(
        report["summary"]["binding_count"].as_u64().unwrap_or(0) >= 7,
        "expected all source evidence binders"
    );
    assert!(
    ...
    let rows = read_jsonl(
        &out_dir.join("testing_observability_replay_expansion_completion_contract.log.jsonl"),
    )?;
...
fn checker_rejects_missing_fuzz_phase2_binding() -> TestResult {
    ...
    let bindings = manifest["evidence_bindings"]
        .as_array_mut()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "evidence_bindings"))?;
    bindings.retain(|binding| binding["binding_id"].as_str() != Some("fuzz_phase2_inventory"));
    ...
    assert!(
        ...
        "checker should fail without phase2 fuzz evidence"
    );
    assert!(
        output_text(&output).contains("fuzz_phase2_inventory"),
        "{}",
        output_text(&output)
    );
    Ok(())
        ...
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "completion_coverage"))?
    {
        if row["missing_item_id"].as_str() == Some("tests.conformance.primary") {
            let binding_ids = row["binding_ids"]
                .as_array_mut()
                .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "binding_ids"))?;
            binding_ids.retain(|value| value.as_str() != Some("per_symbol_fixture_conformance"));
        }
    }
