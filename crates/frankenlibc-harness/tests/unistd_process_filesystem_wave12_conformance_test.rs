//! Deterministic wave-12 unistd/process/filesystem fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave12_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave12.json");
...
#[test]
fn unistd_process_filesystem_wave12_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave12.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave12.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "tracker-blocked-unistd-wave12");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "wave-12-unistd-process-filesystem-fscontext-statfs-statvfs"
    );
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("raw file descriptors")
            ...
            && fixture.description.contains("mount namespaces")
            && fixture.description.contains("stat buffers")
            && fixture.description.contains("filesystem identifiers")
            && fixture.description.contains("ambient file state"),
        "fixture description must reject ambient fd/path/xattr/mount/stat/filesystem capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave12_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "wave-12 fixture should not leave residual symbols"
    );
...
#[test]
fn unistd_process_filesystem_wave12_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    for case in &fixture.cases {
        ...
        assert!(
            matches!(case.expected_errno, 0 | 1 | 9 | 14 | 22),
...
#[test]
fn unistd_process_filesystem_wave12_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for token in [
        "fsconfig",
        "fsetxattr",
...
#[test]
fn unistd_process_filesystem_wave12_coverage_artifacts_bind_first_wave() -> Result<(), String> {
    ...
    for symbol in FIRST_WAVE_SYMBOLS {
        ...
        assert!(
            json_array_has_str(
                &row["fixture_files"],
                "unistd_process_filesystem_wave12.json"
            ),
            "{symbol} coverage must cite unistd_process_filesystem_wave12.json"
        );
        assert!(
    ...
    assert!(
        campaign["current_coverage_pct"].as_f64().unwrap_or(0.0) >= 25.7,
        "unistd/process coverage percent must include wave-12 fixture rows"
    );
    assert!(
        campaign["target_covered"].as_u64().unwrap_or(0) >= 191,
        "unistd/process target_covered must include the 12 wave-12 symbols"
    );
    assert!(
        campaign["target_uncovered"].as_u64().unwrap_or(u64::MAX) <= 551,
        "unistd/process target_uncovered must shrink after wave-12 coverage"
    );
...
#[test]
fn unistd_process_filesystem_wave12_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-12 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
        ...
        assert_eq!(
            ...
            "wave-12 fixture should not use ambient host outputs for {}",
            case.name
        );
