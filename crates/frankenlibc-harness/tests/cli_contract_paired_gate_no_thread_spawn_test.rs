...
//! `crates/frankenlibc-harness/tests/` calls `thread::spawn`
//! (bd-s7yyy). Paired gates are single-threaded Command spawners;
//! cargo test's runner provides per-test parallelism, so a
//! hand-rolled `thread::spawn` is either redundant or a shared-state
//! hack that creates cross-test ordering hazards.
