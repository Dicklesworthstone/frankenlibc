//! Deterministic wave-12 string/memory hot-path fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test string_memory_hotpaths_wave12_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave12.json");
...
#[test]
fn string_memory_hotpaths_wave12_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/string_memory_hotpaths_wave12.json");
    assert!(
        path.exists(),
        "string_memory_hotpaths_wave12.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "string_memory_hotpaths_wave12");
    assert_eq!(fixture.campaign.bead, "bd-kgy3r.8");
    assert_eq!(fixture.campaign.campaign_id, "fcq-string-memory-hotpaths");
    assert_eq!(fixture.campaign.wave_id, "wave-12-string-memory-hotpaths");
    assert_eq!(
    ...
    assert!(
        ...
        "fixture description must reject ambient pointer/allocation/stderr/locale/process capture"
    );
    Ok(())
}

#[test]
fn string_memory_hotpaths_wave12_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave12_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn string_memory_hotpaths_wave12_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn string_memory_hotpaths_wave12_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
