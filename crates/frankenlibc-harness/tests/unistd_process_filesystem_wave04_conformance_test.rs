//! Deterministic wave-04 unistd/process/filesystem AIO/random/argp fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test unistd_process_filesystem_wave04_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave04.json");
...
#[test]
fn unistd_process_filesystem_wave04_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path =
        repo_root()?.join("tests/conformance/fixtures/unistd_process_filesystem_wave04.json");
    assert!(
        path.exists(),
        "unistd_process_filesystem_wave04.json fixture must exist"
    );

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "unistd/process-filesystem");
    assert_eq!(fixture.campaign.bead, "bd-mk28y.1");
    assert_eq!(
    ...
    assert!(
        fixture
            ...
            && fixture.description.contains("random values")
            && fixture.description.contains("alarm leftovers")
            && fixture.description.contains("stream contents"),
        "fixture description must reject ambient random/alarm/argp/AIO metadata capture"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave04_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn unistd_process_filesystem_wave04_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn unistd_process_filesystem_wave04_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert!(
        fixture.spec_reference.contains("POSIX.1 AIO")
            && fixture.spec_reference.contains("glibc LFS AIO")
            && fixture.spec_reference.contains("POSIX alarm")
            && fixture.spec_reference.contains("BSD arc4random")
            && fixture.spec_reference.contains("GNU argp"),
        "fixture must bind POSIX/glibc/BSD/GNU wave-04 semantics"
    );
    Ok(())
}

#[test]
fn unistd_process_filesystem_wave04_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
        fixture.cases.len(),
    ...
    for case in &fixture.cases {
        let result =
            execute_case_via_harness(&case.function, &case.inputs, &case.mode).map_err(|err| {
                format!(
                    "unistd process/filesystem wave-04 case {} ({}) failed via harness: {err}",
                    case.name, case.mode
                )
            })?;
        assert_eq!(
