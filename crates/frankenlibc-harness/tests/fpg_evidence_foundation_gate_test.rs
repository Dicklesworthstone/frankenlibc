...
fn gate_artifact_covers_foundation_rows() -> TestResult {
    ...
    ensure_eq(
        ...
        "fpg-gap-summary-evidence-foundation",
        "owner_family_group",
    )?;
    ...
    ensure_eq(
        freshness_policy
            .get("foundation_evidence_allowed_when_stale")
...
fn gate_rejects_stale_feature_parity_binding() -> TestResult {
    ...
    provenance.insert("line".to_owned(), Value::from(277));
