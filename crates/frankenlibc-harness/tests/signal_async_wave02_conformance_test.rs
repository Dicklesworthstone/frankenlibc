//! Deterministic wave-02 signal/async fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test signal_async_wave02_conformance_test
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/signal_async_wave02.json");
...
#[test]
fn signal_async_wave02_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/signal_async_wave02.json");
    assert!(path.exists(), "signal_async_wave02.json fixture must exist");

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "signal/async");
    assert_eq!(fixture.campaign.bead, "bd-gmbqy.9");
    assert_eq!(fixture.campaign.campaign_id, "fcq-signal-async");
    assert_eq!(fixture.campaign.wave_id, "wave-02-signal-async");
    assert_eq!(
    ...
    for token in [
        ...
        assert!(
            ...
            "fixture description must reject or name signal token {token}"
        );
    }
    Ok(())
}
...
#[test]
fn signal_async_wave02_covers_second_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
...
#[test]
fn signal_async_wave02_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
    ...
    let mut saw_child_isolation = false;
    let mut saw_mask_restore = false;
    let mut saw_set_algebra = false;
    for case in &fixture.cases {
        ...
        saw_child_isolation |= case.expected_output.contains("CHILD_EXIT_0");
        saw_mask_restore |= case.function == "sigprocmask" || case.function == "sigrelse";
        saw_set_algebra |= case.function == "sigisemptyset" || case.function == "sigorset";
        for forbidden in [
            "0x",
            "pid=",
            "tid=",
    ...
    assert!(
        saw_child_isolation,
        "wave must include isolated child checks"
    );
    assert!(saw_mask_restore, "wave must include mask restore checks");
    assert!(
        saw_set_algebra,
        "wave must include signal-set algebra checks"
    );
    Ok(())
}

#[test]
fn signal_async_wave02_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in SECOND_WAVE_SYMBOLS {
        assert!(
    ...
    assert!(
        fixture.spec_reference.contains("POSIX") && fixture.spec_reference.contains("GNU"),
        "spec reference must name POSIX and GNU surfaces"
    );
    Ok(())
}

#[test]
fn signal_async_wave02_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
