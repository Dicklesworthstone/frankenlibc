//! Deterministic wave-01 signal/async fixture tests.
//!
//! Run: cargo test -p frankenlibc-harness --test signal_async_wave01_conformance_test
...
const AMBIENT_POLICY: &str = "forbid_signal_delivery_timing_raw_sigset_bytes_stack_address_handler_address_stderr_stdout_pid_tid_or_wall_clock_capture";
...
fn load_fixture() -> Result<FixtureFile, String> {
    let path = repo_root()?.join("tests/conformance/fixtures/signal_async_wave01.json");
...
#[test]
fn signal_async_wave01_fixture_exists_and_names_campaign() -> Result<(), String> {
    let path = repo_root()?.join("tests/conformance/fixtures/signal_async_wave01.json");
    assert!(path.exists(), "signal_async_wave01.json fixture must exist");

    let fixture = load_fixture()?;
    assert_eq!(fixture.version, "v1");
    assert_eq!(fixture.family, "signal/async");
    assert_eq!(fixture.campaign.bead, "bd-r5k32.12");
    assert_eq!(fixture.campaign.campaign_id, "fcq-signal-async");
    assert_eq!(fixture.campaign.wave_id, "wave-01-signal-async");
    assert_eq!(
    ...
    assert!(
        fixture.description.contains("real-time signal range")
            && fixture.description.contains("process-group error")
            && fixture.description.contains("isolated pause")
            && fixture.description.contains("thread mask")
            && fixture.description.contains("alternate-stack")
            && fixture.description.contains("signal-set intersection")
            && fixture.description.contains("raw signal-set bytes")
            && fixture.description.contains("stack addresses")
            && fixture.description.contains("handler addresses")
            ...
            && fixture.description.contains("wall-clock values"),
        "fixture description must reject ambient signal metadata capture"
    );
    Ok(())
}

#[test]
fn signal_async_wave01_covers_first_wave_in_both_modes() -> Result<(), String> {
    let fixture = load_fixture()?;
    ...
    assert!(
        ...
        "signal/async wave should not leave residual symbols"
    );
...
#[test]
fn signal_async_wave01_logs_without_ambient_leaks() -> Result<(), String> {
    let fixture = load_fixture()?;
    assert_eq!(
...
#[test]
fn signal_async_wave01_spec_reference_names_required_surfaces() -> Result<(), String> {
    let fixture = load_fixture()?;
    for symbol in FIRST_WAVE_SYMBOLS {
        assert!(
    ...
    assert!(
        fixture.spec_reference.contains("POSIX")
            ...
            && fixture.spec_reference.contains("pthread_sigmask")
            && fixture.spec_reference.contains("sigaltstack"),
        "spec reference must name POSIX, GNU, pthread_sigmask, and sigaltstack surfaces"
    );
    Ok(())
}

#[test]
fn signal_async_wave01_executes_via_isolated_harness() -> Result<(), String> {
    let fixture = load_fixture()?;
    for case in &fixture.cases {
        let result =
