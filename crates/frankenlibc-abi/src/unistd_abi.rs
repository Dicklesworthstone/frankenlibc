...
/// The `posix_fallocate` fallback decision, split out from the live syscall so a
/// conformance gate can drive the `EOPNOTSUPP` branch on a host whose filesystem
/// supports `fallocate(2)` natively (every branch here is otherwise unreachable
/// on ext4/tmpfs, which is how the wiring was silently deleted once already —
/// see bd-om1s58).
///
/// glibc (`sysdeps/unix/sysv/linux/posix_fallocate.c`) falls back to the generic
/// zero-fill emulation ONLY on `EOPNOTSUPP`. Every other error, `ENOSYS`
/// included, is returned to the caller unchanged.
#[allow(unsafe_code)]
pub unsafe fn posix_fallocate_dispatch(
    fd: c_int,
    offset: libc::off_t,
    len: libc::off_t,
    raw: Result<(), i32>,
...
pub unsafe extern "C" fn posix_fallocate(
    ...
    let raw = syscall::sys_fallocate(fd, 0, offset, len);
    let rc = unsafe { posix_fallocate_dispatch(fd, offset, len, raw) };
    runtime_policy::observe(ApiFamily::IoFd, decision.profile, 12, rc != 0);
    rc
}
...
pub unsafe extern "C" fn getpass(prompt: *const c_char) -> *mut c_char {
    ...
    //
    // Restored under bd-80rgls: e634aff2a reverted this to a single-fd version
    // that simply returned NULL when /dev/tty could not be opened, so getpass
    // failed outright in any session without a controlling terminal — a cron
    // job, a CI runner, a setsid child — where glibc falls back to stdin.
    ...
    // Only close a descriptor we opened — closing borrowed stdin would be a
    // silent, process-wide fd corruption.
    if owns_fd {
        let _ = syscall::sys_close(in_fd);
    }
    result
...
pub unsafe extern "C" fn getservent_r(
    ...
    if result_buf.is_null()
        || buf.is_null()
        || !tracked_object_fits::<libc::servent>(result_buf.cast())
    {
        return libc::EINVAL;
    }
    ...
    with_serv_iter_state(|state| {
        ...
        loop {
            ...
            let proto_end = match name_end
                .checked_add(entry.protocol.len())
                .and_then(|offset| offset.checked_add(1))
            {
                Some(offset) => offset,
                None => return libc::ERANGE,
            ...
            let name_ptr = buf;
            unsafe {
                std::ptr::copy_nonoverlapping(entry.name.as_ptr(), buf_u8, entry.name.len());
                *buf_u8.add(entry.name.len()) = 0;
            }

            let proto_ptr = unsafe { buf_u8.add(name_end) } as *mut c_char;
            unsafe {
                std::ptr::copy_nonoverlapping(
                    entry.protocol.as_ptr(),
                    buf_u8.add(name_end),
