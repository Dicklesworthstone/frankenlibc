// mbsrtowcs WRITE mode (dst != NULL) head-to-head vs host glibc (dlmopen). Write
// mode ASCII-bulk + scalar `mbtowc` per multibyte char. Output verified
...
fn main() {
    unsafe {
        ...
        type MbsrtowcsFn =
            unsafe extern "C" fn(*mut i32, *mut *const c_char, usize, *mut c_void) -> usize;
        let gl_setlocale: SetlocaleFn = std::mem::transmute::<*mut c_void, SetlocaleFn>(
            libc::dlsym(h, b"setlocale\0".as_ptr().cast()),
        );
        let gl_mbsrtowcs: MbsrtowcsFn = std::mem::transmute::<*mut c_void, MbsrtowcsFn>(
            libc::dlsym(h, b"mbsrtowcs\0".as_ptr().cast()),
        );
        assert!(
        ...
        for &(label, ref src_str) in &[
            ...
            let mut fp = base;
            let fl_len = frankenlibc_abi::wchar_abi::mbsrtowcs(
                fl_dst.as_mut_ptr(),
            ...
            let mut gp = base;
            let gl_len = gl_mbsrtowcs(
                gl_dst.as_mut_ptr(),
                &mut gp as *mut *const c_char,
                cap,
                std::ptr::null_mut(),
            );
            assert_eq!(fl_len, gl_len, "mbsrtowcs write {label} len");
            for k in 0..fl_len {
                assert_eq!(fl_dst[k], gl_dst[k], "mbsrtowcs write {label} char {k}");
            }
            ...
            for _ in 0..iters {
                let mut p = base;
                black_box(frankenlibc_abi::wchar_abi::mbsrtowcs(
            ...
            println!(
                "MBSRTOWCS_WRITE {label} n={n} fl={fl:.0}ns glibc={gl:.0}ns fl/glibc={:.3}x",
                fl / gl
            );
        }
    }
