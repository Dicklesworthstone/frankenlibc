...
fn main() {
    unsafe {
        ...
        let gl_setlocale: SetlocaleFn =
            std::mem::transmute::<*mut c_void, SetlocaleFn>(libc::dlsym(h, b"setlocale\0".as_ptr().cast()));
        let gl_mbstowcs: MbstowcsFn =
            std::mem::transmute::<*mut c_void, MbstowcsFn>(libc::dlsym(h, b"mbstowcs\0".as_ptr().cast()));
        assert!(!gl_setlocale(libc::LC_ALL, b"C.UTF-8\0".as_ptr().cast()).is_null(), "setlocale failed");
        ...
        for &(label, ref src_str) in &[
            ...
            assert_eq!(fl_len, gl_len, "mbstowcs {label} len: fl={fl_len} glibc={gl_len}");
            for k in 0..fl_len {
            ...
            let t0 = Instant::now();
            for _ in 0..iters {
                black_box(frankenlibc_core::string::wchar::mbstowcs(black_box(&mut fl_dest), black_box(src)));
            }
            let fl = t0.elapsed().as_nanos() as f64 / iters as f64;
            let t1 = Instant::now();
            for _ in 0..iters {
                black_box(gl_mbstowcs(black_box(gl_dest.as_mut_ptr()), black_box(src_c.as_ptr()), n + 1));
            }
            let gl = t1.elapsed().as_nanos() as f64 / iters as f64;
            println!("MBSTOWCS {label} n={n} fl={fl:.0}ns glibc={gl:.0}ns fl/glibc={:.3}x", fl / gl);
        }
    }
}
