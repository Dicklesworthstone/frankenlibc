...
fn values() -> Vec<f128> {
    let mut v: Vec<f128> = vec![
        0.0, -0.0f128, 1.0, -1.0, 2.0, -2.0, 3.0, -3.0, 0.5, 5.0, -5.0, 1e300f128, -1e-300f128,
...
fn f128_maxmin_c23_match_glibc() {
    ...
    for &x in &vals {
        for (name, gf, ff) in [
            ("nextup", nextupf128 as unsafe extern "C" fn(f128) -> f128, ma::nextupf128 as unsafe extern "C" fn(f128) -> f128),
            ...
            let fe = unsafe { *el() };
            if g != f || ge != fe {
                mism.push(format!("{name} x={:#034x}: glibc=({g:#034x},e={ge}) fl=({f:#034x},e={fe})", x.to_bits()));
            }
        }
    }
    ...
    let fns: &[(&str, F, F)] = &[
        ...
        ("fmaximum_mag_num", fmaximum_mag_numf128, ma::fmaximum_mag_numf128),
        ("fminimum_mag_num", fminimum_mag_numf128, ma::fminimum_mag_numf128),
    ];
    for &(name, gf, ff) in fns {
        for &x in &vals {
            for &y in &vals {
                ...
                if g != f {
                    mism.push(format!("{name} x={:#034x} y={:#034x}: glibc={g:#034x} fl={f:#034x}", x.to_bits(), y.to_bits()));
                }
            }
        }
    }

    assert!(mism.is_empty(), "f128 C23 max/min diverged ({}):\n{}", mism.len(), mism.iter().take(30).cloned().collect::<Vec<_>>().join("\n"));
}
