...
fn crypt_checksalt_recognized_settings_match_host() {
    ...
    for s in ["$6$abcdefgh$", "$5$abcdefgh$", "$1$abcdefgh$", "$z$bad", "*0", "*1", "$$"] {
        let h = host(s);
        let f = fl(s);
    ...
    assert_eq!(host("$1$abcdefgh$"), 3, "host $1$ should be METHOD_LEGACY(3)");
    assert_eq!(host("$z$bad"), 1, "host unknown scheme should be INVALID(1)");
}
