...
fn math_multi_output_parity_vs_glibc() {
    ...
    for &x in SP {
        ...
        if !exact(fm, gm) || (x.is_finite() && fe != ge) {
            div.push(format!(
                "frexp({x:e}): fl=({fm:?},{fe}) glibc=({gm:?},{ge})"
            ));
        }
    }
    ...
    for &x in SP {
        ...
        if !exact(ff, gf) || !exact(fi, gi) {
            div.push(format!(
                "modf({x:e}): fl=(frac {ff:?}, int {fi:?}) glibc=(frac {gf:?}, int {gi:?})"
            ));
        }
    }
    ...
    for &x in SP {
        for &y in SP {
            ...
            if fr.is_finite() && gr.is_finite() && (fq & 7) != (gq & 7) {
                div.push(format!(
                    "remquo({x:e},{y:e}) quo low3: fl={} glibc={}",
                    fq & 7,
                    gq & 7
                ));
            }
    ...
    for &x in SP {
        ...
        if !approx(fv, gv, 8) {
            div.push(format!("lgamma_r({x:e}) val: fl={fv:?} glibc={gv:?}"));
        }
    ...
    for &x in SP {
        ...
        let mut gsin: f64 = 0.0;
        let mut gcos: f64 = 0.0;
        unsafe { sincos(x, &mut gsin, &mut gcos) };
        if !approx(fsin, gsin, 2) {
            div.push(format!("sincos({x:e}) sin: fl={fsin:?} glibc={gsin:?}"));
        }
        if !approx(fcos, gcos, 2) {
            div.push(format!("sincos({x:e}) cos: fl={fcos:?} glibc={gcos:?}"));
        }
    }
    ...
    for &x in SP {
        for e in [0, 1, -1, 1024, -1074, 2000, -2000] {
            let f = fl::ldexp(x, e);
            let g = unsafe { ldexp(x, e) };
            if !exact(f, g) {
                div.push(format!("ldexp({x:e},{e}): fl={f:?} glibc={g:?}"));
            }
        }
    }
    // f32 frexpf / modff.
    for &x in SP {
        ...
        if !beqf(fm, gm) || (xf.is_finite() && fe != ge) {
            div.push(format!(
                "frexpf({xf:e}): fl=({fm:?},{fe}) glibc=({gm:?},{ge})"
            ));
        }
        ...
        if !beqf(ff, gf) || !beqf(fi, gi) {
            div.push(format!(
                "modff({xf:e}): fl=(frac {ff:?}, int {fi:?}) glibc=(frac {gf:?}, int {gi:?})"
            ));
        }
    }
    ...
    assert!(
        div.is_empty(),
        "multi-output math divergences vs glibc:\n  {}",
        div.join("\n  ")
    );
}
