//! Differential gate: IBM-930 (Japanese EBCDIC) iconv codec vs glibc.
//!
//! IBM-930 is a stateful codec: single-byte EBCDIC host code, with SO (0x0E) /
//! SI (0x0F) shifting into the IBM host double-byte set. glibc is reached via
//! dlsym so its symbols bypass fl's no_mangle interposition.
//!
//! Coverage is byte-for-byte against the live host glibc:
//!   * decode every single byte and every SO-wrapped double-byte pair, comparing
//!     the EXACT single-call outcome (output bytes + consumed-prefix length);
//!   * decode pseudo-random multi-byte streams (SBCS + SO/SI + DBCS);
//!   * encode + round-trip on curated Japanese/ASCII samples (with flush).
#![cfg(target_os = "linux")]
#![allow(unsafe_code)]
#![allow(dead_code)]
...
#[test]
fn ibm930_decode_matches_glibc() {
    let g = glibc();
    let mut mism = Vec::new();
    // Every single byte (SBCS mode).
    for b in 0u16..256 {
        let inp = [b as u8];
        let gr = g_raw(&g, "UTF-32LE", "IBM930", &inp);
        let fr = f_raw("UTF-32LE", "IBM930", &inp);
        if gr != fr && mism.len() < 40 {
            mism.push(format!("sbcs {b:02x}: glibc={gr:x?} fl={fr:x?}"));
        }
    }
    // Every SO-wrapped double-byte pair, then SI.
    for b0 in 0x40u16..=0xFE {
        for b1 in 0x40u16..=0xFE {
            let inp = [0x0E, b0 as u8, b1 as u8, 0x0F];
            let gr = g_raw(&g, "UTF-32LE", "IBM930", &inp);
            let fr = f_raw("UTF-32LE", "IBM930", &inp);
            if gr != fr && mism.len() < 80 {
                mism.push(format!("dbcs {b0:02x}{b1:02x}: glibc={gr:x?} fl={fr:x?}"));
            }
        }
    }
    // Random multi-byte streams (deterministic LCG).
    let mut state: u64 = 0x1B_09_30_42_u64;
    let mut next = || {
        state = state.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
        (state >> 33) as usize
    };
    let alpha: &[u8] = &[
        0x0e, 0x0f, 0x40, 0xc1, 0xc2, 0x81, 0x41, 0x42, 0x4b, 0x5c, 0x7e, 0xfe, 0x00, 0x20,
    ];
    for _ in 0..20_000 {
        let len = 1 + next() % 10;
    ...
    assert!(mism.is_empty(), "IBM-930 decode diverged ({}):\n{}", mism.len(), mism.join("\n"));
}

#[test]
fn ibm930_encode_roundtrip_matches_glibc() {
    let g = glibc();
    let mut mism = Vec::new();
    let samples: &[&str] = &[
        "",
    ...
    for s in samples {
        let u = s.as_bytes();
        let ge = g_full(&g, "IBM930", "UTF-8", u);
        let fe = f_full("IBM930", "UTF-8", u);
        if ge != fe {
            mism.push(format!("ENC {s:?}: glibc={ge:02x?} fl={fe:02x?}"));
            continue;
        }
        if let Some(enc) = ge {
            let gd = g_full(&g, "UTF-8", "IBM930", &enc);
            let fd = f_full("UTF-8", "IBM930", &enc);
            if gd != fd {
    ...
    // Round-trip every decodable single byte and double-byte cell.
    for b in 0u16..256 {
        let inp = [b as u8];
        if let Some(u) = g_full(&g, "UTF-8", "IBM930", &inp) {
            if g_full(&g, "IBM930", "UTF-8", &u) != f_full("IBM930", "UTF-8", &u) && mism.len() < 40 {
                mism.push(format!("reencode sbcs {b:02x}"));
            }
        }
    }
    for alias in ["CP930", "CSIBM930", "IBM-930"] {
        let fe = f_full(alias, "UTF-8", "日".as_bytes());
        let ge = g_full(&g, "IBM930", "UTF-8", "日".as_bytes());
        if fe != ge {
            mism.push(format!("ALIAS {alias}"));
        }
    }
    assert!(mism.is_empty(), "IBM-930 encode/roundtrip diverged ({}):\n{}", mism.len(), mism.join("\n"));
}
