//! Differential gate: remaining EBCDIC country/code pages (DE/FR/GB/IT/ES/PT/NO/FI/JP/KSC5636/GREEK7/Braille/Sami/...),
...
fn gdb(gg:&G,name:&str,b:u8)->Vec<u32>{
    ...
    let mut inb=[b];let mut out=[0u8;64];let mut ip=inb.as_mut_ptr() as *mut c_char;let mut il=1usize;
...
#[test]
fn ebcdic_remainder_match_glibc(){
    let gg=g();
    for &name in CODECS {
        let mut cps=std::collections::BTreeSet::new();
        for b in 0u16..256 {
            let fb=fdb(name,b as u8); let gb=gdb(&gg,name,b as u8);
            assert_eq!(fb,gb,"{name} decode byte {b:#04x}");
            for &cp in &gb { if cp>=0x80 { cps.insert(cp); } }
        }
        for cp in cps { assert_eq!(fe(name,cp),ge(&gg,name,cp),"{name} encode U+{cp:04X}"); }
    }
}
