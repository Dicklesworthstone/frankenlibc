//! Differential gate: batch B of newly-added ASCII-compatible single-byte codecs
//! (bd-4tgqly) — MAC-IS, MAC-UK, MS-MAC-CYRILLIC — plus their
...
const CODECS: &[(&str, &[&str])] = &[
    ("MAC-IS", &["MACIS"]),
    ("MAC-UK", &["MACUK", "MACUKRAINIAN"]),
    ("MS-MAC-CYRILLIC", &["MSMACCYRILLIC"]),
];

#[test]
fn sbcs_batch_b_decode_encode_matches_glibc() {
    let gg = g();
