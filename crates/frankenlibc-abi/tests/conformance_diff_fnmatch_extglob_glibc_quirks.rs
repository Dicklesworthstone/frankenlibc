...
fn period_star_question_group_inconsistency() {
    ...
    assert!(gl_m("*+(?)", "c.", f), "glibc baseline: *+(?) on c. matches");
    assert!(gl_m("?+(?)", "c.", f), "glibc baseline: ?+(?) on c. matches");
    assert!(!gl_m("*?+(?)", "c.", f), "glibc inconsistency: rejects *?+(?) on c.");
