...
fn values() -> Vec<f128> {
    let mut v: Vec<f128> = vec![
        0.0, -0.0f128, 1.0, -1.0, 2.0, 3.0, -3.0, 0.5, 10.0,
        4932.0, 4933.0, 4934.0, -4974.0, -4975.0, -5000.0,
...
fn f128_exp10_matches_glibc() {
    ...
    for &x in &values() {
        ...
        n += 1;
        if g != f || ge != fe {
            mism.push(format!("exp10({:#034x}): glibc=({g:#034x},e={ge}) fl=({f:#034x},e={fe})", x.to_bits()));
        }
    }
    assert!(mism.is_empty(), "exp10f128 diverged ({}/{}):\n{}", mism.len(), n, mism.iter().take(30).cloned().collect::<Vec<_>>().join("\n"));
}
