...
fn getlogin_r_matches_glibc() {
    ...
    assert_eq!((f.0, &f.2), (g.0, &g.2), "getlogin_r: fl=(rc{},name{:?}) glibc=(rc{},name{:?})", f.0, f.2, g.0, g.2);
    if g.0 != 0 {
        assert_eq!(f.1, g.1, "getlogin_r failure errno: fl={} glibc={}", f.1, g.1);
    }
}
...
fn getlogin_r_small_buffer_matches_glibc() {
    ...
    let f = unsafe { (frankenlibc_abi::unistd_abi::getlogin_r(fb.as_mut_ptr() as *mut c_char, 1), errno()) };
    assert_eq!(f.0, g.0, "getlogin_r(len=1) rc: fl={} glibc={}", f.0, g.0);
    if g.0 != 0 {
        assert_eq!(f.1, g.1, "getlogin_r(len=1) errno: fl={} glibc={}", f.1, g.1);
    }
}
