...
fn gdec(gg:&G,name:&str,bytes:&[u8])->Vec<u32>{
    ...
    let mut src=bytes.to_vec(); let mut out=[0u8;64];
    let mut ip=src.as_mut_ptr() as *mut c_char; let mut il=src.len();
    let mut op=out.as_mut_ptr() as *mut c_char; let mut ol=64usize;
    ...
    (gg.close)(cd);
    if r==INVALID&&il!=0 {return vec![u32::MAX];}  // error marker
    let n=64-ol; (0..n/4).map(|i|u32::from_le_bytes([out[i*4],out[i*4+1],out[i*4+2],out[i*4+3]])).collect()
}
fn fdec(name:&str,bytes:&[u8])->Vec<u32>{
    ...
    if r==INVALID&&il!=0 {return vec![u32::MAX];}
    let n=64-ol; (0..n/4).map(|i|u32::from_le_bytes([out[i*4],out[i*4+1],out[i*4+2],out[i*4+3]])).collect()
}
...
fn iso6937_family_match_glibc(){
    ...
    for &name in CODECS {
        ...
        for b in 0u16..256 {
            let inp=[b as u8];
            assert_eq!(fdec(name,&inp),gdec(&gg,name,&inp),"{name} decode {b:#04x}");
            for &c in &gdec(&gg,name,&inp) { if c!=u32::MAX && c>=0x80 { cps.insert(c);} }
        }
        // prefix pairs 0xC0-0xCF x 0..256
        for p in 0xC0u16..=0xCF {
            for l in 0u16..256 {
                let inp=[p as u8,l as u8];
                assert_eq!(fdec(name,&inp),gdec(&gg,name,&inp),"{name} pair {p:#04x},{l:#04x}");
                for &c in &gdec(&gg,name,&inp) { if c!=u32::MAX && c>=0x80 { cps.insert(c);} }
            }
        }
        for cp in cps { assert_eq!(fenc(name,cp),genc(&gg,name,cp),"{name} encode U+{cp:04X}"); }
    }
}
