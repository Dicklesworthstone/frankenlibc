...
fn values() -> Vec<f128> {
    let mut v: Vec<f128> = vec![
        0.0, -0.0f128, 0.25, 0.5, 0.875, 1.0, 1.25, 2.0, 5.0, 9.0, 16.0, 100.0,
        107.0, 108.0, -0.5, -2.0, -9.0, -16.0, -107.0, 1e-20f128, 1e-40f128,
        f128::from_bits(1),
...
fn f128_erf_matches_glibc() {
    ...
    for (name, g, f) in [
        ("erf", erff128 as unsafe extern "C" fn(f128) -> f128, ma::erff128 as unsafe extern "C" fn(f128) -> f128),
