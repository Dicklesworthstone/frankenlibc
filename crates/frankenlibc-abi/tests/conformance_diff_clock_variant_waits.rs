...
mod g {
    use super::*;
    unsafe extern "C" {
        pub fn pthread_mutex_clocklock(m: *mut libc::pthread_mutex_t, c: c_int, t: *const libc::timespec) -> c_int;
        pub fn pthread_cond_clockwait(c: *mut libc::pthread_cond_t, m: *mut libc::pthread_mutex_t, clk: c_int, t: *const libc::timespec) -> c_int;
        pub fn pthread_rwlock_clockrdlock(rw: *mut libc::pthread_rwlock_t, c: c_int, t: *const libc::timespec) -> c_int;
        pub fn pthread_rwlock_clockwrlock(rw: *mut libc::pthread_rwlock_t, c: c_int, t: *const libc::timespec) -> c_int;
...
fn future() -> libc::timespec {
    libc::timespec { tv_sec: i64::MAX / 2, tv_nsec: 0 }
}
...
fn clock_variant_waits_reject_bad_clockid_like_glibc() {
    ...
    for &clk in &BAD_CLOCKS {
        ...
        assert_eq!(f_rd, g_rd, "rwlock_clockrdlock clk={clk}: fl={f_rd} glibc={g_rd}");
        let g_wr = unsafe { g::pthread_rwlock_clockwrlock(&mut grw, clk, &at) };
        let f_wr = unsafe { flp::pthread_rwlock_clockwrlock(&mut frw, clk, &at) };
        assert_eq!(f_wr, g_wr, "rwlock_clockwrlock clk={clk}: fl={f_wr} glibc={g_wr}");
        ...
        assert_eq!(f_cw, g_cw, "cond_clockwait clk={clk}: fl={f_cw} glibc={g_cw}");
        ...
        unsafe { *g::__errno_location() = 0 };
        let f_sw = unsafe { flg::sem_clockwait(fs.as_mut_ptr() as *mut c_void, clk, &at as *const _ as *const c_void) };
        let f_se = unsafe { *g::__errno_location() };
        assert_eq!((f_sw, f_se), (g_sw, g_se), "sem_clockwait clk={clk}: fl=({f_sw},{f_se}) glibc=({g_sw},{g_se})");
    }
}
