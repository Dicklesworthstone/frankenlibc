//! Differential gate: f128 log2p1f128 + log10p1f128 (C23) match glibc
//! bit-for-bit on value + errno (bd-9z5ikz). Both were garbage f64-ABI stubs.
//! The fixes mirror glibc's generic templates: log2(e)/log10(e) · log1p(x)
//! with the near-0 linear path and EDOM/ERANGE. Built on the byte-exact
//! log1pl_f128 → byte-exact.
#![cfg(target_os = "linux")]
#![feature(f128)]
#![allow(unsafe_code)]
...
unsafe extern "C" {
    fn log2p1f128(x: f128) -> f128;
    fn log10p1f128(x: f128) -> f128;
}
fn el() -> *mut c_int {
...
#[test]
fn f128_logp1_base_matches_glibc() {
