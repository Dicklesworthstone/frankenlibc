...
fn iso_and_golden() {
    ...
    for _ in 0..200000 {
        ...
        if rng() & 1 == 0 {
            for x in b.iter_mut() {
                if rng() & 7 == 0 {
                    *x = alpha[(rng() as usize) % alpha.len()];
                }
            }
        } else {
            for x in b.iter_mut() {
        ...
        if fl != gl {
            div += 1;
            if div <= 5 {
                eprintln!("DIV a={:?} b={:?} fl={fl} gl={gl}", ca, cb);
            }
        }
        h.update([fl as u8]);
        n += 1;
    ...
    assert_eq!(n, 200000);
    assert_eq!(
        hex, "37d0b3d211451f288c42aefbffceb890df30c535cc7c313027e451c226f4dea1",
