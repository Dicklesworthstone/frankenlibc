...
fn tmp() -> (std::path::PathBuf, CString) {
    ...
    std::fs::write(
        &p,
        b"root:$6$abc$hash:19000:0:99999:7:::\nsvc:!:18000:1:60000:14:30:25000:0\n",
    )
    .unwrap();
    (
        p.clone(),
...
fn nums(
    ...
    let name = if sp.sp_namp.is_null() {
        String::new()
    } else {
        unsafe { CStr::from_ptr(sp.sp_namp) }
            .to_string_lossy()
            .into_owned()
    };
    (
