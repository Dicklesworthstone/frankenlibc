...
fn gconv(gg: &G, to: &str, from: &str, inp: &[u8]) -> Option<Vec<u8>> {
    ...
    let cd = (gg.open)(ct.as_ptr(), cf.as_ptr());
    if cd as usize == INVALID {
        return None;
    }
    let mut s = inp.to_vec();
    let mut o = vec![0u8; 512];
...
fn utf7imap_encode_matches_glibc() {
    ...
    for s in SAMPLES {
        let b = s.as_bytes();
        assert_eq!(
            flconv("UTF-7-IMAP", "UTF-8", b),
            gconv(&gg, "UTF-7-IMAP", "UTF-8", b),
            "encode {s:?}"
        );
    }
}
...
fn utf7imap_decode_matches_glibc() {
    ...
    for s in SAMPLES {
        if let Some(enc) = gconv(&gg, "UTF-7-IMAP", "UTF-8", s.as_bytes()) {
            if enc == vec![0xDE, 0xAD] {
                continue;
            }
            assert_eq!(
                flconv("UTF-8", "UTF-7-IMAP", &enc),
                gconv(&gg, "UTF-8", "UTF-7-IMAP", &enc),
                "decode of {s:?} -> {enc:02x?}"
            );
        }
    }
    for raw in [&b"A&AOk-B"[..], b"&-", b"&ZeVnLA-", b"x&-y", b"&AAk-"] {
        assert_eq!(
            flconv("UTF-8", "UTF-7-IMAP", raw),
            gconv(&gg, "UTF-8", "UTF-7-IMAP", raw),
            "raw decode {raw:02x?}"
        );
    }
}
