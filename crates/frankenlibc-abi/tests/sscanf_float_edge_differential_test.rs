...
fn sscanf_float_edges_match_glibc() {
    ...
    for input in cases {
        ...
        let cs = CString::new(input).unwrap();
        let ff = unsafe { frankenlibc_abi::stdlib_abi::strtof(cs.as_ptr(), std::ptr::null_mut()) }
            .to_bits();
        let fg = unsafe { strtof(cs.as_ptr(), std::ptr::null_mut()) }.to_bits();
        if ff != fg {
            fails.push(format!("strtof {input:?}: fl={ff:#x} glibc={fg:#x}"));
        }
        let df = unsafe { frankenlibc_abi::stdlib_abi::strtod(cs.as_ptr(), std::ptr::null_mut()) }
            .to_bits();
    ...
    assert!(
        fails.is_empty(),
        "float edges diverged from glibc:\n{}",
        fails.join("\n")
    );
}
