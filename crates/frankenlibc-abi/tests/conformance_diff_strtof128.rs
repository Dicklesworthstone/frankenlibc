...
fn check(s: &str, mism: &mut Vec<String>) {
    ...
    let f = frank(&cs);
    if g != f {
        mism.push(format!("{s:?}: glibc=(bits={:#034x},off={},e={}) fl=(bits={:#034x},off={},e={})", g.0, g.1, g.2, f.0, f.1, f.2));
    }
}
...
fn strtof128_matches_glibc() {
    ...
    let curated = [
        "1", "1.0", "0.5", "0.1", "3.14159", "2.5", "-1.5", "+2.25", "  12.5", "\t-0.0",
        "1e10", "1e-10", "1.23456789012345678901234567890123456e20", "9".repeat(40).leak(),
        "123456789.987654321e-5", "0", "-0", "0.0", "00.00", ".5", "5.", "+.25",
        // exponent / trailing edges
        "1e", "1e+", "1.5e3xyz", "1.5.6", "12abc", "1e1000", "abc", "", "  ", "+", "-", ".",
        // overflow / underflow
        "1e5000", "-1e5000", "1e4932", "1e4933", "1e-4966", "1e-5000", "2e-4970",
        // specials
        "inf", "INF", "-inf", "+infinity", "Infinity", "nan", "NAN", "-nan", "nan(123)", "naX",
        "infi", "infinityX",
        // hex
        "0x1p0", "0x1.8p1", "-0x1.0p-3", "0X1.FFFFFFFFFFFFFFFFFFFFFFFFFFFFp+4", "0x", "0x.8p0",
        "0x1p", "0x1.8", "0xg", "0x10p-4",
    ];
    for s in curated {
        check(s, &mut mism);
    }
    ...
    assert!(mism.is_empty(), "strtof128 diverged ({}):\n{}", mism.len(), mism.join("\n"));
}

fn wide(s: &str) -> Vec<i32> {
    s.bytes().map(|b| b as i32).chain(std::iter::once(0)).collect()
}
...
fn wcstof128_matches_glibc() {
    let mut mism = Vec::new();
    for s in [
        "1", "0.5", "0.1", "-3.14159e2", "  +12.5xyz", "1e-4949", "1e5000", "inf", "-nan(7)",
        "0x1.8p3", "abc", "", "42.0", "123456789012345678901234567890e-15",
    ] {
        let w = wide(s);
        let g = gw(&w);
        let f = fw(&w);
        if g != f {
            mism.push(format!("{s:?}: glibc=(b={:#034x},off={},e={}) fl=(b={:#034x},off={},e={})", g.0, g.1, g.2, f.0, f.1, f.2));
        }
    }
    assert!(mism.is_empty(), "wcstof128 diverged ({}):\n{}", mism.len(), mism.join("\n"));
}
