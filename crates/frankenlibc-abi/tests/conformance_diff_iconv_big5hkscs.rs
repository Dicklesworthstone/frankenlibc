...
fn big5hkscs_encode_roundtrip_matches_glibc() {
    ...
    for s in ["", "Hello, world", "中文字測試", "\u{23ED7}\u{20547}", "\u{00CA}\u{0304}\u{00EA}\u{030C}", "ABC漢字123"] {
        let u = s.as_bytes();
    ...
    assert!(mism.is_empty(), "BIG5-HKSCS encode/roundtrip diverged ({}): {}", mism.len(), mism.join(" "));
}
