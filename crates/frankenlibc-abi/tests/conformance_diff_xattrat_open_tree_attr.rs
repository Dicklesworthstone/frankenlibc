...
fn host_listxattrat(
    ...
    set_host_errno(0);
    let rc = unsafe { host_syscall()(SYS_LISTXATTRAT, dirfd, path, at_flags, list, size) };
    (rc as c_int, host_errno())
}
...
fn host_removexattrat(
    ...
    set_host_errno(0);
    let rc = unsafe { host_syscall()(SYS_REMOVEXATTRAT, dirfd, path, at_flags, name) };
    (rc as c_int, host_errno())
}
...
fn host_open_tree_attr(
    ...
    set_host_errno(0);
    let rc = unsafe { host_syscall()(SYS_OPEN_TREE_ATTR, dirfd, path, flags, attr, size) };
    (rc as c_int, host_errno())
}
