...
fn bessel_jn_yn_parity_vs_glibc() {
    ...
    for n in [-3i32, -2, -1, 0, 1, 2, 3, 5, 10, -5] {
        for &x in SP {
            ...
            if !approx(f, g, 16) {
                div.push(format!(
                    "jn({n},{x:e}): fl={f:?}({:#018x}) glibc={g:?}({:#018x})",
                    f.to_bits(),
                    g.to_bits()
                ));
            }
            let fy = fl::yn(n, x);
            let gy = unsafe { yn(n, x) };
            if !approx(fy, gy, 16) {
                div.push(format!(
                    "yn({n},{x:e}): fl={fy:?}({:#018x}) glibc={gy:?}({:#018x})",
                    fy.to_bits(),
                    gy.to_bits()
                ));
            }
            let xf = x as f32;
            let ff = fl::jnf(n, xf);
            let gf = unsafe { jnf(n, xf) };
            if !approxf(ff, gf, 16) {
                div.push(format!(
                    "jnf({n},{xf:e}): fl={ff:?}({:#010x}) glibc={gf:?}({:#010x})",
                    ff.to_bits(),
                    gf.to_bits()
                ));
            }
            let fyf = fl::ynf(n, xf);
            let gyf = unsafe { ynf(n, xf) };
            if !approxf(fyf, gyf, 16) {
                div.push(format!(
                    "ynf({n},{xf:e}): fl={fyf:?}({:#010x}) glibc={gyf:?}({:#010x})",
                    fyf.to_bits(),
                    gyf.to_bits()
                ));
            }
    ...
    assert!(
        div.is_empty(),
        "Bessel jn/yn divergences vs glibc:\n  {}",
        div.join("\n  ")
    );
}
