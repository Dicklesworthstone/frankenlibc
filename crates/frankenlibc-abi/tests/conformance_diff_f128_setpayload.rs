...
fn f128_setpayload_match_glibc() {
    ...
    for &pl in &payloads() {
        for (name, gf, ff) in [
            (
                "setpayload",
                setpayloadf128 as unsafe extern "C" fn(*mut f128, f128) -> c_int,
                ma::setpayloadf128 as unsafe extern "C" fn(*mut f128, f128) -> c_int,
            ),
            ...
            if grc != frc || gr.to_bits() != fr.to_bits() {
                mism.push(format!(
                    "{name} pl={:#034x}: glibc=(rc={grc},res={:#034x}) fl=(rc={frc},res={:#034x})",
                    pl.to_bits(),
                    gr.to_bits(),
                    fr.to_bits()
                ));
