...
fn fl_u8(a: &[u8], b: &[u8]) -> i32 { (a[0] as i32) - (b[0] as i32) }
fn fl_i8(a: &[u8], b: &[u8]) -> i32 { (a[0] as i8 as i32) - (b[0] as i8 as i32) }
fn fl_u8_desc(a: &[u8], b: &[u8]) -> i32 { (b[0] as i32) - (a[0] as i32) }
...
fn check(label: &str, bytes_in: &[u8], fl_cmp: fn(&[u8], &[u8]) -> i32, gl_cmp: GlCmp) -> Vec<u8> {
    ...
    unsafe { libc::qsort(gl_buf.as_mut_ptr() as *mut c_void, n, 1, Some(gl_cmp)); }
...
fn qsort_count8_lane_matches_glibc() {
    ...
    for &n in &sizes {
        ...
        let dists: Vec<(&str, Box<dyn Fn(usize) -> u8>)> = vec![
            ...
            ("signedish", Box::new(move |i| (mix(seed ^ 0x13, i) as u8) | 0x80)),
            ("twoval", Box::new(move |i| if mix(seed ^ 0x55, i) & 1 == 0 { 0x00 } else { 0xFF })),
        ];
        for (tag, g) in &dists {
