...
fn run_glibc(input: &str, fmt: &str) -> (isize, i64) {
    ...
    let consumed = if end.is_null() {
        ...
        (end as usize - ci.as_ptr() as usize) as isize
    };
...
fn run_fl(input: &str, fmt: &str) -> (isize, i64) {
    ...
    let cf = CString::new(fmt).unwrap();
    let mut tm: libc::tm = unsafe { std::mem::zeroed() };
    let end = unsafe { frankenlibc_abi::time_abi::strptime(ci.as_ptr(), cf.as_ptr(), &mut tm) };
    let consumed = if end.is_null() {
        -1
    } else {
        (end as usize - ci.as_ptr() as usize) as isize
...
fn strptime_tz_directives_match_glibc() {
    ...
    for &(input, fmt) in cases {
        ...
        assert_eq!(
            f, g,
            "strptime({input:?}, {fmt:?}): fl=(consumed {}, gmtoff {}) glibc=(consumed {}, gmtoff {})",
            f.0, f.1, g.0, g.1
        );
    }
}
