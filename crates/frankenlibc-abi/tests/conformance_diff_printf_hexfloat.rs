...
fn printf_hexfloat_a_matches_glibc() {
    let vals: &[f64] = &[
        0.0, -0.0, 1.0, -1.0, 2.0, 0.5, 0.25, 1.5, 3.0, 0.1, -0.1,
        f64::INFINITY, f64::NEG_INFINITY, f64::NAN, -f64::NAN,
        f64::MIN_POSITIVE, -f64::MIN_POSITIVE, f64::MAX, f64::MIN,
        5e-324, -5e-324, 4.9e-324, 1e-323, 2.2250738585072014e-308, // subnormals + boundary
        1.0000000000000002, 0.9999999999999999, 123.875, -123.875,
        3.141592653589793, 2.718281828459045, 1e300, 1e-300, 65504.0,
        // values exercising round-to-even at various precisions
        1.0009765625, 1.00048828125, 1.000244140625, 255.5, 0.000030517578125,
    ];
    ...
    let fmts = [
        "%a", "%A", "%.0a", "%.1a", "%.2a", "%.3a", "%.5a", "%.13a", "%.20a",
        "%20a", "%-20a|", "%020a", "%+a", "% a", "%#a", "%#.0a", "%+.2a",
        "%30.10a", "%-30.4A|", "%.0A", "%#.0A",
    ];
    let mut div = Vec::new();
    for fmt in fmts {
    ...
    assert!(div.is_empty(), "{} printf %a/%A divergences vs glibc", div.len());
}
