...
fn set_arr(d: &mut [c_char], s: &str) { for (x, b) in d.iter_mut().zip(s.bytes()) { *x = b as c_char; } }
fn rec(typ: i16, pid: i32, line: &str, id: &str, user: &str) -> Utmp {
    let mut u: Utmp = unsafe { std::mem::zeroed() };
    u.ut_type = typ; u.ut_pid = pid;
    set_arr(&mut u.ut_line, line); set_arr(&mut u.ut_id, id); set_arr(&mut u.ut_user, user);
    u
}
...
fn write_utmp() -> (std::path::PathBuf, CString) {
    ...
    let recs = [rec(USER_PROCESS, 1234, "tty1", "1", "alice"), rec(USER_PROCESS, 5678, "tty2", "2", "bob"), rec(DEAD_PROCESS, 0, "tty1", "1", "")];
    let mut bytes = Vec::new();
    for r in &recs { bytes.extend_from_slice(unsafe { std::slice::from_raw_parts(r as *const Utmp as *const u8, 384) }); }
    std::fs::write(&p, &bytes).unwrap();
    (p.clone(), CString::new(p.to_string_lossy().as_bytes()).unwrap())
}

type Probe = (i32, Vec<(c_int, (i16, i32, String, String))>, (c_int, Option<(i16, i32, String, String)>), (c_int, Option<(i16, i32, String, String)>));
...
fn glibc_probe(path: &CString) -> Probe {
    unsafe {
        ...
        let line = (lrc, if res.is_null() { None } else { Some(fields(&*res)) });
        g::setutent();
        ...
        let id = (irc, if res2.is_null() { None } else { Some(fields(&*res2)) });
        g::endutent();
        (un, all, line, id)
    }
}
fn fl_probe(path: &CString) -> Probe {
    unsafe {
        ...
        let mut buf: Utmp = std::mem::zeroed();
        let mut res: *mut c_void = std::ptr::null_mut();
        let lrc = fl::getutline_r(&q as *const _ as *const c_void, &mut buf as *mut _ as *mut c_void, &mut res);
        let line = (lrc, if res.is_null() { None } else { Some(fields(&*(res as *const Utmp))) });
        fl::setutent();
        let qid = rec(USER_PROCESS, 0, "", "1", "");
        let mut buf2: Utmp = std::mem::zeroed();
        let mut res2: *mut c_void = std::ptr::null_mut();
        let irc = fl::getutid_r(&qid as *const _ as *const c_void, &mut buf2 as *mut _ as *mut c_void, &mut res2);
        let id = (irc, if res2.is_null() { None } else { Some(fields(&*(res2 as *const Utmp))) });
        fl::endutent();
        (un, all, line, id)
    }
}
...
fn reentrant_utmp_accessors_match_glibc() {
    ...
    assert_eq!(fp.1, gp.1, "getutent_r enumeration: fl={:?} glibc={:?}", fp.1, gp.1);
