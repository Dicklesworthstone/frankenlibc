...
fn bases() -> Vec<f128> {
    let mut v: Vec<f128> = vec![
        0.0, -0.0f128, 1.0, -1.0, -0.5, 0.5, 2.0, -0.9f128, -0.99f128, 3.0,
        0.1, -0.1, 10.0, 100.0, 1e10f128, 1e-10f128, -2.0,
...
fn f128_compoundn_matches_glibc() {
    let exps: Vec<c_longlong> = vec![
        0, 1, -1, 2, -2, 3, -3, 5, -5, 10, -10, 50, -50, 100, -100, 1000, -1000,
        100000, -100000,
    ];
    ...
    for &x in &bases() {
        for &n in &exps {
            ...
            let fe = unsafe { *el() };
            if g != f || ge != fe {
                mism.push(format!("compoundn({:#034x},{n}): glibc=({g:#034x},e={ge}) fl=({f:#034x},e={fe})", x.to_bits()));
            }
        }
    }
    assert!(mism.is_empty(), "compoundnf128 diverged ({}):\n{}", mism.len(), mism.iter().take(30).cloned().collect::<Vec<_>>().join("\n"));
}
