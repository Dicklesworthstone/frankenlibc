...
mod g {
    ...
    unsafe extern "C" {
        ...
        pub fn strfmon_l(s: *mut c_char, max: usize, loc: *mut c_void, fmt: *const c_char, ...) -> isize;
    }
}
...
fn run_fl(fmt: &str, v: f64) -> (isize, Vec<u8>) {
    ...
    unsafe {
        ...
        let n = flu::strfmon_l(buf.as_mut_ptr() as *mut c_char, 128, loc as *mut c_void, cf.as_ptr(), v);
        fll::freelocale(loc);
        let bytes = if n >= 0 { buf[..n as usize].to_vec() } else { Vec::new() };
        (n, bytes)
    }
...
fn strfmon_l_c_locale_matches_glibc() {
    ...
    for &(fmt, v) in cases {
        ...
        assert_eq!(
            ...
            f.0, String::from_utf8_lossy(&f.1), g.0, String::from_utf8_lossy(&g.1),
        );
    }
}
