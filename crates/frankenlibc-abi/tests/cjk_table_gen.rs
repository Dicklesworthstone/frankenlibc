...
fn cjk_table_gen() {
    ...
    for codec in ["SHIFT_JIS", "BIG5", "GBK", "EUC-KR", "CP949", "GB2312", "JOHAB"] {
    ...
    //     formula (not tabled). ---
    {
        ...
        s.push_str("];\n");
        s.push_str(&format!("pub(crate) static GB18030_DBCS2: [(u16, u32); {}] = [\n", dbcs2.len()));
        for chunk in dbcs2.chunks(8) {
            s.push_str("    ");
            for (k, cp) in chunk { s.push_str(&format!("({k:#x},{cp:#x}),")); }
            s.push('\n');
        ...
        s.push_str("];\n");
        s.push_str(&format!("pub(crate) static GB18030_DEC4: [(u32, u32, u32); {}] = [\n", dec4.len()));
        for chunk in dec4.chunks(6) {
            s.push_str("    ");
            for (l, cp, n) in chunk { s.push_str(&format!("({l:#x},{cp:#x},{n}),")); }
            s.push('\n');
        }
        s.push_str("];\n");
        s.push_str(&format!("pub(crate) static GB18030_ENC2: [(u32, u16); {}] = [\n", enc2.len()));
        for chunk in enc2.chunks(8) {
            s.push_str("    ");
            for (cp, k) in chunk { s.push_str(&format!("({cp:#x},{k:#x}),")); }
            s.push('\n');
        }
        s.push_str("];\n");
        s.push_str(&format!("pub(crate) static GB18030_ENC4: [(u32, u32, u32); {}] = [\n", enc4.len()));
        for chunk in enc4.chunks(6) {
            s.push_str("    ");
            for (cp, l, n) in chunk { s.push_str(&format!("({cp:#x},{l:#x},{n}),")); }
            s.push('\n');
        }
        s.push_str("];\n");
        eprint!("{s}");
