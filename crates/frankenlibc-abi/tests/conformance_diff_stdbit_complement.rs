...
fn stdbit_complement_ui_match_glibc() {
    ...
    for &v in CASES {
        assert_eq!(s::stdc_count_zeros_ui(v), unsafe { stdc_count_zeros_ui(v) }, "count_zeros({v:#x})");
        assert_eq!(s::stdc_leading_ones_ui(v), unsafe { stdc_leading_ones_ui(v) }, "leading_ones({v:#x})");
        assert_eq!(s::stdc_trailing_ones_ui(v), unsafe { stdc_trailing_ones_ui(v) }, "trailing_ones({v:#x})");
        assert_eq!(s::stdc_first_leading_zero_ui(v), unsafe { stdc_first_leading_zero_ui(v) }, "first_leading_zero({v:#x})");
        assert_eq!(s::stdc_first_trailing_one_ui(v), unsafe { stdc_first_trailing_one_ui(v) }, "first_trailing_one({v:#x})");
        assert_eq!(s::stdc_first_trailing_zero_ui(v), unsafe { stdc_first_trailing_zero_ui(v) }, "first_trailing_zero({v:#x})");
    }
}
