...
// ===========================================================================
// bd-xh08pf — the only two assertions from c11threads_abi.rs's dead inline
// `#[cfg(test)]` block that were NOT already covered here.
//
// That block sat inside a `#[cfg(not(test))]` module, so it could never compile
// and had never run. Reviewing its five tests against this file:
//   thrd_current non-zero / thrd_equal same-thread -> test_thrd_current_equal
//   mtx init+lock+unlock success codes             -> test_mtx_init_lock_unlock_destroy
//   EBUSY -> thrd_busy                             -> test_mtx_trylock
// are all already covered, so they are NOT duplicated. The two below are not:
// an INVALID mutex kind, and the ETIMEDOUT mapping (test_mtx_timedlock_succeeds_
// when_available only exercises the success path).
//
// The old inline versions reached module-private items (`pthread_rc_to_thrd`,
// the THRD_*/MTX_* constants); these assert the same contract through public
// entry points instead, so no ABI surface had to be widened. Expected values are
// glibc's <threads.h>: thrd_error = 2, thrd_timedout = 4.
// ===========================================================================
...
#[test]
fn test_mtx_init_rejects_invalid_kind() {
    unsafe {
        let mut mtx: libc::pthread_mutex_t = std::mem::zeroed();
        let rc = mtx_init(&mut mtx, 0x4);
        assert_eq!(rc, THRD_ERROR, "unknown mutex kind must be thrd_error=2");
    }
}

#[test]
fn test_mtx_timedlock_times_out_when_held_by_another_thread() {
    // A plain mutex re-locked by its own owner is undefined rather than
    // guaranteed to time out, so the lock is held by a second thread.
    let gate = std::sync::Arc::new(std::sync::Mutex::new(()));
    let mut mtx: libc::pthread_mutex_t = unsafe { std::mem::zeroed() };
    unsafe {
        assert_eq!(mtx_init(&mut mtx, MTX_TIMED), THRD_SUCCESS);
    }

    let addr = &mut mtx as *mut libc::pthread_mutex_t as usize;
    let holder_gate = std::sync::Arc::clone(&gate);
    let held = gate.lock().unwrap();
    let holder = std::thread::spawn(move || {
        let p = addr as *mut libc::pthread_mutex_t;
        unsafe {
            assert_eq!(mtx_lock(p), THRD_SUCCESS);
        }
        let _wait = holder_gate.lock().unwrap();
        unsafe {
            assert_eq!(mtx_unlock(p), THRD_SUCCESS);
        }
    });
    ...
    let rc = unsafe { mtx_timedlock(&mut mtx, &past) };
    assert_eq!(
        rc, THRD_TIMEDOUT,
        "timedlock past its deadline must return thrd_timedout=4"
    );
