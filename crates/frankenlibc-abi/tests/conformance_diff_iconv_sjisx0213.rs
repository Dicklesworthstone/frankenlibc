//! Differential gate: SHIFT_JISX0213 (Shift_JIS-2004) iconv codec vs glibc.
//!
...
const NAME: &str = "SHIFT_JISX0213";

#[test]
fn sjisx0213_decode_matches_glibc() {
    let g = glibc();
    ...
    let alpha: &[u8] = &[0x41, 0x5c, 0x7e, 0xa1, 0xdf, 0x81, 0x82, 0x9f, 0xe0, 0xf0, 0xfc, 0x40, 0x00];
    for _ in 0..15_000 {
        let len = 1 + next() % 8;
    ...
    assert!(mism.is_empty(), "SHIFT_JISX0213 decode diverged ({}): {}", mism.len(), mism.join(" "));
}

#[test]
fn sjisx0213_encode_roundtrip_matches_glibc() {
    let g = glibc();
    let mut mism = Vec::new();
    ...
    for b in 0u16..256 {
        ...
        if let Some(u) = g_full(&g, "UTF-8", NAME, &inp) {
            if !u.is_empty() && g_full(&g, NAME, "UTF-8", &u) != f_full(NAME, "UTF-8", &u) && mism.len() < 30 {
    ...
    // curated samples (ASCII/yen, kana, kanji, astral, combining kana).
    for s in ["", "Hello \u{00A5}", "日本語テスト", "\u{20089}\u{2A6B2}", "\u{304B}\u{309A}き\u{309A}", "ｱｲｳ漢字ABC"] {
        let u = s.as_bytes();
    ...
    let fa = f_full("UTF-8", "SHIFTJISX0213", &[0x82u8, 0xa0]);
    let gc = g_full(&g, "UTF-8", NAME, &[0x82u8, 0xa0]);
    assert_eq!(fa, gc, "alias SHIFTJISX0213 differs");
    assert!(mism.is_empty(), "SHIFT_JISX0213 encode/roundtrip diverged ({}): {}", mism.len(), mism.join(" "));
}
