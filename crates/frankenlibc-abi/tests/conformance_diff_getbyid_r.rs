...
mod g {
    use super::*;
    unsafe extern "C" {
        pub fn getpwuid_r(uid: u32, pw: *mut libc::passwd, buf: *mut c_char, n: usize, res: *mut *mut libc::passwd) -> c_int;
        pub fn getgrgid_r(gid: u32, gr: *mut libc::group, buf: *mut c_char, n: usize, res: *mut *mut libc::group) -> c_int;
        pub fn getgrnam_r(name: *const c_char, gr: *mut libc::group, buf: *mut c_char, n: usize, res: *mut *mut libc::group) -> c_int;
    }
}
...
fn getpwuid_r_uid0_matches_glibc() {
    let g = unsafe {
        ...
        let rc = g::getpwuid_r(0, &mut pw, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut res);
        (rc, res.is_null(), cstr(pw.pw_name), pw.pw_uid, pw.pw_gid, cstr(pw.pw_dir))
    };
    let f = unsafe {
        let mut pw: libc::passwd = std::mem::zeroed();
        let mut buf = [0u8; 2048];
        let mut res: *mut libc::passwd = std::ptr::null_mut();
        let rc = flp::getpwuid_r(0, &mut pw, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut res);
        (rc, res.is_null(), cstr(pw.pw_name), pw.pw_uid, pw.pw_gid, cstr(pw.pw_dir))
    };
...
fn getgrgid_r_gid0_matches_glibc() {
    let g = unsafe {
        ...
        let rc = g::getgrgid_r(0, &mut gr, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut res);
    ...
    let f = unsafe {
        ...
        let mut buf = [0u8; 4096];
        let mut res: *mut libc::group = std::ptr::null_mut();
        let rc = flg::getgrgid_r(0, &mut gr, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut res);
        (rc, res.is_null(), cstr(gr.gr_name), gr.gr_gid, members(&gr))
    };
    ...
    if let (false, name) = (g.1, &g.2) {
        ...
        let gn = unsafe {
            ...
            let mut buf = [0u8; 4096];
            let mut res: *mut libc::group = std::ptr::null_mut();
            let rc = g::getgrnam_r(cn.as_ptr(), &mut gr, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut res);
        ...
        let fn_ = unsafe {
            ...
            let mut buf = [0u8; 4096];
            let mut res: *mut libc::group = std::ptr::null_mut();
            let rc = flg::getgrnam_r(cn.as_ptr(), &mut gr, buf.as_mut_ptr() as *mut c_char, buf.len(), &mut res);
            (rc, cstr(gr.gr_name), gr.gr_gid)
        };
...
fn getpwuid_r_small_buffer_matches_glibc() {
    ...
    assert_eq!(g.0, libc::ERANGE, "glibc returns ERANGE on too-small buffer");
}
