...
unsafe extern "C" {
    ...
    fn stpncpy(d: *mut c_char, s: *const c_char, n: usize) -> *mut c_char;
}
#[test]
fn strncpy_stpncpy_match_glibc() {
    let mut seed: u64 = 0x33;
    let mut rng = || {
        seed ^= seed << 13;
        seed ^= seed >> 7;
        seed ^= seed << 17;
    ...
    for _ in 0..200000 {
        ...
        let mut d1 = vec![0xAAu8; cap];
        let mut d2 = vec![0xAAu8; cap];
        let fr = unsafe { fa::strncpy(d1.as_mut_ptr() as *mut c_char, sp, n) };
        let gr = unsafe { strncpy(d2.as_mut_ptr() as *mut c_char, sp, n) };
        if (fr as usize - d1.as_ptr() as usize) != (gr as usize - d2.as_ptr() as usize)
            || d1[..n] != d2[..n]
        {
            div += 1;
            if div <= 5 {
                eprintln!("DIV strncpy slen={slen} n={n}");
            }
        }
        // stpncpy
        let mut e1 = vec![0xAAu8; cap];
        let mut e2 = vec![0xAAu8; cap];
        let fe = unsafe { fa::stpncpy(e1.as_mut_ptr() as *mut c_char, sp, n) };
        let ge = unsafe { stpncpy(e2.as_mut_ptr() as *mut c_char, sp, n) };
        let (feo, geo) = (
            fe as usize - e1.as_ptr() as usize,
            ge as usize - e2.as_ptr() as usize,
        );
        if feo != geo || e1[..n] != e2[..n] {
            div += 1;
            if div <= 5 {
                eprintln!("DIV stpncpy slen={slen} n={n} feo={feo} geo={geo}");
            }
        }
    ...
    assert_eq!(
        hex, "08265033efc3f82da6ff9e99c4a57e6518d4f8757bb0b6cf7b02462ce35e37b9",
