...
/// Host `fabsf128` via `dlsym`; fl's own definition is handed to the oracle so
...
/// will not coerce.
unsafe extern "C" fn fabsf128(x: f128) -> f128 {
    // SAFETY: prototype matches the C declaration this replaces.
    let f: unsafe extern "C" fn(f128) -> f128 =
        unsafe { dlsym_oracle::host_fn(c"fabsf128", ma::fabsf128 as *const ()) };
    unsafe { f(x) }
}

/// Host `copysignf128` via `dlsym`; fl's own definition is handed to the oracle so
/// it refuses to resolve back to fl and compare it against itself.
///
/// Declared `extern "C"` because these gates store the arms in tables typed
/// `unsafe extern "C" fn`; a plain Rust `unsafe fn` has a different type and
/// will not coerce.
unsafe extern "C" fn copysignf128(x: f128, y: f128) -> f128 {
    // SAFETY: prototype matches the C declaration this replaces.
    let f: unsafe extern "C" fn(f128, f128) -> f128 =
        unsafe { dlsym_oracle::host_fn(c"copysignf128", ma::copysignf128 as *const ()) };
    unsafe { f(x, y) }
}
...
fn f128_exact_ops_match_glibc() {
    ...
    for &x in &vals {
        ...
        ck!("isnan", unsafe { __isnanf128(x) }, unsafe {
            gi::__isnanf128(x)
        });
        ck!("isinf", unsafe { __isinff128(x) }, unsafe {
            gi::__isinff128(x)
        });
        ck!("signbit", unsafe { __signbitf128(x) }, unsafe {
            gi::__signbitf128(x)
        });
        ck!("finite", unsafe { __finitef128(x) }, unsafe {
            ma::__finitef128(x)
        });
        ck!("fpclassify", unsafe { __fpclassifyf128(x) }, unsafe {
            gi::__fpclassifyf128(x)
        });
        ck!("issignaling", unsafe { __issignalingf128(x) }, unsafe {
            ma::__issignalingf128(x)
        });
        let gf = unsafe { fabsf128(x) }.to_bits();
        let ff = unsafe { ma::fabsf128(x) }.to_bits();
        if gf != ff {
            mism.push(format!(
                "fabs bits={:#034x}: glibc={gf:#034x} fl={ff:#034x}",
                x.to_bits()
            ));
        }
        for &y in &vals {
            ...
            if gc != fc {
                mism.push(format!(
                    "copysign x={:#034x} y={:#034x}: glibc={gc:#034x} fl={fc:#034x}",
                    x.to_bits(),
                    y.to_bits()
                ));
            }
