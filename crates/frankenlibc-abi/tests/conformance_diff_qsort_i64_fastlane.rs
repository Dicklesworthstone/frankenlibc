...
fn qsort_i64_fast_lane_matches_glibc() {
    ...
    let sizes = [1usize, 2, 7, 31, 32, 63, 64, 65, 128, 511, 512, 1024, 2047, 2048, 2049, 4096];
    ...
    for &n in &sizes {
        ...
        let mixed: Vec<i64> = (0..n)
            .map(|i| if i % 2 == 0 { lcg(&mut seed) as i64 } else { -(lcg(&mut seed) as i64) })
            .collect();
        ...
        for (tag, data) in [("rand", &rand), ("dups", &dups), ("mixed", &mixed)] {
            ...
            check(&format!("desc/{tag}"), data, fl_cmp_i64_desc, gl_cmp_i64_desc);
