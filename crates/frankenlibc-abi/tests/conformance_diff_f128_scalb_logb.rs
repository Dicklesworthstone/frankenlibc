...
fn values() -> Vec<f128> {
    let mut v: Vec<f128> = vec![
        0.0, -0.0f128, 1.0, -1.0, 2.0, -2.0, 3.0, 0.5, -0.5, 123.456f128, 1e300f128, -1e-300f128,
...
fn f128_scalb_logb_match_glibc() {
    ...
    for &x in &vals {
        for &n in ns {
            for (name, gf, ff) in [
                ("scalbn", scalbnf128 as unsafe extern "C" fn(f128, c_int) -> f128, ma::scalbnf128 as unsafe extern "C" fn(f128, c_int) -> f128),
                ...
                let fe = unsafe { *el() };
                if g != f || ge != fe {
                    mism.push(format!("{name} x={:#034x} n={n}: glibc=({g:#034x},e={ge}) fl=({f:#034x},e={fe})", x.to_bits()));
                }
            }
        }
        // scalbln (c_long n)
        for &n in &[0i64, -16494, 100000, -100000] {
            ...
            let fe = unsafe { *el() };
            if g != f || ge != fe {
                mism.push(format!("scalbln x={:#034x} n={n}: glibc=({g:#034x},e={ge}) fl=({f:#034x},e={fe})", x.to_bits()));
            }
        }
    }
    ...
    for &x in &vals {
        ...
        let fe = unsafe { *el() };
        if g != f || ge != fe {
            mism.push(format!("logb x={:#034x}: glibc=({g:#034x},e={ge}) fl=({f:#034x},e={fe})", x.to_bits()));
        }
    }
    ...
    for &x in &vals {
        ...
        let fe = unsafe { *el() };
        if g != f || ge != fe {
            mism.push(format!("ilogb x={:#034x}: glibc=({g},e={ge}) fl=({f},e={fe})", x.to_bits()));
        }
        unsafe { *el() = 0 };
        ...
        let fe = unsafe { *el() };
        if g != f || ge != fe {
            mism.push(format!("llogb x={:#034x}: glibc=({g},e={ge}) fl=({f},e={fe})", x.to_bits()));
        }
    }

    assert!(mism.is_empty(), "f128 scalb/logb diverged ({}):\n{}", mism.len(), mism.iter().take(30).cloned().collect::<Vec<_>>().join("\n"));
}
