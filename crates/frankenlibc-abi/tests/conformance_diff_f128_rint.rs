...
fn values() -> Vec<f128> {
    let mut v: Vec<f128> = vec![
        0.0, -0.0f128, 0.5, -0.5, 1.5, 2.5, -2.5, 3.5, 1.0, -1.0, 2.4, 2.6, -2.4, -2.6, 1e30f128,
        -1e30f128, 123.456f128, -7.5f128, 8.5f128,
    ...
    for _ in 0..20 {
        ...
        v.push(f128::from_bits(((hi as u128 >> 9 & 1) << 127) | (ef << 112) | mant));
    }
    v
}
...
fn f128_rint_lround_iseqsig_match_glibc() {
    ...
    for &mode in &[0, 0x400, 0x800, 0xc00] {
        ...
        for &x in &vals {
            let (g, f) = (unsafe { rintf128(x) }.to_bits(), unsafe { ma::rintf128(x) }.to_bits());
            if g != f {
                mism.push(format!("rint[{mode:#x}] x={:#034x}: glibc={g:#034x} fl={f:#034x}", x.to_bits()));
            }
            let (g, f) = (unsafe { nearbyintf128(x) }.to_bits(), unsafe { ma::nearbyintf128(x) }.to_bits());
            if g != f {
                mism.push(format!("nearbyint[{mode:#x}] x={:#034x}: glibc={g:#034x} fl={f:#034x}", x.to_bits()));
            }
            let (g, f) = (unsafe { lrintf128(x) }, unsafe { ma::lrintf128(x) });
            if g != f {
                mism.push(format!("lrint[{mode:#x}] x={:#034x}: glibc={g} fl={f}", x.to_bits()));
            }
            let (g, f) = (unsafe { llrintf128(x) }, unsafe { ma::llrintf128(x) });
            if g != f {
                mism.push(format!("llrint[{mode:#x}] x={:#034x}: glibc={g} fl={f}", x.to_bits()));
            }
        }
    }
    ...
    for &x in &vals {
        for &y in &vals {
            ...
            let fe = unsafe { *el() };
            if g != f || ge != fe {
                mism.push(format!("iseqsig x={:#034x} y={:#034x}: glibc=({g},e={ge}) fl=({f},e={fe})", x.to_bits(), y.to_bits()));
            }
        }
    }

    assert!(mism.is_empty(), "f128 rint/lround/iseqsig diverged ({}):\n{}", mism.len(), mism.iter().take(30).cloned().collect::<Vec<_>>().join("\n"));
}
