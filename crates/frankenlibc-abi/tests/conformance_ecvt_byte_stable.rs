...
fn ecvt_fcvt_golden_byte_stable() {
    ...
    let mut run = |v: f64, nd: i32| {
        ...
        let se = if pe.is_null() {
            b"<n>".to_vec()
        } else {
            unsafe { CStr::from_ptr(pe) }.to_bytes().to_vec()
        };
        h.update(&se);
        h.update((*dp).to_le_bytes());
        h.update([*sg as u8]);
        let pf = unsafe { fs::fcvt(v, nd, dp.as_mut() as *mut c_int, sg.as_mut() as *mut c_int) };
        let sf = if pf.is_null() {
            b"<n>".to_vec()
        } else {
            unsafe { CStr::from_ptr(pf) }.to_bytes().to_vec()
        };
        h.update(&sf);
        h.update((*dp).to_le_bytes());
        h.update([*sg as u8]);
    ...
    for _ in 0..80000 {
        let v = f64::from_bits(rng());
        if !v.is_finite() {
            continue;
        }
        let nd = (rng() % 20) as i32;
        run(v, nd);
        n += 1;
    }
    ...
    assert_eq!(
        hex, "f060b908a01660087aa15efd2ff4fa8b120c229c73bee9d81256a05c28b3ea3c",
        "ecvt/fcvt output changed"
    );
}
