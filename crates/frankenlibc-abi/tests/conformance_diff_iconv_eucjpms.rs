...
fn gdec(gg: &G, name: &str, b: &[u8]) -> (u8, u32) {
    ...
    if r != INVALID && 16 - ol == 4 && il == 0 {
        (1, u32::from_le_bytes([o[0], o[1], o[2], o[3]]))
    } else {
        (0, il as u32)
    }
}
...
fn genc(gg: &G, cp: u32) -> Option<Vec<u8>> {
    ...
    let cd = (gg.open)(c"EUC-JP-MS".as_ptr(), c"UTF-8".as_ptr());
...
fn fenc(cp: u32) -> Option<Vec<u8>> {
    let c = char::from_u32(cp)?;
    let cd = unsafe { fl::iconv_open(c"EUC-JP-MS".as_ptr(), c"UTF-8".as_ptr()) };
...
fn eucjpms_decode_matches_glibc() {
    ...
    for b in 0u16..256 {
        let i = [b as u8];
        assert_eq!(fdec(n, &i), gdec(&gg, n, &i), "1b {b:#04x}");
    }
    for a in 0x80u16..=0xFF {
        for b in 0u16..256 {
            let i = [a as u8, b as u8];
            assert_eq!(fdec(n, &i), gdec(&gg, n, &i), "2b {a:#04x},{b:#04x}");
        }
    }
    for b in 0xA0u16..=0xFF {
        for c in 0xA0u16..=0xFF {
            let i = [0x8F, b as u8, c as u8];
            assert_eq!(fdec(n, &i), gdec(&gg, n, &i), "3b 8F,{b:#04x},{c:#04x}");
        }
    }
}
#[test]
fn eucjpms_encode_matches_glibc() {
    ...
    for cp in 0u32..=0xFFFF {
        ...
        assert_eq!(fenc(cp), genc(&gg, cp), "enc U+{cp:04X}");
    }
}
#[test]
