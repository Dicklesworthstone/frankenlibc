...
fn run(tokens: &[CString], opts: &CString, fl: bool) -> (Vec<Step>, Vec<String>) {
    ...
    unsafe {
        ...
        loop {
            let r = if fl {
                fl_getopt(argc, av.as_ptr(), opts.as_ptr())
            } else {
                getopt(argc, av.as_ptr(), opts.as_ptr())
            };
            if r == -1 {
                break;
            }
            let oa = if optarg.is_null() {
                None
            } else {
...
fn getopt_differential_fuzz_vs_glibc() {
    let pool = [
        "-a", "-b", "-c", "-x", "-ab", "-bc", "--", "-", "foo", "bar", "-afoo", "val", "-cval",
        "-d", "-a",
    ];
    let optstrings = [
        "abc", "a:bc", "ab:c", "a::bc", "abcd:", "+abc", "-abc", ":abc", "+a:b", "-a:b:", ":a::b",
        "abc:d::", "ab", "a:b:c:", "+:ab:", "-:a:b",
    ];
    ...
    for _ in 0..8000 {
        ...
        for _ in 0..ntok {
            tokens.push(CString::new(pool[(rng() as usize) % pool.len()]).unwrap());
        }
    ...
    assert!(
        total == 0,
        "getopt diverged in {total}/8000 cases:\n  {}",
        div.join("\n  ")
    );
}
