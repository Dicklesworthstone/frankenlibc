...
fn run_f64(eng: u8, w: &[i32]) -> (u64, isize, c_int) {
    ...
    let consumed = if end.is_null() {
        ...
        (end as isize - w.as_ptr() as isize) / 4
    };
...
fn wcstod_wcstof_parity_vs_glibc() {
    let mut inputs: Vec<String> = INPUTS.iter().map(|s| (*s).to_string()).collect();
    inputs.extend([
        format!("{}1.25tail", " ".repeat(600)),
        format!("2.71828{}tail", " ".repeat(4096)),
        format!("1.5e+{}tail", "2".repeat(600)),
        format!("0x.{}p4tail", "1".repeat(600)),
        format!("nan({})tail", "1234567890abcdef_".repeat(40)),
    ]);
    ...
    assert!(
        div.is_empty(),
        "wcstod/wcstof divergences vs glibc:\n  {}",
        div.join("\n  ")
    );
}
