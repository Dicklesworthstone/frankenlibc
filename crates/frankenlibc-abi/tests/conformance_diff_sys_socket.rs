...
fn diff_socket_create_close() {
    ...
    let cases: &[(&str, c_int, c_int, c_int)] = &[
        ...
        ("AF_INET TCP|CLOEXEC", libc::AF_INET, libc::SOCK_STREAM | libc::SOCK_CLOEXEC, 0),
        ("AF_INET TCP|NONBLOCK", libc::AF_INET, libc::SOCK_STREAM | libc::SOCK_NONBLOCK, 0),
    ];
    ...
    assert!(divs.is_empty(), "socket divergences:\n{}", render_divs(&divs));
}
...
fn make_loopback_addr(port: u16) -> libc::sockaddr_in {
    libc::sockaddr_in {
        ...
        sin_port: port.to_be(),
        sin_addr: libc::in_addr { s_addr: 0x7F000001u32.to_be() }, // 127.0.0.1
        sin_zero: [0; 8],
    }
}
...
fn diff_bind_listen_loopback() {
    ...
    for (label, bind_fn, listen_fn) in [
        ...
        let r_bind = unsafe {
            bind_fn(fd, &addr as *const libc::sockaddr_in as *const libc::sockaddr, addr_len)
        };
    ...
    assert!(divs.is_empty(), "bind/listen divergences:\n{}", render_divs(&divs));
}
...
fn diff_sockopt_roundtrip() {
    ...
    for (label, level, opt, val) in opts {
        ...
        if fd_fl < 0 || fd_lc < 0 {
            unsafe {
                if fd_fl >= 0 { libc::close(fd_fl); }
                if fd_lc >= 0 { libc::close(fd_lc); }
            }
            continue;
        }
        ...
        let _ = unsafe {
            fl::getsockopt(
                ...
                &mut len_fl as *mut u32 as *mut u32,
            )
        };
        let _ = unsafe {
            libc::getsockopt(
                ...
                &mut len_lc as *mut u32 as *mut u32,
            )
        };
    ...
    assert!(divs.is_empty(), "setsockopt round-trip divergences:\n{}", render_divs(&divs));
}
...
fn diff_getsockname_after_bind() {
    ...
    for (label, getsockname_fn) in [
        ...
        ("glibc",
         libc::getsockname as unsafe extern "C" fn(c_int, *mut libc::sockaddr, *mut u32) -> c_int),
    ] {
        let fd = unsafe { libc::socket(libc::AF_INET, libc::SOCK_STREAM, 0) };
        let one: c_int = 1;
        unsafe {
            libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_REUSEADDR,
                &one as *const c_int as *const _, size_of::<c_int>() as u32);
        }
        let addr = make_loopback_addr(0);
        let r = unsafe {
            libc::bind(fd, &addr as *const libc::sockaddr_in as *const libc::sockaddr,
    ...
    assert!(divs.is_empty(), "getsockname divergences:\n{}", render_divs(&divs));
}
...
fn diff_shutdown_invalid_fd() {
    ...
    assert!(divs.is_empty(), "shutdown divergences:\n{}", render_divs(&divs));
}
...
fn diff_socketpair_unix_stream() {
    ...
    unsafe { clear_errno_both() };
    let r_fl = unsafe {
        fl::socketpair(libc::AF_UNIX, libc::SOCK_STREAM, 0, sv_fl.as_mut_ptr())
    };
    let er_fl = unsafe { read_fl_errno() };
    unsafe { clear_errno_both() };
    let r_lc = unsafe {
        libc::socketpair(libc::AF_UNIX, libc::SOCK_STREAM, 0, sv_lc.as_mut_ptr())
    };
    let er_lc = unsafe { read_lc_errno() };
    if r_fl != r_lc {
        divs.push(Divergence {
    ...
    if sv_fl[0] >= 0 { unsafe { libc::close(sv_fl[0]); } }
    if sv_fl[1] >= 0 { unsafe { libc::close(sv_fl[1]); } }
    if sv_lc[0] >= 0 { unsafe { libc::close(sv_lc[0]); } }
    if sv_lc[1] >= 0 { unsafe { libc::close(sv_lc[1]); } }
    assert!(divs.is_empty(), "socketpair divergences:\n{}", render_divs(&divs));
}
