...
//! + f32, vs host glibc, over the full pairwise matrix of signed zeros, signed
...
fn gsym(h: *mut c_void, n: &str) -> Option<extern "C" fn(f64, f64) -> f64> {
...
fn fmaxmin_family_matches_glibc() {
    ...
    let vals = [
        0.0f64, -0.0, 1.0, -1.0, 2.0, -2.0, 0.5, -3.0, 1e300, -1e300,
        f64::INFINITY, f64::NEG_INFINITY, f64::NAN, -f64::NAN,
    ];
    let funcs: &[(&str, fn(f64, f64) -> f64)] = &[
        ...
        ("fmaximum_mag_num", |a, b| unsafe { fl::fmaximum_mag_num(a, b) }),
        ("fminimum_mag_num", |a, b| unsafe { fl::fminimum_mag_num(a, b) }),
    ...
    for (name, flf) in funcs {
        ...
        for &a in &vals {
            for &b in &vals {
                let fv = flf(a, b);
                let gv = gf(a, b);
                let ok = if gv.is_nan() { fv.is_nan() } else { fv.to_bits() == gv.to_bits() };
                if !ok {
                    div.push(format!(
                        "{}({:+},{:+}): fl={:016x} glibc={:016x}",
                        name, a, b, fv.to_bits(), gv.to_bits()
                    ));
                }
            }
        }
    }
    assert_eq!(covered, 10, "expected all 10 family members present in glibc");
    assert!(
        div.is_empty(),
