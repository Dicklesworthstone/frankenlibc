...
|------|--------------|-------|----|----|-------|---------|--------|
| 2026-06-21 | REAL LOSSES found (reliable survey): core `strspn` 6.5x, `strstr` 3x, `strcasestr` 1.5x slower than real glibc (`bd-2g7oyh`, cc/BlackThrush) | `string_inprocess_survey_bench` (NEW; core vs REAL in-process glibc, no abi-bench; warm rch) | strspn 28.9 ns; strstr 121.8 ns; strcasestr 97.4 ns | strspn **4.44 ns**; strstr 41 ns; strcasestr 66 ns | strspn **6.5x**, strstr **3x**, strcasestr **1.5x** LOSS | LOSS (real, trustworthy) → strspn next | The reliable in-process method strikes again — 3 genuine losses the dlmopen gauntlet had masked (these were listed "done" in the perf-frontier memory but are NOT glibc-competitive). glibc's strspn (4.44 ns / 31-char span ≈ 0.14 ns/char) is SIMD/vectorized; fl's bitmap scan (≈0.93 ns/char) is scalar = 6.5x. strstr/strcasestr (fl TwoWay/BMH) lose 3x/1.5x to glibc's SSE search. result-identity asserts green. NEXT: strspn (biggest, clearest — SIMD-ize the bitmap scan or specialize small accept-sets). NOTE these are core string primitives (heavy multi-agent area — check `git log`/coordinate before editing). |
