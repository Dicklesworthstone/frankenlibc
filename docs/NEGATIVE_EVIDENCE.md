...
|------|--------------|-------|----|----|-------|---------|--------|
| 2026-06-21 | `clock_gettime` stack-output + common-clock validation fast path (`bd-2g7oyh.499`, code-only disk-low pass) | `strtol_glibc_bench` `clock_gettime` focused row plus focused time/vDSO conformance | PENDING | PENDING | PENDING | PENDING-BENCH | Source committed only because root filesystem was critical (`sbh status` overall critical, `df` about 40G free / 98% used). No cargo bench/build/check/test was started this turn. This is not the rejected vDSO pointer-cache family: no resolved function pointer cache, no buffered hit counters, and no `time(NULL)`-only runtime-gate reduction. Next turn must run same-worker rch `clock_gettime` row and focused time/vDSO conformance, then keep or revert. Evidence note: `tests/artifacts/perf/bd-2g7oyh.499-clock-gettime-clock-id-fast-pending.md`. |
...
| 2026-06-21 | `pthread_self` default TLS-cache `Cell` hot-hit fast path (`bd-2g7oyh.498`, code-only disk-low pass) | `strtol_glibc_bench` `pthread_self` focused row plus pthread lifecycle conformance | PENDING | PENDING | PENDING | PENDING-BENCH | Source committed only because root filesystem was critical (`sbh status` overall critical, `df` about 47G free / 98% used). No cargo bench/build/test was started this turn. Next turn must run same-worker rch `pthread_self` row and focused pthread lifecycle/conformance gates, then keep or revert. Evidence note: `tests/artifacts/perf/bd-2g7oyh.498-pthread-self-cell-fast-cache-pending.md`. |
