...
|------|--------------|-------|----|----|-------|---------|--------|
| 2026-06-22 | **glob errfunc tests FLAKY = MIXED root/non-root rch fleet (root reads chmod-0o000 dirs) — gauntlet-reliability flag for the glob-owner; NOT a code bug** (`bd-2g7oyh`, cc/BlackThrush) | `rch exec` worker identity (`id -u` gave 1000/ubuntu on one worker; the isolation run selected `hz2 = root@178.104.77.29`) + read `string::glob::tests` helper `unreadable_directory_pattern` | 3 glob tests flip GREEN↔RED across gauntlet runs | n/a | n/a | DIAGNOSIS (glob-owner test-hygiene) | The 3 `string::glob` failures (`directory_error_callback_can_abort`/`_can_continue`, `glob_err_aborts_after_callback`) flip across gauntlet runs. ROOT CAUSE (verified): the rch fleet is MIXED — some workers run as **root** (e.g. `hz2 = root@...`), others as ubuntu (uid 1000). The tests create a dir, `chmod 0o000` it, glob it, and expect glob's opendir/readdir to fail so the errfunc fires. Under **root**, a 0o000 dir is still readable → no error → errfunc never fires → the 3 tests FAIL; on non-root workers they PASS. The helper already uses UNIQUE per-test dirs (`unique_glob_test_dir`), so it is NOT a parallel-collision — purely the root-worker premise violation. NOT a fl code bug, NOT mine (glob is not my area; I touched fnmatch, not glob), and NOT a real regression. FIX (for the glob-owner, since "ONLY your files"): a skip-when-premise-invalid guard — after `set_directory_mode(0o000)`, `if std::fs::read_dir(&blocked_dir).is_ok() { return; }` (robust vs root AND permission-ignoring filesystems). This corrects my prior-turn "non-root, cause unclear" note (the first worker I sampled happened to be non-root). Other agents: an intermittent `string::glob` ×3 in the gauntlet is this fleet-flakiness, not your change. |
...
| 2026-06-21 | **math::float32 RED diagnosis (read-only, for the f32-agent): log2f = 1-ULP glibc-2.42 libm drift; powf = 2-ULP vs hardcoded golden — NOT touching (active owner)** (`bd-2g7oyh`, cc/BlackThrush) | `cargo test math::float32::tests::{log2f_dyadic_profile_grid_matches_libm_bits, powf_profile_exp_1_337_grid_matches_polynomial_bits_and_sha256}` --nocapture | log2f(0.59375) fl 3208677331 vs **live libm** 3208677330 (**1 ULP**); powf(0.5) fl 1053469677 vs **hardcoded golden** 1053469675 (**2 ULP**) | live host libm (glibc 2.42) / in-code golden | n/a | DIAGNOSIS (leave to f32-agent) | The 3 remaining core-gauntlet RED tests (the only non-mine failures). Investigated read-only: **log2f** compares fl to LIVE host libm and is off by exactly **1 ULP** — the signature of the glibc **2.42 libm upgrade** (host is now 2.42; fl's log2f was bit-exact vs the OLD libm; 2.42 nudged log2f(0.59375) by 1 ULP). A real-but-tiny parity gap to RE-ESTABLISH vs 2.42, NOT a refactor regression. **powf** compares fl to a HARDCODED golden grid (not live libm) and is off 2 ULP — cause (fl-powf changed vs stale-golden) needs the f32-agent's bit-exact harness. Both live in `math/float32.rs`, the f32-agent's ACTIVELY-committed file (6 recent commits: coshf/sinhf/tanhf/erff/tgammaf). Unlike the iconv stale tests (inactive owner → I fixed), this owner is ACTIVE + it's subtle bit-exact f32 math → I am NOT touching it (collision + I'd risk a mis-fix). Recording the diagnosis so the re-establishment is framed correctly (glibc-2.42 drift, re-port log2f/powf to 2.42 libm bits — likely re-derive the golden grids too). |
