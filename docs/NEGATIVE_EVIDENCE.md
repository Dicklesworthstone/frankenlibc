...
|------|--------------|-------|----|----|-------|---------|--------|
| 2026-06-20 | deployed strict `snprintf("%s")` SWAR/SIMD strlen+memcpy (`bd-2g7oyh`, cc/BlackThrush) | NEW `snprintf_s_strict_ab_bench` IN-PROCESS A/B (old-kernel vs new-kernel vs host `libc::snprintf`, all in ONE process so worker load cancels in the ratio — defeats the rch cross-worker variance that hid this signal for 2 prior turns) | new 8B 8.61 / 38B 8.53 / 200B 13.03 ns | glibc 8B 24.70 / 38B 25.72 / 200B 23.37 ns | 0.35x / 0.33x / 0.56x | WIN | Keep (commit `6d2cd0c79`). Replaced the fused scalar scan+copy byte loop in `strict_direct_snprintf_s` with `scan_c_string` (page-safe SWAR/SIMD, the exact scanner deployed `strlen` uses; NOT `c_str_bytes`→`known_remaining`, the measured 5x-regression trap from the prior turn) + `memcpy`. Beats glibc at EVERY size; beats old fl 2.1x@38B / 8.9x@200B (old 17.82 / 116.42 ns). Tiny <=8B costs +2.5ns vs old fl (SIMD prologue) but still 2.9x faster than glibc. Byte-identity PROVEN by the bench's `verify()` (new==old across truncation / `%s\n` / NULL→"(null)" / empty / size-0 / size-1, and new==glibc for plain `%s`) which runs in deployed config before every measurement — a lib unit test cannot reach it because `pub mod stdio_abi` is `#[cfg(not(test))]`. Deployed abi release build GREEN. |
| 2026-06-20 | `printf`/`fprintf`/`vfprintf`/`vprintf`/`dprintf` bare-`%s` length: `c_str_bytes`→`scan_c_string` in `direct_printf_string_payload` (`bd-2g7oyh`, cc/BlackThrush) | kernel A/B reuse: `snprintf_s_strict_ab_bench` (same byte-loop vs SWAR `scan_c_string` swap, +`known_remaining` lock removed) | new SWAR strlen 38B 8.53 ns | old `c_str_bytes` byte-loop+lock 38B ≥17.82 ns; glibc strlen-class 25.72 ns | ≤0.48x vs old / 0.33x vs glibc-class | WIN | Keep. `bare_s_or_render` (the FILE-writer bare-`%s` fast path) and the non-strict `snprintf("%s")` path both route through `direct_printf_string_payload`, which computed the arg length via `c_str_bytes` → `scan_c_str_len` → `known_remaining`/`fallback_remaining` (a `lock_fallback_alloc_table()` mutex + up-to-1024-slot hash probe) + scalar byte loop. The length only feeds a copy/write here, so the bound is not load-bearing; swapped to `scan_c_string` (page-safe SWAR/SIMD, the deployed `strlen` scanner). Byte-identical `len` for valid NUL-terminated `%s` (C contract). Deployed `cargo build -p frankenlibc-abi --release` GREEN. Same proven lever as the puts/fputs row; end-to-end printf-vs-glibc not separately benched (buffered-write-dominated; isolation contaminated by cross-libc FILE*). |
| 2026-06-20 | deployed `puts`/`fputs` fast-path strlen: `scan_c_str_len`→`scan_c_string` (`bd-2g7oyh`, cc/BlackThrush) | kernel A/B reuse: `snprintf_s_strict_ab_bench` (byte-loop vs SWAR `scan_c_string` is the identical swap) | new SWAR strlen 38B 8.53 ns | old byte-loop 38B 17.82 ns; glibc strlen-class 25.72 ns | 0.48x vs old / 0.33x vs glibc-class | WIN | Keep. The deployed `bootstrap_passthrough \|\| !heals_enabled` fast-path of `puts`/`fputs` IGNORES the terminated flag (`let (len, _) =`), so `scan_c_str_len`'s `known_remaining` bound was pure overhead: `known_remaining`→`fallback_remaining` takes a `lock_fallback_alloc_table()` MUTEX + up-to-1024-slot hash probe on EVERY call, on top of a scalar byte loop. Swapped to `scan_c_string` (the page-safe SWAR/SIMD scanner the deployed `strlen` already uses) — removes the mutex+probe AND vectorizes the scan. Byte-identical `len` for any valid NUL-terminated C string (the C contract); on UB unterminated-but-tracked input it now reads to the next NUL like glibc instead of stopping at the alloc end. Kernel speedup measured by the shared A/B (SWAR 0.48x of byte-loop); the eliminated `fallback_remaining` lock is pure-additional, unambiguous. Deployed `cargo build -p frankenlibc-abi --release` GREEN. End-to-end `fputs`-vs-glibc not separately benched (cross-libc-instance FILE* + differing rewind cost contaminate an isolated bench). |
