...
| 2026-06-21 | `pthread_self` default TLS-cache `Cell` hot-hit fast path (`bd-2g7oyh.498`, code-only disk-low pass) | Incidental `strtol_glibc_bench` `pthread_self` row from the one allowed rch remote `hz1` bench | 2.14 ns | 2.99 ns | 0.72x | WIN (bench only) | Do not close yet: the one allowed mixed bench shows a real win, but the focused pthread lifecycle/conformance gate was not run this turn. Keep `.498` open until identity/lifecycle conformance passes. Evidence note: `tests/artifacts/perf/bd-2g7oyh.498-pthread-self-cell-fast-cache-pending.md`. |
