...
|------|--------------|-------|----|----|-------|---------|--------|
| 2026-06-21 | CORRECTION to my own root-cause + diagnostic: memmem loses ONLY common-text; fl WINS rare-last 2.3x + adversarial 19x; cause NOT the naive anchor (commonness heuristic exists) — unpinned (`bd-2g7oyh`, cc/BlackThrush) | `string_inprocess_survey_bench` `survey_memmem_rarelast`/`survey_memmem_twoway` (core vs REAL in-process glibc) | rarelast fl **12.02** / twoway fl **14.07** ns | glibc 27.96 / 272.34 ns | rarelast **0.43x fl WINS 2.3x**; twoway **0.05x fl WINS 19x** | CORRECTION (no fix committed) | **I CAUGHT MY OWN WRONG ROOT-CAUSE before committing a fix** (by reading the code I was about to change). The row below claimed memmem "ALWAYS anchors on the last byte" — FALSE: memmem already has `memmem_prefers_last_anchor(first,last) = anchor_commonness(last) <= anchor_commonness(first)` with a static English-frequency table (`' '`/`e`=16, `aionrst`=12, …). For "needle_here" (first `n`=12, last `e`=16) it correctly does NOT anchor on common `e` — it uses the FIRST-byte path on `n`. So the naive-anchor explanation is wrong. DIAGNOSTIC (valid): fl memmem DOMINATES glibc on rare-last (12 vs 28 ns, 2.3x) and on adversarial `aa…ab` (14 vs **272** ns — glibc degrades catastrophically, 19x); Two-Way itself is fast (14 ns). The ONLY loss is common-text "needle_here" (106 vs 34 ns). REAL CAUSE STILL UNPINNED: the first-byte path on a moderately-common byte (`n`, ~4 occurrences in 79 B) is unexpectedly ~106 ns — my candidate-cost model predicts ~40 ns, so something else dominates (memchr per-call fixed cost? verify? path mis-selection?). Needs proper profiling next turn, NOT another guess. |
| 2026-06-21 | ~~FINDING (root-cause SUPERSEDED above — was WRONG)~~: memmem 3.08x slower than glibc (text input); wmemset/wmemcpy PARITY (`bd-2g7oyh`, cc/BlackThrush) | `string_inprocess_survey_bench` `survey_memmem`/`survey_strstr`/`survey_wmemset`/`survey_wmemcpy` (core vs REAL in-process glibc) | memmem **106.89 ns** / strstr 99.77 / wmemset 15.69 / wmemcpy 10.36 | glibc 34.67 / 32.13 / 16.55 / 10.41 | memmem/strstr **3.1x LOSS**; wmemset 0.95x / wmemcpy 0.99x PARITY | RECORDED (deep fix deferred) + wmemset/wmemcpy bound | Reliable survey of a fresh family. wmemset (`[u32]::fill`) + wmemcpy (`copy_from_slice`) lower to optimized memset/memcpy → already PARITY (bound, no action). strstr's 3.1x is NOT its `strlen` pre-pass (~7 ns) — ISOLATED to memmem itself (`survey_memmem` 106.89 vs glibc 34.67 = 3.08x). ROOT CAUSE: memmem's dual-anchor fast path ALWAYS anchors the memchr scan on the needle's LAST byte (assuming it's rare), but in TEXT the last byte is often common — for needle "needle_here" the last byte 'e' occurs ~10x in the 79-byte haystack, so memmem visits ~7 wasted last-byte candidates (each a memchr restart + 11-byte verify) before the Two-Way `miss_work > hay.len()` bailout fires; that wasted prefix + Two-Way ≈ 3x glibc's single skip-table pass. The prior "memmem=TwoWay+BMH DONE" was never reliably head-to-head'd (dlmopen masked it). FIX (deferred — deep, core primitive, risk): rarity-aware anchor (pick the rarer of first/last), or a proper BMH last-byte skip TABLE (skip by up to needle_len, vs visiting every last-byte occurrence), or a lower/ adaptive bailout. Multi-turn; not a clean mask fix. |
