...
| **3** | **L16566** | `strftime` general-loop **~200 ns fixed per-call overhead** | no ratio; recorded as a LOSS/DROP blocker | V1+V2 — dropped on "~0-gain" for the *literal-push* sub-lever, while naming a 200 ns fixed cost it did not attack | **DONE / SURFACE.** ELF `4fd10a27…91dfa`: later fast paths cleared exact/literal shapes, but non-exact `prefix %A suffix` is **269.085 ns** vs glibc **29.021 ns**, median **9.277820**, CI **[9.204705, 9.552748]**, null CI **[0.978942, 1.066469]**. Profile-before-edit frontier remains open. |
| **4** | **L21545** `bd-vihwy9` | exact `%FT%T` `wcsftime` alias emitter | 1.05, frame 267 ns/call | V1+V4 — the ratio is *inside* the floor | **DONE / KEEP.** A 32-timestamp rotating workload made the alias branch discriminating. Exact alias recognition reuses the proven ISO-T emitter: candidate/orig **0.067613**, CI **[0.064222, 0.070854]**, null CI **[0.959509, 1.077104]**; 200,000-case differential remains exact. |
| **5** | **L21131 / L21277** `bd-9x1jcx` | three sub-2× `hosts_*` resolver rows | CAND/ORIG 0.2710 / 0.3802 but **raw-arm CVs 82–475%** | V4 nominally | **DONE / KEEP AT SCANNER BOUNDARY.** An immutable in-memory hosts snapshot isolates the shipped allocating-vs-borrowed mechanism: candidate/orig **0.428468**, CI **[0.411170, 0.444882]**, null CI **[0.952074, 1.043635]**. The file-backed ABI rows remain separately undecidable. |
